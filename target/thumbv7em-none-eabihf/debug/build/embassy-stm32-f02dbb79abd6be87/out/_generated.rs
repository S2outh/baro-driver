embassy_hal_internal::peripherals_definition!(
    PA0,
    PA1,
    PA2,
    PA3,
    PA4,
    PA5,
    PA6,
    PA7,
    PA8,
    PA9,
    PA10,
    PA11,
    PA12,
    PA13,
    PA14,
    PA15,
    PB0,
    PB1,
    PB2,
    PB3,
    PB4,
    PB5,
    PB6,
    PB7,
    PB8,
    PB9,
    PB10,
    PB11,
    PB12,
    PB13,
    PB14,
    PB15,
    PC0,
    PC1,
    PC2,
    PC3,
    PC4,
    PC5,
    PC6,
    PC7,
    PC8,
    PC9,
    PC10,
    PC11,
    PC12,
    PC13,
    PC14,
    PC15,
    PD0,
    PD1,
    PD2,
    PD3,
    PD4,
    PD5,
    PD6,
    PD7,
    PD8,
    PD9,
    PD10,
    PD11,
    PD12,
    PD13,
    PD14,
    PD15,
    PE0,
    PE1,
    PE2,
    PE3,
    PE4,
    PE5,
    PE6,
    PE7,
    PE8,
    PE9,
    PE10,
    PE11,
    PE12,
    PE13,
    PE14,
    PE15,
    PH0,
    PH1,
    ADC1,
    ADC12_COMMON,
    ADC2,
    ADC3,
    ADC3_COMMON,
    BDMA,
    CEC,
    COMP1,
    COMP2,
    CORDIC,
    CRC,
    CRS,
    DAC1,
    DBGMCU,
    DCMI,
    DMA1,
    DMA2,
    DMA2D,
    DMAMUX1,
    DMAMUX2,
    DTS,
    ETH,
    FDCAN1,
    FDCAN2,
    FDCAN3,
    FDCANRAM,
    FLASH,
    FMAC,
    FMC,
    I2C1,
    I2C2,
    I2C3,
    I2C4,
    I2C5,
    IWDG1,
    LPTIM1,
    LPTIM2,
    LPTIM3,
    LPTIM4,
    LPTIM5,
    LPUART1,
    LTDC,
    MDIOS,
    OCTOSPI1,
    OCTOSPI2,
    OCTOSPIM,
    OPAMP1,
    OPAMP2,
    PSSI,
    PWR,
    MCO1,
    MCO2,
    RCC,
    RNG,
    RTC,
    SAI1,
    SAI4,
    SDMMC1,
    SDMMC2,
    SPDIFRX1,
    SPI1,
    SPI2,
    SPI3,
    SPI4,
    SPI6,
    SYSCFG,
    TIM1,
    TIM12,
    TIM13,
    TIM14,
    TIM15,
    TIM16,
    TIM17,
    TIM2,
    TIM23,
    TIM24,
    TIM3,
    TIM4,
    TIM5,
    TIM6,
    TIM7,
    TIM8,
    UART4,
    UART5,
    UART7,
    UART8,
    UART9,
    UID,
    USART1,
    USART10,
    USART2,
    USART3,
    USART6,
    USB_OTG_HS,
    VREFBUF,
    WWDG1,
    EXTI0,
    EXTI1,
    EXTI2,
    EXTI3,
    EXTI4,
    EXTI5,
    EXTI6,
    EXTI7,
    EXTI8,
    EXTI9,
    EXTI10,
    EXTI11,
    EXTI12,
    EXTI13,
    EXTI14,
    EXTI15,
    DMA1_CH0,
    DMA1_CH1,
    DMA1_CH2,
    DMA1_CH3,
    DMA1_CH4,
    DMA1_CH5,
    DMA1_CH6,
    DMA1_CH7,
    DMA2_CH0,
    DMA2_CH1,
    DMA2_CH2,
    DMA2_CH3,
    DMA2_CH4,
    DMA2_CH5,
    DMA2_CH6,
    DMA2_CH7,
    BDMA_CH0,
    BDMA_CH1,
    BDMA_CH2,
    BDMA_CH3,
    BDMA_CH4,
    BDMA_CH5,
    BDMA_CH6,
    BDMA_CH7
);
embassy_hal_internal::peripherals_struct!(
    PA0,
    PA1,
    PA2,
    PA3,
    PA4,
    PA5,
    PA6,
    PA7,
    PA8,
    PA9,
    PA10,
    PA11,
    PA12,
    PA13,
    PA14,
    PA15,
    PB0,
    PB1,
    PB2,
    PB3,
    PB4,
    PB5,
    PB6,
    PB7,
    PB8,
    PB9,
    PB10,
    PB11,
    PB12,
    PB13,
    PB14,
    PB15,
    PC0,
    PC1,
    PC2,
    PC3,
    PC4,
    PC5,
    PC6,
    PC7,
    PC8,
    PC9,
    PC10,
    PC11,
    PC12,
    PC13,
    PC14,
    PC15,
    PD0,
    PD1,
    PD2,
    PD3,
    PD4,
    PD5,
    PD6,
    PD7,
    PD8,
    PD9,
    PD10,
    PD11,
    PD12,
    PD13,
    PD14,
    PD15,
    PE0,
    PE1,
    PE2,
    PE3,
    PE4,
    PE5,
    PE6,
    PE7,
    PE8,
    PE9,
    PE10,
    PE11,
    PE12,
    PE13,
    PE14,
    PE15,
    PH0,
    PH1,
    ADC1,
    ADC12_COMMON,
    ADC2,
    ADC3,
    ADC3_COMMON,
    BDMA,
    CEC,
    COMP1,
    COMP2,
    CORDIC,
    CRC,
    CRS,
    DAC1,
    DBGMCU,
    DCMI,
    DMA1,
    DMA2,
    DMA2D,
    DMAMUX1,
    DMAMUX2,
    DTS,
    ETH,
    FDCAN1,
    FDCAN2,
    FDCAN3,
    FDCANRAM,
    FLASH,
    FMAC,
    FMC,
    I2C1,
    I2C2,
    I2C3,
    I2C4,
    I2C5,
    IWDG1,
    LPTIM1,
    LPTIM2,
    LPTIM3,
    LPTIM4,
    LPTIM5,
    LPUART1,
    LTDC,
    MDIOS,
    OCTOSPI1,
    OCTOSPI2,
    OCTOSPIM,
    OPAMP1,
    OPAMP2,
    PSSI,
    PWR,
    MCO1,
    MCO2,
    RCC,
    RNG,
    RTC,
    SAI1,
    SAI4,
    SDMMC1,
    SDMMC2,
    SPDIFRX1,
    SPI1,
    SPI2,
    SPI3,
    SPI4,
    SPI6,
    SYSCFG,
    TIM1,
    TIM13,
    TIM14,
    TIM15,
    TIM16,
    TIM17,
    TIM2,
    TIM23,
    TIM24,
    TIM3,
    TIM4,
    TIM5,
    TIM6,
    TIM7,
    TIM8,
    UART4,
    UART5,
    UART7,
    UART8,
    UART9,
    UID,
    USART1,
    USART10,
    USART2,
    USART3,
    USART6,
    USB_OTG_HS,
    VREFBUF,
    WWDG1,
    EXTI0,
    EXTI1,
    EXTI2,
    EXTI3,
    EXTI4,
    EXTI5,
    EXTI6,
    EXTI7,
    EXTI8,
    EXTI9,
    EXTI10,
    EXTI11,
    EXTI12,
    EXTI13,
    EXTI14,
    EXTI15,
    DMA1_CH0,
    DMA1_CH1,
    DMA1_CH2,
    DMA1_CH3,
    DMA1_CH4,
    DMA1_CH5,
    DMA1_CH6,
    DMA1_CH7,
    DMA2_CH0,
    DMA2_CH1,
    DMA2_CH2,
    DMA2_CH3,
    DMA2_CH4,
    DMA2_CH5,
    DMA2_CH6,
    DMA2_CH7,
    BDMA_CH0,
    BDMA_CH1,
    BDMA_CH2,
    BDMA_CH3,
    BDMA_CH4,
    BDMA_CH5,
    BDMA_CH6,
    BDMA_CH7
);
embassy_hal_internal::interrupt_mod!(
    WWDG,
    PVD_AVD,
    TAMP_STAMP,
    RTC_WKUP,
    FLASH,
    RCC,
    EXTI0,
    EXTI1,
    EXTI2,
    EXTI3,
    EXTI4,
    DMA1_STREAM0,
    DMA1_STREAM1,
    DMA1_STREAM2,
    DMA1_STREAM3,
    DMA1_STREAM4,
    DMA1_STREAM5,
    DMA1_STREAM6,
    ADC,
    FDCAN1_IT0,
    FDCAN2_IT0,
    FDCAN1_IT1,
    FDCAN2_IT1,
    EXTI9_5,
    TIM1_BRK,
    TIM1_UP,
    TIM1_TRG_COM,
    TIM1_CC,
    TIM2,
    TIM3,
    TIM4,
    I2C1_EV,
    I2C1_ER,
    I2C2_EV,
    I2C2_ER,
    SPI1,
    SPI2,
    USART1,
    USART2,
    USART3,
    EXTI15_10,
    RTC_ALARM,
    TIM8_BRK_TIM12,
    TIM8_UP_TIM13,
    TIM8_TRG_COM_TIM14,
    TIM8_CC,
    DMA1_STREAM7,
    FMC,
    SDMMC1,
    TIM5,
    SPI3,
    UART4,
    UART5,
    TIM6_DAC,
    TIM7,
    DMA2_STREAM0,
    DMA2_STREAM1,
    DMA2_STREAM2,
    DMA2_STREAM3,
    DMA2_STREAM4,
    ETH,
    ETH_WKUP,
    FDCAN_CAL,
    DMA2_STREAM5,
    DMA2_STREAM6,
    DMA2_STREAM7,
    USART6,
    I2C3_EV,
    I2C3_ER,
    OTG_HS_EP1_OUT,
    OTG_HS_EP1_IN,
    OTG_HS_WKUP,
    OTG_HS,
    DCMI_PSSI,
    RNG,
    FPU,
    UART7,
    UART8,
    SPI4,
    SPI5,
    SPI6,
    SAI1,
    LTDC,
    LTDC_ER,
    DMA2D,
    OCTOSPI1,
    LPTIM1,
    CEC,
    I2C4_EV,
    I2C4_ER,
    SPDIF_RX,
    DMAMUX1_OVR,
    DFSDM1_FLT0,
    DFSDM1_FLT1,
    DFSDM1_FLT2,
    DFSDM1_FLT3,
    SWPMI1,
    TIM15,
    TIM16,
    TIM17,
    MDIOS_WKUP,
    MDIOS,
    MDMA,
    SDMMC2,
    HSEM1,
    ADC3,
    DMAMUX2_OVR,
    BDMA_CHANNEL0,
    BDMA_CHANNEL1,
    BDMA_CHANNEL2,
    BDMA_CHANNEL3,
    BDMA_CHANNEL4,
    BDMA_CHANNEL5,
    BDMA_CHANNEL6,
    BDMA_CHANNEL7,
    COMP,
    LPTIM2,
    LPTIM3,
    LPTIM4,
    LPTIM5,
    LPUART1,
    CRS,
    ECC,
    SAI4,
    DTS,
    WAKEUP_PIN,
    OCTOSPI2,
    FMAC,
    CORDIC,
    UART9,
    USART10,
    I2C5_EV,
    I2C5_ER,
    FDCAN3_IT0,
    FDCAN3_IT1,
    TIM23,
    TIM24,
);
pub const MAX_ERASE_SIZE: usize = 131072u32 as usize;
pub mod flash_regions {
    pub const BANK1_REGION: crate::flash::FlashRegion = crate::flash::FlashRegion {
        bank: crate::flash::FlashBank::Bank1,
        base: 134217728u32,
        size: 1048576u32,
        erase_size: 131072u32,
        write_size: 32u32,
        erase_value: 255u8,
        _ensure_internal: (),
    };
    #[cfg(flash)]
    pub struct Bank1Region<'d, MODE = crate::flash::Async>(
        pub &'static crate::flash::FlashRegion,
        pub(crate) embassy_hal_internal::Peri<'d, crate::peripherals::FLASH>,
        pub(crate) core::marker::PhantomData<MODE>,
    );
    #[cfg(flash)]
    pub struct FlashLayout<'d, MODE = crate::flash::Async> {
        pub bank1_region: Bank1Region<'d, MODE>,
        _mode: core::marker::PhantomData<MODE>,
    }
    #[cfg(flash)]
    impl<'d, MODE> FlashLayout<'d, MODE> {
        pub(crate) fn new(p: embassy_hal_internal::Peri<'d, crate::peripherals::FLASH>) -> Self {
            Self {
                bank1_region: Bank1Region(
                    &BANK1_REGION,
                    unsafe { p.clone_unchecked() },
                    core::marker::PhantomData,
                ),
                _mode: core::marker::PhantomData,
            }
        }
    }
    pub const FLASH_REGIONS: [&crate::flash::FlashRegion; 1usize] = [&BANK1_REGION];
}
impl crate::rcc::SealedRccPeripheral for peripherals::ADC1 {
    fn frequency() -> crate::time::Hertz {
        match crate::pac::RCC.d3ccipr().read().adcsel() {
            crate::pac::rcc::vals::Adcsel::PLL2_P => unsafe {
                unwrap ! (crate :: rcc :: get_freqs () . pll2_p . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "ADC1" , "pll2_p")
            },
            crate::pac::rcc::vals::Adcsel::PLL3_R => unsafe {
                unwrap ! (crate :: rcc :: get_freqs () . pll3_r . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "ADC1" , "pll3_r")
            },
            crate::pac::rcc::vals::Adcsel::PER => match crate::pac::RCC.d1ccipr().read().persel() {
                crate::pac::rcc::vals::Persel::HSI => unsafe {
                    unwrap ! (crate :: rcc :: get_freqs () . hsi . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "ADC1" , "hsi")
                },
                crate::pac::rcc::vals::Persel::CSI => unsafe {
                    unwrap ! (crate :: rcc :: get_freqs () . csi . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "ADC1" , "csi")
                },
                crate::pac::rcc::vals::Persel::HSE => unsafe {
                    unwrap ! (crate :: rcc :: get_freqs () . hse . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "ADC1" , "hse")
                },
                #[allow(unreachable_patterns)]
                _ => panic!(
                    "attempted to use peripheral '{}' but its clock mux is not set to a valid \
                         clock. Change 'config.rcc.mux' to another clock.",
                    "ADC1"
                ),
            },
            #[allow(unreachable_patterns)]
            _ => panic!(
                "attempted to use peripheral '{}' but its clock mux is not set to a valid \
                         clock. Change 'config.rcc.mux' to another clock.",
                "ADC1"
            ),
        }
    }
    fn bus_frequency() -> crate::time::Hertz {
        unsafe {
            unwrap ! (crate :: rcc :: get_freqs () . hclk1 . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "ADC1" , "hclk1")
        }
    }
    const RCC_INFO: crate::rcc::RccInfo = unsafe {
        crate::rcc::RccInfo::new(
            Some((32u8, 5u8)),
            (54u8, 5u8),
            Some(0u8),
            #[cfg(feature = "low-power")]
            crate::rcc::StopMode::Stop1,
        )
    };
}
impl crate::rcc::RccPeripheral for peripherals::ADC1 {}
impl crate::rcc::SealedRccPeripheral for peripherals::ADC2 {
    fn frequency() -> crate::time::Hertz {
        match crate::pac::RCC.d3ccipr().read().adcsel() {
            crate::pac::rcc::vals::Adcsel::PLL2_P => unsafe {
                unwrap ! (crate :: rcc :: get_freqs () . pll2_p . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "ADC2" , "pll2_p")
            },
            crate::pac::rcc::vals::Adcsel::PLL3_R => unsafe {
                unwrap ! (crate :: rcc :: get_freqs () . pll3_r . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "ADC2" , "pll3_r")
            },
            crate::pac::rcc::vals::Adcsel::PER => match crate::pac::RCC.d1ccipr().read().persel() {
                crate::pac::rcc::vals::Persel::HSI => unsafe {
                    unwrap ! (crate :: rcc :: get_freqs () . hsi . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "ADC2" , "hsi")
                },
                crate::pac::rcc::vals::Persel::CSI => unsafe {
                    unwrap ! (crate :: rcc :: get_freqs () . csi . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "ADC2" , "csi")
                },
                crate::pac::rcc::vals::Persel::HSE => unsafe {
                    unwrap ! (crate :: rcc :: get_freqs () . hse . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "ADC2" , "hse")
                },
                #[allow(unreachable_patterns)]
                _ => panic!(
                    "attempted to use peripheral '{}' but its clock mux is not set to a valid \
                         clock. Change 'config.rcc.mux' to another clock.",
                    "ADC2"
                ),
            },
            #[allow(unreachable_patterns)]
            _ => panic!(
                "attempted to use peripheral '{}' but its clock mux is not set to a valid \
                         clock. Change 'config.rcc.mux' to another clock.",
                "ADC2"
            ),
        }
    }
    fn bus_frequency() -> crate::time::Hertz {
        unsafe {
            unwrap ! (crate :: rcc :: get_freqs () . hclk1 . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "ADC2" , "hclk1")
        }
    }
    const RCC_INFO: crate::rcc::RccInfo = unsafe {
        crate::rcc::RccInfo::new(
            Some((32u8, 5u8)),
            (54u8, 5u8),
            Some(0u8),
            #[cfg(feature = "low-power")]
            crate::rcc::StopMode::Stop1,
        )
    };
}
impl crate::rcc::RccPeripheral for peripherals::ADC2 {}
impl crate::rcc::SealedRccPeripheral for peripherals::ADC3 {
    fn frequency() -> crate::time::Hertz {
        match crate::pac::RCC.d3ccipr().read().adcsel() {
            crate::pac::rcc::vals::Adcsel::PLL2_P => unsafe {
                unwrap ! (crate :: rcc :: get_freqs () . pll2_p . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "ADC3" , "pll2_p")
            },
            crate::pac::rcc::vals::Adcsel::PLL3_R => unsafe {
                unwrap ! (crate :: rcc :: get_freqs () . pll3_r . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "ADC3" , "pll3_r")
            },
            crate::pac::rcc::vals::Adcsel::PER => match crate::pac::RCC.d1ccipr().read().persel() {
                crate::pac::rcc::vals::Persel::HSI => unsafe {
                    unwrap ! (crate :: rcc :: get_freqs () . hsi . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "ADC3" , "hsi")
                },
                crate::pac::rcc::vals::Persel::CSI => unsafe {
                    unwrap ! (crate :: rcc :: get_freqs () . csi . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "ADC3" , "csi")
                },
                crate::pac::rcc::vals::Persel::HSE => unsafe {
                    unwrap ! (crate :: rcc :: get_freqs () . hse . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "ADC3" , "hse")
                },
                #[allow(unreachable_patterns)]
                _ => panic!(
                    "attempted to use peripheral '{}' but its clock mux is not set to a valid \
                         clock. Change 'config.rcc.mux' to another clock.",
                    "ADC3"
                ),
            },
            #[allow(unreachable_patterns)]
            _ => panic!(
                "attempted to use peripheral '{}' but its clock mux is not set to a valid \
                         clock. Change 'config.rcc.mux' to another clock.",
                "ADC3"
            ),
        }
    }
    fn bus_frequency() -> crate::time::Hertz {
        unsafe {
            unwrap ! (crate :: rcc :: get_freqs () . hclk4 . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "ADC3" , "hclk4")
        }
    }
    const RCC_INFO: crate::rcc::RccInfo = unsafe {
        crate::rcc::RccInfo::new(
            Some((34u8, 24u8)),
            (56u8, 24u8),
            None,
            #[cfg(feature = "low-power")]
            crate::rcc::StopMode::Stop1,
        )
    };
}
impl crate::rcc::RccPeripheral for peripherals::ADC3 {}
impl crate::rcc::SealedRccPeripheral for peripherals::BDMA {
    fn frequency() -> crate::time::Hertz {
        unsafe {
            unwrap ! (crate :: rcc :: get_freqs () . hclk4 . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "BDMA" , "hclk4")
        }
    }
    fn bus_frequency() -> crate::time::Hertz {
        unsafe {
            unwrap ! (crate :: rcc :: get_freqs () . hclk4 . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "BDMA" , "hclk4")
        }
    }
    const RCC_INFO: crate::rcc::RccInfo = unsafe {
        crate::rcc::RccInfo::new(
            Some((34u8, 21u8)),
            (56u8, 21u8),
            None,
            #[cfg(feature = "low-power")]
            crate::rcc::StopMode::Stop1,
        )
    };
}
impl crate::rcc::RccPeripheral for peripherals::BDMA {}
impl crate::rcc::SealedRccPeripheral for peripherals::CEC {
    fn frequency() -> crate::time::Hertz {
        match crate::pac::RCC.d2ccip2r().read().cecsel() {
            crate::pac::rcc::vals::Cecsel::LSE => unsafe {
                unwrap ! (crate :: rcc :: get_freqs () . lse . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "CEC" , "lse")
            },
            crate::pac::rcc::vals::Cecsel::LSI => unsafe {
                unwrap ! (crate :: rcc :: get_freqs () . lsi . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "CEC" , "lsi")
            },
            crate::pac::rcc::vals::Cecsel::CSI => unsafe {
                unwrap ! (crate :: rcc :: get_freqs () . csi . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "CEC" , "csi")
            },
            #[allow(unreachable_patterns)]
            _ => panic!(
                "attempted to use peripheral '{}' but its clock mux is not set to a valid \
                         clock. Change 'config.rcc.mux' to another clock.",
                "CEC"
            ),
        }
    }
    fn bus_frequency() -> crate::time::Hertz {
        unsafe {
            unwrap ! (crate :: rcc :: get_freqs () . pclk1 . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "CEC" , "pclk1")
        }
    }
    const RCC_INFO: crate::rcc::RccInfo = unsafe {
        crate::rcc::RccInfo::new(
            Some((36u8, 27u8)),
            (58u8, 27u8),
            None,
            #[cfg(feature = "low-power")]
            crate::rcc::StopMode::Stop1,
        )
    };
}
impl crate::rcc::RccPeripheral for peripherals::CEC {}
impl crate::rcc::SealedRccPeripheral for peripherals::CORDIC {
    fn frequency() -> crate::time::Hertz {
        unsafe {
            unwrap ! (crate :: rcc :: get_freqs () . hclk2 . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "CORDIC" , "hclk2")
        }
    }
    fn bus_frequency() -> crate::time::Hertz {
        unsafe {
            unwrap ! (crate :: rcc :: get_freqs () . hclk2 . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "CORDIC" , "hclk2")
        }
    }
    const RCC_INFO: crate::rcc::RccInfo = unsafe {
        crate::rcc::RccInfo::new(
            Some((33u8, 17u8)),
            (55u8, 17u8),
            None,
            #[cfg(feature = "low-power")]
            crate::rcc::StopMode::Stop1,
        )
    };
}
impl crate::rcc::RccPeripheral for peripherals::CORDIC {}
impl crate::rcc::SealedRccPeripheral for peripherals::CRC {
    fn frequency() -> crate::time::Hertz {
        unsafe {
            unwrap ! (crate :: rcc :: get_freqs () . hclk4 . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "CRC" , "hclk4")
        }
    }
    fn bus_frequency() -> crate::time::Hertz {
        unsafe {
            unwrap ! (crate :: rcc :: get_freqs () . hclk4 . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "CRC" , "hclk4")
        }
    }
    const RCC_INFO: crate::rcc::RccInfo = unsafe {
        crate::rcc::RccInfo::new(
            Some((34u8, 19u8)),
            (56u8, 19u8),
            None,
            #[cfg(feature = "low-power")]
            crate::rcc::StopMode::Stop1,
        )
    };
}
impl crate::rcc::RccPeripheral for peripherals::CRC {}
impl crate::rcc::SealedRccPeripheral for peripherals::CRS {
    fn frequency() -> crate::time::Hertz {
        unsafe {
            unwrap ! (crate :: rcc :: get_freqs () . pclk1 . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "CRS" , "pclk1")
        }
    }
    fn bus_frequency() -> crate::time::Hertz {
        unsafe {
            unwrap ! (crate :: rcc :: get_freqs () . pclk1 . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "CRS" , "pclk1")
        }
    }
    const RCC_INFO: crate::rcc::RccInfo = unsafe {
        crate::rcc::RccInfo::new(
            Some((37u8, 1u8)),
            (59u8, 1u8),
            None,
            #[cfg(feature = "low-power")]
            crate::rcc::StopMode::Stop1,
        )
    };
}
impl crate::rcc::RccPeripheral for peripherals::CRS {}
impl crate::rcc::SealedRccPeripheral for peripherals::DAC1 {
    fn frequency() -> crate::time::Hertz {
        unsafe {
            unwrap ! (crate :: rcc :: get_freqs () . pclk1 . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "DAC1" , "pclk1")
        }
    }
    fn bus_frequency() -> crate::time::Hertz {
        unsafe {
            unwrap ! (crate :: rcc :: get_freqs () . pclk1 . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "DAC1" , "pclk1")
        }
    }
    const RCC_INFO: crate::rcc::RccInfo = unsafe {
        crate::rcc::RccInfo::new(
            Some((36u8, 29u8)),
            (58u8, 29u8),
            None,
            #[cfg(feature = "low-power")]
            crate::rcc::StopMode::Stop1,
        )
    };
}
impl crate::rcc::RccPeripheral for peripherals::DAC1 {}
impl crate::rcc::SealedRccPeripheral for peripherals::DCMI {
    fn frequency() -> crate::time::Hertz {
        unsafe {
            unwrap ! (crate :: rcc :: get_freqs () . hclk2 . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "DCMI" , "hclk2")
        }
    }
    fn bus_frequency() -> crate::time::Hertz {
        unsafe {
            unwrap ! (crate :: rcc :: get_freqs () . hclk2 . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "DCMI" , "hclk2")
        }
    }
    const RCC_INFO: crate::rcc::RccInfo = unsafe {
        crate::rcc::RccInfo::new(
            Some((33u8, 0u8)),
            (55u8, 0u8),
            None,
            #[cfg(feature = "low-power")]
            crate::rcc::StopMode::Stop1,
        )
    };
}
impl crate::rcc::RccPeripheral for peripherals::DCMI {}
impl crate::rcc::SealedRccPeripheral for peripherals::DMA1 {
    fn frequency() -> crate::time::Hertz {
        unsafe {
            unwrap ! (crate :: rcc :: get_freqs () . hclk1 . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "DMA1" , "hclk1")
        }
    }
    fn bus_frequency() -> crate::time::Hertz {
        unsafe {
            unwrap ! (crate :: rcc :: get_freqs () . hclk1 . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "DMA1" , "hclk1")
        }
    }
    const RCC_INFO: crate::rcc::RccInfo = unsafe {
        crate::rcc::RccInfo::new(
            Some((32u8, 0u8)),
            (54u8, 0u8),
            None,
            #[cfg(feature = "low-power")]
            crate::rcc::StopMode::Stop1,
        )
    };
}
impl crate::rcc::RccPeripheral for peripherals::DMA1 {}
impl crate::rcc::SealedRccPeripheral for peripherals::DMA2 {
    fn frequency() -> crate::time::Hertz {
        unsafe {
            unwrap ! (crate :: rcc :: get_freqs () . hclk1 . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "DMA2" , "hclk1")
        }
    }
    fn bus_frequency() -> crate::time::Hertz {
        unsafe {
            unwrap ! (crate :: rcc :: get_freqs () . hclk1 . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "DMA2" , "hclk1")
        }
    }
    const RCC_INFO: crate::rcc::RccInfo = unsafe {
        crate::rcc::RccInfo::new(
            Some((32u8, 1u8)),
            (54u8, 1u8),
            None,
            #[cfg(feature = "low-power")]
            crate::rcc::StopMode::Stop1,
        )
    };
}
impl crate::rcc::RccPeripheral for peripherals::DMA2 {}
impl crate::rcc::SealedRccPeripheral for peripherals::DMA2D {
    fn frequency() -> crate::time::Hertz {
        unsafe {
            unwrap ! (crate :: rcc :: get_freqs () . hclk3 . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "DMA2D" , "hclk3")
        }
    }
    fn bus_frequency() -> crate::time::Hertz {
        unsafe {
            unwrap ! (crate :: rcc :: get_freqs () . hclk3 . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "DMA2D" , "hclk3")
        }
    }
    const RCC_INFO: crate::rcc::RccInfo = unsafe {
        crate::rcc::RccInfo::new(
            Some((31u8, 4u8)),
            (53u8, 4u8),
            None,
            #[cfg(feature = "low-power")]
            crate::rcc::StopMode::Stop1,
        )
    };
}
impl crate::rcc::RccPeripheral for peripherals::DMA2D {}
impl crate::rcc::SealedRccPeripheral for peripherals::DTS {
    fn frequency() -> crate::time::Hertz {
        unsafe {
            unwrap ! (crate :: rcc :: get_freqs () . pclk4 . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "DTS" , "pclk4")
        }
    }
    fn bus_frequency() -> crate::time::Hertz {
        unsafe {
            unwrap ! (crate :: rcc :: get_freqs () . pclk4 . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "DTS" , "pclk4")
        }
    }
    const RCC_INFO: crate::rcc::RccInfo = unsafe {
        crate::rcc::RccInfo::new(
            Some((39u8, 26u8)),
            (61u8, 26u8),
            None,
            #[cfg(feature = "low-power")]
            crate::rcc::StopMode::Stop1,
        )
    };
}
impl crate::rcc::RccPeripheral for peripherals::DTS {}
impl crate::rcc::SealedRccPeripheral for peripherals::ETH {
    fn frequency() -> crate::time::Hertz {
        unsafe {
            unwrap ! (crate :: rcc :: get_freqs () . hclk1 . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "ETH" , "hclk1")
        }
    }
    fn bus_frequency() -> crate::time::Hertz {
        unsafe {
            unwrap ! (crate :: rcc :: get_freqs () . hclk1 . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "ETH" , "hclk1")
        }
    }
    const RCC_INFO: crate::rcc::RccInfo = unsafe {
        crate::rcc::RccInfo::new(
            Some((32u8, 15u8)),
            (54u8, 15u8),
            None,
            #[cfg(feature = "low-power")]
            crate::rcc::StopMode::Stop1,
        )
    };
}
impl crate::rcc::RccPeripheral for peripherals::ETH {}
impl crate::rcc::SealedRccPeripheral for peripherals::FDCAN1 {
    fn frequency() -> crate::time::Hertz {
        match crate::pac::RCC.d2ccip1r().read().fdcansel() {
            crate::pac::rcc::vals::Fdcansel::HSE => unsafe {
                unwrap ! (crate :: rcc :: get_freqs () . hse . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "FDCAN1" , "hse")
            },
            crate::pac::rcc::vals::Fdcansel::PLL1_Q => unsafe {
                unwrap ! (crate :: rcc :: get_freqs () . pll1_q . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "FDCAN1" , "pll1_q")
            },
            crate::pac::rcc::vals::Fdcansel::PLL2_Q => unsafe {
                unwrap ! (crate :: rcc :: get_freqs () . pll2_q . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "FDCAN1" , "pll2_q")
            },
            #[allow(unreachable_patterns)]
            _ => panic!(
                "attempted to use peripheral '{}' but its clock mux is not set to a valid \
                         clock. Change 'config.rcc.mux' to another clock.",
                "FDCAN1"
            ),
        }
    }
    fn bus_frequency() -> crate::time::Hertz {
        unsafe {
            unwrap ! (crate :: rcc :: get_freqs () . pclk1 . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "FDCAN1" , "pclk1")
        }
    }
    const RCC_INFO: crate::rcc::RccInfo = unsafe {
        crate::rcc::RccInfo::new(
            Some((37u8, 8u8)),
            (59u8, 8u8),
            Some(1u8),
            #[cfg(feature = "low-power")]
            crate::rcc::StopMode::Stop1,
        )
    };
}
impl crate::rcc::RccPeripheral for peripherals::FDCAN1 {}
impl crate::rcc::SealedRccPeripheral for peripherals::FDCAN2 {
    fn frequency() -> crate::time::Hertz {
        match crate::pac::RCC.d2ccip1r().read().fdcansel() {
            crate::pac::rcc::vals::Fdcansel::HSE => unsafe {
                unwrap ! (crate :: rcc :: get_freqs () . hse . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "FDCAN2" , "hse")
            },
            crate::pac::rcc::vals::Fdcansel::PLL1_Q => unsafe {
                unwrap ! (crate :: rcc :: get_freqs () . pll1_q . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "FDCAN2" , "pll1_q")
            },
            crate::pac::rcc::vals::Fdcansel::PLL2_Q => unsafe {
                unwrap ! (crate :: rcc :: get_freqs () . pll2_q . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "FDCAN2" , "pll2_q")
            },
            #[allow(unreachable_patterns)]
            _ => panic!(
                "attempted to use peripheral '{}' but its clock mux is not set to a valid \
                         clock. Change 'config.rcc.mux' to another clock.",
                "FDCAN2"
            ),
        }
    }
    fn bus_frequency() -> crate::time::Hertz {
        unsafe {
            unwrap ! (crate :: rcc :: get_freqs () . pclk1 . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "FDCAN2" , "pclk1")
        }
    }
    const RCC_INFO: crate::rcc::RccInfo = unsafe {
        crate::rcc::RccInfo::new(
            Some((37u8, 8u8)),
            (59u8, 8u8),
            Some(1u8),
            #[cfg(feature = "low-power")]
            crate::rcc::StopMode::Stop1,
        )
    };
}
impl crate::rcc::RccPeripheral for peripherals::FDCAN2 {}
impl crate::rcc::SealedRccPeripheral for peripherals::FDCAN3 {
    fn frequency() -> crate::time::Hertz {
        match crate::pac::RCC.d2ccip1r().read().fdcansel() {
            crate::pac::rcc::vals::Fdcansel::HSE => unsafe {
                unwrap ! (crate :: rcc :: get_freqs () . hse . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "FDCAN3" , "hse")
            },
            crate::pac::rcc::vals::Fdcansel::PLL1_Q => unsafe {
                unwrap ! (crate :: rcc :: get_freqs () . pll1_q . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "FDCAN3" , "pll1_q")
            },
            crate::pac::rcc::vals::Fdcansel::PLL2_Q => unsafe {
                unwrap ! (crate :: rcc :: get_freqs () . pll2_q . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "FDCAN3" , "pll2_q")
            },
            #[allow(unreachable_patterns)]
            _ => panic!(
                "attempted to use peripheral '{}' but its clock mux is not set to a valid \
                         clock. Change 'config.rcc.mux' to another clock.",
                "FDCAN3"
            ),
        }
    }
    fn bus_frequency() -> crate::time::Hertz {
        unsafe {
            unwrap ! (crate :: rcc :: get_freqs () . pclk1 . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "FDCAN3" , "pclk1")
        }
    }
    const RCC_INFO: crate::rcc::RccInfo = unsafe {
        crate::rcc::RccInfo::new(
            Some((37u8, 8u8)),
            (59u8, 8u8),
            Some(1u8),
            #[cfg(feature = "low-power")]
            crate::rcc::StopMode::Stop1,
        )
    };
}
impl crate::rcc::RccPeripheral for peripherals::FDCAN3 {}
impl crate::rcc::SealedRccPeripheral for peripherals::FMAC {
    fn frequency() -> crate::time::Hertz {
        unsafe {
            unwrap ! (crate :: rcc :: get_freqs () . hclk2 . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "FMAC" , "hclk2")
        }
    }
    fn bus_frequency() -> crate::time::Hertz {
        unsafe {
            unwrap ! (crate :: rcc :: get_freqs () . hclk2 . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "FMAC" , "hclk2")
        }
    }
    const RCC_INFO: crate::rcc::RccInfo = unsafe {
        crate::rcc::RccInfo::new(
            Some((33u8, 16u8)),
            (55u8, 16u8),
            None,
            #[cfg(feature = "low-power")]
            crate::rcc::StopMode::Stop1,
        )
    };
}
impl crate::rcc::RccPeripheral for peripherals::FMAC {}
impl crate::rcc::SealedRccPeripheral for peripherals::FMC {
    fn frequency() -> crate::time::Hertz {
        match crate::pac::RCC.d1ccipr().read().fmcsel() {
            crate::pac::rcc::vals::Fmcsel::HCLK3 => unsafe {
                unwrap ! (crate :: rcc :: get_freqs () . hclk3 . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "FMC" , "hclk3")
            },
            crate::pac::rcc::vals::Fmcsel::PLL1_Q => unsafe {
                unwrap ! (crate :: rcc :: get_freqs () . pll1_q . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "FMC" , "pll1_q")
            },
            crate::pac::rcc::vals::Fmcsel::PLL2_R => unsafe {
                unwrap ! (crate :: rcc :: get_freqs () . pll2_r . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "FMC" , "pll2_r")
            },
            crate::pac::rcc::vals::Fmcsel::PER => match crate::pac::RCC.d1ccipr().read().persel() {
                crate::pac::rcc::vals::Persel::HSI => unsafe {
                    unwrap ! (crate :: rcc :: get_freqs () . hsi . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "FMC" , "hsi")
                },
                crate::pac::rcc::vals::Persel::CSI => unsafe {
                    unwrap ! (crate :: rcc :: get_freqs () . csi . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "FMC" , "csi")
                },
                crate::pac::rcc::vals::Persel::HSE => unsafe {
                    unwrap ! (crate :: rcc :: get_freqs () . hse . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "FMC" , "hse")
                },
                #[allow(unreachable_patterns)]
                _ => panic!(
                    "attempted to use peripheral '{}' but its clock mux is not set to a valid \
                         clock. Change 'config.rcc.mux' to another clock.",
                    "FMC"
                ),
            },
            #[allow(unreachable_patterns)]
            _ => panic!(
                "attempted to use peripheral '{}' but its clock mux is not set to a valid \
                         clock. Change 'config.rcc.mux' to another clock.",
                "FMC"
            ),
        }
    }
    fn bus_frequency() -> crate::time::Hertz {
        unsafe {
            unwrap ! (crate :: rcc :: get_freqs () . hclk3 . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "FMC" , "hclk3")
        }
    }
    const RCC_INFO: crate::rcc::RccInfo = unsafe {
        crate::rcc::RccInfo::new(
            Some((31u8, 12u8)),
            (53u8, 12u8),
            None,
            #[cfg(feature = "low-power")]
            crate::rcc::StopMode::Stop1,
        )
    };
}
impl crate::rcc::RccPeripheral for peripherals::FMC {}
impl crate::rcc::SealedRccPeripheral for peripherals::I2C1 {
    fn frequency() -> crate::time::Hertz {
        match crate::pac::RCC.d2ccip2r().read().i2c1235sel() {
            crate::pac::rcc::vals::I2c1235sel::PCLK1 => unsafe {
                unwrap ! (crate :: rcc :: get_freqs () . pclk1 . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "I2C1" , "pclk1")
            },
            crate::pac::rcc::vals::I2c1235sel::PLL3_R => unsafe {
                unwrap ! (crate :: rcc :: get_freqs () . pll3_r . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "I2C1" , "pll3_r")
            },
            crate::pac::rcc::vals::I2c1235sel::HSI => unsafe {
                unwrap ! (crate :: rcc :: get_freqs () . hsi . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "I2C1" , "hsi")
            },
            crate::pac::rcc::vals::I2c1235sel::CSI => unsafe {
                unwrap ! (crate :: rcc :: get_freqs () . csi . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "I2C1" , "csi")
            },
            #[allow(unreachable_patterns)]
            _ => panic!(
                "attempted to use peripheral '{}' but its clock mux is not set to a valid \
                         clock. Change 'config.rcc.mux' to another clock.",
                "I2C1"
            ),
        }
    }
    fn bus_frequency() -> crate::time::Hertz {
        unsafe {
            unwrap ! (crate :: rcc :: get_freqs () . pclk1 . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "I2C1" , "pclk1")
        }
    }
    const RCC_INFO: crate::rcc::RccInfo = unsafe {
        crate::rcc::RccInfo::new(
            Some((36u8, 21u8)),
            (58u8, 21u8),
            None,
            #[cfg(feature = "low-power")]
            crate::rcc::StopMode::Stop1,
        )
    };
}
impl crate::rcc::RccPeripheral for peripherals::I2C1 {}
impl crate::rcc::SealedRccPeripheral for peripherals::I2C2 {
    fn frequency() -> crate::time::Hertz {
        match crate::pac::RCC.d2ccip2r().read().i2c1235sel() {
            crate::pac::rcc::vals::I2c1235sel::PCLK1 => unsafe {
                unwrap ! (crate :: rcc :: get_freqs () . pclk1 . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "I2C2" , "pclk1")
            },
            crate::pac::rcc::vals::I2c1235sel::PLL3_R => unsafe {
                unwrap ! (crate :: rcc :: get_freqs () . pll3_r . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "I2C2" , "pll3_r")
            },
            crate::pac::rcc::vals::I2c1235sel::HSI => unsafe {
                unwrap ! (crate :: rcc :: get_freqs () . hsi . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "I2C2" , "hsi")
            },
            crate::pac::rcc::vals::I2c1235sel::CSI => unsafe {
                unwrap ! (crate :: rcc :: get_freqs () . csi . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "I2C2" , "csi")
            },
            #[allow(unreachable_patterns)]
            _ => panic!(
                "attempted to use peripheral '{}' but its clock mux is not set to a valid \
                         clock. Change 'config.rcc.mux' to another clock.",
                "I2C2"
            ),
        }
    }
    fn bus_frequency() -> crate::time::Hertz {
        unsafe {
            unwrap ! (crate :: rcc :: get_freqs () . pclk1 . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "I2C2" , "pclk1")
        }
    }
    const RCC_INFO: crate::rcc::RccInfo = unsafe {
        crate::rcc::RccInfo::new(
            Some((36u8, 22u8)),
            (58u8, 22u8),
            None,
            #[cfg(feature = "low-power")]
            crate::rcc::StopMode::Stop1,
        )
    };
}
impl crate::rcc::RccPeripheral for peripherals::I2C2 {}
impl crate::rcc::SealedRccPeripheral for peripherals::I2C3 {
    fn frequency() -> crate::time::Hertz {
        match crate::pac::RCC.d2ccip2r().read().i2c1235sel() {
            crate::pac::rcc::vals::I2c1235sel::PCLK1 => unsafe {
                unwrap ! (crate :: rcc :: get_freqs () . pclk1 . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "I2C3" , "pclk1")
            },
            crate::pac::rcc::vals::I2c1235sel::PLL3_R => unsafe {
                unwrap ! (crate :: rcc :: get_freqs () . pll3_r . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "I2C3" , "pll3_r")
            },
            crate::pac::rcc::vals::I2c1235sel::HSI => unsafe {
                unwrap ! (crate :: rcc :: get_freqs () . hsi . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "I2C3" , "hsi")
            },
            crate::pac::rcc::vals::I2c1235sel::CSI => unsafe {
                unwrap ! (crate :: rcc :: get_freqs () . csi . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "I2C3" , "csi")
            },
            #[allow(unreachable_patterns)]
            _ => panic!(
                "attempted to use peripheral '{}' but its clock mux is not set to a valid \
                         clock. Change 'config.rcc.mux' to another clock.",
                "I2C3"
            ),
        }
    }
    fn bus_frequency() -> crate::time::Hertz {
        unsafe {
            unwrap ! (crate :: rcc :: get_freqs () . pclk1 . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "I2C3" , "pclk1")
        }
    }
    const RCC_INFO: crate::rcc::RccInfo = unsafe {
        crate::rcc::RccInfo::new(
            Some((36u8, 23u8)),
            (58u8, 23u8),
            None,
            #[cfg(feature = "low-power")]
            crate::rcc::StopMode::Stop1,
        )
    };
}
impl crate::rcc::RccPeripheral for peripherals::I2C3 {}
impl crate::rcc::SealedRccPeripheral for peripherals::I2C4 {
    fn frequency() -> crate::time::Hertz {
        match crate::pac::RCC.d3ccipr().read().i2c4sel() {
            crate::pac::rcc::vals::I2c4sel::PCLK4 => unsafe {
                unwrap ! (crate :: rcc :: get_freqs () . pclk4 . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "I2C4" , "pclk4")
            },
            crate::pac::rcc::vals::I2c4sel::PLL3_R => unsafe {
                unwrap ! (crate :: rcc :: get_freqs () . pll3_r . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "I2C4" , "pll3_r")
            },
            crate::pac::rcc::vals::I2c4sel::HSI => unsafe {
                unwrap ! (crate :: rcc :: get_freqs () . hsi . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "I2C4" , "hsi")
            },
            crate::pac::rcc::vals::I2c4sel::CSI => unsafe {
                unwrap ! (crate :: rcc :: get_freqs () . csi . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "I2C4" , "csi")
            },
            #[allow(unreachable_patterns)]
            _ => panic!(
                "attempted to use peripheral '{}' but its clock mux is not set to a valid \
                         clock. Change 'config.rcc.mux' to another clock.",
                "I2C4"
            ),
        }
    }
    fn bus_frequency() -> crate::time::Hertz {
        unsafe {
            unwrap ! (crate :: rcc :: get_freqs () . pclk4 . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "I2C4" , "pclk4")
        }
    }
    const RCC_INFO: crate::rcc::RccInfo = unsafe {
        crate::rcc::RccInfo::new(
            Some((39u8, 7u8)),
            (61u8, 7u8),
            None,
            #[cfg(feature = "low-power")]
            crate::rcc::StopMode::Stop1,
        )
    };
}
impl crate::rcc::RccPeripheral for peripherals::I2C4 {}
impl crate::rcc::SealedRccPeripheral for peripherals::I2C5 {
    fn frequency() -> crate::time::Hertz {
        match crate::pac::RCC.d2ccip2r().read().i2c1235sel() {
            crate::pac::rcc::vals::I2c1235sel::PCLK1 => unsafe {
                unwrap ! (crate :: rcc :: get_freqs () . pclk1 . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "I2C5" , "pclk1")
            },
            crate::pac::rcc::vals::I2c1235sel::PLL3_R => unsafe {
                unwrap ! (crate :: rcc :: get_freqs () . pll3_r . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "I2C5" , "pll3_r")
            },
            crate::pac::rcc::vals::I2c1235sel::HSI => unsafe {
                unwrap ! (crate :: rcc :: get_freqs () . hsi . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "I2C5" , "hsi")
            },
            crate::pac::rcc::vals::I2c1235sel::CSI => unsafe {
                unwrap ! (crate :: rcc :: get_freqs () . csi . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "I2C5" , "csi")
            },
            #[allow(unreachable_patterns)]
            _ => panic!(
                "attempted to use peripheral '{}' but its clock mux is not set to a valid \
                         clock. Change 'config.rcc.mux' to another clock.",
                "I2C5"
            ),
        }
    }
    fn bus_frequency() -> crate::time::Hertz {
        unsafe {
            unwrap ! (crate :: rcc :: get_freqs () . pclk1 . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "I2C5" , "pclk1")
        }
    }
    const RCC_INFO: crate::rcc::RccInfo = unsafe {
        crate::rcc::RccInfo::new(
            Some((36u8, 25u8)),
            (58u8, 25u8),
            None,
            #[cfg(feature = "low-power")]
            crate::rcc::StopMode::Stop1,
        )
    };
}
impl crate::rcc::RccPeripheral for peripherals::I2C5 {}
impl crate::rcc::SealedRccPeripheral for peripherals::LPTIM1 {
    fn frequency() -> crate::time::Hertz {
        match crate::pac::RCC.d2ccip2r().read().lptim1sel() {
            crate::pac::rcc::vals::Lptim1sel::PCLK1 => unsafe {
                unwrap ! (crate :: rcc :: get_freqs () . pclk1 . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "LPTIM1" , "pclk1")
            },
            crate::pac::rcc::vals::Lptim1sel::PLL2_P => unsafe {
                unwrap ! (crate :: rcc :: get_freqs () . pll2_p . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "LPTIM1" , "pll2_p")
            },
            crate::pac::rcc::vals::Lptim1sel::PLL3_R => unsafe {
                unwrap ! (crate :: rcc :: get_freqs () . pll3_r . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "LPTIM1" , "pll3_r")
            },
            crate::pac::rcc::vals::Lptim1sel::LSE => unsafe {
                unwrap ! (crate :: rcc :: get_freqs () . lse . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "LPTIM1" , "lse")
            },
            crate::pac::rcc::vals::Lptim1sel::LSI => unsafe {
                unwrap ! (crate :: rcc :: get_freqs () . lsi . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "LPTIM1" , "lsi")
            },
            crate::pac::rcc::vals::Lptim1sel::PER => {
                match crate::pac::RCC.d1ccipr().read().persel() {
                    crate::pac::rcc::vals::Persel::HSI => unsafe {
                        unwrap ! (crate :: rcc :: get_freqs () . hsi . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "LPTIM1" , "hsi")
                    },
                    crate::pac::rcc::vals::Persel::CSI => unsafe {
                        unwrap ! (crate :: rcc :: get_freqs () . csi . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "LPTIM1" , "csi")
                    },
                    crate::pac::rcc::vals::Persel::HSE => unsafe {
                        unwrap ! (crate :: rcc :: get_freqs () . hse . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "LPTIM1" , "hse")
                    },
                    #[allow(unreachable_patterns)]
                    _ => panic!(
                        "attempted to use peripheral '{}' but its clock mux is not set to a valid \
                         clock. Change 'config.rcc.mux' to another clock.",
                        "LPTIM1"
                    ),
                }
            }
            #[allow(unreachable_patterns)]
            _ => panic!(
                "attempted to use peripheral '{}' but its clock mux is not set to a valid \
                         clock. Change 'config.rcc.mux' to another clock.",
                "LPTIM1"
            ),
        }
    }
    fn bus_frequency() -> crate::time::Hertz {
        unsafe {
            unwrap ! (crate :: rcc :: get_freqs () . pclk1 . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "LPTIM1" , "pclk1")
        }
    }
    const RCC_INFO: crate::rcc::RccInfo = unsafe {
        crate::rcc::RccInfo::new(
            Some((36u8, 9u8)),
            (58u8, 9u8),
            None,
            #[cfg(feature = "low-power")]
            crate::rcc::StopMode::Stop2,
        )
    };
}
impl crate::rcc::RccPeripheral for peripherals::LPTIM1 {}
impl crate::rcc::SealedRccPeripheral for peripherals::LPTIM2 {
    fn frequency() -> crate::time::Hertz {
        match crate::pac::RCC.d3ccipr().read().lptim2sel() {
            crate::pac::rcc::vals::Lptim2sel::PCLK4 => unsafe {
                unwrap ! (crate :: rcc :: get_freqs () . pclk4 . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "LPTIM2" , "pclk4")
            },
            crate::pac::rcc::vals::Lptim2sel::PLL2_P => unsafe {
                unwrap ! (crate :: rcc :: get_freqs () . pll2_p . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "LPTIM2" , "pll2_p")
            },
            crate::pac::rcc::vals::Lptim2sel::PLL3_R => unsafe {
                unwrap ! (crate :: rcc :: get_freqs () . pll3_r . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "LPTIM2" , "pll3_r")
            },
            crate::pac::rcc::vals::Lptim2sel::LSE => unsafe {
                unwrap ! (crate :: rcc :: get_freqs () . lse . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "LPTIM2" , "lse")
            },
            crate::pac::rcc::vals::Lptim2sel::LSI => unsafe {
                unwrap ! (crate :: rcc :: get_freqs () . lsi . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "LPTIM2" , "lsi")
            },
            crate::pac::rcc::vals::Lptim2sel::PER => {
                match crate::pac::RCC.d1ccipr().read().persel() {
                    crate::pac::rcc::vals::Persel::HSI => unsafe {
                        unwrap ! (crate :: rcc :: get_freqs () . hsi . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "LPTIM2" , "hsi")
                    },
                    crate::pac::rcc::vals::Persel::CSI => unsafe {
                        unwrap ! (crate :: rcc :: get_freqs () . csi . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "LPTIM2" , "csi")
                    },
                    crate::pac::rcc::vals::Persel::HSE => unsafe {
                        unwrap ! (crate :: rcc :: get_freqs () . hse . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "LPTIM2" , "hse")
                    },
                    #[allow(unreachable_patterns)]
                    _ => panic!(
                        "attempted to use peripheral '{}' but its clock mux is not set to a valid \
                         clock. Change 'config.rcc.mux' to another clock.",
                        "LPTIM2"
                    ),
                }
            }
            #[allow(unreachable_patterns)]
            _ => panic!(
                "attempted to use peripheral '{}' but its clock mux is not set to a valid \
                         clock. Change 'config.rcc.mux' to another clock.",
                "LPTIM2"
            ),
        }
    }
    fn bus_frequency() -> crate::time::Hertz {
        unsafe {
            unwrap ! (crate :: rcc :: get_freqs () . pclk4 . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "LPTIM2" , "pclk4")
        }
    }
    const RCC_INFO: crate::rcc::RccInfo = unsafe {
        crate::rcc::RccInfo::new(
            Some((39u8, 9u8)),
            (61u8, 9u8),
            None,
            #[cfg(feature = "low-power")]
            crate::rcc::StopMode::Stop2,
        )
    };
}
impl crate::rcc::RccPeripheral for peripherals::LPTIM2 {}
impl crate::rcc::SealedRccPeripheral for peripherals::LPTIM3 {
    fn frequency() -> crate::time::Hertz {
        unsafe {
            unwrap ! (crate :: rcc :: get_freqs () . pclk4 . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "LPTIM3" , "pclk4")
        }
    }
    fn bus_frequency() -> crate::time::Hertz {
        unsafe {
            unwrap ! (crate :: rcc :: get_freqs () . pclk4 . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "LPTIM3" , "pclk4")
        }
    }
    const RCC_INFO: crate::rcc::RccInfo = unsafe {
        crate::rcc::RccInfo::new(
            Some((39u8, 10u8)),
            (61u8, 10u8),
            None,
            #[cfg(feature = "low-power")]
            crate::rcc::StopMode::Stop2,
        )
    };
}
impl crate::rcc::RccPeripheral for peripherals::LPTIM3 {}
impl crate::rcc::SealedRccPeripheral for peripherals::LPTIM4 {
    fn frequency() -> crate::time::Hertz {
        unsafe {
            unwrap ! (crate :: rcc :: get_freqs () . pclk4 . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "LPTIM4" , "pclk4")
        }
    }
    fn bus_frequency() -> crate::time::Hertz {
        unsafe {
            unwrap ! (crate :: rcc :: get_freqs () . pclk4 . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "LPTIM4" , "pclk4")
        }
    }
    const RCC_INFO: crate::rcc::RccInfo = unsafe {
        crate::rcc::RccInfo::new(
            Some((39u8, 11u8)),
            (61u8, 11u8),
            None,
            #[cfg(feature = "low-power")]
            crate::rcc::StopMode::Stop2,
        )
    };
}
impl crate::rcc::RccPeripheral for peripherals::LPTIM4 {}
impl crate::rcc::SealedRccPeripheral for peripherals::LPTIM5 {
    fn frequency() -> crate::time::Hertz {
        unsafe {
            unwrap ! (crate :: rcc :: get_freqs () . pclk4 . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "LPTIM5" , "pclk4")
        }
    }
    fn bus_frequency() -> crate::time::Hertz {
        unsafe {
            unwrap ! (crate :: rcc :: get_freqs () . pclk4 . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "LPTIM5" , "pclk4")
        }
    }
    const RCC_INFO: crate::rcc::RccInfo = unsafe {
        crate::rcc::RccInfo::new(
            Some((39u8, 12u8)),
            (61u8, 12u8),
            None,
            #[cfg(feature = "low-power")]
            crate::rcc::StopMode::Stop2,
        )
    };
}
impl crate::rcc::RccPeripheral for peripherals::LPTIM5 {}
impl crate::rcc::SealedRccPeripheral for peripherals::LPUART1 {
    fn frequency() -> crate::time::Hertz {
        match crate::pac::RCC.d3ccipr().read().lpuart1sel() {
            crate::pac::rcc::vals::Lpuartsel::PCLK4 => unsafe {
                unwrap ! (crate :: rcc :: get_freqs () . pclk4 . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "LPUART1" , "pclk4")
            },
            crate::pac::rcc::vals::Lpuartsel::PLL2_Q => unsafe {
                unwrap ! (crate :: rcc :: get_freqs () . pll2_q . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "LPUART1" , "pll2_q")
            },
            crate::pac::rcc::vals::Lpuartsel::PLL3_Q => unsafe {
                unwrap ! (crate :: rcc :: get_freqs () . pll3_q . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "LPUART1" , "pll3_q")
            },
            crate::pac::rcc::vals::Lpuartsel::HSI => unsafe {
                unwrap ! (crate :: rcc :: get_freqs () . hsi . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "LPUART1" , "hsi")
            },
            crate::pac::rcc::vals::Lpuartsel::CSI => unsafe {
                unwrap ! (crate :: rcc :: get_freqs () . csi . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "LPUART1" , "csi")
            },
            crate::pac::rcc::vals::Lpuartsel::LSE => unsafe {
                unwrap ! (crate :: rcc :: get_freqs () . lse . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "LPUART1" , "lse")
            },
            #[allow(unreachable_patterns)]
            _ => panic!(
                "attempted to use peripheral '{}' but its clock mux is not set to a valid \
                         clock. Change 'config.rcc.mux' to another clock.",
                "LPUART1"
            ),
        }
    }
    fn bus_frequency() -> crate::time::Hertz {
        unsafe {
            unwrap ! (crate :: rcc :: get_freqs () . pclk4 . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "LPUART1" , "pclk4")
        }
    }
    const RCC_INFO: crate::rcc::RccInfo = unsafe {
        crate::rcc::RccInfo::new(
            Some((39u8, 3u8)),
            (61u8, 3u8),
            None,
            #[cfg(feature = "low-power")]
            crate::rcc::StopMode::Stop2,
        )
    };
}
impl crate::rcc::RccPeripheral for peripherals::LPUART1 {}
impl crate::rcc::SealedRccPeripheral for peripherals::LTDC {
    fn frequency() -> crate::time::Hertz {
        unsafe {
            unwrap ! (crate :: rcc :: get_freqs () . pclk3 . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "LTDC" , "pclk3")
        }
    }
    fn bus_frequency() -> crate::time::Hertz {
        unsafe {
            unwrap ! (crate :: rcc :: get_freqs () . pclk3 . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "LTDC" , "pclk3")
        }
    }
    const RCC_INFO: crate::rcc::RccInfo = unsafe {
        crate::rcc::RccInfo::new(
            Some((35u8, 3u8)),
            (57u8, 3u8),
            None,
            #[cfg(feature = "low-power")]
            crate::rcc::StopMode::Stop1,
        )
    };
}
impl crate::rcc::RccPeripheral for peripherals::LTDC {}
impl crate::rcc::SealedRccPeripheral for peripherals::MDIOS {
    fn frequency() -> crate::time::Hertz {
        unsafe {
            unwrap ! (crate :: rcc :: get_freqs () . pclk1 . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "MDIOS" , "pclk1")
        }
    }
    fn bus_frequency() -> crate::time::Hertz {
        unsafe {
            unwrap ! (crate :: rcc :: get_freqs () . pclk1 . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "MDIOS" , "pclk1")
        }
    }
    const RCC_INFO: crate::rcc::RccInfo = unsafe {
        crate::rcc::RccInfo::new(
            Some((37u8, 5u8)),
            (59u8, 5u8),
            None,
            #[cfg(feature = "low-power")]
            crate::rcc::StopMode::Stop1,
        )
    };
}
impl crate::rcc::RccPeripheral for peripherals::MDIOS {}
impl crate::rcc::SealedRccPeripheral for peripherals::OCTOSPI1 {
    fn frequency() -> crate::time::Hertz {
        match crate::pac::RCC.d1ccipr().read().octospisel() {
            crate::pac::rcc::vals::Fmcsel::HCLK3 => unsafe {
                unwrap ! (crate :: rcc :: get_freqs () . hclk3 . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "OCTOSPI1" , "hclk3")
            },
            crate::pac::rcc::vals::Fmcsel::PLL1_Q => unsafe {
                unwrap ! (crate :: rcc :: get_freqs () . pll1_q . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "OCTOSPI1" , "pll1_q")
            },
            crate::pac::rcc::vals::Fmcsel::PLL2_R => unsafe {
                unwrap ! (crate :: rcc :: get_freqs () . pll2_r . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "OCTOSPI1" , "pll2_r")
            },
            crate::pac::rcc::vals::Fmcsel::PER => match crate::pac::RCC.d1ccipr().read().persel() {
                crate::pac::rcc::vals::Persel::HSI => unsafe {
                    unwrap ! (crate :: rcc :: get_freqs () . hsi . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "OCTOSPI1" , "hsi")
                },
                crate::pac::rcc::vals::Persel::CSI => unsafe {
                    unwrap ! (crate :: rcc :: get_freqs () . csi . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "OCTOSPI1" , "csi")
                },
                crate::pac::rcc::vals::Persel::HSE => unsafe {
                    unwrap ! (crate :: rcc :: get_freqs () . hse . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "OCTOSPI1" , "hse")
                },
                #[allow(unreachable_patterns)]
                _ => panic!(
                    "attempted to use peripheral '{}' but its clock mux is not set to a valid \
                         clock. Change 'config.rcc.mux' to another clock.",
                    "OCTOSPI1"
                ),
            },
            #[allow(unreachable_patterns)]
            _ => panic!(
                "attempted to use peripheral '{}' but its clock mux is not set to a valid \
                         clock. Change 'config.rcc.mux' to another clock.",
                "OCTOSPI1"
            ),
        }
    }
    fn bus_frequency() -> crate::time::Hertz {
        unsafe {
            unwrap ! (crate :: rcc :: get_freqs () . hclk3 . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "OCTOSPI1" , "hclk3")
        }
    }
    const RCC_INFO: crate::rcc::RccInfo = unsafe {
        crate::rcc::RccInfo::new(
            Some((31u8, 14u8)),
            (53u8, 14u8),
            None,
            #[cfg(feature = "low-power")]
            crate::rcc::StopMode::Stop1,
        )
    };
}
impl crate::rcc::RccPeripheral for peripherals::OCTOSPI1 {}
impl crate::rcc::SealedRccPeripheral for peripherals::OCTOSPI2 {
    fn frequency() -> crate::time::Hertz {
        match crate::pac::RCC.d1ccipr().read().octospisel() {
            crate::pac::rcc::vals::Fmcsel::HCLK3 => unsafe {
                unwrap ! (crate :: rcc :: get_freqs () . hclk3 . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "OCTOSPI2" , "hclk3")
            },
            crate::pac::rcc::vals::Fmcsel::PLL1_Q => unsafe {
                unwrap ! (crate :: rcc :: get_freqs () . pll1_q . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "OCTOSPI2" , "pll1_q")
            },
            crate::pac::rcc::vals::Fmcsel::PLL2_R => unsafe {
                unwrap ! (crate :: rcc :: get_freqs () . pll2_r . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "OCTOSPI2" , "pll2_r")
            },
            crate::pac::rcc::vals::Fmcsel::PER => match crate::pac::RCC.d1ccipr().read().persel() {
                crate::pac::rcc::vals::Persel::HSI => unsafe {
                    unwrap ! (crate :: rcc :: get_freqs () . hsi . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "OCTOSPI2" , "hsi")
                },
                crate::pac::rcc::vals::Persel::CSI => unsafe {
                    unwrap ! (crate :: rcc :: get_freqs () . csi . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "OCTOSPI2" , "csi")
                },
                crate::pac::rcc::vals::Persel::HSE => unsafe {
                    unwrap ! (crate :: rcc :: get_freqs () . hse . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "OCTOSPI2" , "hse")
                },
                #[allow(unreachable_patterns)]
                _ => panic!(
                    "attempted to use peripheral '{}' but its clock mux is not set to a valid \
                         clock. Change 'config.rcc.mux' to another clock.",
                    "OCTOSPI2"
                ),
            },
            #[allow(unreachable_patterns)]
            _ => panic!(
                "attempted to use peripheral '{}' but its clock mux is not set to a valid \
                         clock. Change 'config.rcc.mux' to another clock.",
                "OCTOSPI2"
            ),
        }
    }
    fn bus_frequency() -> crate::time::Hertz {
        unsafe {
            unwrap ! (crate :: rcc :: get_freqs () . hclk3 . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "OCTOSPI2" , "hclk3")
        }
    }
    const RCC_INFO: crate::rcc::RccInfo = unsafe {
        crate::rcc::RccInfo::new(
            Some((31u8, 19u8)),
            (53u8, 19u8),
            None,
            #[cfg(feature = "low-power")]
            crate::rcc::StopMode::Stop1,
        )
    };
}
impl crate::rcc::RccPeripheral for peripherals::OCTOSPI2 {}
impl crate::rcc::SealedRccPeripheral for peripherals::OPAMP1 {
    fn frequency() -> crate::time::Hertz {
        unsafe {
            unwrap ! (crate :: rcc :: get_freqs () . pclk1 . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "OPAMP1" , "pclk1")
        }
    }
    fn bus_frequency() -> crate::time::Hertz {
        unsafe {
            unwrap ! (crate :: rcc :: get_freqs () . pclk1 . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "OPAMP1" , "pclk1")
        }
    }
    const RCC_INFO: crate::rcc::RccInfo = unsafe {
        crate::rcc::RccInfo::new(
            Some((37u8, 4u8)),
            (59u8, 4u8),
            Some(2u8),
            #[cfg(feature = "low-power")]
            crate::rcc::StopMode::Stop1,
        )
    };
}
impl crate::rcc::RccPeripheral for peripherals::OPAMP1 {}
impl crate::rcc::SealedRccPeripheral for peripherals::OPAMP2 {
    fn frequency() -> crate::time::Hertz {
        unsafe {
            unwrap ! (crate :: rcc :: get_freqs () . pclk1 . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "OPAMP2" , "pclk1")
        }
    }
    fn bus_frequency() -> crate::time::Hertz {
        unsafe {
            unwrap ! (crate :: rcc :: get_freqs () . pclk1 . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "OPAMP2" , "pclk1")
        }
    }
    const RCC_INFO: crate::rcc::RccInfo = unsafe {
        crate::rcc::RccInfo::new(
            Some((37u8, 4u8)),
            (59u8, 4u8),
            Some(2u8),
            #[cfg(feature = "low-power")]
            crate::rcc::StopMode::Stop1,
        )
    };
}
impl crate::rcc::RccPeripheral for peripherals::OPAMP2 {}
impl crate::rcc::SealedRccPeripheral for peripherals::RNG {
    fn frequency() -> crate::time::Hertz {
        match crate::pac::RCC.d2ccip2r().read().rngsel() {
            crate::pac::rcc::vals::Rngsel::HSI48 => unsafe {
                unwrap ! (crate :: rcc :: get_freqs () . hsi48 . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "RNG" , "hsi48")
            },
            crate::pac::rcc::vals::Rngsel::PLL1_Q => unsafe {
                unwrap ! (crate :: rcc :: get_freqs () . pll1_q . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "RNG" , "pll1_q")
            },
            crate::pac::rcc::vals::Rngsel::LSE => unsafe {
                unwrap ! (crate :: rcc :: get_freqs () . lse . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "RNG" , "lse")
            },
            crate::pac::rcc::vals::Rngsel::LSI => unsafe {
                unwrap ! (crate :: rcc :: get_freqs () . lsi . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "RNG" , "lsi")
            },
            #[allow(unreachable_patterns)]
            _ => panic!(
                "attempted to use peripheral '{}' but its clock mux is not set to a valid \
                         clock. Change 'config.rcc.mux' to another clock.",
                "RNG"
            ),
        }
    }
    fn bus_frequency() -> crate::time::Hertz {
        unsafe {
            unwrap ! (crate :: rcc :: get_freqs () . hclk2 . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "RNG" , "hclk2")
        }
    }
    const RCC_INFO: crate::rcc::RccInfo = unsafe {
        crate::rcc::RccInfo::new(
            Some((33u8, 6u8)),
            (55u8, 6u8),
            None,
            #[cfg(feature = "low-power")]
            crate::rcc::StopMode::Stop1,
        )
    };
}
impl crate::rcc::RccPeripheral for peripherals::RNG {}
impl crate::rcc::SealedRccPeripheral for peripherals::RTC {
    fn frequency() -> crate::time::Hertz {
        match crate::pac::RCC.bdcr().read().rtcsel() {
            crate::pac::rcc::vals::Rtcsel::LSE => unsafe {
                unwrap ! (crate :: rcc :: get_freqs () . lse . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "RTC" , "lse")
            },
            crate::pac::rcc::vals::Rtcsel::LSI => unsafe {
                unwrap ! (crate :: rcc :: get_freqs () . lsi . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "RTC" , "lsi")
            },
            crate::pac::rcc::vals::Rtcsel::HSE => unsafe {
                unwrap ! (crate :: rcc :: get_freqs () . hse . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "RTC" , "hse")
            },
            #[allow(unreachable_patterns)]
            _ => panic!(
                "attempted to use peripheral '{}' but its clock mux is not set to a valid \
                         clock. Change 'config.rcc.mux' to another clock.",
                "RTC"
            ),
        }
    }
    fn bus_frequency() -> crate::time::Hertz {
        unsafe {
            unwrap ! (crate :: rcc :: get_freqs () . pclk4 . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "RTC" , "pclk4")
        }
    }
    const RCC_INFO: crate::rcc::RccInfo = unsafe {
        crate::rcc::RccInfo::new(
            None,
            (61u8, 16u8),
            None,
            #[cfg(feature = "low-power")]
            crate::rcc::StopMode::Standby,
        )
    };
}
impl crate::rcc::RccPeripheral for peripherals::RTC {}
impl crate::rcc::SealedRccPeripheral for peripherals::SAI1 {
    fn frequency() -> crate::time::Hertz {
        match crate::pac::RCC.d2ccip1r().read().sai1sel() {
            crate::pac::rcc::vals::Saisel::PLL1_Q => unsafe {
                unwrap ! (crate :: rcc :: get_freqs () . pll1_q . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "SAI1" , "pll1_q")
            },
            crate::pac::rcc::vals::Saisel::PLL2_P => unsafe {
                unwrap ! (crate :: rcc :: get_freqs () . pll2_p . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "SAI1" , "pll2_p")
            },
            crate::pac::rcc::vals::Saisel::PLL3_P => unsafe {
                unwrap ! (crate :: rcc :: get_freqs () . pll3_p . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "SAI1" , "pll3_p")
            },
            crate::pac::rcc::vals::Saisel::I2S_CKIN => unsafe {
                unwrap ! (crate :: rcc :: get_freqs () . i2s_ckin . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "SAI1" , "i2s_ckin")
            },
            crate::pac::rcc::vals::Saisel::PER => match crate::pac::RCC.d1ccipr().read().persel() {
                crate::pac::rcc::vals::Persel::HSI => unsafe {
                    unwrap ! (crate :: rcc :: get_freqs () . hsi . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "SAI1" , "hsi")
                },
                crate::pac::rcc::vals::Persel::CSI => unsafe {
                    unwrap ! (crate :: rcc :: get_freqs () . csi . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "SAI1" , "csi")
                },
                crate::pac::rcc::vals::Persel::HSE => unsafe {
                    unwrap ! (crate :: rcc :: get_freqs () . hse . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "SAI1" , "hse")
                },
                #[allow(unreachable_patterns)]
                _ => panic!(
                    "attempted to use peripheral '{}' but its clock mux is not set to a valid \
                         clock. Change 'config.rcc.mux' to another clock.",
                    "SAI1"
                ),
            },
            #[allow(unreachable_patterns)]
            _ => panic!(
                "attempted to use peripheral '{}' but its clock mux is not set to a valid \
                         clock. Change 'config.rcc.mux' to another clock.",
                "SAI1"
            ),
        }
    }
    fn bus_frequency() -> crate::time::Hertz {
        unsafe {
            unwrap ! (crate :: rcc :: get_freqs () . pclk2 . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "SAI1" , "pclk2")
        }
    }
    const RCC_INFO: crate::rcc::RccInfo = unsafe {
        crate::rcc::RccInfo::new(
            Some((38u8, 22u8)),
            (60u8, 22u8),
            None,
            #[cfg(feature = "low-power")]
            crate::rcc::StopMode::Stop1,
        )
    };
}
impl crate::rcc::RccPeripheral for peripherals::SAI1 {}
impl crate::rcc::SealedRccPeripheral for peripherals::SAI4 {
    fn frequency() -> crate::time::Hertz {
        unsafe {
            unwrap ! (crate :: rcc :: get_freqs () . pclk4 . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "SAI4" , "pclk4")
        }
    }
    fn bus_frequency() -> crate::time::Hertz {
        unsafe {
            unwrap ! (crate :: rcc :: get_freqs () . pclk4 . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "SAI4" , "pclk4")
        }
    }
    const RCC_INFO: crate::rcc::RccInfo = unsafe {
        crate::rcc::RccInfo::new(
            Some((39u8, 21u8)),
            (61u8, 21u8),
            None,
            #[cfg(feature = "low-power")]
            crate::rcc::StopMode::Stop1,
        )
    };
}
impl crate::rcc::RccPeripheral for peripherals::SAI4 {}
impl crate::rcc::SealedRccPeripheral for peripherals::SDMMC1 {
    fn frequency() -> crate::time::Hertz {
        match crate::pac::RCC.d1ccipr().read().sdmmcsel() {
            crate::pac::rcc::vals::Sdmmcsel::PLL1_Q => unsafe {
                unwrap ! (crate :: rcc :: get_freqs () . pll1_q . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "SDMMC1" , "pll1_q")
            },
            crate::pac::rcc::vals::Sdmmcsel::PLL2_R => unsafe {
                unwrap ! (crate :: rcc :: get_freqs () . pll2_r . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "SDMMC1" , "pll2_r")
            },
            #[allow(unreachable_patterns)]
            _ => panic!(
                "attempted to use peripheral '{}' but its clock mux is not set to a valid \
                         clock. Change 'config.rcc.mux' to another clock.",
                "SDMMC1"
            ),
        }
    }
    fn bus_frequency() -> crate::time::Hertz {
        unsafe {
            unwrap ! (crate :: rcc :: get_freqs () . hclk3 . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "SDMMC1" , "hclk3")
        }
    }
    const RCC_INFO: crate::rcc::RccInfo = unsafe {
        crate::rcc::RccInfo::new(
            Some((31u8, 16u8)),
            (53u8, 16u8),
            None,
            #[cfg(feature = "low-power")]
            crate::rcc::StopMode::Stop1,
        )
    };
}
impl crate::rcc::RccPeripheral for peripherals::SDMMC1 {}
impl crate::rcc::SealedRccPeripheral for peripherals::SDMMC2 {
    fn frequency() -> crate::time::Hertz {
        match crate::pac::RCC.d1ccipr().read().sdmmcsel() {
            crate::pac::rcc::vals::Sdmmcsel::PLL1_Q => unsafe {
                unwrap ! (crate :: rcc :: get_freqs () . pll1_q . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "SDMMC2" , "pll1_q")
            },
            crate::pac::rcc::vals::Sdmmcsel::PLL2_R => unsafe {
                unwrap ! (crate :: rcc :: get_freqs () . pll2_r . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "SDMMC2" , "pll2_r")
            },
            #[allow(unreachable_patterns)]
            _ => panic!(
                "attempted to use peripheral '{}' but its clock mux is not set to a valid \
                         clock. Change 'config.rcc.mux' to another clock.",
                "SDMMC2"
            ),
        }
    }
    fn bus_frequency() -> crate::time::Hertz {
        unsafe {
            unwrap ! (crate :: rcc :: get_freqs () . hclk2 . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "SDMMC2" , "hclk2")
        }
    }
    const RCC_INFO: crate::rcc::RccInfo = unsafe {
        crate::rcc::RccInfo::new(
            Some((33u8, 9u8)),
            (55u8, 9u8),
            None,
            #[cfg(feature = "low-power")]
            crate::rcc::StopMode::Stop1,
        )
    };
}
impl crate::rcc::RccPeripheral for peripherals::SDMMC2 {}
impl crate::rcc::SealedRccPeripheral for peripherals::SPDIFRX1 {
    fn frequency() -> crate::time::Hertz {
        match crate::pac::RCC.d2ccip1r().read().spdifrxsel() {
            crate::pac::rcc::vals::Spdifrxsel::PLL1_Q => unsafe {
                unwrap ! (crate :: rcc :: get_freqs () . pll1_q . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "SPDIFRX1" , "pll1_q")
            },
            crate::pac::rcc::vals::Spdifrxsel::PLL2_R => unsafe {
                unwrap ! (crate :: rcc :: get_freqs () . pll2_r . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "SPDIFRX1" , "pll2_r")
            },
            crate::pac::rcc::vals::Spdifrxsel::PLL3_R => unsafe {
                unwrap ! (crate :: rcc :: get_freqs () . pll3_r . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "SPDIFRX1" , "pll3_r")
            },
            crate::pac::rcc::vals::Spdifrxsel::HSI => unsafe {
                unwrap ! (crate :: rcc :: get_freqs () . hsi . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "SPDIFRX1" , "hsi")
            },
            #[allow(unreachable_patterns)]
            _ => panic!(
                "attempted to use peripheral '{}' but its clock mux is not set to a valid \
                         clock. Change 'config.rcc.mux' to another clock.",
                "SPDIFRX1"
            ),
        }
    }
    fn bus_frequency() -> crate::time::Hertz {
        unsafe {
            unwrap ! (crate :: rcc :: get_freqs () . pclk1 . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "SPDIFRX1" , "pclk1")
        }
    }
    const RCC_INFO: crate::rcc::RccInfo = unsafe {
        crate::rcc::RccInfo::new(
            Some((36u8, 16u8)),
            (58u8, 16u8),
            None,
            #[cfg(feature = "low-power")]
            crate::rcc::StopMode::Stop1,
        )
    };
}
impl crate::rcc::RccPeripheral for peripherals::SPDIFRX1 {}
impl crate::rcc::SealedRccPeripheral for peripherals::SPI1 {
    fn frequency() -> crate::time::Hertz {
        match crate::pac::RCC.d2ccip1r().read().spi123sel() {
            crate::pac::rcc::vals::Saisel::PLL1_Q => unsafe {
                unwrap ! (crate :: rcc :: get_freqs () . pll1_q . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "SPI1" , "pll1_q")
            },
            crate::pac::rcc::vals::Saisel::PLL2_P => unsafe {
                unwrap ! (crate :: rcc :: get_freqs () . pll2_p . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "SPI1" , "pll2_p")
            },
            crate::pac::rcc::vals::Saisel::PLL3_P => unsafe {
                unwrap ! (crate :: rcc :: get_freqs () . pll3_p . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "SPI1" , "pll3_p")
            },
            crate::pac::rcc::vals::Saisel::I2S_CKIN => unsafe {
                unwrap ! (crate :: rcc :: get_freqs () . i2s_ckin . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "SPI1" , "i2s_ckin")
            },
            crate::pac::rcc::vals::Saisel::PER => match crate::pac::RCC.d1ccipr().read().persel() {
                crate::pac::rcc::vals::Persel::HSI => unsafe {
                    unwrap ! (crate :: rcc :: get_freqs () . hsi . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "SPI1" , "hsi")
                },
                crate::pac::rcc::vals::Persel::CSI => unsafe {
                    unwrap ! (crate :: rcc :: get_freqs () . csi . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "SPI1" , "csi")
                },
                crate::pac::rcc::vals::Persel::HSE => unsafe {
                    unwrap ! (crate :: rcc :: get_freqs () . hse . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "SPI1" , "hse")
                },
                #[allow(unreachable_patterns)]
                _ => panic!(
                    "attempted to use peripheral '{}' but its clock mux is not set to a valid \
                         clock. Change 'config.rcc.mux' to another clock.",
                    "SPI1"
                ),
            },
            #[allow(unreachable_patterns)]
            _ => panic!(
                "attempted to use peripheral '{}' but its clock mux is not set to a valid \
                         clock. Change 'config.rcc.mux' to another clock.",
                "SPI1"
            ),
        }
    }
    fn bus_frequency() -> crate::time::Hertz {
        unsafe {
            unwrap ! (crate :: rcc :: get_freqs () . pclk2 . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "SPI1" , "pclk2")
        }
    }
    const RCC_INFO: crate::rcc::RccInfo = unsafe {
        crate::rcc::RccInfo::new(
            Some((38u8, 12u8)),
            (60u8, 12u8),
            None,
            #[cfg(feature = "low-power")]
            crate::rcc::StopMode::Stop1,
        )
    };
}
impl crate::rcc::RccPeripheral for peripherals::SPI1 {}
impl crate::rcc::SealedRccPeripheral for peripherals::SPI2 {
    fn frequency() -> crate::time::Hertz {
        match crate::pac::RCC.d2ccip1r().read().spi123sel() {
            crate::pac::rcc::vals::Saisel::PLL1_Q => unsafe {
                unwrap ! (crate :: rcc :: get_freqs () . pll1_q . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "SPI2" , "pll1_q")
            },
            crate::pac::rcc::vals::Saisel::PLL2_P => unsafe {
                unwrap ! (crate :: rcc :: get_freqs () . pll2_p . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "SPI2" , "pll2_p")
            },
            crate::pac::rcc::vals::Saisel::PLL3_P => unsafe {
                unwrap ! (crate :: rcc :: get_freqs () . pll3_p . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "SPI2" , "pll3_p")
            },
            crate::pac::rcc::vals::Saisel::I2S_CKIN => unsafe {
                unwrap ! (crate :: rcc :: get_freqs () . i2s_ckin . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "SPI2" , "i2s_ckin")
            },
            crate::pac::rcc::vals::Saisel::PER => match crate::pac::RCC.d1ccipr().read().persel() {
                crate::pac::rcc::vals::Persel::HSI => unsafe {
                    unwrap ! (crate :: rcc :: get_freqs () . hsi . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "SPI2" , "hsi")
                },
                crate::pac::rcc::vals::Persel::CSI => unsafe {
                    unwrap ! (crate :: rcc :: get_freqs () . csi . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "SPI2" , "csi")
                },
                crate::pac::rcc::vals::Persel::HSE => unsafe {
                    unwrap ! (crate :: rcc :: get_freqs () . hse . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "SPI2" , "hse")
                },
                #[allow(unreachable_patterns)]
                _ => panic!(
                    "attempted to use peripheral '{}' but its clock mux is not set to a valid \
                         clock. Change 'config.rcc.mux' to another clock.",
                    "SPI2"
                ),
            },
            #[allow(unreachable_patterns)]
            _ => panic!(
                "attempted to use peripheral '{}' but its clock mux is not set to a valid \
                         clock. Change 'config.rcc.mux' to another clock.",
                "SPI2"
            ),
        }
    }
    fn bus_frequency() -> crate::time::Hertz {
        unsafe {
            unwrap ! (crate :: rcc :: get_freqs () . pclk1 . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "SPI2" , "pclk1")
        }
    }
    const RCC_INFO: crate::rcc::RccInfo = unsafe {
        crate::rcc::RccInfo::new(
            Some((36u8, 14u8)),
            (58u8, 14u8),
            None,
            #[cfg(feature = "low-power")]
            crate::rcc::StopMode::Stop1,
        )
    };
}
impl crate::rcc::RccPeripheral for peripherals::SPI2 {}
impl crate::rcc::SealedRccPeripheral for peripherals::SPI3 {
    fn frequency() -> crate::time::Hertz {
        match crate::pac::RCC.d2ccip1r().read().spi123sel() {
            crate::pac::rcc::vals::Saisel::PLL1_Q => unsafe {
                unwrap ! (crate :: rcc :: get_freqs () . pll1_q . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "SPI3" , "pll1_q")
            },
            crate::pac::rcc::vals::Saisel::PLL2_P => unsafe {
                unwrap ! (crate :: rcc :: get_freqs () . pll2_p . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "SPI3" , "pll2_p")
            },
            crate::pac::rcc::vals::Saisel::PLL3_P => unsafe {
                unwrap ! (crate :: rcc :: get_freqs () . pll3_p . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "SPI3" , "pll3_p")
            },
            crate::pac::rcc::vals::Saisel::I2S_CKIN => unsafe {
                unwrap ! (crate :: rcc :: get_freqs () . i2s_ckin . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "SPI3" , "i2s_ckin")
            },
            crate::pac::rcc::vals::Saisel::PER => match crate::pac::RCC.d1ccipr().read().persel() {
                crate::pac::rcc::vals::Persel::HSI => unsafe {
                    unwrap ! (crate :: rcc :: get_freqs () . hsi . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "SPI3" , "hsi")
                },
                crate::pac::rcc::vals::Persel::CSI => unsafe {
                    unwrap ! (crate :: rcc :: get_freqs () . csi . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "SPI3" , "csi")
                },
                crate::pac::rcc::vals::Persel::HSE => unsafe {
                    unwrap ! (crate :: rcc :: get_freqs () . hse . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "SPI3" , "hse")
                },
                #[allow(unreachable_patterns)]
                _ => panic!(
                    "attempted to use peripheral '{}' but its clock mux is not set to a valid \
                         clock. Change 'config.rcc.mux' to another clock.",
                    "SPI3"
                ),
            },
            #[allow(unreachable_patterns)]
            _ => panic!(
                "attempted to use peripheral '{}' but its clock mux is not set to a valid \
                         clock. Change 'config.rcc.mux' to another clock.",
                "SPI3"
            ),
        }
    }
    fn bus_frequency() -> crate::time::Hertz {
        unsafe {
            unwrap ! (crate :: rcc :: get_freqs () . pclk1 . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "SPI3" , "pclk1")
        }
    }
    const RCC_INFO: crate::rcc::RccInfo = unsafe {
        crate::rcc::RccInfo::new(
            Some((36u8, 15u8)),
            (58u8, 15u8),
            None,
            #[cfg(feature = "low-power")]
            crate::rcc::StopMode::Stop1,
        )
    };
}
impl crate::rcc::RccPeripheral for peripherals::SPI3 {}
impl crate::rcc::SealedRccPeripheral for peripherals::SPI4 {
    fn frequency() -> crate::time::Hertz {
        match crate::pac::RCC.d2ccip1r().read().spi45sel() {
            crate::pac::rcc::vals::Spi45sel::PCLK2 => unsafe {
                unwrap ! (crate :: rcc :: get_freqs () . pclk2 . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "SPI4" , "pclk2")
            },
            crate::pac::rcc::vals::Spi45sel::PLL2_Q => unsafe {
                unwrap ! (crate :: rcc :: get_freqs () . pll2_q . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "SPI4" , "pll2_q")
            },
            crate::pac::rcc::vals::Spi45sel::PLL3_Q => unsafe {
                unwrap ! (crate :: rcc :: get_freqs () . pll3_q . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "SPI4" , "pll3_q")
            },
            crate::pac::rcc::vals::Spi45sel::HSI => unsafe {
                unwrap ! (crate :: rcc :: get_freqs () . hsi . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "SPI4" , "hsi")
            },
            crate::pac::rcc::vals::Spi45sel::CSI => unsafe {
                unwrap ! (crate :: rcc :: get_freqs () . csi . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "SPI4" , "csi")
            },
            crate::pac::rcc::vals::Spi45sel::HSE => unsafe {
                unwrap ! (crate :: rcc :: get_freqs () . hse . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "SPI4" , "hse")
            },
            #[allow(unreachable_patterns)]
            _ => panic!(
                "attempted to use peripheral '{}' but its clock mux is not set to a valid \
                         clock. Change 'config.rcc.mux' to another clock.",
                "SPI4"
            ),
        }
    }
    fn bus_frequency() -> crate::time::Hertz {
        unsafe {
            unwrap ! (crate :: rcc :: get_freqs () . pclk2 . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "SPI4" , "pclk2")
        }
    }
    const RCC_INFO: crate::rcc::RccInfo = unsafe {
        crate::rcc::RccInfo::new(
            Some((38u8, 13u8)),
            (60u8, 13u8),
            None,
            #[cfg(feature = "low-power")]
            crate::rcc::StopMode::Stop1,
        )
    };
}
impl crate::rcc::RccPeripheral for peripherals::SPI4 {}
impl crate::rcc::SealedRccPeripheral for peripherals::SPI6 {
    fn frequency() -> crate::time::Hertz {
        match crate::pac::RCC.d3ccipr().read().spi6sel() {
            crate::pac::rcc::vals::Spi6sel::PCLK4 => unsafe {
                unwrap ! (crate :: rcc :: get_freqs () . pclk4 . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "SPI6" , "pclk4")
            },
            crate::pac::rcc::vals::Spi6sel::PLL2_Q => unsafe {
                unwrap ! (crate :: rcc :: get_freqs () . pll2_q . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "SPI6" , "pll2_q")
            },
            crate::pac::rcc::vals::Spi6sel::PLL3_Q => unsafe {
                unwrap ! (crate :: rcc :: get_freqs () . pll3_q . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "SPI6" , "pll3_q")
            },
            crate::pac::rcc::vals::Spi6sel::HSI => unsafe {
                unwrap ! (crate :: rcc :: get_freqs () . hsi . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "SPI6" , "hsi")
            },
            crate::pac::rcc::vals::Spi6sel::CSI => unsafe {
                unwrap ! (crate :: rcc :: get_freqs () . csi . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "SPI6" , "csi")
            },
            crate::pac::rcc::vals::Spi6sel::HSE => unsafe {
                unwrap ! (crate :: rcc :: get_freqs () . hse . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "SPI6" , "hse")
            },
            #[allow(unreachable_patterns)]
            _ => panic!(
                "attempted to use peripheral '{}' but its clock mux is not set to a valid \
                         clock. Change 'config.rcc.mux' to another clock.",
                "SPI6"
            ),
        }
    }
    fn bus_frequency() -> crate::time::Hertz {
        unsafe {
            unwrap ! (crate :: rcc :: get_freqs () . pclk4 . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "SPI6" , "pclk4")
        }
    }
    const RCC_INFO: crate::rcc::RccInfo = unsafe {
        crate::rcc::RccInfo::new(
            Some((39u8, 5u8)),
            (61u8, 5u8),
            None,
            #[cfg(feature = "low-power")]
            crate::rcc::StopMode::Stop1,
        )
    };
}
impl crate::rcc::RccPeripheral for peripherals::SPI6 {}
impl crate::rcc::SealedRccPeripheral for peripherals::SYSCFG {
    fn frequency() -> crate::time::Hertz {
        unsafe {
            unwrap ! (crate :: rcc :: get_freqs () . pclk4 . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "SYSCFG" , "pclk4")
        }
    }
    fn bus_frequency() -> crate::time::Hertz {
        unsafe {
            unwrap ! (crate :: rcc :: get_freqs () . pclk4 . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "SYSCFG" , "pclk4")
        }
    }
    const RCC_INFO: crate::rcc::RccInfo = unsafe {
        crate::rcc::RccInfo::new(
            Some((39u8, 1u8)),
            (61u8, 1u8),
            None,
            #[cfg(feature = "low-power")]
            crate::rcc::StopMode::Stop1,
        )
    };
}
impl crate::rcc::RccPeripheral for peripherals::SYSCFG {}
impl crate::rcc::SealedRccPeripheral for peripherals::TIM1 {
    fn frequency() -> crate::time::Hertz {
        unsafe {
            unwrap ! (crate :: rcc :: get_freqs () . pclk2_tim . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "TIM1" , "pclk2_tim")
        }
    }
    fn bus_frequency() -> crate::time::Hertz {
        unsafe {
            unwrap ! (crate :: rcc :: get_freqs () . pclk2 . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "TIM1" , "pclk2")
        }
    }
    const RCC_INFO: crate::rcc::RccInfo = unsafe {
        crate::rcc::RccInfo::new(
            Some((38u8, 0u8)),
            (60u8, 0u8),
            None,
            #[cfg(feature = "low-power")]
            crate::rcc::StopMode::Stop1,
        )
    };
}
impl crate::rcc::RccPeripheral for peripherals::TIM1 {}
impl crate::rcc::SealedRccPeripheral for peripherals::TIM12 {
    fn frequency() -> crate::time::Hertz {
        unsafe {
            unwrap ! (crate :: rcc :: get_freqs () . pclk1_tim . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "TIM12" , "pclk1_tim")
        }
    }
    fn bus_frequency() -> crate::time::Hertz {
        unsafe {
            unwrap ! (crate :: rcc :: get_freqs () . pclk1 . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "TIM12" , "pclk1")
        }
    }
    const RCC_INFO: crate::rcc::RccInfo = unsafe {
        crate::rcc::RccInfo::new(
            Some((36u8, 6u8)),
            (58u8, 6u8),
            None,
            #[cfg(feature = "low-power")]
            crate::rcc::StopMode::Stop1,
        )
    };
}
impl crate::rcc::RccPeripheral for peripherals::TIM12 {}
impl crate::rcc::SealedRccPeripheral for peripherals::TIM13 {
    fn frequency() -> crate::time::Hertz {
        unsafe {
            unwrap ! (crate :: rcc :: get_freqs () . pclk1_tim . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "TIM13" , "pclk1_tim")
        }
    }
    fn bus_frequency() -> crate::time::Hertz {
        unsafe {
            unwrap ! (crate :: rcc :: get_freqs () . pclk1 . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "TIM13" , "pclk1")
        }
    }
    const RCC_INFO: crate::rcc::RccInfo = unsafe {
        crate::rcc::RccInfo::new(
            Some((36u8, 7u8)),
            (58u8, 7u8),
            None,
            #[cfg(feature = "low-power")]
            crate::rcc::StopMode::Stop1,
        )
    };
}
impl crate::rcc::RccPeripheral for peripherals::TIM13 {}
impl crate::rcc::SealedRccPeripheral for peripherals::TIM14 {
    fn frequency() -> crate::time::Hertz {
        unsafe {
            unwrap ! (crate :: rcc :: get_freqs () . pclk1_tim . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "TIM14" , "pclk1_tim")
        }
    }
    fn bus_frequency() -> crate::time::Hertz {
        unsafe {
            unwrap ! (crate :: rcc :: get_freqs () . pclk1 . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "TIM14" , "pclk1")
        }
    }
    const RCC_INFO: crate::rcc::RccInfo = unsafe {
        crate::rcc::RccInfo::new(
            Some((36u8, 8u8)),
            (58u8, 8u8),
            None,
            #[cfg(feature = "low-power")]
            crate::rcc::StopMode::Stop1,
        )
    };
}
impl crate::rcc::RccPeripheral for peripherals::TIM14 {}
impl crate::rcc::SealedRccPeripheral for peripherals::TIM15 {
    fn frequency() -> crate::time::Hertz {
        unsafe {
            unwrap ! (crate :: rcc :: get_freqs () . pclk2_tim . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "TIM15" , "pclk2_tim")
        }
    }
    fn bus_frequency() -> crate::time::Hertz {
        unsafe {
            unwrap ! (crate :: rcc :: get_freqs () . pclk2 . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "TIM15" , "pclk2")
        }
    }
    const RCC_INFO: crate::rcc::RccInfo = unsafe {
        crate::rcc::RccInfo::new(
            Some((38u8, 16u8)),
            (60u8, 16u8),
            None,
            #[cfg(feature = "low-power")]
            crate::rcc::StopMode::Stop1,
        )
    };
}
impl crate::rcc::RccPeripheral for peripherals::TIM15 {}
impl crate::rcc::SealedRccPeripheral for peripherals::TIM16 {
    fn frequency() -> crate::time::Hertz {
        unsafe {
            unwrap ! (crate :: rcc :: get_freqs () . pclk2_tim . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "TIM16" , "pclk2_tim")
        }
    }
    fn bus_frequency() -> crate::time::Hertz {
        unsafe {
            unwrap ! (crate :: rcc :: get_freqs () . pclk2 . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "TIM16" , "pclk2")
        }
    }
    const RCC_INFO: crate::rcc::RccInfo = unsafe {
        crate::rcc::RccInfo::new(
            Some((38u8, 17u8)),
            (60u8, 17u8),
            None,
            #[cfg(feature = "low-power")]
            crate::rcc::StopMode::Stop1,
        )
    };
}
impl crate::rcc::RccPeripheral for peripherals::TIM16 {}
impl crate::rcc::SealedRccPeripheral for peripherals::TIM17 {
    fn frequency() -> crate::time::Hertz {
        unsafe {
            unwrap ! (crate :: rcc :: get_freqs () . pclk2_tim . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "TIM17" , "pclk2_tim")
        }
    }
    fn bus_frequency() -> crate::time::Hertz {
        unsafe {
            unwrap ! (crate :: rcc :: get_freqs () . pclk2 . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "TIM17" , "pclk2")
        }
    }
    const RCC_INFO: crate::rcc::RccInfo = unsafe {
        crate::rcc::RccInfo::new(
            Some((38u8, 18u8)),
            (60u8, 18u8),
            None,
            #[cfg(feature = "low-power")]
            crate::rcc::StopMode::Stop1,
        )
    };
}
impl crate::rcc::RccPeripheral for peripherals::TIM17 {}
impl crate::rcc::SealedRccPeripheral for peripherals::TIM2 {
    fn frequency() -> crate::time::Hertz {
        unsafe {
            unwrap ! (crate :: rcc :: get_freqs () . pclk1_tim . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "TIM2" , "pclk1_tim")
        }
    }
    fn bus_frequency() -> crate::time::Hertz {
        unsafe {
            unwrap ! (crate :: rcc :: get_freqs () . pclk1 . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "TIM2" , "pclk1")
        }
    }
    const RCC_INFO: crate::rcc::RccInfo = unsafe {
        crate::rcc::RccInfo::new(
            Some((36u8, 0u8)),
            (58u8, 0u8),
            None,
            #[cfg(feature = "low-power")]
            crate::rcc::StopMode::Stop1,
        )
    };
}
impl crate::rcc::RccPeripheral for peripherals::TIM2 {}
impl crate::rcc::SealedRccPeripheral for peripherals::TIM23 {
    fn frequency() -> crate::time::Hertz {
        unsafe {
            unwrap ! (crate :: rcc :: get_freqs () . pclk1_tim . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "TIM23" , "pclk1_tim")
        }
    }
    fn bus_frequency() -> crate::time::Hertz {
        unsafe {
            unwrap ! (crate :: rcc :: get_freqs () . pclk1 . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "TIM23" , "pclk1")
        }
    }
    const RCC_INFO: crate::rcc::RccInfo = unsafe {
        crate::rcc::RccInfo::new(
            Some((37u8, 24u8)),
            (59u8, 24u8),
            None,
            #[cfg(feature = "low-power")]
            crate::rcc::StopMode::Stop1,
        )
    };
}
impl crate::rcc::RccPeripheral for peripherals::TIM23 {}
impl crate::rcc::SealedRccPeripheral for peripherals::TIM24 {
    fn frequency() -> crate::time::Hertz {
        unsafe {
            unwrap ! (crate :: rcc :: get_freqs () . pclk1_tim . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "TIM24" , "pclk1_tim")
        }
    }
    fn bus_frequency() -> crate::time::Hertz {
        unsafe {
            unwrap ! (crate :: rcc :: get_freqs () . pclk1 . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "TIM24" , "pclk1")
        }
    }
    const RCC_INFO: crate::rcc::RccInfo = unsafe {
        crate::rcc::RccInfo::new(
            Some((37u8, 25u8)),
            (59u8, 25u8),
            None,
            #[cfg(feature = "low-power")]
            crate::rcc::StopMode::Stop1,
        )
    };
}
impl crate::rcc::RccPeripheral for peripherals::TIM24 {}
impl crate::rcc::SealedRccPeripheral for peripherals::TIM3 {
    fn frequency() -> crate::time::Hertz {
        unsafe {
            unwrap ! (crate :: rcc :: get_freqs () . pclk1_tim . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "TIM3" , "pclk1_tim")
        }
    }
    fn bus_frequency() -> crate::time::Hertz {
        unsafe {
            unwrap ! (crate :: rcc :: get_freqs () . pclk1 . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "TIM3" , "pclk1")
        }
    }
    const RCC_INFO: crate::rcc::RccInfo = unsafe {
        crate::rcc::RccInfo::new(
            Some((36u8, 1u8)),
            (58u8, 1u8),
            None,
            #[cfg(feature = "low-power")]
            crate::rcc::StopMode::Stop1,
        )
    };
}
impl crate::rcc::RccPeripheral for peripherals::TIM3 {}
impl crate::rcc::SealedRccPeripheral for peripherals::TIM4 {
    fn frequency() -> crate::time::Hertz {
        unsafe {
            unwrap ! (crate :: rcc :: get_freqs () . pclk1_tim . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "TIM4" , "pclk1_tim")
        }
    }
    fn bus_frequency() -> crate::time::Hertz {
        unsafe {
            unwrap ! (crate :: rcc :: get_freqs () . pclk1 . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "TIM4" , "pclk1")
        }
    }
    const RCC_INFO: crate::rcc::RccInfo = unsafe {
        crate::rcc::RccInfo::new(
            Some((36u8, 2u8)),
            (58u8, 2u8),
            None,
            #[cfg(feature = "low-power")]
            crate::rcc::StopMode::Stop1,
        )
    };
}
impl crate::rcc::RccPeripheral for peripherals::TIM4 {}
impl crate::rcc::SealedRccPeripheral for peripherals::TIM5 {
    fn frequency() -> crate::time::Hertz {
        unsafe {
            unwrap ! (crate :: rcc :: get_freqs () . pclk1_tim . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "TIM5" , "pclk1_tim")
        }
    }
    fn bus_frequency() -> crate::time::Hertz {
        unsafe {
            unwrap ! (crate :: rcc :: get_freqs () . pclk1 . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "TIM5" , "pclk1")
        }
    }
    const RCC_INFO: crate::rcc::RccInfo = unsafe {
        crate::rcc::RccInfo::new(
            Some((36u8, 3u8)),
            (58u8, 3u8),
            None,
            #[cfg(feature = "low-power")]
            crate::rcc::StopMode::Stop1,
        )
    };
}
impl crate::rcc::RccPeripheral for peripherals::TIM5 {}
impl crate::rcc::SealedRccPeripheral for peripherals::TIM6 {
    fn frequency() -> crate::time::Hertz {
        unsafe {
            unwrap ! (crate :: rcc :: get_freqs () . pclk1_tim . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "TIM6" , "pclk1_tim")
        }
    }
    fn bus_frequency() -> crate::time::Hertz {
        unsafe {
            unwrap ! (crate :: rcc :: get_freqs () . pclk1 . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "TIM6" , "pclk1")
        }
    }
    const RCC_INFO: crate::rcc::RccInfo = unsafe {
        crate::rcc::RccInfo::new(
            Some((36u8, 4u8)),
            (58u8, 4u8),
            None,
            #[cfg(feature = "low-power")]
            crate::rcc::StopMode::Stop1,
        )
    };
}
impl crate::rcc::RccPeripheral for peripherals::TIM6 {}
impl crate::rcc::SealedRccPeripheral for peripherals::TIM7 {
    fn frequency() -> crate::time::Hertz {
        unsafe {
            unwrap ! (crate :: rcc :: get_freqs () . pclk1_tim . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "TIM7" , "pclk1_tim")
        }
    }
    fn bus_frequency() -> crate::time::Hertz {
        unsafe {
            unwrap ! (crate :: rcc :: get_freqs () . pclk1 . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "TIM7" , "pclk1")
        }
    }
    const RCC_INFO: crate::rcc::RccInfo = unsafe {
        crate::rcc::RccInfo::new(
            Some((36u8, 5u8)),
            (58u8, 5u8),
            None,
            #[cfg(feature = "low-power")]
            crate::rcc::StopMode::Stop1,
        )
    };
}
impl crate::rcc::RccPeripheral for peripherals::TIM7 {}
impl crate::rcc::SealedRccPeripheral for peripherals::TIM8 {
    fn frequency() -> crate::time::Hertz {
        unsafe {
            unwrap ! (crate :: rcc :: get_freqs () . pclk2_tim . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "TIM8" , "pclk2_tim")
        }
    }
    fn bus_frequency() -> crate::time::Hertz {
        unsafe {
            unwrap ! (crate :: rcc :: get_freqs () . pclk2 . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "TIM8" , "pclk2")
        }
    }
    const RCC_INFO: crate::rcc::RccInfo = unsafe {
        crate::rcc::RccInfo::new(
            Some((38u8, 1u8)),
            (60u8, 1u8),
            None,
            #[cfg(feature = "low-power")]
            crate::rcc::StopMode::Stop1,
        )
    };
}
impl crate::rcc::RccPeripheral for peripherals::TIM8 {}
impl crate::rcc::SealedRccPeripheral for peripherals::UART4 {
    fn frequency() -> crate::time::Hertz {
        match crate::pac::RCC.d2ccip2r().read().usart234578sel() {
            crate::pac::rcc::vals::Usart234578sel::PCLK1 => unsafe {
                unwrap ! (crate :: rcc :: get_freqs () . pclk1 . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "UART4" , "pclk1")
            },
            crate::pac::rcc::vals::Usart234578sel::PLL2_Q => unsafe {
                unwrap ! (crate :: rcc :: get_freqs () . pll2_q . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "UART4" , "pll2_q")
            },
            crate::pac::rcc::vals::Usart234578sel::PLL3_Q => unsafe {
                unwrap ! (crate :: rcc :: get_freqs () . pll3_q . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "UART4" , "pll3_q")
            },
            crate::pac::rcc::vals::Usart234578sel::HSI => unsafe {
                unwrap ! (crate :: rcc :: get_freqs () . hsi . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "UART4" , "hsi")
            },
            crate::pac::rcc::vals::Usart234578sel::CSI => unsafe {
                unwrap ! (crate :: rcc :: get_freqs () . csi . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "UART4" , "csi")
            },
            crate::pac::rcc::vals::Usart234578sel::LSE => unsafe {
                unwrap ! (crate :: rcc :: get_freqs () . lse . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "UART4" , "lse")
            },
            #[allow(unreachable_patterns)]
            _ => panic!(
                "attempted to use peripheral '{}' but its clock mux is not set to a valid \
                         clock. Change 'config.rcc.mux' to another clock.",
                "UART4"
            ),
        }
    }
    fn bus_frequency() -> crate::time::Hertz {
        unsafe {
            unwrap ! (crate :: rcc :: get_freqs () . pclk1 . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "UART4" , "pclk1")
        }
    }
    const RCC_INFO: crate::rcc::RccInfo = unsafe {
        crate::rcc::RccInfo::new(
            Some((36u8, 19u8)),
            (58u8, 19u8),
            None,
            #[cfg(feature = "low-power")]
            crate::rcc::StopMode::Stop1,
        )
    };
}
impl crate::rcc::RccPeripheral for peripherals::UART4 {}
impl crate::rcc::SealedRccPeripheral for peripherals::UART5 {
    fn frequency() -> crate::time::Hertz {
        match crate::pac::RCC.d2ccip2r().read().usart234578sel() {
            crate::pac::rcc::vals::Usart234578sel::PCLK1 => unsafe {
                unwrap ! (crate :: rcc :: get_freqs () . pclk1 . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "UART5" , "pclk1")
            },
            crate::pac::rcc::vals::Usart234578sel::PLL2_Q => unsafe {
                unwrap ! (crate :: rcc :: get_freqs () . pll2_q . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "UART5" , "pll2_q")
            },
            crate::pac::rcc::vals::Usart234578sel::PLL3_Q => unsafe {
                unwrap ! (crate :: rcc :: get_freqs () . pll3_q . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "UART5" , "pll3_q")
            },
            crate::pac::rcc::vals::Usart234578sel::HSI => unsafe {
                unwrap ! (crate :: rcc :: get_freqs () . hsi . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "UART5" , "hsi")
            },
            crate::pac::rcc::vals::Usart234578sel::CSI => unsafe {
                unwrap ! (crate :: rcc :: get_freqs () . csi . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "UART5" , "csi")
            },
            crate::pac::rcc::vals::Usart234578sel::LSE => unsafe {
                unwrap ! (crate :: rcc :: get_freqs () . lse . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "UART5" , "lse")
            },
            #[allow(unreachable_patterns)]
            _ => panic!(
                "attempted to use peripheral '{}' but its clock mux is not set to a valid \
                         clock. Change 'config.rcc.mux' to another clock.",
                "UART5"
            ),
        }
    }
    fn bus_frequency() -> crate::time::Hertz {
        unsafe {
            unwrap ! (crate :: rcc :: get_freqs () . pclk1 . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "UART5" , "pclk1")
        }
    }
    const RCC_INFO: crate::rcc::RccInfo = unsafe {
        crate::rcc::RccInfo::new(
            Some((36u8, 20u8)),
            (58u8, 20u8),
            None,
            #[cfg(feature = "low-power")]
            crate::rcc::StopMode::Stop1,
        )
    };
}
impl crate::rcc::RccPeripheral for peripherals::UART5 {}
impl crate::rcc::SealedRccPeripheral for peripherals::UART7 {
    fn frequency() -> crate::time::Hertz {
        match crate::pac::RCC.d2ccip2r().read().usart234578sel() {
            crate::pac::rcc::vals::Usart234578sel::PCLK1 => unsafe {
                unwrap ! (crate :: rcc :: get_freqs () . pclk1 . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "UART7" , "pclk1")
            },
            crate::pac::rcc::vals::Usart234578sel::PLL2_Q => unsafe {
                unwrap ! (crate :: rcc :: get_freqs () . pll2_q . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "UART7" , "pll2_q")
            },
            crate::pac::rcc::vals::Usart234578sel::PLL3_Q => unsafe {
                unwrap ! (crate :: rcc :: get_freqs () . pll3_q . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "UART7" , "pll3_q")
            },
            crate::pac::rcc::vals::Usart234578sel::HSI => unsafe {
                unwrap ! (crate :: rcc :: get_freqs () . hsi . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "UART7" , "hsi")
            },
            crate::pac::rcc::vals::Usart234578sel::CSI => unsafe {
                unwrap ! (crate :: rcc :: get_freqs () . csi . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "UART7" , "csi")
            },
            crate::pac::rcc::vals::Usart234578sel::LSE => unsafe {
                unwrap ! (crate :: rcc :: get_freqs () . lse . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "UART7" , "lse")
            },
            #[allow(unreachable_patterns)]
            _ => panic!(
                "attempted to use peripheral '{}' but its clock mux is not set to a valid \
                         clock. Change 'config.rcc.mux' to another clock.",
                "UART7"
            ),
        }
    }
    fn bus_frequency() -> crate::time::Hertz {
        unsafe {
            unwrap ! (crate :: rcc :: get_freqs () . pclk1 . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "UART7" , "pclk1")
        }
    }
    const RCC_INFO: crate::rcc::RccInfo = unsafe {
        crate::rcc::RccInfo::new(
            Some((36u8, 30u8)),
            (58u8, 30u8),
            None,
            #[cfg(feature = "low-power")]
            crate::rcc::StopMode::Stop1,
        )
    };
}
impl crate::rcc::RccPeripheral for peripherals::UART7 {}
impl crate::rcc::SealedRccPeripheral for peripherals::UART8 {
    fn frequency() -> crate::time::Hertz {
        match crate::pac::RCC.d2ccip2r().read().usart234578sel() {
            crate::pac::rcc::vals::Usart234578sel::PCLK1 => unsafe {
                unwrap ! (crate :: rcc :: get_freqs () . pclk1 . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "UART8" , "pclk1")
            },
            crate::pac::rcc::vals::Usart234578sel::PLL2_Q => unsafe {
                unwrap ! (crate :: rcc :: get_freqs () . pll2_q . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "UART8" , "pll2_q")
            },
            crate::pac::rcc::vals::Usart234578sel::PLL3_Q => unsafe {
                unwrap ! (crate :: rcc :: get_freqs () . pll3_q . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "UART8" , "pll3_q")
            },
            crate::pac::rcc::vals::Usart234578sel::HSI => unsafe {
                unwrap ! (crate :: rcc :: get_freqs () . hsi . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "UART8" , "hsi")
            },
            crate::pac::rcc::vals::Usart234578sel::CSI => unsafe {
                unwrap ! (crate :: rcc :: get_freqs () . csi . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "UART8" , "csi")
            },
            crate::pac::rcc::vals::Usart234578sel::LSE => unsafe {
                unwrap ! (crate :: rcc :: get_freqs () . lse . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "UART8" , "lse")
            },
            #[allow(unreachable_patterns)]
            _ => panic!(
                "attempted to use peripheral '{}' but its clock mux is not set to a valid \
                         clock. Change 'config.rcc.mux' to another clock.",
                "UART8"
            ),
        }
    }
    fn bus_frequency() -> crate::time::Hertz {
        unsafe {
            unwrap ! (crate :: rcc :: get_freqs () . pclk1 . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "UART8" , "pclk1")
        }
    }
    const RCC_INFO: crate::rcc::RccInfo = unsafe {
        crate::rcc::RccInfo::new(
            Some((36u8, 31u8)),
            (58u8, 31u8),
            None,
            #[cfg(feature = "low-power")]
            crate::rcc::StopMode::Stop1,
        )
    };
}
impl crate::rcc::RccPeripheral for peripherals::UART8 {}
impl crate::rcc::SealedRccPeripheral for peripherals::UART9 {
    fn frequency() -> crate::time::Hertz {
        match crate::pac::RCC.d2ccip2r().read().usart16910sel() {
            crate::pac::rcc::vals::Usart16910sel::PCLK2 => unsafe {
                unwrap ! (crate :: rcc :: get_freqs () . pclk2 . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "UART9" , "pclk2")
            },
            crate::pac::rcc::vals::Usart16910sel::PLL2_Q => unsafe {
                unwrap ! (crate :: rcc :: get_freqs () . pll2_q . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "UART9" , "pll2_q")
            },
            crate::pac::rcc::vals::Usart16910sel::PLL3_Q => unsafe {
                unwrap ! (crate :: rcc :: get_freqs () . pll3_q . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "UART9" , "pll3_q")
            },
            crate::pac::rcc::vals::Usart16910sel::HSI => unsafe {
                unwrap ! (crate :: rcc :: get_freqs () . hsi . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "UART9" , "hsi")
            },
            crate::pac::rcc::vals::Usart16910sel::CSI => unsafe {
                unwrap ! (crate :: rcc :: get_freqs () . csi . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "UART9" , "csi")
            },
            crate::pac::rcc::vals::Usart16910sel::LSE => unsafe {
                unwrap ! (crate :: rcc :: get_freqs () . lse . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "UART9" , "lse")
            },
            #[allow(unreachable_patterns)]
            _ => panic!(
                "attempted to use peripheral '{}' but its clock mux is not set to a valid \
                         clock. Change 'config.rcc.mux' to another clock.",
                "UART9"
            ),
        }
    }
    fn bus_frequency() -> crate::time::Hertz {
        unsafe {
            unwrap ! (crate :: rcc :: get_freqs () . pclk2 . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "UART9" , "pclk2")
        }
    }
    const RCC_INFO: crate::rcc::RccInfo = unsafe {
        crate::rcc::RccInfo::new(
            Some((38u8, 6u8)),
            (60u8, 6u8),
            None,
            #[cfg(feature = "low-power")]
            crate::rcc::StopMode::Stop1,
        )
    };
}
impl crate::rcc::RccPeripheral for peripherals::UART9 {}
impl crate::rcc::SealedRccPeripheral for peripherals::USART1 {
    fn frequency() -> crate::time::Hertz {
        match crate::pac::RCC.d2ccip2r().read().usart16910sel() {
            crate::pac::rcc::vals::Usart16910sel::PCLK2 => unsafe {
                unwrap ! (crate :: rcc :: get_freqs () . pclk2 . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "USART1" , "pclk2")
            },
            crate::pac::rcc::vals::Usart16910sel::PLL2_Q => unsafe {
                unwrap ! (crate :: rcc :: get_freqs () . pll2_q . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "USART1" , "pll2_q")
            },
            crate::pac::rcc::vals::Usart16910sel::PLL3_Q => unsafe {
                unwrap ! (crate :: rcc :: get_freqs () . pll3_q . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "USART1" , "pll3_q")
            },
            crate::pac::rcc::vals::Usart16910sel::HSI => unsafe {
                unwrap ! (crate :: rcc :: get_freqs () . hsi . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "USART1" , "hsi")
            },
            crate::pac::rcc::vals::Usart16910sel::CSI => unsafe {
                unwrap ! (crate :: rcc :: get_freqs () . csi . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "USART1" , "csi")
            },
            crate::pac::rcc::vals::Usart16910sel::LSE => unsafe {
                unwrap ! (crate :: rcc :: get_freqs () . lse . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "USART1" , "lse")
            },
            #[allow(unreachable_patterns)]
            _ => panic!(
                "attempted to use peripheral '{}' but its clock mux is not set to a valid \
                         clock. Change 'config.rcc.mux' to another clock.",
                "USART1"
            ),
        }
    }
    fn bus_frequency() -> crate::time::Hertz {
        unsafe {
            unwrap ! (crate :: rcc :: get_freqs () . pclk2 . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "USART1" , "pclk2")
        }
    }
    const RCC_INFO: crate::rcc::RccInfo = unsafe {
        crate::rcc::RccInfo::new(
            Some((38u8, 4u8)),
            (60u8, 4u8),
            None,
            #[cfg(feature = "low-power")]
            crate::rcc::StopMode::Stop1,
        )
    };
}
impl crate::rcc::RccPeripheral for peripherals::USART1 {}
impl crate::rcc::SealedRccPeripheral for peripherals::USART10 {
    fn frequency() -> crate::time::Hertz {
        match crate::pac::RCC.d2ccip2r().read().usart16910sel() {
            crate::pac::rcc::vals::Usart16910sel::PCLK2 => unsafe {
                unwrap ! (crate :: rcc :: get_freqs () . pclk2 . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "USART10" , "pclk2")
            },
            crate::pac::rcc::vals::Usart16910sel::PLL2_Q => unsafe {
                unwrap ! (crate :: rcc :: get_freqs () . pll2_q . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "USART10" , "pll2_q")
            },
            crate::pac::rcc::vals::Usart16910sel::PLL3_Q => unsafe {
                unwrap ! (crate :: rcc :: get_freqs () . pll3_q . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "USART10" , "pll3_q")
            },
            crate::pac::rcc::vals::Usart16910sel::HSI => unsafe {
                unwrap ! (crate :: rcc :: get_freqs () . hsi . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "USART10" , "hsi")
            },
            crate::pac::rcc::vals::Usart16910sel::CSI => unsafe {
                unwrap ! (crate :: rcc :: get_freqs () . csi . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "USART10" , "csi")
            },
            crate::pac::rcc::vals::Usart16910sel::LSE => unsafe {
                unwrap ! (crate :: rcc :: get_freqs () . lse . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "USART10" , "lse")
            },
            #[allow(unreachable_patterns)]
            _ => panic!(
                "attempted to use peripheral '{}' but its clock mux is not set to a valid \
                         clock. Change 'config.rcc.mux' to another clock.",
                "USART10"
            ),
        }
    }
    fn bus_frequency() -> crate::time::Hertz {
        unsafe {
            unwrap ! (crate :: rcc :: get_freqs () . pclk2 . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "USART10" , "pclk2")
        }
    }
    const RCC_INFO: crate::rcc::RccInfo = unsafe {
        crate::rcc::RccInfo::new(
            Some((38u8, 7u8)),
            (60u8, 7u8),
            None,
            #[cfg(feature = "low-power")]
            crate::rcc::StopMode::Stop1,
        )
    };
}
impl crate::rcc::RccPeripheral for peripherals::USART10 {}
impl crate::rcc::SealedRccPeripheral for peripherals::USART2 {
    fn frequency() -> crate::time::Hertz {
        match crate::pac::RCC.d2ccip2r().read().usart234578sel() {
            crate::pac::rcc::vals::Usart234578sel::PCLK1 => unsafe {
                unwrap ! (crate :: rcc :: get_freqs () . pclk1 . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "USART2" , "pclk1")
            },
            crate::pac::rcc::vals::Usart234578sel::PLL2_Q => unsafe {
                unwrap ! (crate :: rcc :: get_freqs () . pll2_q . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "USART2" , "pll2_q")
            },
            crate::pac::rcc::vals::Usart234578sel::PLL3_Q => unsafe {
                unwrap ! (crate :: rcc :: get_freqs () . pll3_q . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "USART2" , "pll3_q")
            },
            crate::pac::rcc::vals::Usart234578sel::HSI => unsafe {
                unwrap ! (crate :: rcc :: get_freqs () . hsi . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "USART2" , "hsi")
            },
            crate::pac::rcc::vals::Usart234578sel::CSI => unsafe {
                unwrap ! (crate :: rcc :: get_freqs () . csi . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "USART2" , "csi")
            },
            crate::pac::rcc::vals::Usart234578sel::LSE => unsafe {
                unwrap ! (crate :: rcc :: get_freqs () . lse . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "USART2" , "lse")
            },
            #[allow(unreachable_patterns)]
            _ => panic!(
                "attempted to use peripheral '{}' but its clock mux is not set to a valid \
                         clock. Change 'config.rcc.mux' to another clock.",
                "USART2"
            ),
        }
    }
    fn bus_frequency() -> crate::time::Hertz {
        unsafe {
            unwrap ! (crate :: rcc :: get_freqs () . pclk1 . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "USART2" , "pclk1")
        }
    }
    const RCC_INFO: crate::rcc::RccInfo = unsafe {
        crate::rcc::RccInfo::new(
            Some((36u8, 17u8)),
            (58u8, 17u8),
            None,
            #[cfg(feature = "low-power")]
            crate::rcc::StopMode::Stop1,
        )
    };
}
impl crate::rcc::RccPeripheral for peripherals::USART2 {}
impl crate::rcc::SealedRccPeripheral for peripherals::USART3 {
    fn frequency() -> crate::time::Hertz {
        match crate::pac::RCC.d2ccip2r().read().usart234578sel() {
            crate::pac::rcc::vals::Usart234578sel::PCLK1 => unsafe {
                unwrap ! (crate :: rcc :: get_freqs () . pclk1 . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "USART3" , "pclk1")
            },
            crate::pac::rcc::vals::Usart234578sel::PLL2_Q => unsafe {
                unwrap ! (crate :: rcc :: get_freqs () . pll2_q . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "USART3" , "pll2_q")
            },
            crate::pac::rcc::vals::Usart234578sel::PLL3_Q => unsafe {
                unwrap ! (crate :: rcc :: get_freqs () . pll3_q . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "USART3" , "pll3_q")
            },
            crate::pac::rcc::vals::Usart234578sel::HSI => unsafe {
                unwrap ! (crate :: rcc :: get_freqs () . hsi . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "USART3" , "hsi")
            },
            crate::pac::rcc::vals::Usart234578sel::CSI => unsafe {
                unwrap ! (crate :: rcc :: get_freqs () . csi . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "USART3" , "csi")
            },
            crate::pac::rcc::vals::Usart234578sel::LSE => unsafe {
                unwrap ! (crate :: rcc :: get_freqs () . lse . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "USART3" , "lse")
            },
            #[allow(unreachable_patterns)]
            _ => panic!(
                "attempted to use peripheral '{}' but its clock mux is not set to a valid \
                         clock. Change 'config.rcc.mux' to another clock.",
                "USART3"
            ),
        }
    }
    fn bus_frequency() -> crate::time::Hertz {
        unsafe {
            unwrap ! (crate :: rcc :: get_freqs () . pclk1 . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "USART3" , "pclk1")
        }
    }
    const RCC_INFO: crate::rcc::RccInfo = unsafe {
        crate::rcc::RccInfo::new(
            Some((36u8, 18u8)),
            (58u8, 18u8),
            None,
            #[cfg(feature = "low-power")]
            crate::rcc::StopMode::Stop1,
        )
    };
}
impl crate::rcc::RccPeripheral for peripherals::USART3 {}
impl crate::rcc::SealedRccPeripheral for peripherals::USART6 {
    fn frequency() -> crate::time::Hertz {
        match crate::pac::RCC.d2ccip2r().read().usart16910sel() {
            crate::pac::rcc::vals::Usart16910sel::PCLK2 => unsafe {
                unwrap ! (crate :: rcc :: get_freqs () . pclk2 . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "USART6" , "pclk2")
            },
            crate::pac::rcc::vals::Usart16910sel::PLL2_Q => unsafe {
                unwrap ! (crate :: rcc :: get_freqs () . pll2_q . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "USART6" , "pll2_q")
            },
            crate::pac::rcc::vals::Usart16910sel::PLL3_Q => unsafe {
                unwrap ! (crate :: rcc :: get_freqs () . pll3_q . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "USART6" , "pll3_q")
            },
            crate::pac::rcc::vals::Usart16910sel::HSI => unsafe {
                unwrap ! (crate :: rcc :: get_freqs () . hsi . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "USART6" , "hsi")
            },
            crate::pac::rcc::vals::Usart16910sel::CSI => unsafe {
                unwrap ! (crate :: rcc :: get_freqs () . csi . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "USART6" , "csi")
            },
            crate::pac::rcc::vals::Usart16910sel::LSE => unsafe {
                unwrap ! (crate :: rcc :: get_freqs () . lse . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "USART6" , "lse")
            },
            #[allow(unreachable_patterns)]
            _ => panic!(
                "attempted to use peripheral '{}' but its clock mux is not set to a valid \
                         clock. Change 'config.rcc.mux' to another clock.",
                "USART6"
            ),
        }
    }
    fn bus_frequency() -> crate::time::Hertz {
        unsafe {
            unwrap ! (crate :: rcc :: get_freqs () . pclk2 . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "USART6" , "pclk2")
        }
    }
    const RCC_INFO: crate::rcc::RccInfo = unsafe {
        crate::rcc::RccInfo::new(
            Some((38u8, 5u8)),
            (60u8, 5u8),
            None,
            #[cfg(feature = "low-power")]
            crate::rcc::StopMode::Stop1,
        )
    };
}
impl crate::rcc::RccPeripheral for peripherals::USART6 {}
impl crate::rcc::SealedRccPeripheral for peripherals::USB_OTG_HS {
    fn frequency() -> crate::time::Hertz {
        match crate::pac::RCC.d2ccip2r().read().usbsel() {
            crate::pac::rcc::vals::Usbsel::PLL1_Q => unsafe {
                unwrap ! (crate :: rcc :: get_freqs () . pll1_q . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "USB_OTG_HS" , "pll1_q")
            },
            crate::pac::rcc::vals::Usbsel::PLL3_Q => unsafe {
                unwrap ! (crate :: rcc :: get_freqs () . pll3_q . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "USB_OTG_HS" , "pll3_q")
            },
            crate::pac::rcc::vals::Usbsel::HSI48 => unsafe {
                unwrap ! (crate :: rcc :: get_freqs () . hsi48 . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "USB_OTG_HS" , "hsi48")
            },
            #[allow(unreachable_patterns)]
            _ => panic!(
                "attempted to use peripheral '{}' but its clock mux is not set to a valid \
                         clock. Change 'config.rcc.mux' to another clock.",
                "USB_OTG_HS"
            ),
        }
    }
    fn bus_frequency() -> crate::time::Hertz {
        unsafe {
            unwrap ! (crate :: rcc :: get_freqs () . hclk1 . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "USB_OTG_HS" , "hclk1")
        }
    }
    const RCC_INFO: crate::rcc::RccInfo = unsafe {
        crate::rcc::RccInfo::new(
            Some((32u8, 25u8)),
            (54u8, 25u8),
            None,
            #[cfg(feature = "low-power")]
            crate::rcc::StopMode::Stop1,
        )
    };
}
impl crate::rcc::RccPeripheral for peripherals::USB_OTG_HS {}
impl crate::rcc::SealedRccPeripheral for peripherals::WWDG1 {
    fn frequency() -> crate::time::Hertz {
        unsafe {
            unwrap ! (crate :: rcc :: get_freqs () . pclk3 . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "WWDG1" , "pclk3")
        }
    }
    fn bus_frequency() -> crate::time::Hertz {
        unsafe {
            unwrap ! (crate :: rcc :: get_freqs () . pclk3 . to_hertz () , "peripheral '{}' is configured to use the '{}' clock, which is not running. \
                    Either enable it in 'config.rcc' or change 'config.rcc.mux' to use another clock" , "WWDG1" , "pclk3")
        }
    }
    const RCC_INFO: crate::rcc::RccInfo = unsafe {
        crate::rcc::RccInfo::new(
            None,
            (57u8, 6u8),
            None,
            #[cfg(feature = "low-power")]
            crate::rcc::StopMode::Stop1,
        )
    };
}
impl crate::rcc::RccPeripheral for peripherals::WWDG1 {}
pub(crate) static mut REFCOUNTS: [u8; 3usize] = [0u8, 0u8, 0u8];
pub mod mux {
    pub use crate::pac::rcc::vals::Adcsel;
    pub use crate::pac::rcc::vals::Cecsel;
    pub use crate::pac::rcc::vals::Fdcansel;
    pub use crate::pac::rcc::vals::Fmcsel;
    pub use crate::pac::rcc::vals::I2c1235sel;
    pub use crate::pac::rcc::vals::I2c4sel;
    pub use crate::pac::rcc::vals::Lptim1sel;
    pub use crate::pac::rcc::vals::Lptim2sel;
    pub use crate::pac::rcc::vals::Lpuartsel;
    pub use crate::pac::rcc::vals::Persel;
    pub use crate::pac::rcc::vals::Rngsel;
    pub use crate::pac::rcc::vals::Rtcsel;
    pub use crate::pac::rcc::vals::Saisel;
    pub use crate::pac::rcc::vals::Sdmmcsel;
    pub use crate::pac::rcc::vals::Spdifrxsel;
    pub use crate::pac::rcc::vals::Spi45sel;
    pub use crate::pac::rcc::vals::Spi6sel;
    pub use crate::pac::rcc::vals::Usart16910sel;
    pub use crate::pac::rcc::vals::Usart234578sel;
    pub use crate::pac::rcc::vals::Usbsel;
    #[derive(Clone, Copy)]
    #[non_exhaustive]
    pub struct ClockMux {
        pub rtcsel: Rtcsel,
        pub fmcsel: Fmcsel,
        pub octospisel: Fmcsel,
        pub persel: Persel,
        pub sdmmcsel: Sdmmcsel,
        pub fdcansel: Fdcansel,
        pub sai1sel: Saisel,
        pub spdifrxsel: Spdifrxsel,
        pub spi123sel: Saisel,
        pub spi45sel: Spi45sel,
        pub cecsel: Cecsel,
        pub i2c1235sel: I2c1235sel,
        pub lptim1sel: Lptim1sel,
        pub rngsel: Rngsel,
        pub usart16910sel: Usart16910sel,
        pub usart234578sel: Usart234578sel,
        pub usbsel: Usbsel,
        pub adcsel: Adcsel,
        pub i2c4sel: I2c4sel,
        pub lptim2sel: Lptim2sel,
        pub lpuart1sel: Lpuartsel,
        pub spi6sel: Spi6sel,
    }
    impl ClockMux {
        pub(crate) const fn default() -> Self {
            unsafe { ::core::mem::zeroed() }
        }
    }
    impl Default for ClockMux {
        fn default() -> Self {
            Self::default()
        }
    }
    impl ClockMux {
        pub(crate) fn init(&self) {
            crate::pac::RCC.bdcr().modify(|w| {
                w.set_rtcsel(self.rtcsel);
            });
            crate::pac::RCC.d1ccipr().modify(|w| {
                w.set_fmcsel(self.fmcsel);
                w.set_octospisel(self.octospisel);
                w.set_persel(self.persel);
                w.set_sdmmcsel(self.sdmmcsel);
            });
            crate::pac::RCC.d2ccip1r().modify(|w| {
                w.set_fdcansel(self.fdcansel);
                w.set_sai1sel(self.sai1sel);
                w.set_spdifrxsel(self.spdifrxsel);
                w.set_spi123sel(self.spi123sel);
                w.set_spi45sel(self.spi45sel);
            });
            crate::pac::RCC.d2ccip2r().modify(|w| {
                w.set_cecsel(self.cecsel);
                w.set_i2c1235sel(self.i2c1235sel);
                w.set_lptim1sel(self.lptim1sel);
                w.set_rngsel(self.rngsel);
                w.set_usart16910sel(self.usart16910sel);
                w.set_usart234578sel(self.usart234578sel);
                w.set_usbsel(self.usbsel);
            });
            crate::pac::RCC.d3ccipr().modify(|w| {
                w.set_adcsel(self.adcsel);
                w.set_i2c4sel(self.i2c4sel);
                w.set_lptim2sel(self.lptim2sel);
                w.set_lpuart1sel(self.lpuart1sel);
                w.set_spi6sel(self.spi6sel);
            });
        }
    }
}
#[derive(Clone, Copy, Debug)]
#[cfg_attr(feature = "defmt", derive(defmt::Format))]
#[repr(C)]
pub struct Clocks {
    pub csi: crate::time::MaybeHertz,
    pub hclk1: crate::time::MaybeHertz,
    pub hclk2: crate::time::MaybeHertz,
    pub hclk3: crate::time::MaybeHertz,
    pub hclk4: crate::time::MaybeHertz,
    pub hse: crate::time::MaybeHertz,
    pub hsi: crate::time::MaybeHertz,
    pub hsi48: crate::time::MaybeHertz,
    pub i2s_ckin: crate::time::MaybeHertz,
    pub lse: crate::time::MaybeHertz,
    pub lsi: crate::time::MaybeHertz,
    pub pclk1: crate::time::MaybeHertz,
    pub pclk1_tim: crate::time::MaybeHertz,
    pub pclk2: crate::time::MaybeHertz,
    pub pclk2_tim: crate::time::MaybeHertz,
    pub pclk3: crate::time::MaybeHertz,
    pub pclk4: crate::time::MaybeHertz,
    pub pll1_q: crate::time::MaybeHertz,
    pub pll2_p: crate::time::MaybeHertz,
    pub pll2_q: crate::time::MaybeHertz,
    pub pll2_r: crate::time::MaybeHertz,
    pub pll3_p: crate::time::MaybeHertz,
    pub pll3_q: crate::time::MaybeHertz,
    pub pll3_r: crate::time::MaybeHertz,
    pub rtc: crate::time::MaybeHertz,
    pub sys: crate::time::MaybeHertz,
}
pub unsafe fn init_dma() {
    crate::pac::RCC.ahb1enr().modify(|w| w.set_dma1en(true));
    crate::pac::RCC.ahb1enr().modify(|w| w.set_dma2en(true));
}
pub unsafe fn init_bdma() {
    crate::pac::RCC.ahb4enr().modify(|w| w.set_bdmaen(true));
}
pub unsafe fn init_dmamux() {}
pub unsafe fn init_gpdma() {}
pub unsafe fn init_gpio() {
    crate::pac::RCC.ahb4enr().modify(|w| w.set_gpioaen(true));
    crate::pac::RCC.ahb4enr().modify(|w| w.set_gpioben(true));
    crate::pac::RCC.ahb4enr().modify(|w| w.set_gpiocen(true));
    crate::pac::RCC.ahb4enr().modify(|w| w.set_gpioden(true));
    crate::pac::RCC.ahb4enr().modify(|w| w.set_gpioeen(true));
    crate::pac::RCC.ahb4enr().modify(|w| w.set_gpiofen(true));
    crate::pac::RCC.ahb4enr().modify(|w| w.set_gpiogen(true));
    crate::pac::RCC.ahb4enr().modify(|w| w.set_gpiohen(true));
    crate::pac::RCC.ahb4enr().modify(|w| w.set_gpiojen(true));
    crate::pac::RCC.ahb4enr().modify(|w| w.set_gpioken(true));
}
impl_adc_pin!(ADC1, PA0, 16u8);
impl_adc_pin!(ADC1, PA1, 17u8);
impl_adc_pin!(ADC1, PA2, 14u8);
impl_adc_pin!(ADC1, PA3, 15u8);
impl_adc_pin!(ADC1, PA4, 18u8);
impl_adc_pin!(ADC1, PA5, 19u8);
impl_adc_pin!(ADC1, PA6, 3u8);
impl_adc_pin!(ADC1, PA7, 7u8);
impl_adc_pin!(ADC1, PB0, 9u8);
impl_adc_pin!(ADC1, PB1, 5u8);
impl_adc_pin!(ADC1, PC0, 10u8);
impl_adc_pin!(ADC1, PC1, 11u8);
impl_adc_pin!(ADC1, PC4, 4u8);
impl_adc_pin!(ADC1, PC5, 8u8);
impl_adc_pin!(ADC2, PA2, 14u8);
impl_adc_pin!(ADC2, PA3, 15u8);
impl_adc_pin!(ADC2, PA4, 18u8);
impl_adc_pin!(ADC2, PA5, 19u8);
impl_adc_pin!(ADC2, PA6, 3u8);
impl_adc_pin!(ADC2, PA7, 7u8);
impl_adc_pin!(ADC2, PB0, 9u8);
impl_adc_pin!(ADC2, PB1, 5u8);
impl_adc_pin!(ADC2, PC0, 10u8);
impl_adc_pin!(ADC2, PC1, 11u8);
impl_adc_pin!(ADC2, PC4, 4u8);
impl_adc_pin!(ADC2, PC5, 8u8);
impl_adc_pin!(ADC3, PC0, 10u8);
impl_adc_pin!(ADC3, PC1, 11u8);
pin_trait_impl!(crate::dac::DacPin<Ch1>, DAC1, PA4, 0u8);
pin_trait_impl!(crate::dac::DacPin<Ch2>, DAC1, PA5, 0u8);
pin_trait_impl!(crate::dcmi::D1Pin, DCMI, PA10, 13u8);
pin_trait_impl!(crate::dcmi::HSyncPin, DCMI, PA4, 13u8);
pin_trait_impl!(crate::dcmi::PixClkPin, DCMI, PA6, 13u8);
pin_trait_impl!(crate::dcmi::D0Pin, DCMI, PA9, 13u8);
pin_trait_impl!(crate::dcmi::D2Pin, DCMI, PB13, 13u8);
pin_trait_impl!(crate::dcmi::D10Pin, DCMI, PB5, 13u8);
pin_trait_impl!(crate::dcmi::D5Pin, DCMI, PB6, 13u8);
pin_trait_impl!(crate::dcmi::VSyncPin, DCMI, PB7, 13u8);
pin_trait_impl!(crate::dcmi::D6Pin, DCMI, PB8, 13u8);
pin_trait_impl!(crate::dcmi::D7Pin, DCMI, PB9, 13u8);
pin_trait_impl!(crate::dcmi::D8Pin, DCMI, PC10, 13u8);
pin_trait_impl!(crate::dcmi::D4Pin, DCMI, PC11, 13u8);
pin_trait_impl!(crate::dcmi::D9Pin, DCMI, PC12, 13u8);
pin_trait_impl!(crate::dcmi::D0Pin, DCMI, PC6, 13u8);
pin_trait_impl!(crate::dcmi::D1Pin, DCMI, PC7, 13u8);
pin_trait_impl!(crate::dcmi::D2Pin, DCMI, PC8, 13u8);
pin_trait_impl!(crate::dcmi::D3Pin, DCMI, PC9, 13u8);
pin_trait_impl!(crate::dcmi::D12Pin, DCMI, PD12, 13u8);
pin_trait_impl!(crate::dcmi::D13Pin, DCMI, PD13, 13u8);
pin_trait_impl!(crate::dcmi::D11Pin, DCMI, PD2, 13u8);
pin_trait_impl!(crate::dcmi::D5Pin, DCMI, PD3, 13u8);
pin_trait_impl!(crate::dcmi::D10Pin, DCMI, PD6, 13u8);
pin_trait_impl!(crate::dcmi::D2Pin, DCMI, PE0, 13u8);
pin_trait_impl!(crate::dcmi::D3Pin, DCMI, PE1, 13u8);
pin_trait_impl!(crate::dcmi::D4Pin, DCMI, PE4, 13u8);
pin_trait_impl!(crate::dcmi::D6Pin, DCMI, PE5, 13u8);
pin_trait_impl!(crate::dcmi::D7Pin, DCMI, PE6, 13u8);
pin_trait_impl!(crate::eth::RefClkPin, ETH, PA1, 11u8);
pin_trait_impl!(crate::eth::RXClkPin, ETH, PA1, 11u8);
pin_trait_impl!(crate::eth::MDIOPin, ETH, PA2, 11u8);
pin_trait_impl!(crate::eth::CRSPin, ETH, PA7, 11u8);
pin_trait_impl!(crate::eth::RXDVPin, ETH, PA7, 11u8);
pin_trait_impl!(crate::eth::RXD2Pin, ETH, PB0, 11u8);
pin_trait_impl!(crate::eth::RXD3Pin, ETH, PB1, 11u8);
pin_trait_impl!(crate::eth::TXEnPin, ETH, PB11, 11u8);
pin_trait_impl!(crate::eth::TXD0Pin, ETH, PB12, 11u8);
pin_trait_impl!(crate::eth::TXD1Pin, ETH, PB13, 11u8);
pin_trait_impl!(crate::eth::TXD3Pin, ETH, PB8, 11u8);
pin_trait_impl!(crate::eth::MDCPin, ETH, PC1, 11u8);
pin_trait_impl!(crate::eth::TXD2Pin, ETH, PC2, 11u8);
pin_trait_impl!(crate::eth::TXClkPin, ETH, PC3, 11u8);
pin_trait_impl!(crate::eth::RXD0Pin, ETH, PC4, 11u8);
pin_trait_impl!(crate::eth::RXD1Pin, ETH, PC5, 11u8);
pin_trait_impl!(crate::eth::TXD3Pin, ETH, PE2, 11u8);
pin_trait_impl!(crate::can::RxPin, FDCAN1, PA11, 9u8);
pin_trait_impl!(crate::can::TxPin, FDCAN1, PA12, 9u8);
pin_trait_impl!(crate::can::RxPin, FDCAN1, PB8, 9u8);
pin_trait_impl!(crate::can::TxPin, FDCAN1, PB9, 9u8);
pin_trait_impl!(crate::can::RxPin, FDCAN1, PD0, 9u8);
pin_trait_impl!(crate::can::TxPin, FDCAN1, PD1, 9u8);
pin_trait_impl!(crate::can::RxPin, FDCAN2, PB12, 9u8);
pin_trait_impl!(crate::can::TxPin, FDCAN2, PB13, 9u8);
pin_trait_impl!(crate::can::RxPin, FDCAN2, PB5, 9u8);
pin_trait_impl!(crate::can::TxPin, FDCAN2, PB6, 9u8);
pin_trait_impl!(crate::can::RxPin, FDCAN3, PD12, 5u8);
pin_trait_impl!(crate::can::TxPin, FDCAN3, PD13, 5u8);
pin_trait_impl!(crate::fmc::A19Pin, FMC, PA0, 12u8);
pin_trait_impl!(crate::fmc::D8Pin, FMC, PA4, 12u8);
pin_trait_impl!(crate::fmc::DA8Pin, FMC, PA4, 12u8);
pin_trait_impl!(crate::fmc::D9Pin, FMC, PA5, 12u8);
pin_trait_impl!(crate::fmc::DA9Pin, FMC, PA5, 12u8);
pin_trait_impl!(crate::fmc::SDNWEPin, FMC, PA7, 12u8);
pin_trait_impl!(crate::fmc::D10Pin, FMC, PB14, 12u8);
pin_trait_impl!(crate::fmc::DA10Pin, FMC, PB14, 12u8);
pin_trait_impl!(crate::fmc::D11Pin, FMC, PB15, 12u8);
pin_trait_impl!(crate::fmc::DA11Pin, FMC, PB15, 12u8);
pin_trait_impl!(crate::fmc::SDCKE1Pin, FMC, PB5, 12u8);
pin_trait_impl!(crate::fmc::SDNE1Pin, FMC, PB6, 12u8);
pin_trait_impl!(crate::fmc::NLPin, FMC, PB7, 12u8);
pin_trait_impl!(crate::fmc::A25Pin, FMC, PC0, 9u8);
pin_trait_impl!(crate::fmc::D12Pin, FMC, PC0, 1u8);
pin_trait_impl!(crate::fmc::DA12Pin, FMC, PC0, 1u8);
pin_trait_impl!(crate::fmc::SDNWEPin, FMC, PC0, 12u8);
pin_trait_impl!(crate::fmc::D6Pin, FMC, PC12, 1u8);
pin_trait_impl!(crate::fmc::DA6Pin, FMC, PC12, 1u8);
pin_trait_impl!(crate::fmc::SDNE0Pin, FMC, PC2, 12u8);
pin_trait_impl!(crate::fmc::SDCKE0Pin, FMC, PC3, 12u8);
pin_trait_impl!(crate::fmc::A22Pin, FMC, PC4, 1u8);
pin_trait_impl!(crate::fmc::SDNE0Pin, FMC, PC4, 12u8);
pin_trait_impl!(crate::fmc::SDCKE0Pin, FMC, PC5, 12u8);
pin_trait_impl!(crate::fmc::NWaitPin, FMC, PC6, 9u8);
pin_trait_impl!(crate::fmc::NE1Pin, FMC, PC7, 9u8);
pin_trait_impl!(crate::fmc::INTPin, FMC, PC8, 10u8);
pin_trait_impl!(crate::fmc::NCEPin, FMC, PC8, 9u8);
pin_trait_impl!(crate::fmc::NE2Pin, FMC, PC8, 9u8);
pin_trait_impl!(crate::fmc::D2Pin, FMC, PD0, 12u8);
pin_trait_impl!(crate::fmc::DA2Pin, FMC, PD0, 12u8);
pin_trait_impl!(crate::fmc::D3Pin, FMC, PD1, 12u8);
pin_trait_impl!(crate::fmc::DA3Pin, FMC, PD1, 12u8);
pin_trait_impl!(crate::fmc::D15Pin, FMC, PD10, 12u8);
pin_trait_impl!(crate::fmc::DA15Pin, FMC, PD10, 12u8);
pin_trait_impl!(crate::fmc::A16Pin, FMC, PD11, 12u8);
pin_trait_impl!(crate::fmc::A17Pin, FMC, PD12, 12u8);
pin_trait_impl!(crate::fmc::A18Pin, FMC, PD13, 12u8);
pin_trait_impl!(crate::fmc::D0Pin, FMC, PD14, 12u8);
pin_trait_impl!(crate::fmc::DA0Pin, FMC, PD14, 12u8);
pin_trait_impl!(crate::fmc::D1Pin, FMC, PD15, 12u8);
pin_trait_impl!(crate::fmc::DA1Pin, FMC, PD15, 12u8);
pin_trait_impl!(crate::fmc::D7Pin, FMC, PD2, 1u8);
pin_trait_impl!(crate::fmc::DA7Pin, FMC, PD2, 1u8);
pin_trait_impl!(crate::fmc::ClkPin, FMC, PD3, 12u8);
pin_trait_impl!(crate::fmc::NOEPin, FMC, PD4, 12u8);
pin_trait_impl!(crate::fmc::NWEPin, FMC, PD5, 12u8);
pin_trait_impl!(crate::fmc::NWaitPin, FMC, PD6, 12u8);
pin_trait_impl!(crate::fmc::NE1Pin, FMC, PD7, 12u8);
pin_trait_impl!(crate::fmc::D13Pin, FMC, PD8, 12u8);
pin_trait_impl!(crate::fmc::DA13Pin, FMC, PD8, 12u8);
pin_trait_impl!(crate::fmc::D14Pin, FMC, PD9, 12u8);
pin_trait_impl!(crate::fmc::DA14Pin, FMC, PD9, 12u8);
pin_trait_impl!(crate::fmc::NBL0Pin, FMC, PE0, 12u8);
pin_trait_impl!(crate::fmc::NBL1Pin, FMC, PE1, 12u8);
pin_trait_impl!(crate::fmc::D7Pin, FMC, PE10, 12u8);
pin_trait_impl!(crate::fmc::DA7Pin, FMC, PE10, 12u8);
pin_trait_impl!(crate::fmc::D8Pin, FMC, PE11, 12u8);
pin_trait_impl!(crate::fmc::DA8Pin, FMC, PE11, 12u8);
pin_trait_impl!(crate::fmc::D9Pin, FMC, PE12, 12u8);
pin_trait_impl!(crate::fmc::DA9Pin, FMC, PE12, 12u8);
pin_trait_impl!(crate::fmc::D10Pin, FMC, PE13, 12u8);
pin_trait_impl!(crate::fmc::DA10Pin, FMC, PE13, 12u8);
pin_trait_impl!(crate::fmc::D11Pin, FMC, PE14, 12u8);
pin_trait_impl!(crate::fmc::DA11Pin, FMC, PE14, 12u8);
pin_trait_impl!(crate::fmc::D12Pin, FMC, PE15, 12u8);
pin_trait_impl!(crate::fmc::DA12Pin, FMC, PE15, 12u8);
pin_trait_impl!(crate::fmc::A23Pin, FMC, PE2, 12u8);
pin_trait_impl!(crate::fmc::A19Pin, FMC, PE3, 12u8);
pin_trait_impl!(crate::fmc::A20Pin, FMC, PE4, 12u8);
pin_trait_impl!(crate::fmc::A21Pin, FMC, PE5, 12u8);
pin_trait_impl!(crate::fmc::A22Pin, FMC, PE6, 12u8);
pin_trait_impl!(crate::fmc::D4Pin, FMC, PE7, 12u8);
pin_trait_impl!(crate::fmc::DA4Pin, FMC, PE7, 12u8);
pin_trait_impl!(crate::fmc::D5Pin, FMC, PE8, 12u8);
pin_trait_impl!(crate::fmc::DA5Pin, FMC, PE8, 12u8);
pin_trait_impl!(crate::fmc::D6Pin, FMC, PE9, 12u8);
pin_trait_impl!(crate::fmc::DA6Pin, FMC, PE9, 12u8);
pin_trait_impl!(crate::i2c::SclPin, I2C1, PB6, 4u8);
pin_trait_impl!(crate::i2c::SdaPin, I2C1, PB7, 4u8);
pin_trait_impl!(crate::i2c::SclPin, I2C1, PB8, 4u8);
pin_trait_impl!(crate::i2c::SdaPin, I2C1, PB9, 4u8);
pin_trait_impl!(crate::i2c::SclPin, I2C2, PB10, 4u8);
pin_trait_impl!(crate::i2c::SdaPin, I2C2, PB11, 4u8);
pin_trait_impl!(crate::i2c::SclPin, I2C3, PA8, 4u8);
pin_trait_impl!(crate::i2c::SdaPin, I2C3, PC9, 4u8);
pin_trait_impl!(crate::i2c::SclPin, I2C4, PB6, 6u8);
pin_trait_impl!(crate::i2c::SdaPin, I2C4, PB7, 6u8);
pin_trait_impl!(crate::i2c::SclPin, I2C4, PB8, 6u8);
pin_trait_impl!(crate::i2c::SdaPin, I2C4, PB9, 6u8);
pin_trait_impl!(crate::i2c::SclPin, I2C4, PD12, 4u8);
pin_trait_impl!(crate::i2c::SdaPin, I2C4, PD13, 4u8);
pin_trait_impl!(crate::i2c::SclPin, I2C5, PA8, 6u8);
pin_trait_impl!(crate::i2c::SdaPin, I2C5, PC10, 4u8);
pin_trait_impl!(crate::i2c::SclPin, I2C5, PC11, 4u8);
pin_trait_impl!(crate::i2c::SdaPin, I2C5, PC9, 6u8);
pin_trait_impl!(crate::lptim::OutputPin, LPTIM1, PD13, 1u8);
pin_trait_impl!(crate::lptim::OutputPin, LPTIM2, PB13, 3u8);
pin_trait_impl!(crate::lptim::OutputPin, LPTIM3, PA1, 3u8);
pin_trait_impl!(crate::lptim::OutputPin, LPTIM4, PA2, 3u8);
pin_trait_impl!(crate::lptim::OutputPin, LPTIM5, PA3, 3u8);
pin_trait_impl!(crate::usart::RxPin, LPUART1, PA10, 3u8);
pin_trait_impl!(crate::usart::CtsPin, LPUART1, PA11, 3u8);
pin_trait_impl!(crate::usart::DePin, LPUART1, PA12, 3u8);
pin_trait_impl!(crate::usart::RtsPin, LPUART1, PA12, 3u8);
pin_trait_impl!(crate::usart::TxPin, LPUART1, PA9, 3u8);
pin_trait_impl!(crate::usart::TxPin, LPUART1, PB6, 8u8);
pin_trait_impl!(crate::usart::RxPin, LPUART1, PB7, 8u8);
pin_trait_impl!(crate::ltdc::R2Pin, LTDC, PA1, 14u8);
pin_trait_impl!(crate::ltdc::B1Pin, LTDC, PA10, 14u8);
pin_trait_impl!(crate::ltdc::B4Pin, LTDC, PA10, 12u8);
pin_trait_impl!(crate::ltdc::R4Pin, LTDC, PA11, 14u8);
pin_trait_impl!(crate::ltdc::R5Pin, LTDC, PA12, 14u8);
pin_trait_impl!(crate::ltdc::B6Pin, LTDC, PA15, 14u8);
pin_trait_impl!(crate::ltdc::R3Pin, LTDC, PA15, 9u8);
pin_trait_impl!(crate::ltdc::R1Pin, LTDC, PA2, 14u8);
pin_trait_impl!(crate::ltdc::B2Pin, LTDC, PA3, 9u8);
pin_trait_impl!(crate::ltdc::B5Pin, LTDC, PA3, 14u8);
pin_trait_impl!(crate::ltdc::VsyncPin, LTDC, PA4, 14u8);
pin_trait_impl!(crate::ltdc::R4Pin, LTDC, PA5, 14u8);
pin_trait_impl!(crate::ltdc::G2Pin, LTDC, PA6, 14u8);
pin_trait_impl!(crate::ltdc::VsyncPin, LTDC, PA7, 14u8);
pin_trait_impl!(crate::ltdc::B3Pin, LTDC, PA8, 13u8);
pin_trait_impl!(crate::ltdc::R6Pin, LTDC, PA8, 14u8);
pin_trait_impl!(crate::ltdc::R5Pin, LTDC, PA9, 14u8);
pin_trait_impl!(crate::ltdc::G1Pin, LTDC, PB0, 14u8);
pin_trait_impl!(crate::ltdc::R3Pin, LTDC, PB0, 9u8);
pin_trait_impl!(crate::ltdc::G0Pin, LTDC, PB1, 14u8);
pin_trait_impl!(crate::ltdc::R6Pin, LTDC, PB1, 9u8);
pin_trait_impl!(crate::ltdc::G4Pin, LTDC, PB10, 14u8);
pin_trait_impl!(crate::ltdc::G5Pin, LTDC, PB11, 14u8);
pin_trait_impl!(crate::ltdc::ClkPin, LTDC, PB14, 14u8);
pin_trait_impl!(crate::ltdc::G7Pin, LTDC, PB15, 14u8);
pin_trait_impl!(crate::ltdc::B5Pin, LTDC, PB5, 3u8);
pin_trait_impl!(crate::ltdc::B6Pin, LTDC, PB8, 14u8);
pin_trait_impl!(crate::ltdc::B7Pin, LTDC, PB9, 14u8);
pin_trait_impl!(crate::ltdc::G2Pin, LTDC, PC0, 11u8);
pin_trait_impl!(crate::ltdc::R5Pin, LTDC, PC0, 14u8);
pin_trait_impl!(crate::ltdc::G5Pin, LTDC, PC1, 14u8);
pin_trait_impl!(crate::ltdc::B1Pin, LTDC, PC10, 10u8);
pin_trait_impl!(crate::ltdc::R2Pin, LTDC, PC10, 14u8);
pin_trait_impl!(crate::ltdc::B4Pin, LTDC, PC11, 14u8);
pin_trait_impl!(crate::ltdc::R6Pin, LTDC, PC12, 14u8);
pin_trait_impl!(crate::ltdc::R7Pin, LTDC, PC4, 14u8);
pin_trait_impl!(crate::ltdc::DePin, LTDC, PC5, 14u8);
pin_trait_impl!(crate::ltdc::HsyncPin, LTDC, PC6, 14u8);
pin_trait_impl!(crate::ltdc::G6Pin, LTDC, PC7, 14u8);
pin_trait_impl!(crate::ltdc::B2Pin, LTDC, PC9, 14u8);
pin_trait_impl!(crate::ltdc::G3Pin, LTDC, PC9, 10u8);
pin_trait_impl!(crate::ltdc::B1Pin, LTDC, PD0, 14u8);
pin_trait_impl!(crate::ltdc::B3Pin, LTDC, PD10, 14u8);
pin_trait_impl!(crate::ltdc::B2Pin, LTDC, PD2, 14u8);
pin_trait_impl!(crate::ltdc::B7Pin, LTDC, PD2, 9u8);
pin_trait_impl!(crate::ltdc::G7Pin, LTDC, PD3, 14u8);
pin_trait_impl!(crate::ltdc::B2Pin, LTDC, PD6, 14u8);
pin_trait_impl!(crate::ltdc::R0Pin, LTDC, PE0, 14u8);
pin_trait_impl!(crate::ltdc::R6Pin, LTDC, PE1, 14u8);
pin_trait_impl!(crate::ltdc::G3Pin, LTDC, PE11, 14u8);
pin_trait_impl!(crate::ltdc::B4Pin, LTDC, PE12, 14u8);
pin_trait_impl!(crate::ltdc::DePin, LTDC, PE13, 14u8);
pin_trait_impl!(crate::ltdc::ClkPin, LTDC, PE14, 14u8);
pin_trait_impl!(crate::ltdc::R7Pin, LTDC, PE15, 14u8);
pin_trait_impl!(crate::ltdc::B0Pin, LTDC, PE4, 14u8);
pin_trait_impl!(crate::ltdc::G0Pin, LTDC, PE5, 14u8);
pin_trait_impl!(crate::ltdc::G1Pin, LTDC, PE6, 14u8);
pin_trait_impl!(crate::ospi::DQSPin, OCTOSPI2, PA1, 12u8);
pin_trait_impl!(crate::ospi::DQSPin, OCTOSPI1, PA1, 12u8);
pin_trait_impl!(crate::ospi::D3Pin, OCTOSPI2, PA1, 9u8);
pin_trait_impl!(crate::ospi::D3Pin, OCTOSPI1, PA1, 9u8);
pin_trait_impl!(crate::ospi::D0Pin, OCTOSPI2, PA2, 6u8);
pin_trait_impl!(crate::ospi::D0Pin, OCTOSPI1, PA2, 6u8);
pin_trait_impl!(crate::ospi::SckPin, OCTOSPI2, PA3, 12u8);
pin_trait_impl!(crate::ospi::SckPin, OCTOSPI1, PA3, 12u8);
pin_trait_impl!(crate::ospi::D2Pin, OCTOSPI2, PA3, 6u8);
pin_trait_impl!(crate::ospi::D2Pin, OCTOSPI1, PA3, 6u8);
pin_trait_impl!(crate::ospi::D3Pin, OCTOSPI2, PA6, 6u8);
pin_trait_impl!(crate::ospi::D3Pin, OCTOSPI1, PA6, 6u8);
pin_trait_impl!(crate::ospi::D2Pin, OCTOSPI2, PA7, 10u8);
pin_trait_impl!(crate::ospi::D2Pin, OCTOSPI1, PA7, 10u8);
pin_trait_impl!(crate::ospi::D1Pin, OCTOSPI2, PB0, 4u8);
pin_trait_impl!(crate::ospi::D1Pin, OCTOSPI1, PB0, 4u8);
pin_trait_impl!(crate::ospi::D0Pin, OCTOSPI2, PB1, 4u8);
pin_trait_impl!(crate::ospi::D0Pin, OCTOSPI1, PB1, 4u8);
pin_trait_impl!(crate::ospi::NSSPin, OCTOSPI2, PB10, 9u8);
pin_trait_impl!(crate::ospi::NSSPin, OCTOSPI1, PB10, 9u8);
pin_trait_impl!(crate::ospi::D0Pin, OCTOSPI2, PB12, 12u8);
pin_trait_impl!(crate::ospi::D0Pin, OCTOSPI1, PB12, 12u8);
pin_trait_impl!(crate::ospi::NckPin, OCTOSPI2, PB12, 3u8);
pin_trait_impl!(crate::ospi::NckPin, OCTOSPI1, PB12, 3u8);
pin_trait_impl!(crate::ospi::D2Pin, OCTOSPI2, PB13, 4u8);
pin_trait_impl!(crate::ospi::D2Pin, OCTOSPI1, PB13, 4u8);
pin_trait_impl!(crate::ospi::SckPin, OCTOSPI2, PB2, 9u8);
pin_trait_impl!(crate::ospi::SckPin, OCTOSPI1, PB2, 9u8);
pin_trait_impl!(crate::ospi::DQSPin, OCTOSPI2, PB2, 10u8);
pin_trait_impl!(crate::ospi::DQSPin, OCTOSPI1, PB2, 10u8);
pin_trait_impl!(crate::ospi::NSSPin, OCTOSPI2, PB6, 10u8);
pin_trait_impl!(crate::ospi::NSSPin, OCTOSPI1, PB6, 10u8);
pin_trait_impl!(crate::ospi::D4Pin, OCTOSPI2, PC1, 10u8);
pin_trait_impl!(crate::ospi::D4Pin, OCTOSPI1, PC1, 10u8);
pin_trait_impl!(crate::ospi::D1Pin, OCTOSPI2, PC10, 9u8);
pin_trait_impl!(crate::ospi::D1Pin, OCTOSPI1, PC10, 9u8);
pin_trait_impl!(crate::ospi::NSSPin, OCTOSPI2, PC11, 9u8);
pin_trait_impl!(crate::ospi::NSSPin, OCTOSPI1, PC11, 9u8);
pin_trait_impl!(crate::ospi::D2Pin, OCTOSPI2, PC2, 9u8);
pin_trait_impl!(crate::ospi::D2Pin, OCTOSPI1, PC2, 9u8);
pin_trait_impl!(crate::ospi::D5Pin, OCTOSPI2, PC2, 4u8);
pin_trait_impl!(crate::ospi::D5Pin, OCTOSPI1, PC2, 4u8);
pin_trait_impl!(crate::ospi::D0Pin, OCTOSPI2, PC3, 9u8);
pin_trait_impl!(crate::ospi::D0Pin, OCTOSPI1, PC3, 9u8);
pin_trait_impl!(crate::ospi::D6Pin, OCTOSPI2, PC3, 4u8);
pin_trait_impl!(crate::ospi::D6Pin, OCTOSPI1, PC3, 4u8);
pin_trait_impl!(crate::ospi::DQSPin, OCTOSPI2, PC5, 10u8);
pin_trait_impl!(crate::ospi::DQSPin, OCTOSPI1, PC5, 10u8);
pin_trait_impl!(crate::ospi::D0Pin, OCTOSPI2, PC9, 9u8);
pin_trait_impl!(crate::ospi::D0Pin, OCTOSPI1, PC9, 9u8);
pin_trait_impl!(crate::ospi::D0Pin, OCTOSPI2, PD11, 9u8);
pin_trait_impl!(crate::ospi::D0Pin, OCTOSPI1, PD11, 9u8);
pin_trait_impl!(crate::ospi::D1Pin, OCTOSPI2, PD12, 9u8);
pin_trait_impl!(crate::ospi::D1Pin, OCTOSPI1, PD12, 9u8);
pin_trait_impl!(crate::ospi::D3Pin, OCTOSPI2, PD13, 9u8);
pin_trait_impl!(crate::ospi::D3Pin, OCTOSPI1, PD13, 9u8);
pin_trait_impl!(crate::ospi::D4Pin, OCTOSPI2, PD4, 10u8);
pin_trait_impl!(crate::ospi::D4Pin, OCTOSPI1, PD4, 10u8);
pin_trait_impl!(crate::ospi::D5Pin, OCTOSPI2, PD5, 10u8);
pin_trait_impl!(crate::ospi::D5Pin, OCTOSPI1, PD5, 10u8);
pin_trait_impl!(crate::ospi::D6Pin, OCTOSPI2, PD6, 10u8);
pin_trait_impl!(crate::ospi::D6Pin, OCTOSPI1, PD6, 10u8);
pin_trait_impl!(crate::ospi::D7Pin, OCTOSPI2, PD7, 10u8);
pin_trait_impl!(crate::ospi::D7Pin, OCTOSPI1, PD7, 10u8);
pin_trait_impl!(crate::ospi::D7Pin, OCTOSPI2, PE10, 10u8);
pin_trait_impl!(crate::ospi::D7Pin, OCTOSPI1, PE10, 10u8);
pin_trait_impl!(crate::ospi::NSSPin, OCTOSPI2, PE11, 11u8);
pin_trait_impl!(crate::ospi::NSSPin, OCTOSPI1, PE11, 11u8);
pin_trait_impl!(crate::ospi::D2Pin, OCTOSPI2, PE2, 9u8);
pin_trait_impl!(crate::ospi::D2Pin, OCTOSPI1, PE2, 9u8);
pin_trait_impl!(crate::ospi::D4Pin, OCTOSPI2, PE7, 10u8);
pin_trait_impl!(crate::ospi::D4Pin, OCTOSPI1, PE7, 10u8);
pin_trait_impl!(crate::ospi::D5Pin, OCTOSPI2, PE8, 10u8);
pin_trait_impl!(crate::ospi::D5Pin, OCTOSPI1, PE8, 10u8);
pin_trait_impl!(crate::ospi::D6Pin, OCTOSPI2, PE9, 10u8);
pin_trait_impl!(crate::ospi::D6Pin, OCTOSPI1, PE9, 10u8);
impl_opamp_vn_pin!(OPAMP1, PA7, 1u8);
impl_opamp_vp_pin!(OPAMP1, PB0, 0u8);
impl_opamp_vout_pin!(OPAMP1, PC4);
impl_opamp_vn_pin!(OPAMP1, PC5, 0u8);
impl_opamp_vout_pin!(OPAMP2, PE7);
impl_opamp_vn_pin!(OPAMP2, PE8, 0u8);
impl_opamp_vp_pin!(OPAMP2, PE9, 0u8);
pin_trait_impl!(crate::rcc::McoPin, MCO1, PA8, 0u8);
pin_trait_impl!(crate::rcc::McoPin, MCO2, PC9, 0u8);
pin_trait_impl!(crate::sai::SdPin<A>, SAI1, PB2, 6u8);
pin_trait_impl!(crate::sai::SdPin<A>, SAI1, PC1, 6u8);
pin_trait_impl!(crate::sai::SdPin<A>, SAI1, PD6, 6u8);
pin_trait_impl!(crate::sai::MclkPin<A>, SAI1, PE2, 6u8);
pin_trait_impl!(crate::sai::SdPin<B>, SAI1, PE3, 6u8);
pin_trait_impl!(crate::sai::FsPin<A>, SAI1, PE4, 6u8);
pin_trait_impl!(crate::sai::SckPin<A>, SAI1, PE5, 6u8);
pin_trait_impl!(crate::sai::SdPin<A>, SAI1, PE6, 6u8);
pin_trait_impl!(crate::sai::SdPin<B>, SAI4, PA0, 10u8);
pin_trait_impl!(crate::sai::MclkPin<B>, SAI4, PA1, 10u8);
pin_trait_impl!(crate::sai::FsPin<B>, SAI4, PA12, 8u8);
pin_trait_impl!(crate::sai::SckPin<B>, SAI4, PA2, 8u8);
pin_trait_impl!(crate::sai::SdPin<A>, SAI4, PB2, 8u8);
pin_trait_impl!(crate::sai::FsPin<B>, SAI4, PC0, 8u8);
pin_trait_impl!(crate::sai::SdPin<A>, SAI4, PC1, 8u8);
pin_trait_impl!(crate::sai::SdPin<A>, SAI4, PD11, 10u8);
pin_trait_impl!(crate::sai::FsPin<A>, SAI4, PD12, 10u8);
pin_trait_impl!(crate::sai::SckPin<A>, SAI4, PD13, 10u8);
pin_trait_impl!(crate::sai::SdPin<A>, SAI4, PD6, 8u8);
pin_trait_impl!(crate::sai::MclkPin<A>, SAI4, PE0, 10u8);
pin_trait_impl!(crate::sai::SdPin<B>, SAI4, PE11, 10u8);
pin_trait_impl!(crate::sai::SckPin<B>, SAI4, PE12, 10u8);
pin_trait_impl!(crate::sai::FsPin<B>, SAI4, PE13, 10u8);
pin_trait_impl!(crate::sai::MclkPin<B>, SAI4, PE14, 10u8);
pin_trait_impl!(crate::sai::MclkPin<A>, SAI4, PE2, 8u8);
pin_trait_impl!(crate::sai::SdPin<B>, SAI4, PE3, 8u8);
pin_trait_impl!(crate::sai::FsPin<A>, SAI4, PE4, 8u8);
pin_trait_impl!(crate::sai::SckPin<A>, SAI4, PE5, 8u8);
pin_trait_impl!(crate::sai::MclkPin<B>, SAI4, PE6, 10u8);
pin_trait_impl!(crate::sai::SdPin<A>, SAI4, PE6, 8u8);
pin_trait_impl!(crate::sdmmc::D0Pin, SDMMC1, PB13, 12u8);
pin_trait_impl!(crate::sdmmc::D4Pin, SDMMC1, PB8, 12u8);
pin_trait_impl!(crate::sdmmc::D5Pin, SDMMC1, PB9, 12u8);
pin_trait_impl!(crate::sdmmc::D2Pin, SDMMC1, PC10, 12u8);
pin_trait_impl!(crate::sdmmc::D3Pin, SDMMC1, PC11, 12u8);
pin_trait_impl!(crate::sdmmc::CkPin, SDMMC1, PC12, 12u8);
pin_trait_impl!(crate::sdmmc::D6Pin, SDMMC1, PC6, 12u8);
pin_trait_impl!(crate::sdmmc::D7Pin, SDMMC1, PC7, 12u8);
pin_trait_impl!(crate::sdmmc::D0Pin, SDMMC1, PC8, 12u8);
pin_trait_impl!(crate::sdmmc::D1Pin, SDMMC1, PC9, 12u8);
pin_trait_impl!(crate::sdmmc::CmdPin, SDMMC1, PD2, 12u8);
pin_trait_impl!(crate::sdmmc::CmdPin, SDMMC2, PA0, 9u8);
pin_trait_impl!(crate::sdmmc::D0Pin, SDMMC2, PB14, 9u8);
pin_trait_impl!(crate::sdmmc::D1Pin, SDMMC2, PB15, 9u8);
pin_trait_impl!(crate::sdmmc::D2Pin, SDMMC2, PB3, 9u8);
pin_trait_impl!(crate::sdmmc::D3Pin, SDMMC2, PB4, 9u8);
pin_trait_impl!(crate::sdmmc::D4Pin, SDMMC2, PB8, 10u8);
pin_trait_impl!(crate::sdmmc::D5Pin, SDMMC2, PB9, 10u8);
pin_trait_impl!(crate::sdmmc::CkPin, SDMMC2, PC1, 9u8);
pin_trait_impl!(crate::sdmmc::D6Pin, SDMMC2, PC6, 10u8);
pin_trait_impl!(crate::sdmmc::D7Pin, SDMMC2, PC7, 10u8);
pin_trait_impl!(crate::sdmmc::CkPin, SDMMC2, PD6, 11u8);
pin_trait_impl!(crate::sdmmc::CmdPin, SDMMC2, PD7, 11u8);
impl_spdifrx_pin!(SPDIFRX1, PC4, 9u8, 2u8);
impl_spdifrx_pin!(SPDIFRX1, PC5, 9u8, 3u8);
impl_spdifrx_pin!(SPDIFRX1, PD7, 9u8, 0u8);
impl_spdifrx_pin!(SPDIFRX1, PD8, 9u8, 1u8);
pin_trait_impl!(crate::spi::WsPin, SPI1, PA15, 5u8);
pin_trait_impl!(crate::spi::CsPin, SPI1, PA15, 5u8);
pin_trait_impl!(crate::spi::WsPin, SPI1, PA4, 5u8);
pin_trait_impl!(crate::spi::CsPin, SPI1, PA4, 5u8);
pin_trait_impl!(crate::spi::CkPin, SPI1, PA5, 5u8);
pin_trait_impl!(crate::spi::SckPin, SPI1, PA5, 5u8);
pin_trait_impl!(crate::spi::MisoPin, SPI1, PA6, 5u8);
pin_trait_impl!(crate::spi::MosiPin, SPI1, PA7, 5u8);
pin_trait_impl!(crate::spi::CkPin, SPI1, PB3, 5u8);
pin_trait_impl!(crate::spi::SckPin, SPI1, PB3, 5u8);
pin_trait_impl!(crate::spi::MisoPin, SPI1, PB4, 5u8);
pin_trait_impl!(crate::spi::MosiPin, SPI1, PB5, 5u8);
pin_trait_impl!(crate::spi::MckPin, SPI1, PC4, 5u8);
pin_trait_impl!(crate::spi::MosiPin, SPI1, PD7, 5u8);
pin_trait_impl!(crate::spi::WsPin, SPI2, PA11, 5u8);
pin_trait_impl!(crate::spi::CsPin, SPI2, PA11, 5u8);
pin_trait_impl!(crate::spi::CkPin, SPI2, PA12, 5u8);
pin_trait_impl!(crate::spi::SckPin, SPI2, PA12, 5u8);
pin_trait_impl!(crate::spi::CkPin, SPI2, PA9, 5u8);
pin_trait_impl!(crate::spi::SckPin, SPI2, PA9, 5u8);
pin_trait_impl!(crate::spi::CkPin, SPI2, PB10, 5u8);
pin_trait_impl!(crate::spi::SckPin, SPI2, PB10, 5u8);
pin_trait_impl!(crate::spi::WsPin, SPI2, PB12, 5u8);
pin_trait_impl!(crate::spi::CsPin, SPI2, PB12, 5u8);
pin_trait_impl!(crate::spi::CkPin, SPI2, PB13, 5u8);
pin_trait_impl!(crate::spi::SckPin, SPI2, PB13, 5u8);
pin_trait_impl!(crate::spi::MisoPin, SPI2, PB14, 5u8);
pin_trait_impl!(crate::spi::MosiPin, SPI2, PB15, 5u8);
pin_trait_impl!(crate::spi::WsPin, SPI2, PB4, 7u8);
pin_trait_impl!(crate::spi::CsPin, SPI2, PB4, 7u8);
pin_trait_impl!(crate::spi::WsPin, SPI2, PB9, 5u8);
pin_trait_impl!(crate::spi::CsPin, SPI2, PB9, 5u8);
pin_trait_impl!(crate::spi::MosiPin, SPI2, PC1, 5u8);
pin_trait_impl!(crate::spi::MisoPin, SPI2, PC2, 5u8);
pin_trait_impl!(crate::spi::MosiPin, SPI2, PC3, 5u8);
pin_trait_impl!(crate::spi::MckPin, SPI2, PC6, 5u8);
pin_trait_impl!(crate::spi::CkPin, SPI2, PD3, 5u8);
pin_trait_impl!(crate::spi::SckPin, SPI2, PD3, 5u8);
pin_trait_impl!(crate::spi::WsPin, SPI3, PA15, 6u8);
pin_trait_impl!(crate::spi::CsPin, SPI3, PA15, 6u8);
pin_trait_impl!(crate::spi::WsPin, SPI3, PA4, 6u8);
pin_trait_impl!(crate::spi::CsPin, SPI3, PA4, 6u8);
pin_trait_impl!(crate::spi::MosiPin, SPI3, PB2, 7u8);
pin_trait_impl!(crate::spi::CkPin, SPI3, PB3, 6u8);
pin_trait_impl!(crate::spi::SckPin, SPI3, PB3, 6u8);
pin_trait_impl!(crate::spi::MisoPin, SPI3, PB4, 6u8);
pin_trait_impl!(crate::spi::MosiPin, SPI3, PB5, 7u8);
pin_trait_impl!(crate::spi::CkPin, SPI3, PC10, 6u8);
pin_trait_impl!(crate::spi::SckPin, SPI3, PC10, 6u8);
pin_trait_impl!(crate::spi::MisoPin, SPI3, PC11, 6u8);
pin_trait_impl!(crate::spi::MosiPin, SPI3, PC12, 6u8);
pin_trait_impl!(crate::spi::MckPin, SPI3, PC7, 6u8);
pin_trait_impl!(crate::spi::MosiPin, SPI3, PD6, 5u8);
pin_trait_impl!(crate::spi::CsPin, SPI4, PE11, 5u8);
pin_trait_impl!(crate::spi::SckPin, SPI4, PE12, 5u8);
pin_trait_impl!(crate::spi::MisoPin, SPI4, PE13, 5u8);
pin_trait_impl!(crate::spi::MosiPin, SPI4, PE14, 5u8);
pin_trait_impl!(crate::spi::SckPin, SPI4, PE2, 5u8);
pin_trait_impl!(crate::spi::CsPin, SPI4, PE4, 5u8);
pin_trait_impl!(crate::spi::MisoPin, SPI4, PE5, 5u8);
pin_trait_impl!(crate::spi::MosiPin, SPI4, PE6, 5u8);
pin_trait_impl!(crate::spi::WsPin, SPI6, PA0, 5u8);
pin_trait_impl!(crate::spi::CsPin, SPI6, PA0, 5u8);
pin_trait_impl!(crate::spi::WsPin, SPI6, PA15, 7u8);
pin_trait_impl!(crate::spi::CsPin, SPI6, PA15, 7u8);
pin_trait_impl!(crate::spi::MckPin, SPI6, PA3, 5u8);
pin_trait_impl!(crate::spi::WsPin, SPI6, PA4, 8u8);
pin_trait_impl!(crate::spi::CsPin, SPI6, PA4, 8u8);
pin_trait_impl!(crate::spi::CkPin, SPI6, PA5, 8u8);
pin_trait_impl!(crate::spi::SckPin, SPI6, PA5, 8u8);
pin_trait_impl!(crate::spi::MisoPin, SPI6, PA6, 8u8);
pin_trait_impl!(crate::spi::MosiPin, SPI6, PA7, 8u8);
pin_trait_impl!(crate::spi::CkPin, SPI6, PB3, 8u8);
pin_trait_impl!(crate::spi::SckPin, SPI6, PB3, 8u8);
pin_trait_impl!(crate::spi::MisoPin, SPI6, PB4, 8u8);
pin_trait_impl!(crate::spi::MosiPin, SPI6, PB5, 8u8);
pin_trait_impl!(crate::spi::CkPin, SPI6, PC12, 5u8);
pin_trait_impl!(crate::spi::SckPin, SPI6, PC12, 5u8);
pin_trait_impl!(crate::timer::TimerPin<Ch3>, TIM1, PA10, 1u8);
pin_trait_impl!(crate::timer::TimerPin<Ch4>, TIM1, PA11, 1u8);
pin_trait_impl!(crate::timer::BreakInputPin<BkIn2>, TIM1, PA12, 12u8);
pin_trait_impl!(crate::timer::ExternalTriggerPin, TIM1, PA12, 1u8);
pin_trait_impl!(crate::timer::BreakInputPin<BkIn1>, TIM1, PA6, 1u8);
pin_trait_impl!(
    crate::timer::BreakInputComparator1Pin<BkIn1>,
    TIM1,
    PA6,
    12u8
);
pin_trait_impl!(
    crate::timer::BreakInputComparator2Pin<BkIn1>,
    TIM1,
    PA6,
    12u8
);
pin_trait_impl!(crate::timer::TimerComplementaryPin<Ch1>, TIM1, PA7, 1u8);
pin_trait_impl!(crate::timer::TimerPin<Ch1>, TIM1, PA8, 1u8);
pin_trait_impl!(crate::timer::TimerPin<Ch2>, TIM1, PA9, 1u8);
pin_trait_impl!(crate::timer::TimerComplementaryPin<Ch2>, TIM1, PB0, 1u8);
pin_trait_impl!(crate::timer::TimerComplementaryPin<Ch3>, TIM1, PB1, 1u8);
pin_trait_impl!(crate::timer::BreakInputPin<BkIn1>, TIM1, PB12, 1u8);
pin_trait_impl!(
    crate::timer::BreakInputComparator1Pin<BkIn1>,
    TIM1,
    PB12,
    13u8
);
pin_trait_impl!(
    crate::timer::BreakInputComparator2Pin<BkIn1>,
    TIM1,
    PB12,
    13u8
);
pin_trait_impl!(crate::timer::TimerComplementaryPin<Ch1>, TIM1, PB13, 1u8);
pin_trait_impl!(crate::timer::TimerComplementaryPin<Ch2>, TIM1, PB14, 1u8);
pin_trait_impl!(crate::timer::TimerComplementaryPin<Ch3>, TIM1, PB15, 1u8);
pin_trait_impl!(crate::timer::TimerComplementaryPin<Ch2>, TIM1, PE10, 1u8);
pin_trait_impl!(crate::timer::TimerPin<Ch2>, TIM1, PE11, 1u8);
pin_trait_impl!(crate::timer::TimerComplementaryPin<Ch3>, TIM1, PE12, 1u8);
pin_trait_impl!(crate::timer::TimerPin<Ch3>, TIM1, PE13, 1u8);
pin_trait_impl!(crate::timer::TimerPin<Ch4>, TIM1, PE14, 1u8);
pin_trait_impl!(crate::timer::BreakInputPin<BkIn1>, TIM1, PE15, 1u8);
pin_trait_impl!(
    crate::timer::BreakInputComparator1Pin<BkIn1>,
    TIM1,
    PE15,
    13u8
);
pin_trait_impl!(
    crate::timer::BreakInputComparator2Pin<BkIn1>,
    TIM1,
    PE15,
    13u8
);
pin_trait_impl!(crate::timer::BreakInputPin<BkIn2>, TIM1, PE6, 1u8);
pin_trait_impl!(
    crate::timer::BreakInputComparator1Pin<BkIn2>,
    TIM1,
    PE6,
    11u8
);
pin_trait_impl!(
    crate::timer::BreakInputComparator2Pin<BkIn2>,
    TIM1,
    PE6,
    11u8
);
pin_trait_impl!(crate::timer::ExternalTriggerPin, TIM1, PE7, 1u8);
pin_trait_impl!(crate::timer::TimerComplementaryPin<Ch1>, TIM1, PE8, 1u8);
pin_trait_impl!(crate::timer::TimerPin<Ch1>, TIM1, PE9, 1u8);
pin_trait_impl!(crate::timer::TimerPin<Ch1>, TIM12, PB14, 2u8);
pin_trait_impl!(crate::timer::TimerPin<Ch2>, TIM12, PB15, 2u8);
pin_trait_impl!(crate::timer::TimerPin<Ch1>, TIM13, PA6, 9u8);
pin_trait_impl!(crate::timer::TimerPin<Ch1>, TIM14, PA7, 9u8);
pin_trait_impl!(crate::timer::BreakInputPin<BkIn1>, TIM15, PA0, 4u8);
pin_trait_impl!(crate::timer::TimerComplementaryPin<Ch1>, TIM15, PA1, 4u8);
pin_trait_impl!(crate::timer::TimerPin<Ch1>, TIM15, PA2, 4u8);
pin_trait_impl!(crate::timer::TimerPin<Ch2>, TIM15, PA3, 4u8);
pin_trait_impl!(crate::timer::TimerPin<Ch1>, TIM15, PC12, 2u8);
pin_trait_impl!(crate::timer::BreakInputPin<BkIn1>, TIM15, PD2, 4u8);
pin_trait_impl!(crate::timer::BreakInputPin<BkIn1>, TIM15, PE3, 4u8);
pin_trait_impl!(crate::timer::TimerComplementaryPin<Ch1>, TIM15, PE4, 4u8);
pin_trait_impl!(crate::timer::TimerPin<Ch1>, TIM15, PE5, 4u8);
pin_trait_impl!(crate::timer::TimerPin<Ch2>, TIM15, PE6, 4u8);
pin_trait_impl!(crate::timer::BreakInputPin<BkIn1>, TIM16, PB4, 1u8);
pin_trait_impl!(crate::timer::TimerComplementaryPin<Ch1>, TIM16, PB6, 1u8);
pin_trait_impl!(crate::timer::TimerPin<Ch1>, TIM16, PB8, 1u8);
pin_trait_impl!(crate::timer::BreakInputPin<BkIn1>, TIM17, PB5, 1u8);
pin_trait_impl!(crate::timer::TimerComplementaryPin<Ch1>, TIM17, PB7, 1u8);
pin_trait_impl!(crate::timer::TimerPin<Ch1>, TIM17, PB9, 1u8);
pin_trait_impl!(crate::timer::TimerPin<Ch1>, TIM2, PA0, 1u8);
pin_trait_impl!(crate::timer::ExternalTriggerPin, TIM2, PA0, 1u8);
pin_trait_impl!(crate::timer::TimerPin<Ch2>, TIM2, PA1, 1u8);
pin_trait_impl!(crate::timer::TimerPin<Ch1>, TIM2, PA15, 1u8);
pin_trait_impl!(crate::timer::ExternalTriggerPin, TIM2, PA15, 1u8);
pin_trait_impl!(crate::timer::TimerPin<Ch3>, TIM2, PA2, 1u8);
pin_trait_impl!(crate::timer::TimerPin<Ch4>, TIM2, PA3, 1u8);
pin_trait_impl!(crate::timer::TimerPin<Ch1>, TIM2, PA5, 1u8);
pin_trait_impl!(crate::timer::ExternalTriggerPin, TIM2, PA5, 1u8);
pin_trait_impl!(crate::timer::TimerPin<Ch3>, TIM2, PB10, 1u8);
pin_trait_impl!(crate::timer::TimerPin<Ch4>, TIM2, PB11, 1u8);
pin_trait_impl!(crate::timer::TimerPin<Ch2>, TIM2, PB3, 1u8);
pin_trait_impl!(crate::timer::ExternalTriggerPin, TIM23, PB2, 13u8);
pin_trait_impl!(crate::timer::ExternalTriggerPin, TIM24, PB3, 14u8);
pin_trait_impl!(crate::timer::TimerPin<Ch1>, TIM3, PA6, 2u8);
pin_trait_impl!(crate::timer::TimerPin<Ch2>, TIM3, PA7, 2u8);
pin_trait_impl!(crate::timer::TimerPin<Ch3>, TIM3, PB0, 2u8);
pin_trait_impl!(crate::timer::TimerPin<Ch4>, TIM3, PB1, 2u8);
pin_trait_impl!(crate::timer::TimerPin<Ch1>, TIM3, PB4, 2u8);
pin_trait_impl!(crate::timer::TimerPin<Ch2>, TIM3, PB5, 2u8);
pin_trait_impl!(crate::timer::TimerPin<Ch1>, TIM3, PC6, 2u8);
pin_trait_impl!(crate::timer::TimerPin<Ch2>, TIM3, PC7, 2u8);
pin_trait_impl!(crate::timer::TimerPin<Ch3>, TIM3, PC8, 2u8);
pin_trait_impl!(crate::timer::TimerPin<Ch4>, TIM3, PC9, 2u8);
pin_trait_impl!(crate::timer::ExternalTriggerPin, TIM3, PD2, 2u8);
pin_trait_impl!(crate::timer::TimerPin<Ch1>, TIM4, PB6, 2u8);
pin_trait_impl!(crate::timer::TimerPin<Ch2>, TIM4, PB7, 2u8);
pin_trait_impl!(crate::timer::TimerPin<Ch3>, TIM4, PB8, 2u8);
pin_trait_impl!(crate::timer::TimerPin<Ch4>, TIM4, PB9, 2u8);
pin_trait_impl!(crate::timer::TimerPin<Ch1>, TIM4, PD12, 2u8);
pin_trait_impl!(crate::timer::TimerPin<Ch2>, TIM4, PD13, 2u8);
pin_trait_impl!(crate::timer::TimerPin<Ch3>, TIM4, PD14, 2u8);
pin_trait_impl!(crate::timer::TimerPin<Ch4>, TIM4, PD15, 2u8);
pin_trait_impl!(crate::timer::ExternalTriggerPin, TIM4, PE0, 2u8);
pin_trait_impl!(crate::timer::TimerPin<Ch1>, TIM5, PA0, 2u8);
pin_trait_impl!(crate::timer::TimerPin<Ch2>, TIM5, PA1, 2u8);
pin_trait_impl!(crate::timer::TimerPin<Ch3>, TIM5, PA2, 2u8);
pin_trait_impl!(crate::timer::TimerPin<Ch4>, TIM5, PA3, 2u8);
pin_trait_impl!(crate::timer::ExternalTriggerPin, TIM5, PA4, 2u8);
pin_trait_impl!(crate::timer::ExternalTriggerPin, TIM8, PA0, 3u8);
pin_trait_impl!(crate::timer::TimerComplementaryPin<Ch1>, TIM8, PA5, 3u8);
pin_trait_impl!(crate::timer::BreakInputPin<BkIn1>, TIM8, PA6, 3u8);
pin_trait_impl!(
    crate::timer::BreakInputComparator1Pin<BkIn1>,
    TIM8,
    PA6,
    10u8
);
pin_trait_impl!(
    crate::timer::BreakInputComparator2Pin<BkIn1>,
    TIM8,
    PA6,
    10u8
);
pin_trait_impl!(crate::timer::TimerComplementaryPin<Ch1>, TIM8, PA7, 3u8);
pin_trait_impl!(crate::timer::BreakInputPin<BkIn2>, TIM8, PA8, 3u8);
pin_trait_impl!(
    crate::timer::BreakInputComparator1Pin<BkIn2>,
    TIM8,
    PA8,
    12u8
);
pin_trait_impl!(
    crate::timer::BreakInputComparator2Pin<BkIn2>,
    TIM8,
    PA8,
    12u8
);
pin_trait_impl!(crate::timer::TimerComplementaryPin<Ch2>, TIM8, PB0, 3u8);
pin_trait_impl!(crate::timer::TimerComplementaryPin<Ch3>, TIM8, PB1, 3u8);
pin_trait_impl!(crate::timer::TimerComplementaryPin<Ch2>, TIM8, PB14, 3u8);
pin_trait_impl!(crate::timer::TimerComplementaryPin<Ch3>, TIM8, PB15, 3u8);
pin_trait_impl!(crate::timer::TimerPin<Ch1>, TIM8, PC6, 3u8);
pin_trait_impl!(crate::timer::TimerPin<Ch2>, TIM8, PC7, 3u8);
pin_trait_impl!(crate::timer::TimerPin<Ch3>, TIM8, PC8, 3u8);
pin_trait_impl!(crate::timer::TimerPin<Ch4>, TIM8, PC9, 3u8);
pin_trait_impl!(crate::usart::TxPin, UART4, PA0, 8u8);
pin_trait_impl!(crate::usart::RxPin, UART4, PA1, 8u8);
pin_trait_impl!(crate::usart::RxPin, UART4, PA11, 6u8);
pin_trait_impl!(crate::usart::TxPin, UART4, PA12, 6u8);
pin_trait_impl!(crate::usart::DePin, UART4, PA15, 8u8);
pin_trait_impl!(crate::usart::RtsPin, UART4, PA15, 8u8);
pin_trait_impl!(crate::usart::CtsPin, UART4, PB0, 8u8);
pin_trait_impl!(crate::usart::DePin, UART4, PB14, 8u8);
pin_trait_impl!(crate::usart::RtsPin, UART4, PB14, 8u8);
pin_trait_impl!(crate::usart::CtsPin, UART4, PB15, 8u8);
pin_trait_impl!(crate::usart::RxPin, UART4, PB8, 8u8);
pin_trait_impl!(crate::usart::TxPin, UART4, PB9, 8u8);
pin_trait_impl!(crate::usart::TxPin, UART4, PC10, 8u8);
pin_trait_impl!(crate::usart::RxPin, UART4, PC11, 8u8);
pin_trait_impl!(crate::usart::RxPin, UART4, PD0, 8u8);
pin_trait_impl!(crate::usart::TxPin, UART4, PD1, 8u8);
pin_trait_impl!(crate::usart::RxPin, UART5, PB12, 14u8);
pin_trait_impl!(crate::usart::TxPin, UART5, PB13, 14u8);
pin_trait_impl!(crate::usart::RxPin, UART5, PB5, 14u8);
pin_trait_impl!(crate::usart::TxPin, UART5, PB6, 14u8);
pin_trait_impl!(crate::usart::TxPin, UART5, PC12, 8u8);
pin_trait_impl!(crate::usart::DePin, UART5, PC8, 8u8);
pin_trait_impl!(crate::usart::RtsPin, UART5, PC8, 8u8);
pin_trait_impl!(crate::usart::CtsPin, UART5, PC9, 8u8);
pin_trait_impl!(crate::usart::RxPin, UART5, PD2, 8u8);
pin_trait_impl!(crate::usart::TxPin, UART7, PA15, 11u8);
pin_trait_impl!(crate::usart::RxPin, UART7, PA8, 11u8);
pin_trait_impl!(crate::usart::RxPin, UART7, PB3, 11u8);
pin_trait_impl!(crate::usart::TxPin, UART7, PB4, 11u8);
pin_trait_impl!(crate::usart::CtsPin, UART7, PE10, 7u8);
pin_trait_impl!(crate::usart::RxPin, UART7, PE7, 7u8);
pin_trait_impl!(crate::usart::TxPin, UART7, PE8, 7u8);
pin_trait_impl!(crate::usart::DePin, UART7, PE9, 7u8);
pin_trait_impl!(crate::usart::RtsPin, UART7, PE9, 7u8);
pin_trait_impl!(crate::usart::CtsPin, UART8, PD14, 8u8);
pin_trait_impl!(crate::usart::DePin, UART8, PD15, 8u8);
pin_trait_impl!(crate::usart::RtsPin, UART8, PD15, 8u8);
pin_trait_impl!(crate::usart::RxPin, UART8, PE0, 8u8);
pin_trait_impl!(crate::usart::TxPin, UART8, PE1, 8u8);
pin_trait_impl!(crate::usart::CtsPin, UART9, PD0, 11u8);
pin_trait_impl!(crate::usart::DePin, UART9, PD13, 11u8);
pin_trait_impl!(crate::usart::RtsPin, UART9, PD13, 11u8);
pin_trait_impl!(crate::usart::RxPin, UART9, PD14, 11u8);
pin_trait_impl!(crate::usart::TxPin, UART9, PD15, 11u8);
pin_trait_impl!(crate::usart::RxPin, USART1, PA10, 7u8);
pin_trait_impl!(crate::usart::CtsPin, USART1, PA11, 7u8);
pin_trait_impl!(crate::usart::DePin, USART1, PA12, 7u8);
pin_trait_impl!(crate::usart::RtsPin, USART1, PA12, 7u8);
pin_trait_impl!(crate::usart::CkPin, USART1, PA8, 7u8);
pin_trait_impl!(crate::usart::TxPin, USART1, PA9, 7u8);
pin_trait_impl!(crate::usart::TxPin, USART1, PB14, 4u8);
pin_trait_impl!(crate::usart::RxPin, USART1, PB15, 4u8);
pin_trait_impl!(crate::usart::TxPin, USART1, PB6, 7u8);
pin_trait_impl!(crate::usart::RxPin, USART1, PB7, 7u8);
pin_trait_impl!(crate::usart::CkPin, USART10, PE15, 11u8);
pin_trait_impl!(crate::usart::RxPin, USART10, PE2, 4u8);
pin_trait_impl!(crate::usart::TxPin, USART10, PE3, 11u8);
pin_trait_impl!(crate::usart::CtsPin, USART2, PA0, 7u8);
pin_trait_impl!(crate::usart::DePin, USART2, PA1, 7u8);
pin_trait_impl!(crate::usart::RtsPin, USART2, PA1, 7u8);
pin_trait_impl!(crate::usart::TxPin, USART2, PA2, 7u8);
pin_trait_impl!(crate::usart::RxPin, USART2, PA3, 7u8);
pin_trait_impl!(crate::usart::CkPin, USART2, PA4, 7u8);
pin_trait_impl!(crate::usart::CtsPin, USART2, PD3, 7u8);
pin_trait_impl!(crate::usart::DePin, USART2, PD4, 7u8);
pin_trait_impl!(crate::usart::RtsPin, USART2, PD4, 7u8);
pin_trait_impl!(crate::usart::TxPin, USART2, PD5, 7u8);
pin_trait_impl!(crate::usart::RxPin, USART2, PD6, 7u8);
pin_trait_impl!(crate::usart::CkPin, USART2, PD7, 7u8);
pin_trait_impl!(crate::usart::TxPin, USART3, PB10, 7u8);
pin_trait_impl!(crate::usart::RxPin, USART3, PB11, 7u8);
pin_trait_impl!(crate::usart::CkPin, USART3, PB12, 7u8);
pin_trait_impl!(crate::usart::CtsPin, USART3, PB13, 7u8);
pin_trait_impl!(crate::usart::DePin, USART3, PB14, 7u8);
pin_trait_impl!(crate::usart::RtsPin, USART3, PB14, 7u8);
pin_trait_impl!(crate::usart::TxPin, USART3, PC10, 7u8);
pin_trait_impl!(crate::usart::RxPin, USART3, PC11, 7u8);
pin_trait_impl!(crate::usart::CkPin, USART3, PC12, 7u8);
pin_trait_impl!(crate::usart::CkPin, USART3, PD10, 7u8);
pin_trait_impl!(crate::usart::CtsPin, USART3, PD11, 7u8);
pin_trait_impl!(crate::usart::DePin, USART3, PD12, 7u8);
pin_trait_impl!(crate::usart::RtsPin, USART3, PD12, 7u8);
pin_trait_impl!(crate::usart::TxPin, USART3, PD8, 7u8);
pin_trait_impl!(crate::usart::RxPin, USART3, PD9, 7u8);
pin_trait_impl!(crate::usart::TxPin, USART6, PC6, 7u8);
pin_trait_impl!(crate::usart::RxPin, USART6, PC7, 7u8);
pin_trait_impl!(crate::usart::CkPin, USART6, PC8, 7u8);
pin_trait_impl!(crate::usb::DmPin, USB_OTG_HS, PA11, 0u8);
pin_trait_impl!(crate::usb::DpPin, USB_OTG_HS, PA12, 0u8);
pin_trait_impl!(crate::usb::UlpiD0Pin, USB_OTG_HS, PA3, 10u8);
pin_trait_impl!(crate::usb::UlpiClkPin, USB_OTG_HS, PA5, 10u8);
pin_trait_impl!(crate::usb::UlpiD1Pin, USB_OTG_HS, PB0, 10u8);
pin_trait_impl!(crate::usb::UlpiD2Pin, USB_OTG_HS, PB1, 10u8);
pin_trait_impl!(crate::usb::UlpiD3Pin, USB_OTG_HS, PB10, 10u8);
pin_trait_impl!(crate::usb::UlpiD4Pin, USB_OTG_HS, PB11, 10u8);
pin_trait_impl!(crate::usb::UlpiD5Pin, USB_OTG_HS, PB12, 10u8);
pin_trait_impl!(crate::usb::UlpiD6Pin, USB_OTG_HS, PB13, 10u8);
pin_trait_impl!(crate::usb::UlpiD7Pin, USB_OTG_HS, PB5, 10u8);
pin_trait_impl!(crate::usb::UlpiStpPin, USB_OTG_HS, PC0, 10u8);
pin_trait_impl!(crate::usb::UlpiDirPin, USB_OTG_HS, PC2, 10u8);
pin_trait_impl!(crate::usb::UlpiNxtPin, USB_OTG_HS, PC3, 10u8);
dma_trait_impl!(crate::adc::RxDma, ADC1, DMA1_CH0, 9u8, {});
dma_trait_impl!(crate::adc::RxDma, ADC1, DMA1_CH1, 9u8, {});
dma_trait_impl!(crate::adc::RxDma, ADC1, DMA1_CH2, 9u8, {});
dma_trait_impl!(crate::adc::RxDma, ADC1, DMA1_CH3, 9u8, {});
dma_trait_impl!(crate::adc::RxDma, ADC1, DMA1_CH4, 9u8, {});
dma_trait_impl!(crate::adc::RxDma, ADC1, DMA1_CH5, 9u8, {});
dma_trait_impl!(crate::adc::RxDma, ADC1, DMA1_CH6, 9u8, {});
dma_trait_impl!(crate::adc::RxDma, ADC1, DMA1_CH7, 9u8, {});
dma_trait_impl!(crate::adc::RxDma, ADC1, DMA2_CH0, 9u8, {});
dma_trait_impl!(crate::adc::RxDma, ADC1, DMA2_CH1, 9u8, {});
dma_trait_impl!(crate::adc::RxDma, ADC1, DMA2_CH2, 9u8, {});
dma_trait_impl!(crate::adc::RxDma, ADC1, DMA2_CH3, 9u8, {});
dma_trait_impl!(crate::adc::RxDma, ADC1, DMA2_CH4, 9u8, {});
dma_trait_impl!(crate::adc::RxDma, ADC1, DMA2_CH5, 9u8, {});
dma_trait_impl!(crate::adc::RxDma, ADC1, DMA2_CH6, 9u8, {});
dma_trait_impl!(crate::adc::RxDma, ADC1, DMA2_CH7, 9u8, {});
dma_trait_impl!(crate::adc::RxDma, ADC2, DMA1_CH0, 10u8, {});
dma_trait_impl!(crate::adc::RxDma, ADC2, DMA1_CH1, 10u8, {});
dma_trait_impl!(crate::adc::RxDma, ADC2, DMA1_CH2, 10u8, {});
dma_trait_impl!(crate::adc::RxDma, ADC2, DMA1_CH3, 10u8, {});
dma_trait_impl!(crate::adc::RxDma, ADC2, DMA1_CH4, 10u8, {});
dma_trait_impl!(crate::adc::RxDma, ADC2, DMA1_CH5, 10u8, {});
dma_trait_impl!(crate::adc::RxDma, ADC2, DMA1_CH6, 10u8, {});
dma_trait_impl!(crate::adc::RxDma, ADC2, DMA1_CH7, 10u8, {});
dma_trait_impl!(crate::adc::RxDma, ADC2, DMA2_CH0, 10u8, {});
dma_trait_impl!(crate::adc::RxDma, ADC2, DMA2_CH1, 10u8, {});
dma_trait_impl!(crate::adc::RxDma, ADC2, DMA2_CH2, 10u8, {});
dma_trait_impl!(crate::adc::RxDma, ADC2, DMA2_CH3, 10u8, {});
dma_trait_impl!(crate::adc::RxDma, ADC2, DMA2_CH4, 10u8, {});
dma_trait_impl!(crate::adc::RxDma, ADC2, DMA2_CH5, 10u8, {});
dma_trait_impl!(crate::adc::RxDma, ADC2, DMA2_CH6, 10u8, {});
dma_trait_impl!(crate::adc::RxDma, ADC2, DMA2_CH7, 10u8, {});
dma_trait_impl!(crate::adc::RxDma, ADC3, DMA1_CH0, 115u8, {});
dma_trait_impl!(crate::adc::RxDma, ADC3, DMA1_CH1, 115u8, {});
dma_trait_impl!(crate::adc::RxDma, ADC3, DMA1_CH2, 115u8, {});
dma_trait_impl!(crate::adc::RxDma, ADC3, DMA1_CH3, 115u8, {});
dma_trait_impl!(crate::adc::RxDma, ADC3, DMA1_CH4, 115u8, {});
dma_trait_impl!(crate::adc::RxDma, ADC3, DMA1_CH5, 115u8, {});
dma_trait_impl!(crate::adc::RxDma, ADC3, DMA1_CH6, 115u8, {});
dma_trait_impl!(crate::adc::RxDma, ADC3, DMA1_CH7, 115u8, {});
dma_trait_impl!(crate::adc::RxDma, ADC3, DMA2_CH0, 115u8, {});
dma_trait_impl!(crate::adc::RxDma, ADC3, DMA2_CH1, 115u8, {});
dma_trait_impl!(crate::adc::RxDma, ADC3, DMA2_CH2, 115u8, {});
dma_trait_impl!(crate::adc::RxDma, ADC3, DMA2_CH3, 115u8, {});
dma_trait_impl!(crate::adc::RxDma, ADC3, DMA2_CH4, 115u8, {});
dma_trait_impl!(crate::adc::RxDma, ADC3, DMA2_CH5, 115u8, {});
dma_trait_impl!(crate::adc::RxDma, ADC3, DMA2_CH6, 115u8, {});
dma_trait_impl!(crate::adc::RxDma, ADC3, DMA2_CH7, 115u8, {});
dma_trait_impl!(crate::adc::RxDma, ADC3, BDMA_CH0, 17u8, {});
dma_trait_impl!(crate::adc::RxDma, ADC3, BDMA_CH1, 17u8, {});
dma_trait_impl!(crate::adc::RxDma, ADC3, BDMA_CH2, 17u8, {});
dma_trait_impl!(crate::adc::RxDma, ADC3, BDMA_CH3, 17u8, {});
dma_trait_impl!(crate::adc::RxDma, ADC3, BDMA_CH4, 17u8, {});
dma_trait_impl!(crate::adc::RxDma, ADC3, BDMA_CH5, 17u8, {});
dma_trait_impl!(crate::adc::RxDma, ADC3, BDMA_CH6, 17u8, {});
dma_trait_impl!(crate::adc::RxDma, ADC3, BDMA_CH7, 17u8, {});
dma_trait_impl!(crate::cordic::ReadDma, CORDIC, DMA1_CH0, 122u8, {});
dma_trait_impl!(crate::cordic::ReadDma, CORDIC, DMA1_CH1, 122u8, {});
dma_trait_impl!(crate::cordic::ReadDma, CORDIC, DMA1_CH2, 122u8, {});
dma_trait_impl!(crate::cordic::ReadDma, CORDIC, DMA1_CH3, 122u8, {});
dma_trait_impl!(crate::cordic::ReadDma, CORDIC, DMA1_CH4, 122u8, {});
dma_trait_impl!(crate::cordic::ReadDma, CORDIC, DMA1_CH5, 122u8, {});
dma_trait_impl!(crate::cordic::ReadDma, CORDIC, DMA1_CH6, 122u8, {});
dma_trait_impl!(crate::cordic::ReadDma, CORDIC, DMA1_CH7, 122u8, {});
dma_trait_impl!(crate::cordic::ReadDma, CORDIC, DMA2_CH0, 122u8, {});
dma_trait_impl!(crate::cordic::ReadDma, CORDIC, DMA2_CH1, 122u8, {});
dma_trait_impl!(crate::cordic::ReadDma, CORDIC, DMA2_CH2, 122u8, {});
dma_trait_impl!(crate::cordic::ReadDma, CORDIC, DMA2_CH3, 122u8, {});
dma_trait_impl!(crate::cordic::ReadDma, CORDIC, DMA2_CH4, 122u8, {});
dma_trait_impl!(crate::cordic::ReadDma, CORDIC, DMA2_CH5, 122u8, {});
dma_trait_impl!(crate::cordic::ReadDma, CORDIC, DMA2_CH6, 122u8, {});
dma_trait_impl!(crate::cordic::ReadDma, CORDIC, DMA2_CH7, 122u8, {});
dma_trait_impl!(crate::cordic::WriteDma, CORDIC, DMA1_CH0, 123u8, {});
dma_trait_impl!(crate::cordic::WriteDma, CORDIC, DMA1_CH1, 123u8, {});
dma_trait_impl!(crate::cordic::WriteDma, CORDIC, DMA1_CH2, 123u8, {});
dma_trait_impl!(crate::cordic::WriteDma, CORDIC, DMA1_CH3, 123u8, {});
dma_trait_impl!(crate::cordic::WriteDma, CORDIC, DMA1_CH4, 123u8, {});
dma_trait_impl!(crate::cordic::WriteDma, CORDIC, DMA1_CH5, 123u8, {});
dma_trait_impl!(crate::cordic::WriteDma, CORDIC, DMA1_CH6, 123u8, {});
dma_trait_impl!(crate::cordic::WriteDma, CORDIC, DMA1_CH7, 123u8, {});
dma_trait_impl!(crate::cordic::WriteDma, CORDIC, DMA2_CH0, 123u8, {});
dma_trait_impl!(crate::cordic::WriteDma, CORDIC, DMA2_CH1, 123u8, {});
dma_trait_impl!(crate::cordic::WriteDma, CORDIC, DMA2_CH2, 123u8, {});
dma_trait_impl!(crate::cordic::WriteDma, CORDIC, DMA2_CH3, 123u8, {});
dma_trait_impl!(crate::cordic::WriteDma, CORDIC, DMA2_CH4, 123u8, {});
dma_trait_impl!(crate::cordic::WriteDma, CORDIC, DMA2_CH5, 123u8, {});
dma_trait_impl!(crate::cordic::WriteDma, CORDIC, DMA2_CH6, 123u8, {});
dma_trait_impl!(crate::cordic::WriteDma, CORDIC, DMA2_CH7, 123u8, {});
dma_trait_impl!(crate::dac::Dma<Ch1>, DAC1, DMA1_CH0, 67u8, {});
dma_trait_impl!(crate::dac::Dma<Ch1>, DAC1, DMA1_CH1, 67u8, {});
dma_trait_impl!(crate::dac::Dma<Ch1>, DAC1, DMA1_CH2, 67u8, {});
dma_trait_impl!(crate::dac::Dma<Ch1>, DAC1, DMA1_CH3, 67u8, {});
dma_trait_impl!(crate::dac::Dma<Ch1>, DAC1, DMA1_CH4, 67u8, {});
dma_trait_impl!(crate::dac::Dma<Ch1>, DAC1, DMA1_CH5, 67u8, {});
dma_trait_impl!(crate::dac::Dma<Ch1>, DAC1, DMA1_CH6, 67u8, {});
dma_trait_impl!(crate::dac::Dma<Ch1>, DAC1, DMA1_CH7, 67u8, {});
dma_trait_impl!(crate::dac::Dma<Ch1>, DAC1, DMA2_CH0, 67u8, {});
dma_trait_impl!(crate::dac::Dma<Ch1>, DAC1, DMA2_CH1, 67u8, {});
dma_trait_impl!(crate::dac::Dma<Ch1>, DAC1, DMA2_CH2, 67u8, {});
dma_trait_impl!(crate::dac::Dma<Ch1>, DAC1, DMA2_CH3, 67u8, {});
dma_trait_impl!(crate::dac::Dma<Ch1>, DAC1, DMA2_CH4, 67u8, {});
dma_trait_impl!(crate::dac::Dma<Ch1>, DAC1, DMA2_CH5, 67u8, {});
dma_trait_impl!(crate::dac::Dma<Ch1>, DAC1, DMA2_CH6, 67u8, {});
dma_trait_impl!(crate::dac::Dma<Ch1>, DAC1, DMA2_CH7, 67u8, {});
dma_trait_impl!(crate::dac::Dma<Ch2>, DAC1, DMA1_CH0, 68u8, {});
dma_trait_impl!(crate::dac::Dma<Ch2>, DAC1, DMA1_CH1, 68u8, {});
dma_trait_impl!(crate::dac::Dma<Ch2>, DAC1, DMA1_CH2, 68u8, {});
dma_trait_impl!(crate::dac::Dma<Ch2>, DAC1, DMA1_CH3, 68u8, {});
dma_trait_impl!(crate::dac::Dma<Ch2>, DAC1, DMA1_CH4, 68u8, {});
dma_trait_impl!(crate::dac::Dma<Ch2>, DAC1, DMA1_CH5, 68u8, {});
dma_trait_impl!(crate::dac::Dma<Ch2>, DAC1, DMA1_CH6, 68u8, {});
dma_trait_impl!(crate::dac::Dma<Ch2>, DAC1, DMA1_CH7, 68u8, {});
dma_trait_impl!(crate::dac::Dma<Ch2>, DAC1, DMA2_CH0, 68u8, {});
dma_trait_impl!(crate::dac::Dma<Ch2>, DAC1, DMA2_CH1, 68u8, {});
dma_trait_impl!(crate::dac::Dma<Ch2>, DAC1, DMA2_CH2, 68u8, {});
dma_trait_impl!(crate::dac::Dma<Ch2>, DAC1, DMA2_CH3, 68u8, {});
dma_trait_impl!(crate::dac::Dma<Ch2>, DAC1, DMA2_CH4, 68u8, {});
dma_trait_impl!(crate::dac::Dma<Ch2>, DAC1, DMA2_CH5, 68u8, {});
dma_trait_impl!(crate::dac::Dma<Ch2>, DAC1, DMA2_CH6, 68u8, {});
dma_trait_impl!(crate::dac::Dma<Ch2>, DAC1, DMA2_CH7, 68u8, {});
dma_trait_impl!(crate::dcmi::FrameDma, DCMI, DMA1_CH0, 75u8, {});
dma_trait_impl!(crate::dcmi::FrameDma, DCMI, DMA1_CH1, 75u8, {});
dma_trait_impl!(crate::dcmi::FrameDma, DCMI, DMA1_CH2, 75u8, {});
dma_trait_impl!(crate::dcmi::FrameDma, DCMI, DMA1_CH3, 75u8, {});
dma_trait_impl!(crate::dcmi::FrameDma, DCMI, DMA1_CH4, 75u8, {});
dma_trait_impl!(crate::dcmi::FrameDma, DCMI, DMA1_CH5, 75u8, {});
dma_trait_impl!(crate::dcmi::FrameDma, DCMI, DMA1_CH6, 75u8, {});
dma_trait_impl!(crate::dcmi::FrameDma, DCMI, DMA1_CH7, 75u8, {});
dma_trait_impl!(crate::dcmi::FrameDma, DCMI, DMA2_CH0, 75u8, {});
dma_trait_impl!(crate::dcmi::FrameDma, DCMI, DMA2_CH1, 75u8, {});
dma_trait_impl!(crate::dcmi::FrameDma, DCMI, DMA2_CH2, 75u8, {});
dma_trait_impl!(crate::dcmi::FrameDma, DCMI, DMA2_CH3, 75u8, {});
dma_trait_impl!(crate::dcmi::FrameDma, DCMI, DMA2_CH4, 75u8, {});
dma_trait_impl!(crate::dcmi::FrameDma, DCMI, DMA2_CH5, 75u8, {});
dma_trait_impl!(crate::dcmi::FrameDma, DCMI, DMA2_CH6, 75u8, {});
dma_trait_impl!(crate::dcmi::FrameDma, DCMI, DMA2_CH7, 75u8, {});
dma_trait_impl!(crate::i2c::RxDma, I2C1, DMA1_CH0, 33u8, {});
dma_trait_impl!(crate::i2c::RxDma, I2C1, DMA1_CH1, 33u8, {});
dma_trait_impl!(crate::i2c::RxDma, I2C1, DMA1_CH2, 33u8, {});
dma_trait_impl!(crate::i2c::RxDma, I2C1, DMA1_CH3, 33u8, {});
dma_trait_impl!(crate::i2c::RxDma, I2C1, DMA1_CH4, 33u8, {});
dma_trait_impl!(crate::i2c::RxDma, I2C1, DMA1_CH5, 33u8, {});
dma_trait_impl!(crate::i2c::RxDma, I2C1, DMA1_CH6, 33u8, {});
dma_trait_impl!(crate::i2c::RxDma, I2C1, DMA1_CH7, 33u8, {});
dma_trait_impl!(crate::i2c::RxDma, I2C1, DMA2_CH0, 33u8, {});
dma_trait_impl!(crate::i2c::RxDma, I2C1, DMA2_CH1, 33u8, {});
dma_trait_impl!(crate::i2c::RxDma, I2C1, DMA2_CH2, 33u8, {});
dma_trait_impl!(crate::i2c::RxDma, I2C1, DMA2_CH3, 33u8, {});
dma_trait_impl!(crate::i2c::RxDma, I2C1, DMA2_CH4, 33u8, {});
dma_trait_impl!(crate::i2c::RxDma, I2C1, DMA2_CH5, 33u8, {});
dma_trait_impl!(crate::i2c::RxDma, I2C1, DMA2_CH6, 33u8, {});
dma_trait_impl!(crate::i2c::RxDma, I2C1, DMA2_CH7, 33u8, {});
dma_trait_impl!(crate::i2c::TxDma, I2C1, DMA1_CH0, 34u8, {});
dma_trait_impl!(crate::i2c::TxDma, I2C1, DMA1_CH1, 34u8, {});
dma_trait_impl!(crate::i2c::TxDma, I2C1, DMA1_CH2, 34u8, {});
dma_trait_impl!(crate::i2c::TxDma, I2C1, DMA1_CH3, 34u8, {});
dma_trait_impl!(crate::i2c::TxDma, I2C1, DMA1_CH4, 34u8, {});
dma_trait_impl!(crate::i2c::TxDma, I2C1, DMA1_CH5, 34u8, {});
dma_trait_impl!(crate::i2c::TxDma, I2C1, DMA1_CH6, 34u8, {});
dma_trait_impl!(crate::i2c::TxDma, I2C1, DMA1_CH7, 34u8, {});
dma_trait_impl!(crate::i2c::TxDma, I2C1, DMA2_CH0, 34u8, {});
dma_trait_impl!(crate::i2c::TxDma, I2C1, DMA2_CH1, 34u8, {});
dma_trait_impl!(crate::i2c::TxDma, I2C1, DMA2_CH2, 34u8, {});
dma_trait_impl!(crate::i2c::TxDma, I2C1, DMA2_CH3, 34u8, {});
dma_trait_impl!(crate::i2c::TxDma, I2C1, DMA2_CH4, 34u8, {});
dma_trait_impl!(crate::i2c::TxDma, I2C1, DMA2_CH5, 34u8, {});
dma_trait_impl!(crate::i2c::TxDma, I2C1, DMA2_CH6, 34u8, {});
dma_trait_impl!(crate::i2c::TxDma, I2C1, DMA2_CH7, 34u8, {});
dma_trait_impl!(crate::i2c::RxDma, I2C2, DMA1_CH0, 35u8, {});
dma_trait_impl!(crate::i2c::RxDma, I2C2, DMA1_CH1, 35u8, {});
dma_trait_impl!(crate::i2c::RxDma, I2C2, DMA1_CH2, 35u8, {});
dma_trait_impl!(crate::i2c::RxDma, I2C2, DMA1_CH3, 35u8, {});
dma_trait_impl!(crate::i2c::RxDma, I2C2, DMA1_CH4, 35u8, {});
dma_trait_impl!(crate::i2c::RxDma, I2C2, DMA1_CH5, 35u8, {});
dma_trait_impl!(crate::i2c::RxDma, I2C2, DMA1_CH6, 35u8, {});
dma_trait_impl!(crate::i2c::RxDma, I2C2, DMA1_CH7, 35u8, {});
dma_trait_impl!(crate::i2c::RxDma, I2C2, DMA2_CH0, 35u8, {});
dma_trait_impl!(crate::i2c::RxDma, I2C2, DMA2_CH1, 35u8, {});
dma_trait_impl!(crate::i2c::RxDma, I2C2, DMA2_CH2, 35u8, {});
dma_trait_impl!(crate::i2c::RxDma, I2C2, DMA2_CH3, 35u8, {});
dma_trait_impl!(crate::i2c::RxDma, I2C2, DMA2_CH4, 35u8, {});
dma_trait_impl!(crate::i2c::RxDma, I2C2, DMA2_CH5, 35u8, {});
dma_trait_impl!(crate::i2c::RxDma, I2C2, DMA2_CH6, 35u8, {});
dma_trait_impl!(crate::i2c::RxDma, I2C2, DMA2_CH7, 35u8, {});
dma_trait_impl!(crate::i2c::TxDma, I2C2, DMA1_CH0, 36u8, {});
dma_trait_impl!(crate::i2c::TxDma, I2C2, DMA1_CH1, 36u8, {});
dma_trait_impl!(crate::i2c::TxDma, I2C2, DMA1_CH2, 36u8, {});
dma_trait_impl!(crate::i2c::TxDma, I2C2, DMA1_CH3, 36u8, {});
dma_trait_impl!(crate::i2c::TxDma, I2C2, DMA1_CH4, 36u8, {});
dma_trait_impl!(crate::i2c::TxDma, I2C2, DMA1_CH5, 36u8, {});
dma_trait_impl!(crate::i2c::TxDma, I2C2, DMA1_CH6, 36u8, {});
dma_trait_impl!(crate::i2c::TxDma, I2C2, DMA1_CH7, 36u8, {});
dma_trait_impl!(crate::i2c::TxDma, I2C2, DMA2_CH0, 36u8, {});
dma_trait_impl!(crate::i2c::TxDma, I2C2, DMA2_CH1, 36u8, {});
dma_trait_impl!(crate::i2c::TxDma, I2C2, DMA2_CH2, 36u8, {});
dma_trait_impl!(crate::i2c::TxDma, I2C2, DMA2_CH3, 36u8, {});
dma_trait_impl!(crate::i2c::TxDma, I2C2, DMA2_CH4, 36u8, {});
dma_trait_impl!(crate::i2c::TxDma, I2C2, DMA2_CH5, 36u8, {});
dma_trait_impl!(crate::i2c::TxDma, I2C2, DMA2_CH6, 36u8, {});
dma_trait_impl!(crate::i2c::TxDma, I2C2, DMA2_CH7, 36u8, {});
dma_trait_impl!(crate::i2c::RxDma, I2C3, DMA1_CH0, 73u8, {});
dma_trait_impl!(crate::i2c::RxDma, I2C3, DMA1_CH1, 73u8, {});
dma_trait_impl!(crate::i2c::RxDma, I2C3, DMA1_CH2, 73u8, {});
dma_trait_impl!(crate::i2c::RxDma, I2C3, DMA1_CH3, 73u8, {});
dma_trait_impl!(crate::i2c::RxDma, I2C3, DMA1_CH4, 73u8, {});
dma_trait_impl!(crate::i2c::RxDma, I2C3, DMA1_CH5, 73u8, {});
dma_trait_impl!(crate::i2c::RxDma, I2C3, DMA1_CH6, 73u8, {});
dma_trait_impl!(crate::i2c::RxDma, I2C3, DMA1_CH7, 73u8, {});
dma_trait_impl!(crate::i2c::RxDma, I2C3, DMA2_CH0, 73u8, {});
dma_trait_impl!(crate::i2c::RxDma, I2C3, DMA2_CH1, 73u8, {});
dma_trait_impl!(crate::i2c::RxDma, I2C3, DMA2_CH2, 73u8, {});
dma_trait_impl!(crate::i2c::RxDma, I2C3, DMA2_CH3, 73u8, {});
dma_trait_impl!(crate::i2c::RxDma, I2C3, DMA2_CH4, 73u8, {});
dma_trait_impl!(crate::i2c::RxDma, I2C3, DMA2_CH5, 73u8, {});
dma_trait_impl!(crate::i2c::RxDma, I2C3, DMA2_CH6, 73u8, {});
dma_trait_impl!(crate::i2c::RxDma, I2C3, DMA2_CH7, 73u8, {});
dma_trait_impl!(crate::i2c::TxDma, I2C3, DMA1_CH0, 74u8, {});
dma_trait_impl!(crate::i2c::TxDma, I2C3, DMA1_CH1, 74u8, {});
dma_trait_impl!(crate::i2c::TxDma, I2C3, DMA1_CH2, 74u8, {});
dma_trait_impl!(crate::i2c::TxDma, I2C3, DMA1_CH3, 74u8, {});
dma_trait_impl!(crate::i2c::TxDma, I2C3, DMA1_CH4, 74u8, {});
dma_trait_impl!(crate::i2c::TxDma, I2C3, DMA1_CH5, 74u8, {});
dma_trait_impl!(crate::i2c::TxDma, I2C3, DMA1_CH6, 74u8, {});
dma_trait_impl!(crate::i2c::TxDma, I2C3, DMA1_CH7, 74u8, {});
dma_trait_impl!(crate::i2c::TxDma, I2C3, DMA2_CH0, 74u8, {});
dma_trait_impl!(crate::i2c::TxDma, I2C3, DMA2_CH1, 74u8, {});
dma_trait_impl!(crate::i2c::TxDma, I2C3, DMA2_CH2, 74u8, {});
dma_trait_impl!(crate::i2c::TxDma, I2C3, DMA2_CH3, 74u8, {});
dma_trait_impl!(crate::i2c::TxDma, I2C3, DMA2_CH4, 74u8, {});
dma_trait_impl!(crate::i2c::TxDma, I2C3, DMA2_CH5, 74u8, {});
dma_trait_impl!(crate::i2c::TxDma, I2C3, DMA2_CH6, 74u8, {});
dma_trait_impl!(crate::i2c::TxDma, I2C3, DMA2_CH7, 74u8, {});
dma_trait_impl!(crate::i2c::RxDma, I2C4, BDMA_CH0, 13u8, {});
dma_trait_impl!(crate::i2c::RxDma, I2C4, BDMA_CH1, 13u8, {});
dma_trait_impl!(crate::i2c::RxDma, I2C4, BDMA_CH2, 13u8, {});
dma_trait_impl!(crate::i2c::RxDma, I2C4, BDMA_CH3, 13u8, {});
dma_trait_impl!(crate::i2c::RxDma, I2C4, BDMA_CH4, 13u8, {});
dma_trait_impl!(crate::i2c::RxDma, I2C4, BDMA_CH5, 13u8, {});
dma_trait_impl!(crate::i2c::RxDma, I2C4, BDMA_CH6, 13u8, {});
dma_trait_impl!(crate::i2c::RxDma, I2C4, BDMA_CH7, 13u8, {});
dma_trait_impl!(crate::i2c::TxDma, I2C4, BDMA_CH0, 14u8, {});
dma_trait_impl!(crate::i2c::TxDma, I2C4, BDMA_CH1, 14u8, {});
dma_trait_impl!(crate::i2c::TxDma, I2C4, BDMA_CH2, 14u8, {});
dma_trait_impl!(crate::i2c::TxDma, I2C4, BDMA_CH3, 14u8, {});
dma_trait_impl!(crate::i2c::TxDma, I2C4, BDMA_CH4, 14u8, {});
dma_trait_impl!(crate::i2c::TxDma, I2C4, BDMA_CH5, 14u8, {});
dma_trait_impl!(crate::i2c::TxDma, I2C4, BDMA_CH6, 14u8, {});
dma_trait_impl!(crate::i2c::TxDma, I2C4, BDMA_CH7, 14u8, {});
dma_trait_impl!(crate::i2c::RxDma, I2C5, DMA1_CH0, 124u8, {});
dma_trait_impl!(crate::i2c::RxDma, I2C5, DMA1_CH1, 124u8, {});
dma_trait_impl!(crate::i2c::RxDma, I2C5, DMA1_CH2, 124u8, {});
dma_trait_impl!(crate::i2c::RxDma, I2C5, DMA1_CH3, 124u8, {});
dma_trait_impl!(crate::i2c::RxDma, I2C5, DMA1_CH4, 124u8, {});
dma_trait_impl!(crate::i2c::RxDma, I2C5, DMA1_CH5, 124u8, {});
dma_trait_impl!(crate::i2c::RxDma, I2C5, DMA1_CH6, 124u8, {});
dma_trait_impl!(crate::i2c::RxDma, I2C5, DMA1_CH7, 124u8, {});
dma_trait_impl!(crate::i2c::RxDma, I2C5, DMA2_CH0, 124u8, {});
dma_trait_impl!(crate::i2c::RxDma, I2C5, DMA2_CH1, 124u8, {});
dma_trait_impl!(crate::i2c::RxDma, I2C5, DMA2_CH2, 124u8, {});
dma_trait_impl!(crate::i2c::RxDma, I2C5, DMA2_CH3, 124u8, {});
dma_trait_impl!(crate::i2c::RxDma, I2C5, DMA2_CH4, 124u8, {});
dma_trait_impl!(crate::i2c::RxDma, I2C5, DMA2_CH5, 124u8, {});
dma_trait_impl!(crate::i2c::RxDma, I2C5, DMA2_CH6, 124u8, {});
dma_trait_impl!(crate::i2c::RxDma, I2C5, DMA2_CH7, 124u8, {});
dma_trait_impl!(crate::i2c::TxDma, I2C5, DMA1_CH0, 125u8, {});
dma_trait_impl!(crate::i2c::TxDma, I2C5, DMA1_CH1, 125u8, {});
dma_trait_impl!(crate::i2c::TxDma, I2C5, DMA1_CH2, 125u8, {});
dma_trait_impl!(crate::i2c::TxDma, I2C5, DMA1_CH3, 125u8, {});
dma_trait_impl!(crate::i2c::TxDma, I2C5, DMA1_CH4, 125u8, {});
dma_trait_impl!(crate::i2c::TxDma, I2C5, DMA1_CH5, 125u8, {});
dma_trait_impl!(crate::i2c::TxDma, I2C5, DMA1_CH6, 125u8, {});
dma_trait_impl!(crate::i2c::TxDma, I2C5, DMA1_CH7, 125u8, {});
dma_trait_impl!(crate::i2c::TxDma, I2C5, DMA2_CH0, 125u8, {});
dma_trait_impl!(crate::i2c::TxDma, I2C5, DMA2_CH1, 125u8, {});
dma_trait_impl!(crate::i2c::TxDma, I2C5, DMA2_CH2, 125u8, {});
dma_trait_impl!(crate::i2c::TxDma, I2C5, DMA2_CH3, 125u8, {});
dma_trait_impl!(crate::i2c::TxDma, I2C5, DMA2_CH4, 125u8, {});
dma_trait_impl!(crate::i2c::TxDma, I2C5, DMA2_CH5, 125u8, {});
dma_trait_impl!(crate::i2c::TxDma, I2C5, DMA2_CH6, 125u8, {});
dma_trait_impl!(crate::i2c::TxDma, I2C5, DMA2_CH7, 125u8, {});
dma_trait_impl!(crate::usart::RxDma, LPUART1, BDMA_CH0, 9u8, {});
dma_trait_impl!(crate::usart::RxDma, LPUART1, BDMA_CH1, 9u8, {});
dma_trait_impl!(crate::usart::RxDma, LPUART1, BDMA_CH2, 9u8, {});
dma_trait_impl!(crate::usart::RxDma, LPUART1, BDMA_CH3, 9u8, {});
dma_trait_impl!(crate::usart::RxDma, LPUART1, BDMA_CH4, 9u8, {});
dma_trait_impl!(crate::usart::RxDma, LPUART1, BDMA_CH5, 9u8, {});
dma_trait_impl!(crate::usart::RxDma, LPUART1, BDMA_CH6, 9u8, {});
dma_trait_impl!(crate::usart::RxDma, LPUART1, BDMA_CH7, 9u8, {});
dma_trait_impl!(crate::usart::TxDma, LPUART1, BDMA_CH0, 10u8, {});
dma_trait_impl!(crate::usart::TxDma, LPUART1, BDMA_CH1, 10u8, {});
dma_trait_impl!(crate::usart::TxDma, LPUART1, BDMA_CH2, 10u8, {});
dma_trait_impl!(crate::usart::TxDma, LPUART1, BDMA_CH3, 10u8, {});
dma_trait_impl!(crate::usart::TxDma, LPUART1, BDMA_CH4, 10u8, {});
dma_trait_impl!(crate::usart::TxDma, LPUART1, BDMA_CH5, 10u8, {});
dma_trait_impl!(crate::usart::TxDma, LPUART1, BDMA_CH6, 10u8, {});
dma_trait_impl!(crate::usart::TxDma, LPUART1, BDMA_CH7, 10u8, {});
dma_trait_impl!(crate::sai::Dma<A>, SAI1, DMA1_CH0, 87u8, {});
dma_trait_impl!(crate::sai::Dma<A>, SAI1, DMA1_CH1, 87u8, {});
dma_trait_impl!(crate::sai::Dma<A>, SAI1, DMA1_CH2, 87u8, {});
dma_trait_impl!(crate::sai::Dma<A>, SAI1, DMA1_CH3, 87u8, {});
dma_trait_impl!(crate::sai::Dma<A>, SAI1, DMA1_CH4, 87u8, {});
dma_trait_impl!(crate::sai::Dma<A>, SAI1, DMA1_CH5, 87u8, {});
dma_trait_impl!(crate::sai::Dma<A>, SAI1, DMA1_CH6, 87u8, {});
dma_trait_impl!(crate::sai::Dma<A>, SAI1, DMA1_CH7, 87u8, {});
dma_trait_impl!(crate::sai::Dma<A>, SAI1, DMA2_CH0, 87u8, {});
dma_trait_impl!(crate::sai::Dma<A>, SAI1, DMA2_CH1, 87u8, {});
dma_trait_impl!(crate::sai::Dma<A>, SAI1, DMA2_CH2, 87u8, {});
dma_trait_impl!(crate::sai::Dma<A>, SAI1, DMA2_CH3, 87u8, {});
dma_trait_impl!(crate::sai::Dma<A>, SAI1, DMA2_CH4, 87u8, {});
dma_trait_impl!(crate::sai::Dma<A>, SAI1, DMA2_CH5, 87u8, {});
dma_trait_impl!(crate::sai::Dma<A>, SAI1, DMA2_CH6, 87u8, {});
dma_trait_impl!(crate::sai::Dma<A>, SAI1, DMA2_CH7, 87u8, {});
dma_trait_impl!(crate::sai::Dma<B>, SAI1, DMA1_CH0, 88u8, {});
dma_trait_impl!(crate::sai::Dma<B>, SAI1, DMA1_CH1, 88u8, {});
dma_trait_impl!(crate::sai::Dma<B>, SAI1, DMA1_CH2, 88u8, {});
dma_trait_impl!(crate::sai::Dma<B>, SAI1, DMA1_CH3, 88u8, {});
dma_trait_impl!(crate::sai::Dma<B>, SAI1, DMA1_CH4, 88u8, {});
dma_trait_impl!(crate::sai::Dma<B>, SAI1, DMA1_CH5, 88u8, {});
dma_trait_impl!(crate::sai::Dma<B>, SAI1, DMA1_CH6, 88u8, {});
dma_trait_impl!(crate::sai::Dma<B>, SAI1, DMA1_CH7, 88u8, {});
dma_trait_impl!(crate::sai::Dma<B>, SAI1, DMA2_CH0, 88u8, {});
dma_trait_impl!(crate::sai::Dma<B>, SAI1, DMA2_CH1, 88u8, {});
dma_trait_impl!(crate::sai::Dma<B>, SAI1, DMA2_CH2, 88u8, {});
dma_trait_impl!(crate::sai::Dma<B>, SAI1, DMA2_CH3, 88u8, {});
dma_trait_impl!(crate::sai::Dma<B>, SAI1, DMA2_CH4, 88u8, {});
dma_trait_impl!(crate::sai::Dma<B>, SAI1, DMA2_CH5, 88u8, {});
dma_trait_impl!(crate::sai::Dma<B>, SAI1, DMA2_CH6, 88u8, {});
dma_trait_impl!(crate::sai::Dma<B>, SAI1, DMA2_CH7, 88u8, {});
dma_trait_impl!(crate::sai::Dma<A>, SAI4, BDMA_CH0, 15u8, {});
dma_trait_impl!(crate::sai::Dma<A>, SAI4, BDMA_CH1, 15u8, {});
dma_trait_impl!(crate::sai::Dma<A>, SAI4, BDMA_CH2, 15u8, {});
dma_trait_impl!(crate::sai::Dma<A>, SAI4, BDMA_CH3, 15u8, {});
dma_trait_impl!(crate::sai::Dma<A>, SAI4, BDMA_CH4, 15u8, {});
dma_trait_impl!(crate::sai::Dma<A>, SAI4, BDMA_CH5, 15u8, {});
dma_trait_impl!(crate::sai::Dma<A>, SAI4, BDMA_CH6, 15u8, {});
dma_trait_impl!(crate::sai::Dma<A>, SAI4, BDMA_CH7, 15u8, {});
dma_trait_impl!(crate::sai::Dma<B>, SAI4, BDMA_CH0, 16u8, {});
dma_trait_impl!(crate::sai::Dma<B>, SAI4, BDMA_CH1, 16u8, {});
dma_trait_impl!(crate::sai::Dma<B>, SAI4, BDMA_CH2, 16u8, {});
dma_trait_impl!(crate::sai::Dma<B>, SAI4, BDMA_CH3, 16u8, {});
dma_trait_impl!(crate::sai::Dma<B>, SAI4, BDMA_CH4, 16u8, {});
dma_trait_impl!(crate::sai::Dma<B>, SAI4, BDMA_CH5, 16u8, {});
dma_trait_impl!(crate::sai::Dma<B>, SAI4, BDMA_CH6, 16u8, {});
dma_trait_impl!(crate::sai::Dma<B>, SAI4, BDMA_CH7, 16u8, {});
dma_trait_impl!(crate::spdifrx::Dma, SPDIFRX1, DMA1_CH0, 93u8, {});
dma_trait_impl!(crate::spdifrx::Dma, SPDIFRX1, DMA1_CH1, 93u8, {});
dma_trait_impl!(crate::spdifrx::Dma, SPDIFRX1, DMA1_CH2, 93u8, {});
dma_trait_impl!(crate::spdifrx::Dma, SPDIFRX1, DMA1_CH3, 93u8, {});
dma_trait_impl!(crate::spdifrx::Dma, SPDIFRX1, DMA1_CH4, 93u8, {});
dma_trait_impl!(crate::spdifrx::Dma, SPDIFRX1, DMA1_CH5, 93u8, {});
dma_trait_impl!(crate::spdifrx::Dma, SPDIFRX1, DMA1_CH6, 93u8, {});
dma_trait_impl!(crate::spdifrx::Dma, SPDIFRX1, DMA1_CH7, 93u8, {});
dma_trait_impl!(crate::spdifrx::Dma, SPDIFRX1, DMA2_CH0, 93u8, {});
dma_trait_impl!(crate::spdifrx::Dma, SPDIFRX1, DMA2_CH1, 93u8, {});
dma_trait_impl!(crate::spdifrx::Dma, SPDIFRX1, DMA2_CH2, 93u8, {});
dma_trait_impl!(crate::spdifrx::Dma, SPDIFRX1, DMA2_CH3, 93u8, {});
dma_trait_impl!(crate::spdifrx::Dma, SPDIFRX1, DMA2_CH4, 93u8, {});
dma_trait_impl!(crate::spdifrx::Dma, SPDIFRX1, DMA2_CH5, 93u8, {});
dma_trait_impl!(crate::spdifrx::Dma, SPDIFRX1, DMA2_CH6, 93u8, {});
dma_trait_impl!(crate::spdifrx::Dma, SPDIFRX1, DMA2_CH7, 93u8, {});
dma_trait_impl!(crate::spi::RxDma, SPI1, DMA1_CH0, 37u8, {});
dma_trait_impl!(crate::spi::RxDma, SPI1, DMA1_CH1, 37u8, {});
dma_trait_impl!(crate::spi::RxDma, SPI1, DMA1_CH2, 37u8, {});
dma_trait_impl!(crate::spi::RxDma, SPI1, DMA1_CH3, 37u8, {});
dma_trait_impl!(crate::spi::RxDma, SPI1, DMA1_CH4, 37u8, {});
dma_trait_impl!(crate::spi::RxDma, SPI1, DMA1_CH5, 37u8, {});
dma_trait_impl!(crate::spi::RxDma, SPI1, DMA1_CH6, 37u8, {});
dma_trait_impl!(crate::spi::RxDma, SPI1, DMA1_CH7, 37u8, {});
dma_trait_impl!(crate::spi::RxDma, SPI1, DMA2_CH0, 37u8, {});
dma_trait_impl!(crate::spi::RxDma, SPI1, DMA2_CH1, 37u8, {});
dma_trait_impl!(crate::spi::RxDma, SPI1, DMA2_CH2, 37u8, {});
dma_trait_impl!(crate::spi::RxDma, SPI1, DMA2_CH3, 37u8, {});
dma_trait_impl!(crate::spi::RxDma, SPI1, DMA2_CH4, 37u8, {});
dma_trait_impl!(crate::spi::RxDma, SPI1, DMA2_CH5, 37u8, {});
dma_trait_impl!(crate::spi::RxDma, SPI1, DMA2_CH6, 37u8, {});
dma_trait_impl!(crate::spi::RxDma, SPI1, DMA2_CH7, 37u8, {});
dma_trait_impl!(crate::spi::TxDma, SPI1, DMA1_CH0, 38u8, {});
dma_trait_impl!(crate::spi::TxDma, SPI1, DMA1_CH1, 38u8, {});
dma_trait_impl!(crate::spi::TxDma, SPI1, DMA1_CH2, 38u8, {});
dma_trait_impl!(crate::spi::TxDma, SPI1, DMA1_CH3, 38u8, {});
dma_trait_impl!(crate::spi::TxDma, SPI1, DMA1_CH4, 38u8, {});
dma_trait_impl!(crate::spi::TxDma, SPI1, DMA1_CH5, 38u8, {});
dma_trait_impl!(crate::spi::TxDma, SPI1, DMA1_CH6, 38u8, {});
dma_trait_impl!(crate::spi::TxDma, SPI1, DMA1_CH7, 38u8, {});
dma_trait_impl!(crate::spi::TxDma, SPI1, DMA2_CH0, 38u8, {});
dma_trait_impl!(crate::spi::TxDma, SPI1, DMA2_CH1, 38u8, {});
dma_trait_impl!(crate::spi::TxDma, SPI1, DMA2_CH2, 38u8, {});
dma_trait_impl!(crate::spi::TxDma, SPI1, DMA2_CH3, 38u8, {});
dma_trait_impl!(crate::spi::TxDma, SPI1, DMA2_CH4, 38u8, {});
dma_trait_impl!(crate::spi::TxDma, SPI1, DMA2_CH5, 38u8, {});
dma_trait_impl!(crate::spi::TxDma, SPI1, DMA2_CH6, 38u8, {});
dma_trait_impl!(crate::spi::TxDma, SPI1, DMA2_CH7, 38u8, {});
dma_trait_impl!(crate::spi::RxDma, SPI2, DMA1_CH0, 39u8, {});
dma_trait_impl!(crate::spi::RxDma, SPI2, DMA1_CH1, 39u8, {});
dma_trait_impl!(crate::spi::RxDma, SPI2, DMA1_CH2, 39u8, {});
dma_trait_impl!(crate::spi::RxDma, SPI2, DMA1_CH3, 39u8, {});
dma_trait_impl!(crate::spi::RxDma, SPI2, DMA1_CH4, 39u8, {});
dma_trait_impl!(crate::spi::RxDma, SPI2, DMA1_CH5, 39u8, {});
dma_trait_impl!(crate::spi::RxDma, SPI2, DMA1_CH6, 39u8, {});
dma_trait_impl!(crate::spi::RxDma, SPI2, DMA1_CH7, 39u8, {});
dma_trait_impl!(crate::spi::RxDma, SPI2, DMA2_CH0, 39u8, {});
dma_trait_impl!(crate::spi::RxDma, SPI2, DMA2_CH1, 39u8, {});
dma_trait_impl!(crate::spi::RxDma, SPI2, DMA2_CH2, 39u8, {});
dma_trait_impl!(crate::spi::RxDma, SPI2, DMA2_CH3, 39u8, {});
dma_trait_impl!(crate::spi::RxDma, SPI2, DMA2_CH4, 39u8, {});
dma_trait_impl!(crate::spi::RxDma, SPI2, DMA2_CH5, 39u8, {});
dma_trait_impl!(crate::spi::RxDma, SPI2, DMA2_CH6, 39u8, {});
dma_trait_impl!(crate::spi::RxDma, SPI2, DMA2_CH7, 39u8, {});
dma_trait_impl!(crate::spi::TxDma, SPI2, DMA1_CH0, 40u8, {});
dma_trait_impl!(crate::spi::TxDma, SPI2, DMA1_CH1, 40u8, {});
dma_trait_impl!(crate::spi::TxDma, SPI2, DMA1_CH2, 40u8, {});
dma_trait_impl!(crate::spi::TxDma, SPI2, DMA1_CH3, 40u8, {});
dma_trait_impl!(crate::spi::TxDma, SPI2, DMA1_CH4, 40u8, {});
dma_trait_impl!(crate::spi::TxDma, SPI2, DMA1_CH5, 40u8, {});
dma_trait_impl!(crate::spi::TxDma, SPI2, DMA1_CH6, 40u8, {});
dma_trait_impl!(crate::spi::TxDma, SPI2, DMA1_CH7, 40u8, {});
dma_trait_impl!(crate::spi::TxDma, SPI2, DMA2_CH0, 40u8, {});
dma_trait_impl!(crate::spi::TxDma, SPI2, DMA2_CH1, 40u8, {});
dma_trait_impl!(crate::spi::TxDma, SPI2, DMA2_CH2, 40u8, {});
dma_trait_impl!(crate::spi::TxDma, SPI2, DMA2_CH3, 40u8, {});
dma_trait_impl!(crate::spi::TxDma, SPI2, DMA2_CH4, 40u8, {});
dma_trait_impl!(crate::spi::TxDma, SPI2, DMA2_CH5, 40u8, {});
dma_trait_impl!(crate::spi::TxDma, SPI2, DMA2_CH6, 40u8, {});
dma_trait_impl!(crate::spi::TxDma, SPI2, DMA2_CH7, 40u8, {});
dma_trait_impl!(crate::spi::RxDma, SPI3, DMA1_CH0, 61u8, {});
dma_trait_impl!(crate::spi::RxDma, SPI3, DMA1_CH1, 61u8, {});
dma_trait_impl!(crate::spi::RxDma, SPI3, DMA1_CH2, 61u8, {});
dma_trait_impl!(crate::spi::RxDma, SPI3, DMA1_CH3, 61u8, {});
dma_trait_impl!(crate::spi::RxDma, SPI3, DMA1_CH4, 61u8, {});
dma_trait_impl!(crate::spi::RxDma, SPI3, DMA1_CH5, 61u8, {});
dma_trait_impl!(crate::spi::RxDma, SPI3, DMA1_CH6, 61u8, {});
dma_trait_impl!(crate::spi::RxDma, SPI3, DMA1_CH7, 61u8, {});
dma_trait_impl!(crate::spi::RxDma, SPI3, DMA2_CH0, 61u8, {});
dma_trait_impl!(crate::spi::RxDma, SPI3, DMA2_CH1, 61u8, {});
dma_trait_impl!(crate::spi::RxDma, SPI3, DMA2_CH2, 61u8, {});
dma_trait_impl!(crate::spi::RxDma, SPI3, DMA2_CH3, 61u8, {});
dma_trait_impl!(crate::spi::RxDma, SPI3, DMA2_CH4, 61u8, {});
dma_trait_impl!(crate::spi::RxDma, SPI3, DMA2_CH5, 61u8, {});
dma_trait_impl!(crate::spi::RxDma, SPI3, DMA2_CH6, 61u8, {});
dma_trait_impl!(crate::spi::RxDma, SPI3, DMA2_CH7, 61u8, {});
dma_trait_impl!(crate::spi::TxDma, SPI3, DMA1_CH0, 62u8, {});
dma_trait_impl!(crate::spi::TxDma, SPI3, DMA1_CH1, 62u8, {});
dma_trait_impl!(crate::spi::TxDma, SPI3, DMA1_CH2, 62u8, {});
dma_trait_impl!(crate::spi::TxDma, SPI3, DMA1_CH3, 62u8, {});
dma_trait_impl!(crate::spi::TxDma, SPI3, DMA1_CH4, 62u8, {});
dma_trait_impl!(crate::spi::TxDma, SPI3, DMA1_CH5, 62u8, {});
dma_trait_impl!(crate::spi::TxDma, SPI3, DMA1_CH6, 62u8, {});
dma_trait_impl!(crate::spi::TxDma, SPI3, DMA1_CH7, 62u8, {});
dma_trait_impl!(crate::spi::TxDma, SPI3, DMA2_CH0, 62u8, {});
dma_trait_impl!(crate::spi::TxDma, SPI3, DMA2_CH1, 62u8, {});
dma_trait_impl!(crate::spi::TxDma, SPI3, DMA2_CH2, 62u8, {});
dma_trait_impl!(crate::spi::TxDma, SPI3, DMA2_CH3, 62u8, {});
dma_trait_impl!(crate::spi::TxDma, SPI3, DMA2_CH4, 62u8, {});
dma_trait_impl!(crate::spi::TxDma, SPI3, DMA2_CH5, 62u8, {});
dma_trait_impl!(crate::spi::TxDma, SPI3, DMA2_CH6, 62u8, {});
dma_trait_impl!(crate::spi::TxDma, SPI3, DMA2_CH7, 62u8, {});
dma_trait_impl!(crate::spi::RxDma, SPI4, DMA1_CH0, 83u8, {});
dma_trait_impl!(crate::spi::RxDma, SPI4, DMA1_CH1, 83u8, {});
dma_trait_impl!(crate::spi::RxDma, SPI4, DMA1_CH2, 83u8, {});
dma_trait_impl!(crate::spi::RxDma, SPI4, DMA1_CH3, 83u8, {});
dma_trait_impl!(crate::spi::RxDma, SPI4, DMA1_CH4, 83u8, {});
dma_trait_impl!(crate::spi::RxDma, SPI4, DMA1_CH5, 83u8, {});
dma_trait_impl!(crate::spi::RxDma, SPI4, DMA1_CH6, 83u8, {});
dma_trait_impl!(crate::spi::RxDma, SPI4, DMA1_CH7, 83u8, {});
dma_trait_impl!(crate::spi::RxDma, SPI4, DMA2_CH0, 83u8, {});
dma_trait_impl!(crate::spi::RxDma, SPI4, DMA2_CH1, 83u8, {});
dma_trait_impl!(crate::spi::RxDma, SPI4, DMA2_CH2, 83u8, {});
dma_trait_impl!(crate::spi::RxDma, SPI4, DMA2_CH3, 83u8, {});
dma_trait_impl!(crate::spi::RxDma, SPI4, DMA2_CH4, 83u8, {});
dma_trait_impl!(crate::spi::RxDma, SPI4, DMA2_CH5, 83u8, {});
dma_trait_impl!(crate::spi::RxDma, SPI4, DMA2_CH6, 83u8, {});
dma_trait_impl!(crate::spi::RxDma, SPI4, DMA2_CH7, 83u8, {});
dma_trait_impl!(crate::spi::TxDma, SPI4, DMA1_CH0, 84u8, {});
dma_trait_impl!(crate::spi::TxDma, SPI4, DMA1_CH1, 84u8, {});
dma_trait_impl!(crate::spi::TxDma, SPI4, DMA1_CH2, 84u8, {});
dma_trait_impl!(crate::spi::TxDma, SPI4, DMA1_CH3, 84u8, {});
dma_trait_impl!(crate::spi::TxDma, SPI4, DMA1_CH4, 84u8, {});
dma_trait_impl!(crate::spi::TxDma, SPI4, DMA1_CH5, 84u8, {});
dma_trait_impl!(crate::spi::TxDma, SPI4, DMA1_CH6, 84u8, {});
dma_trait_impl!(crate::spi::TxDma, SPI4, DMA1_CH7, 84u8, {});
dma_trait_impl!(crate::spi::TxDma, SPI4, DMA2_CH0, 84u8, {});
dma_trait_impl!(crate::spi::TxDma, SPI4, DMA2_CH1, 84u8, {});
dma_trait_impl!(crate::spi::TxDma, SPI4, DMA2_CH2, 84u8, {});
dma_trait_impl!(crate::spi::TxDma, SPI4, DMA2_CH3, 84u8, {});
dma_trait_impl!(crate::spi::TxDma, SPI4, DMA2_CH4, 84u8, {});
dma_trait_impl!(crate::spi::TxDma, SPI4, DMA2_CH5, 84u8, {});
dma_trait_impl!(crate::spi::TxDma, SPI4, DMA2_CH6, 84u8, {});
dma_trait_impl!(crate::spi::TxDma, SPI4, DMA2_CH7, 84u8, {});
dma_trait_impl!(crate::spi::RxDma, SPI6, BDMA_CH0, 11u8, {});
dma_trait_impl!(crate::spi::RxDma, SPI6, BDMA_CH1, 11u8, {});
dma_trait_impl!(crate::spi::RxDma, SPI6, BDMA_CH2, 11u8, {});
dma_trait_impl!(crate::spi::RxDma, SPI6, BDMA_CH3, 11u8, {});
dma_trait_impl!(crate::spi::RxDma, SPI6, BDMA_CH4, 11u8, {});
dma_trait_impl!(crate::spi::RxDma, SPI6, BDMA_CH5, 11u8, {});
dma_trait_impl!(crate::spi::RxDma, SPI6, BDMA_CH6, 11u8, {});
dma_trait_impl!(crate::spi::RxDma, SPI6, BDMA_CH7, 11u8, {});
dma_trait_impl!(crate::spi::TxDma, SPI6, BDMA_CH0, 12u8, {});
dma_trait_impl!(crate::spi::TxDma, SPI6, BDMA_CH1, 12u8, {});
dma_trait_impl!(crate::spi::TxDma, SPI6, BDMA_CH2, 12u8, {});
dma_trait_impl!(crate::spi::TxDma, SPI6, BDMA_CH3, 12u8, {});
dma_trait_impl!(crate::spi::TxDma, SPI6, BDMA_CH4, 12u8, {});
dma_trait_impl!(crate::spi::TxDma, SPI6, BDMA_CH5, 12u8, {});
dma_trait_impl!(crate::spi::TxDma, SPI6, BDMA_CH6, 12u8, {});
dma_trait_impl!(crate::spi::TxDma, SPI6, BDMA_CH7, 12u8, {});
dma_trait_impl!(crate::timer::Dma<Ch1>, TIM1, DMA1_CH0, 11u8, {});
dma_trait_impl!(crate::timer::Dma<Ch1>, TIM1, DMA1_CH1, 11u8, {});
dma_trait_impl!(crate::timer::Dma<Ch1>, TIM1, DMA1_CH2, 11u8, {});
dma_trait_impl!(crate::timer::Dma<Ch1>, TIM1, DMA1_CH3, 11u8, {});
dma_trait_impl!(crate::timer::Dma<Ch1>, TIM1, DMA1_CH4, 11u8, {});
dma_trait_impl!(crate::timer::Dma<Ch1>, TIM1, DMA1_CH5, 11u8, {});
dma_trait_impl!(crate::timer::Dma<Ch1>, TIM1, DMA1_CH6, 11u8, {});
dma_trait_impl!(crate::timer::Dma<Ch1>, TIM1, DMA1_CH7, 11u8, {});
dma_trait_impl!(crate::timer::Dma<Ch1>, TIM1, DMA2_CH0, 11u8, {});
dma_trait_impl!(crate::timer::Dma<Ch1>, TIM1, DMA2_CH1, 11u8, {});
dma_trait_impl!(crate::timer::Dma<Ch1>, TIM1, DMA2_CH2, 11u8, {});
dma_trait_impl!(crate::timer::Dma<Ch1>, TIM1, DMA2_CH3, 11u8, {});
dma_trait_impl!(crate::timer::Dma<Ch1>, TIM1, DMA2_CH4, 11u8, {});
dma_trait_impl!(crate::timer::Dma<Ch1>, TIM1, DMA2_CH5, 11u8, {});
dma_trait_impl!(crate::timer::Dma<Ch1>, TIM1, DMA2_CH6, 11u8, {});
dma_trait_impl!(crate::timer::Dma<Ch1>, TIM1, DMA2_CH7, 11u8, {});
dma_trait_impl!(crate::timer::Dma<Ch2>, TIM1, DMA1_CH0, 12u8, {});
dma_trait_impl!(crate::timer::Dma<Ch2>, TIM1, DMA1_CH1, 12u8, {});
dma_trait_impl!(crate::timer::Dma<Ch2>, TIM1, DMA1_CH2, 12u8, {});
dma_trait_impl!(crate::timer::Dma<Ch2>, TIM1, DMA1_CH3, 12u8, {});
dma_trait_impl!(crate::timer::Dma<Ch2>, TIM1, DMA1_CH4, 12u8, {});
dma_trait_impl!(crate::timer::Dma<Ch2>, TIM1, DMA1_CH5, 12u8, {});
dma_trait_impl!(crate::timer::Dma<Ch2>, TIM1, DMA1_CH6, 12u8, {});
dma_trait_impl!(crate::timer::Dma<Ch2>, TIM1, DMA1_CH7, 12u8, {});
dma_trait_impl!(crate::timer::Dma<Ch2>, TIM1, DMA2_CH0, 12u8, {});
dma_trait_impl!(crate::timer::Dma<Ch2>, TIM1, DMA2_CH1, 12u8, {});
dma_trait_impl!(crate::timer::Dma<Ch2>, TIM1, DMA2_CH2, 12u8, {});
dma_trait_impl!(crate::timer::Dma<Ch2>, TIM1, DMA2_CH3, 12u8, {});
dma_trait_impl!(crate::timer::Dma<Ch2>, TIM1, DMA2_CH4, 12u8, {});
dma_trait_impl!(crate::timer::Dma<Ch2>, TIM1, DMA2_CH5, 12u8, {});
dma_trait_impl!(crate::timer::Dma<Ch2>, TIM1, DMA2_CH6, 12u8, {});
dma_trait_impl!(crate::timer::Dma<Ch2>, TIM1, DMA2_CH7, 12u8, {});
dma_trait_impl!(crate::timer::Dma<Ch3>, TIM1, DMA1_CH0, 13u8, {});
dma_trait_impl!(crate::timer::Dma<Ch3>, TIM1, DMA1_CH1, 13u8, {});
dma_trait_impl!(crate::timer::Dma<Ch3>, TIM1, DMA1_CH2, 13u8, {});
dma_trait_impl!(crate::timer::Dma<Ch3>, TIM1, DMA1_CH3, 13u8, {});
dma_trait_impl!(crate::timer::Dma<Ch3>, TIM1, DMA1_CH4, 13u8, {});
dma_trait_impl!(crate::timer::Dma<Ch3>, TIM1, DMA1_CH5, 13u8, {});
dma_trait_impl!(crate::timer::Dma<Ch3>, TIM1, DMA1_CH6, 13u8, {});
dma_trait_impl!(crate::timer::Dma<Ch3>, TIM1, DMA1_CH7, 13u8, {});
dma_trait_impl!(crate::timer::Dma<Ch3>, TIM1, DMA2_CH0, 13u8, {});
dma_trait_impl!(crate::timer::Dma<Ch3>, TIM1, DMA2_CH1, 13u8, {});
dma_trait_impl!(crate::timer::Dma<Ch3>, TIM1, DMA2_CH2, 13u8, {});
dma_trait_impl!(crate::timer::Dma<Ch3>, TIM1, DMA2_CH3, 13u8, {});
dma_trait_impl!(crate::timer::Dma<Ch3>, TIM1, DMA2_CH4, 13u8, {});
dma_trait_impl!(crate::timer::Dma<Ch3>, TIM1, DMA2_CH5, 13u8, {});
dma_trait_impl!(crate::timer::Dma<Ch3>, TIM1, DMA2_CH6, 13u8, {});
dma_trait_impl!(crate::timer::Dma<Ch3>, TIM1, DMA2_CH7, 13u8, {});
dma_trait_impl!(crate::timer::Dma<Ch4>, TIM1, DMA1_CH0, 14u8, {});
dma_trait_impl!(crate::timer::Dma<Ch4>, TIM1, DMA1_CH1, 14u8, {});
dma_trait_impl!(crate::timer::Dma<Ch4>, TIM1, DMA1_CH2, 14u8, {});
dma_trait_impl!(crate::timer::Dma<Ch4>, TIM1, DMA1_CH3, 14u8, {});
dma_trait_impl!(crate::timer::Dma<Ch4>, TIM1, DMA1_CH4, 14u8, {});
dma_trait_impl!(crate::timer::Dma<Ch4>, TIM1, DMA1_CH5, 14u8, {});
dma_trait_impl!(crate::timer::Dma<Ch4>, TIM1, DMA1_CH6, 14u8, {});
dma_trait_impl!(crate::timer::Dma<Ch4>, TIM1, DMA1_CH7, 14u8, {});
dma_trait_impl!(crate::timer::Dma<Ch4>, TIM1, DMA2_CH0, 14u8, {});
dma_trait_impl!(crate::timer::Dma<Ch4>, TIM1, DMA2_CH1, 14u8, {});
dma_trait_impl!(crate::timer::Dma<Ch4>, TIM1, DMA2_CH2, 14u8, {});
dma_trait_impl!(crate::timer::Dma<Ch4>, TIM1, DMA2_CH3, 14u8, {});
dma_trait_impl!(crate::timer::Dma<Ch4>, TIM1, DMA2_CH4, 14u8, {});
dma_trait_impl!(crate::timer::Dma<Ch4>, TIM1, DMA2_CH5, 14u8, {});
dma_trait_impl!(crate::timer::Dma<Ch4>, TIM1, DMA2_CH6, 14u8, {});
dma_trait_impl!(crate::timer::Dma<Ch4>, TIM1, DMA2_CH7, 14u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM1, DMA1_CH0, 15u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM1, DMA1_CH1, 15u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM1, DMA1_CH2, 15u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM1, DMA1_CH3, 15u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM1, DMA1_CH4, 15u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM1, DMA1_CH5, 15u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM1, DMA1_CH6, 15u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM1, DMA1_CH7, 15u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM1, DMA2_CH0, 15u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM1, DMA2_CH1, 15u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM1, DMA2_CH2, 15u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM1, DMA2_CH3, 15u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM1, DMA2_CH4, 15u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM1, DMA2_CH5, 15u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM1, DMA2_CH6, 15u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM1, DMA2_CH7, 15u8, {});
dma_trait_impl!(crate::timer::Dma<Ch1>, TIM15, DMA1_CH0, 105u8, {});
dma_trait_impl!(crate::timer::Dma<Ch1>, TIM15, DMA1_CH1, 105u8, {});
dma_trait_impl!(crate::timer::Dma<Ch1>, TIM15, DMA1_CH2, 105u8, {});
dma_trait_impl!(crate::timer::Dma<Ch1>, TIM15, DMA1_CH3, 105u8, {});
dma_trait_impl!(crate::timer::Dma<Ch1>, TIM15, DMA1_CH4, 105u8, {});
dma_trait_impl!(crate::timer::Dma<Ch1>, TIM15, DMA1_CH5, 105u8, {});
dma_trait_impl!(crate::timer::Dma<Ch1>, TIM15, DMA1_CH6, 105u8, {});
dma_trait_impl!(crate::timer::Dma<Ch1>, TIM15, DMA1_CH7, 105u8, {});
dma_trait_impl!(crate::timer::Dma<Ch1>, TIM15, DMA2_CH0, 105u8, {});
dma_trait_impl!(crate::timer::Dma<Ch1>, TIM15, DMA2_CH1, 105u8, {});
dma_trait_impl!(crate::timer::Dma<Ch1>, TIM15, DMA2_CH2, 105u8, {});
dma_trait_impl!(crate::timer::Dma<Ch1>, TIM15, DMA2_CH3, 105u8, {});
dma_trait_impl!(crate::timer::Dma<Ch1>, TIM15, DMA2_CH4, 105u8, {});
dma_trait_impl!(crate::timer::Dma<Ch1>, TIM15, DMA2_CH5, 105u8, {});
dma_trait_impl!(crate::timer::Dma<Ch1>, TIM15, DMA2_CH6, 105u8, {});
dma_trait_impl!(crate::timer::Dma<Ch1>, TIM15, DMA2_CH7, 105u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM15, DMA1_CH0, 106u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM15, DMA1_CH1, 106u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM15, DMA1_CH2, 106u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM15, DMA1_CH3, 106u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM15, DMA1_CH4, 106u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM15, DMA1_CH5, 106u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM15, DMA1_CH6, 106u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM15, DMA1_CH7, 106u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM15, DMA2_CH0, 106u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM15, DMA2_CH1, 106u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM15, DMA2_CH2, 106u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM15, DMA2_CH3, 106u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM15, DMA2_CH4, 106u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM15, DMA2_CH5, 106u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM15, DMA2_CH6, 106u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM15, DMA2_CH7, 106u8, {});
dma_trait_impl!(crate::timer::Dma<Ch1>, TIM16, DMA1_CH0, 109u8, {});
dma_trait_impl!(crate::timer::Dma<Ch1>, TIM16, DMA1_CH1, 109u8, {});
dma_trait_impl!(crate::timer::Dma<Ch1>, TIM16, DMA1_CH2, 109u8, {});
dma_trait_impl!(crate::timer::Dma<Ch1>, TIM16, DMA1_CH3, 109u8, {});
dma_trait_impl!(crate::timer::Dma<Ch1>, TIM16, DMA1_CH4, 109u8, {});
dma_trait_impl!(crate::timer::Dma<Ch1>, TIM16, DMA1_CH5, 109u8, {});
dma_trait_impl!(crate::timer::Dma<Ch1>, TIM16, DMA1_CH6, 109u8, {});
dma_trait_impl!(crate::timer::Dma<Ch1>, TIM16, DMA1_CH7, 109u8, {});
dma_trait_impl!(crate::timer::Dma<Ch1>, TIM16, DMA2_CH0, 109u8, {});
dma_trait_impl!(crate::timer::Dma<Ch1>, TIM16, DMA2_CH1, 109u8, {});
dma_trait_impl!(crate::timer::Dma<Ch1>, TIM16, DMA2_CH2, 109u8, {});
dma_trait_impl!(crate::timer::Dma<Ch1>, TIM16, DMA2_CH3, 109u8, {});
dma_trait_impl!(crate::timer::Dma<Ch1>, TIM16, DMA2_CH4, 109u8, {});
dma_trait_impl!(crate::timer::Dma<Ch1>, TIM16, DMA2_CH5, 109u8, {});
dma_trait_impl!(crate::timer::Dma<Ch1>, TIM16, DMA2_CH6, 109u8, {});
dma_trait_impl!(crate::timer::Dma<Ch1>, TIM16, DMA2_CH7, 109u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM16, DMA1_CH0, 110u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM16, DMA1_CH1, 110u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM16, DMA1_CH2, 110u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM16, DMA1_CH3, 110u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM16, DMA1_CH4, 110u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM16, DMA1_CH5, 110u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM16, DMA1_CH6, 110u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM16, DMA1_CH7, 110u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM16, DMA2_CH0, 110u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM16, DMA2_CH1, 110u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM16, DMA2_CH2, 110u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM16, DMA2_CH3, 110u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM16, DMA2_CH4, 110u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM16, DMA2_CH5, 110u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM16, DMA2_CH6, 110u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM16, DMA2_CH7, 110u8, {});
dma_trait_impl!(crate::timer::Dma<Ch1>, TIM17, DMA1_CH0, 111u8, {});
dma_trait_impl!(crate::timer::Dma<Ch1>, TIM17, DMA1_CH1, 111u8, {});
dma_trait_impl!(crate::timer::Dma<Ch1>, TIM17, DMA1_CH2, 111u8, {});
dma_trait_impl!(crate::timer::Dma<Ch1>, TIM17, DMA1_CH3, 111u8, {});
dma_trait_impl!(crate::timer::Dma<Ch1>, TIM17, DMA1_CH4, 111u8, {});
dma_trait_impl!(crate::timer::Dma<Ch1>, TIM17, DMA1_CH5, 111u8, {});
dma_trait_impl!(crate::timer::Dma<Ch1>, TIM17, DMA1_CH6, 111u8, {});
dma_trait_impl!(crate::timer::Dma<Ch1>, TIM17, DMA1_CH7, 111u8, {});
dma_trait_impl!(crate::timer::Dma<Ch1>, TIM17, DMA2_CH0, 111u8, {});
dma_trait_impl!(crate::timer::Dma<Ch1>, TIM17, DMA2_CH1, 111u8, {});
dma_trait_impl!(crate::timer::Dma<Ch1>, TIM17, DMA2_CH2, 111u8, {});
dma_trait_impl!(crate::timer::Dma<Ch1>, TIM17, DMA2_CH3, 111u8, {});
dma_trait_impl!(crate::timer::Dma<Ch1>, TIM17, DMA2_CH4, 111u8, {});
dma_trait_impl!(crate::timer::Dma<Ch1>, TIM17, DMA2_CH5, 111u8, {});
dma_trait_impl!(crate::timer::Dma<Ch1>, TIM17, DMA2_CH6, 111u8, {});
dma_trait_impl!(crate::timer::Dma<Ch1>, TIM17, DMA2_CH7, 111u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM17, DMA1_CH0, 112u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM17, DMA1_CH1, 112u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM17, DMA1_CH2, 112u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM17, DMA1_CH3, 112u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM17, DMA1_CH4, 112u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM17, DMA1_CH5, 112u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM17, DMA1_CH6, 112u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM17, DMA1_CH7, 112u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM17, DMA2_CH0, 112u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM17, DMA2_CH1, 112u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM17, DMA2_CH2, 112u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM17, DMA2_CH3, 112u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM17, DMA2_CH4, 112u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM17, DMA2_CH5, 112u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM17, DMA2_CH6, 112u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM17, DMA2_CH7, 112u8, {});
dma_trait_impl!(crate::timer::Dma<Ch1>, TIM2, DMA1_CH0, 18u8, {});
dma_trait_impl!(crate::timer::Dma<Ch1>, TIM2, DMA1_CH1, 18u8, {});
dma_trait_impl!(crate::timer::Dma<Ch1>, TIM2, DMA1_CH2, 18u8, {});
dma_trait_impl!(crate::timer::Dma<Ch1>, TIM2, DMA1_CH3, 18u8, {});
dma_trait_impl!(crate::timer::Dma<Ch1>, TIM2, DMA1_CH4, 18u8, {});
dma_trait_impl!(crate::timer::Dma<Ch1>, TIM2, DMA1_CH5, 18u8, {});
dma_trait_impl!(crate::timer::Dma<Ch1>, TIM2, DMA1_CH6, 18u8, {});
dma_trait_impl!(crate::timer::Dma<Ch1>, TIM2, DMA1_CH7, 18u8, {});
dma_trait_impl!(crate::timer::Dma<Ch1>, TIM2, DMA2_CH0, 18u8, {});
dma_trait_impl!(crate::timer::Dma<Ch1>, TIM2, DMA2_CH1, 18u8, {});
dma_trait_impl!(crate::timer::Dma<Ch1>, TIM2, DMA2_CH2, 18u8, {});
dma_trait_impl!(crate::timer::Dma<Ch1>, TIM2, DMA2_CH3, 18u8, {});
dma_trait_impl!(crate::timer::Dma<Ch1>, TIM2, DMA2_CH4, 18u8, {});
dma_trait_impl!(crate::timer::Dma<Ch1>, TIM2, DMA2_CH5, 18u8, {});
dma_trait_impl!(crate::timer::Dma<Ch1>, TIM2, DMA2_CH6, 18u8, {});
dma_trait_impl!(crate::timer::Dma<Ch1>, TIM2, DMA2_CH7, 18u8, {});
dma_trait_impl!(crate::timer::Dma<Ch2>, TIM2, DMA1_CH0, 19u8, {});
dma_trait_impl!(crate::timer::Dma<Ch2>, TIM2, DMA1_CH1, 19u8, {});
dma_trait_impl!(crate::timer::Dma<Ch2>, TIM2, DMA1_CH2, 19u8, {});
dma_trait_impl!(crate::timer::Dma<Ch2>, TIM2, DMA1_CH3, 19u8, {});
dma_trait_impl!(crate::timer::Dma<Ch2>, TIM2, DMA1_CH4, 19u8, {});
dma_trait_impl!(crate::timer::Dma<Ch2>, TIM2, DMA1_CH5, 19u8, {});
dma_trait_impl!(crate::timer::Dma<Ch2>, TIM2, DMA1_CH6, 19u8, {});
dma_trait_impl!(crate::timer::Dma<Ch2>, TIM2, DMA1_CH7, 19u8, {});
dma_trait_impl!(crate::timer::Dma<Ch2>, TIM2, DMA2_CH0, 19u8, {});
dma_trait_impl!(crate::timer::Dma<Ch2>, TIM2, DMA2_CH1, 19u8, {});
dma_trait_impl!(crate::timer::Dma<Ch2>, TIM2, DMA2_CH2, 19u8, {});
dma_trait_impl!(crate::timer::Dma<Ch2>, TIM2, DMA2_CH3, 19u8, {});
dma_trait_impl!(crate::timer::Dma<Ch2>, TIM2, DMA2_CH4, 19u8, {});
dma_trait_impl!(crate::timer::Dma<Ch2>, TIM2, DMA2_CH5, 19u8, {});
dma_trait_impl!(crate::timer::Dma<Ch2>, TIM2, DMA2_CH6, 19u8, {});
dma_trait_impl!(crate::timer::Dma<Ch2>, TIM2, DMA2_CH7, 19u8, {});
dma_trait_impl!(crate::timer::Dma<Ch3>, TIM2, DMA1_CH0, 20u8, {});
dma_trait_impl!(crate::timer::Dma<Ch3>, TIM2, DMA1_CH1, 20u8, {});
dma_trait_impl!(crate::timer::Dma<Ch3>, TIM2, DMA1_CH2, 20u8, {});
dma_trait_impl!(crate::timer::Dma<Ch3>, TIM2, DMA1_CH3, 20u8, {});
dma_trait_impl!(crate::timer::Dma<Ch3>, TIM2, DMA1_CH4, 20u8, {});
dma_trait_impl!(crate::timer::Dma<Ch3>, TIM2, DMA1_CH5, 20u8, {});
dma_trait_impl!(crate::timer::Dma<Ch3>, TIM2, DMA1_CH6, 20u8, {});
dma_trait_impl!(crate::timer::Dma<Ch3>, TIM2, DMA1_CH7, 20u8, {});
dma_trait_impl!(crate::timer::Dma<Ch3>, TIM2, DMA2_CH0, 20u8, {});
dma_trait_impl!(crate::timer::Dma<Ch3>, TIM2, DMA2_CH1, 20u8, {});
dma_trait_impl!(crate::timer::Dma<Ch3>, TIM2, DMA2_CH2, 20u8, {});
dma_trait_impl!(crate::timer::Dma<Ch3>, TIM2, DMA2_CH3, 20u8, {});
dma_trait_impl!(crate::timer::Dma<Ch3>, TIM2, DMA2_CH4, 20u8, {});
dma_trait_impl!(crate::timer::Dma<Ch3>, TIM2, DMA2_CH5, 20u8, {});
dma_trait_impl!(crate::timer::Dma<Ch3>, TIM2, DMA2_CH6, 20u8, {});
dma_trait_impl!(crate::timer::Dma<Ch3>, TIM2, DMA2_CH7, 20u8, {});
dma_trait_impl!(crate::timer::Dma<Ch4>, TIM2, DMA1_CH0, 21u8, {});
dma_trait_impl!(crate::timer::Dma<Ch4>, TIM2, DMA1_CH1, 21u8, {});
dma_trait_impl!(crate::timer::Dma<Ch4>, TIM2, DMA1_CH2, 21u8, {});
dma_trait_impl!(crate::timer::Dma<Ch4>, TIM2, DMA1_CH3, 21u8, {});
dma_trait_impl!(crate::timer::Dma<Ch4>, TIM2, DMA1_CH4, 21u8, {});
dma_trait_impl!(crate::timer::Dma<Ch4>, TIM2, DMA1_CH5, 21u8, {});
dma_trait_impl!(crate::timer::Dma<Ch4>, TIM2, DMA1_CH6, 21u8, {});
dma_trait_impl!(crate::timer::Dma<Ch4>, TIM2, DMA1_CH7, 21u8, {});
dma_trait_impl!(crate::timer::Dma<Ch4>, TIM2, DMA2_CH0, 21u8, {});
dma_trait_impl!(crate::timer::Dma<Ch4>, TIM2, DMA2_CH1, 21u8, {});
dma_trait_impl!(crate::timer::Dma<Ch4>, TIM2, DMA2_CH2, 21u8, {});
dma_trait_impl!(crate::timer::Dma<Ch4>, TIM2, DMA2_CH3, 21u8, {});
dma_trait_impl!(crate::timer::Dma<Ch4>, TIM2, DMA2_CH4, 21u8, {});
dma_trait_impl!(crate::timer::Dma<Ch4>, TIM2, DMA2_CH5, 21u8, {});
dma_trait_impl!(crate::timer::Dma<Ch4>, TIM2, DMA2_CH6, 21u8, {});
dma_trait_impl!(crate::timer::Dma<Ch4>, TIM2, DMA2_CH7, 21u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM2, DMA1_CH0, 22u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM2, DMA1_CH1, 22u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM2, DMA1_CH2, 22u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM2, DMA1_CH3, 22u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM2, DMA1_CH4, 22u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM2, DMA1_CH5, 22u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM2, DMA1_CH6, 22u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM2, DMA1_CH7, 22u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM2, DMA2_CH0, 22u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM2, DMA2_CH1, 22u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM2, DMA2_CH2, 22u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM2, DMA2_CH3, 22u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM2, DMA2_CH4, 22u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM2, DMA2_CH5, 22u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM2, DMA2_CH6, 22u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM2, DMA2_CH7, 22u8, {});
dma_trait_impl!(crate::timer::Dma<Ch1>, TIM23, DMA1_CH0, 126u8, {});
dma_trait_impl!(crate::timer::Dma<Ch1>, TIM23, DMA1_CH1, 126u8, {});
dma_trait_impl!(crate::timer::Dma<Ch1>, TIM23, DMA1_CH2, 126u8, {});
dma_trait_impl!(crate::timer::Dma<Ch1>, TIM23, DMA1_CH3, 126u8, {});
dma_trait_impl!(crate::timer::Dma<Ch1>, TIM23, DMA1_CH4, 126u8, {});
dma_trait_impl!(crate::timer::Dma<Ch1>, TIM23, DMA1_CH5, 126u8, {});
dma_trait_impl!(crate::timer::Dma<Ch1>, TIM23, DMA1_CH6, 126u8, {});
dma_trait_impl!(crate::timer::Dma<Ch1>, TIM23, DMA1_CH7, 126u8, {});
dma_trait_impl!(crate::timer::Dma<Ch1>, TIM23, DMA2_CH0, 126u8, {});
dma_trait_impl!(crate::timer::Dma<Ch1>, TIM23, DMA2_CH1, 126u8, {});
dma_trait_impl!(crate::timer::Dma<Ch1>, TIM23, DMA2_CH2, 126u8, {});
dma_trait_impl!(crate::timer::Dma<Ch1>, TIM23, DMA2_CH3, 126u8, {});
dma_trait_impl!(crate::timer::Dma<Ch1>, TIM23, DMA2_CH4, 126u8, {});
dma_trait_impl!(crate::timer::Dma<Ch1>, TIM23, DMA2_CH5, 126u8, {});
dma_trait_impl!(crate::timer::Dma<Ch1>, TIM23, DMA2_CH6, 126u8, {});
dma_trait_impl!(crate::timer::Dma<Ch1>, TIM23, DMA2_CH7, 126u8, {});
dma_trait_impl!(crate::timer::Dma<Ch2>, TIM23, DMA1_CH0, 127u8, {});
dma_trait_impl!(crate::timer::Dma<Ch2>, TIM23, DMA1_CH1, 127u8, {});
dma_trait_impl!(crate::timer::Dma<Ch2>, TIM23, DMA1_CH2, 127u8, {});
dma_trait_impl!(crate::timer::Dma<Ch2>, TIM23, DMA1_CH3, 127u8, {});
dma_trait_impl!(crate::timer::Dma<Ch2>, TIM23, DMA1_CH4, 127u8, {});
dma_trait_impl!(crate::timer::Dma<Ch2>, TIM23, DMA1_CH5, 127u8, {});
dma_trait_impl!(crate::timer::Dma<Ch2>, TIM23, DMA1_CH6, 127u8, {});
dma_trait_impl!(crate::timer::Dma<Ch2>, TIM23, DMA1_CH7, 127u8, {});
dma_trait_impl!(crate::timer::Dma<Ch2>, TIM23, DMA2_CH0, 127u8, {});
dma_trait_impl!(crate::timer::Dma<Ch2>, TIM23, DMA2_CH1, 127u8, {});
dma_trait_impl!(crate::timer::Dma<Ch2>, TIM23, DMA2_CH2, 127u8, {});
dma_trait_impl!(crate::timer::Dma<Ch2>, TIM23, DMA2_CH3, 127u8, {});
dma_trait_impl!(crate::timer::Dma<Ch2>, TIM23, DMA2_CH4, 127u8, {});
dma_trait_impl!(crate::timer::Dma<Ch2>, TIM23, DMA2_CH5, 127u8, {});
dma_trait_impl!(crate::timer::Dma<Ch2>, TIM23, DMA2_CH6, 127u8, {});
dma_trait_impl!(crate::timer::Dma<Ch2>, TIM23, DMA2_CH7, 127u8, {});
dma_trait_impl!(crate::timer::Dma<Ch3>, TIM23, DMA1_CH0, 128u8, {});
dma_trait_impl!(crate::timer::Dma<Ch3>, TIM23, DMA1_CH1, 128u8, {});
dma_trait_impl!(crate::timer::Dma<Ch3>, TIM23, DMA1_CH2, 128u8, {});
dma_trait_impl!(crate::timer::Dma<Ch3>, TIM23, DMA1_CH3, 128u8, {});
dma_trait_impl!(crate::timer::Dma<Ch3>, TIM23, DMA1_CH4, 128u8, {});
dma_trait_impl!(crate::timer::Dma<Ch3>, TIM23, DMA1_CH5, 128u8, {});
dma_trait_impl!(crate::timer::Dma<Ch3>, TIM23, DMA1_CH6, 128u8, {});
dma_trait_impl!(crate::timer::Dma<Ch3>, TIM23, DMA1_CH7, 128u8, {});
dma_trait_impl!(crate::timer::Dma<Ch3>, TIM23, DMA2_CH0, 128u8, {});
dma_trait_impl!(crate::timer::Dma<Ch3>, TIM23, DMA2_CH1, 128u8, {});
dma_trait_impl!(crate::timer::Dma<Ch3>, TIM23, DMA2_CH2, 128u8, {});
dma_trait_impl!(crate::timer::Dma<Ch3>, TIM23, DMA2_CH3, 128u8, {});
dma_trait_impl!(crate::timer::Dma<Ch3>, TIM23, DMA2_CH4, 128u8, {});
dma_trait_impl!(crate::timer::Dma<Ch3>, TIM23, DMA2_CH5, 128u8, {});
dma_trait_impl!(crate::timer::Dma<Ch3>, TIM23, DMA2_CH6, 128u8, {});
dma_trait_impl!(crate::timer::Dma<Ch3>, TIM23, DMA2_CH7, 128u8, {});
dma_trait_impl!(crate::timer::Dma<Ch4>, TIM23, DMA1_CH0, 129u8, {});
dma_trait_impl!(crate::timer::Dma<Ch4>, TIM23, DMA1_CH1, 129u8, {});
dma_trait_impl!(crate::timer::Dma<Ch4>, TIM23, DMA1_CH2, 129u8, {});
dma_trait_impl!(crate::timer::Dma<Ch4>, TIM23, DMA1_CH3, 129u8, {});
dma_trait_impl!(crate::timer::Dma<Ch4>, TIM23, DMA1_CH4, 129u8, {});
dma_trait_impl!(crate::timer::Dma<Ch4>, TIM23, DMA1_CH5, 129u8, {});
dma_trait_impl!(crate::timer::Dma<Ch4>, TIM23, DMA1_CH6, 129u8, {});
dma_trait_impl!(crate::timer::Dma<Ch4>, TIM23, DMA1_CH7, 129u8, {});
dma_trait_impl!(crate::timer::Dma<Ch4>, TIM23, DMA2_CH0, 129u8, {});
dma_trait_impl!(crate::timer::Dma<Ch4>, TIM23, DMA2_CH1, 129u8, {});
dma_trait_impl!(crate::timer::Dma<Ch4>, TIM23, DMA2_CH2, 129u8, {});
dma_trait_impl!(crate::timer::Dma<Ch4>, TIM23, DMA2_CH3, 129u8, {});
dma_trait_impl!(crate::timer::Dma<Ch4>, TIM23, DMA2_CH4, 129u8, {});
dma_trait_impl!(crate::timer::Dma<Ch4>, TIM23, DMA2_CH5, 129u8, {});
dma_trait_impl!(crate::timer::Dma<Ch4>, TIM23, DMA2_CH6, 129u8, {});
dma_trait_impl!(crate::timer::Dma<Ch4>, TIM23, DMA2_CH7, 129u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM23, DMA1_CH0, 130u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM23, DMA1_CH1, 130u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM23, DMA1_CH2, 130u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM23, DMA1_CH3, 130u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM23, DMA1_CH4, 130u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM23, DMA1_CH5, 130u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM23, DMA1_CH6, 130u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM23, DMA1_CH7, 130u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM23, DMA2_CH0, 130u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM23, DMA2_CH1, 130u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM23, DMA2_CH2, 130u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM23, DMA2_CH3, 130u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM23, DMA2_CH4, 130u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM23, DMA2_CH5, 130u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM23, DMA2_CH6, 130u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM23, DMA2_CH7, 130u8, {});
dma_trait_impl!(crate::timer::Dma<Ch1>, TIM24, DMA1_CH0, 132u8, {});
dma_trait_impl!(crate::timer::Dma<Ch1>, TIM24, DMA1_CH1, 132u8, {});
dma_trait_impl!(crate::timer::Dma<Ch1>, TIM24, DMA1_CH2, 132u8, {});
dma_trait_impl!(crate::timer::Dma<Ch1>, TIM24, DMA1_CH3, 132u8, {});
dma_trait_impl!(crate::timer::Dma<Ch1>, TIM24, DMA1_CH4, 132u8, {});
dma_trait_impl!(crate::timer::Dma<Ch1>, TIM24, DMA1_CH5, 132u8, {});
dma_trait_impl!(crate::timer::Dma<Ch1>, TIM24, DMA1_CH6, 132u8, {});
dma_trait_impl!(crate::timer::Dma<Ch1>, TIM24, DMA1_CH7, 132u8, {});
dma_trait_impl!(crate::timer::Dma<Ch1>, TIM24, DMA2_CH0, 132u8, {});
dma_trait_impl!(crate::timer::Dma<Ch1>, TIM24, DMA2_CH1, 132u8, {});
dma_trait_impl!(crate::timer::Dma<Ch1>, TIM24, DMA2_CH2, 132u8, {});
dma_trait_impl!(crate::timer::Dma<Ch1>, TIM24, DMA2_CH3, 132u8, {});
dma_trait_impl!(crate::timer::Dma<Ch1>, TIM24, DMA2_CH4, 132u8, {});
dma_trait_impl!(crate::timer::Dma<Ch1>, TIM24, DMA2_CH5, 132u8, {});
dma_trait_impl!(crate::timer::Dma<Ch1>, TIM24, DMA2_CH6, 132u8, {});
dma_trait_impl!(crate::timer::Dma<Ch1>, TIM24, DMA2_CH7, 132u8, {});
dma_trait_impl!(crate::timer::Dma<Ch2>, TIM24, DMA1_CH0, 133u8, {});
dma_trait_impl!(crate::timer::Dma<Ch2>, TIM24, DMA1_CH1, 133u8, {});
dma_trait_impl!(crate::timer::Dma<Ch2>, TIM24, DMA1_CH2, 133u8, {});
dma_trait_impl!(crate::timer::Dma<Ch2>, TIM24, DMA1_CH3, 133u8, {});
dma_trait_impl!(crate::timer::Dma<Ch2>, TIM24, DMA1_CH4, 133u8, {});
dma_trait_impl!(crate::timer::Dma<Ch2>, TIM24, DMA1_CH5, 133u8, {});
dma_trait_impl!(crate::timer::Dma<Ch2>, TIM24, DMA1_CH6, 133u8, {});
dma_trait_impl!(crate::timer::Dma<Ch2>, TIM24, DMA1_CH7, 133u8, {});
dma_trait_impl!(crate::timer::Dma<Ch2>, TIM24, DMA2_CH0, 133u8, {});
dma_trait_impl!(crate::timer::Dma<Ch2>, TIM24, DMA2_CH1, 133u8, {});
dma_trait_impl!(crate::timer::Dma<Ch2>, TIM24, DMA2_CH2, 133u8, {});
dma_trait_impl!(crate::timer::Dma<Ch2>, TIM24, DMA2_CH3, 133u8, {});
dma_trait_impl!(crate::timer::Dma<Ch2>, TIM24, DMA2_CH4, 133u8, {});
dma_trait_impl!(crate::timer::Dma<Ch2>, TIM24, DMA2_CH5, 133u8, {});
dma_trait_impl!(crate::timer::Dma<Ch2>, TIM24, DMA2_CH6, 133u8, {});
dma_trait_impl!(crate::timer::Dma<Ch2>, TIM24, DMA2_CH7, 133u8, {});
dma_trait_impl!(crate::timer::Dma<Ch3>, TIM24, DMA1_CH0, 134u8, {});
dma_trait_impl!(crate::timer::Dma<Ch3>, TIM24, DMA1_CH1, 134u8, {});
dma_trait_impl!(crate::timer::Dma<Ch3>, TIM24, DMA1_CH2, 134u8, {});
dma_trait_impl!(crate::timer::Dma<Ch3>, TIM24, DMA1_CH3, 134u8, {});
dma_trait_impl!(crate::timer::Dma<Ch3>, TIM24, DMA1_CH4, 134u8, {});
dma_trait_impl!(crate::timer::Dma<Ch3>, TIM24, DMA1_CH5, 134u8, {});
dma_trait_impl!(crate::timer::Dma<Ch3>, TIM24, DMA1_CH6, 134u8, {});
dma_trait_impl!(crate::timer::Dma<Ch3>, TIM24, DMA1_CH7, 134u8, {});
dma_trait_impl!(crate::timer::Dma<Ch3>, TIM24, DMA2_CH0, 134u8, {});
dma_trait_impl!(crate::timer::Dma<Ch3>, TIM24, DMA2_CH1, 134u8, {});
dma_trait_impl!(crate::timer::Dma<Ch3>, TIM24, DMA2_CH2, 134u8, {});
dma_trait_impl!(crate::timer::Dma<Ch3>, TIM24, DMA2_CH3, 134u8, {});
dma_trait_impl!(crate::timer::Dma<Ch3>, TIM24, DMA2_CH4, 134u8, {});
dma_trait_impl!(crate::timer::Dma<Ch3>, TIM24, DMA2_CH5, 134u8, {});
dma_trait_impl!(crate::timer::Dma<Ch3>, TIM24, DMA2_CH6, 134u8, {});
dma_trait_impl!(crate::timer::Dma<Ch3>, TIM24, DMA2_CH7, 134u8, {});
dma_trait_impl!(crate::timer::Dma<Ch4>, TIM24, DMA1_CH0, 135u8, {});
dma_trait_impl!(crate::timer::Dma<Ch4>, TIM24, DMA1_CH1, 135u8, {});
dma_trait_impl!(crate::timer::Dma<Ch4>, TIM24, DMA1_CH2, 135u8, {});
dma_trait_impl!(crate::timer::Dma<Ch4>, TIM24, DMA1_CH3, 135u8, {});
dma_trait_impl!(crate::timer::Dma<Ch4>, TIM24, DMA1_CH4, 135u8, {});
dma_trait_impl!(crate::timer::Dma<Ch4>, TIM24, DMA1_CH5, 135u8, {});
dma_trait_impl!(crate::timer::Dma<Ch4>, TIM24, DMA1_CH6, 135u8, {});
dma_trait_impl!(crate::timer::Dma<Ch4>, TIM24, DMA1_CH7, 135u8, {});
dma_trait_impl!(crate::timer::Dma<Ch4>, TIM24, DMA2_CH0, 135u8, {});
dma_trait_impl!(crate::timer::Dma<Ch4>, TIM24, DMA2_CH1, 135u8, {});
dma_trait_impl!(crate::timer::Dma<Ch4>, TIM24, DMA2_CH2, 135u8, {});
dma_trait_impl!(crate::timer::Dma<Ch4>, TIM24, DMA2_CH3, 135u8, {});
dma_trait_impl!(crate::timer::Dma<Ch4>, TIM24, DMA2_CH4, 135u8, {});
dma_trait_impl!(crate::timer::Dma<Ch4>, TIM24, DMA2_CH5, 135u8, {});
dma_trait_impl!(crate::timer::Dma<Ch4>, TIM24, DMA2_CH6, 135u8, {});
dma_trait_impl!(crate::timer::Dma<Ch4>, TIM24, DMA2_CH7, 135u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM24, DMA1_CH0, 136u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM24, DMA1_CH1, 136u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM24, DMA1_CH2, 136u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM24, DMA1_CH3, 136u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM24, DMA1_CH4, 136u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM24, DMA1_CH5, 136u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM24, DMA1_CH6, 136u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM24, DMA1_CH7, 136u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM24, DMA2_CH0, 136u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM24, DMA2_CH1, 136u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM24, DMA2_CH2, 136u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM24, DMA2_CH3, 136u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM24, DMA2_CH4, 136u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM24, DMA2_CH5, 136u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM24, DMA2_CH6, 136u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM24, DMA2_CH7, 136u8, {});
dma_trait_impl!(crate::timer::Dma<Ch1>, TIM3, DMA1_CH0, 23u8, {});
dma_trait_impl!(crate::timer::Dma<Ch1>, TIM3, DMA1_CH1, 23u8, {});
dma_trait_impl!(crate::timer::Dma<Ch1>, TIM3, DMA1_CH2, 23u8, {});
dma_trait_impl!(crate::timer::Dma<Ch1>, TIM3, DMA1_CH3, 23u8, {});
dma_trait_impl!(crate::timer::Dma<Ch1>, TIM3, DMA1_CH4, 23u8, {});
dma_trait_impl!(crate::timer::Dma<Ch1>, TIM3, DMA1_CH5, 23u8, {});
dma_trait_impl!(crate::timer::Dma<Ch1>, TIM3, DMA1_CH6, 23u8, {});
dma_trait_impl!(crate::timer::Dma<Ch1>, TIM3, DMA1_CH7, 23u8, {});
dma_trait_impl!(crate::timer::Dma<Ch1>, TIM3, DMA2_CH0, 23u8, {});
dma_trait_impl!(crate::timer::Dma<Ch1>, TIM3, DMA2_CH1, 23u8, {});
dma_trait_impl!(crate::timer::Dma<Ch1>, TIM3, DMA2_CH2, 23u8, {});
dma_trait_impl!(crate::timer::Dma<Ch1>, TIM3, DMA2_CH3, 23u8, {});
dma_trait_impl!(crate::timer::Dma<Ch1>, TIM3, DMA2_CH4, 23u8, {});
dma_trait_impl!(crate::timer::Dma<Ch1>, TIM3, DMA2_CH5, 23u8, {});
dma_trait_impl!(crate::timer::Dma<Ch1>, TIM3, DMA2_CH6, 23u8, {});
dma_trait_impl!(crate::timer::Dma<Ch1>, TIM3, DMA2_CH7, 23u8, {});
dma_trait_impl!(crate::timer::Dma<Ch2>, TIM3, DMA1_CH0, 24u8, {});
dma_trait_impl!(crate::timer::Dma<Ch2>, TIM3, DMA1_CH1, 24u8, {});
dma_trait_impl!(crate::timer::Dma<Ch2>, TIM3, DMA1_CH2, 24u8, {});
dma_trait_impl!(crate::timer::Dma<Ch2>, TIM3, DMA1_CH3, 24u8, {});
dma_trait_impl!(crate::timer::Dma<Ch2>, TIM3, DMA1_CH4, 24u8, {});
dma_trait_impl!(crate::timer::Dma<Ch2>, TIM3, DMA1_CH5, 24u8, {});
dma_trait_impl!(crate::timer::Dma<Ch2>, TIM3, DMA1_CH6, 24u8, {});
dma_trait_impl!(crate::timer::Dma<Ch2>, TIM3, DMA1_CH7, 24u8, {});
dma_trait_impl!(crate::timer::Dma<Ch2>, TIM3, DMA2_CH0, 24u8, {});
dma_trait_impl!(crate::timer::Dma<Ch2>, TIM3, DMA2_CH1, 24u8, {});
dma_trait_impl!(crate::timer::Dma<Ch2>, TIM3, DMA2_CH2, 24u8, {});
dma_trait_impl!(crate::timer::Dma<Ch2>, TIM3, DMA2_CH3, 24u8, {});
dma_trait_impl!(crate::timer::Dma<Ch2>, TIM3, DMA2_CH4, 24u8, {});
dma_trait_impl!(crate::timer::Dma<Ch2>, TIM3, DMA2_CH5, 24u8, {});
dma_trait_impl!(crate::timer::Dma<Ch2>, TIM3, DMA2_CH6, 24u8, {});
dma_trait_impl!(crate::timer::Dma<Ch2>, TIM3, DMA2_CH7, 24u8, {});
dma_trait_impl!(crate::timer::Dma<Ch3>, TIM3, DMA1_CH0, 25u8, {});
dma_trait_impl!(crate::timer::Dma<Ch3>, TIM3, DMA1_CH1, 25u8, {});
dma_trait_impl!(crate::timer::Dma<Ch3>, TIM3, DMA1_CH2, 25u8, {});
dma_trait_impl!(crate::timer::Dma<Ch3>, TIM3, DMA1_CH3, 25u8, {});
dma_trait_impl!(crate::timer::Dma<Ch3>, TIM3, DMA1_CH4, 25u8, {});
dma_trait_impl!(crate::timer::Dma<Ch3>, TIM3, DMA1_CH5, 25u8, {});
dma_trait_impl!(crate::timer::Dma<Ch3>, TIM3, DMA1_CH6, 25u8, {});
dma_trait_impl!(crate::timer::Dma<Ch3>, TIM3, DMA1_CH7, 25u8, {});
dma_trait_impl!(crate::timer::Dma<Ch3>, TIM3, DMA2_CH0, 25u8, {});
dma_trait_impl!(crate::timer::Dma<Ch3>, TIM3, DMA2_CH1, 25u8, {});
dma_trait_impl!(crate::timer::Dma<Ch3>, TIM3, DMA2_CH2, 25u8, {});
dma_trait_impl!(crate::timer::Dma<Ch3>, TIM3, DMA2_CH3, 25u8, {});
dma_trait_impl!(crate::timer::Dma<Ch3>, TIM3, DMA2_CH4, 25u8, {});
dma_trait_impl!(crate::timer::Dma<Ch3>, TIM3, DMA2_CH5, 25u8, {});
dma_trait_impl!(crate::timer::Dma<Ch3>, TIM3, DMA2_CH6, 25u8, {});
dma_trait_impl!(crate::timer::Dma<Ch3>, TIM3, DMA2_CH7, 25u8, {});
dma_trait_impl!(crate::timer::Dma<Ch4>, TIM3, DMA1_CH0, 26u8, {});
dma_trait_impl!(crate::timer::Dma<Ch4>, TIM3, DMA1_CH1, 26u8, {});
dma_trait_impl!(crate::timer::Dma<Ch4>, TIM3, DMA1_CH2, 26u8, {});
dma_trait_impl!(crate::timer::Dma<Ch4>, TIM3, DMA1_CH3, 26u8, {});
dma_trait_impl!(crate::timer::Dma<Ch4>, TIM3, DMA1_CH4, 26u8, {});
dma_trait_impl!(crate::timer::Dma<Ch4>, TIM3, DMA1_CH5, 26u8, {});
dma_trait_impl!(crate::timer::Dma<Ch4>, TIM3, DMA1_CH6, 26u8, {});
dma_trait_impl!(crate::timer::Dma<Ch4>, TIM3, DMA1_CH7, 26u8, {});
dma_trait_impl!(crate::timer::Dma<Ch4>, TIM3, DMA2_CH0, 26u8, {});
dma_trait_impl!(crate::timer::Dma<Ch4>, TIM3, DMA2_CH1, 26u8, {});
dma_trait_impl!(crate::timer::Dma<Ch4>, TIM3, DMA2_CH2, 26u8, {});
dma_trait_impl!(crate::timer::Dma<Ch4>, TIM3, DMA2_CH3, 26u8, {});
dma_trait_impl!(crate::timer::Dma<Ch4>, TIM3, DMA2_CH4, 26u8, {});
dma_trait_impl!(crate::timer::Dma<Ch4>, TIM3, DMA2_CH5, 26u8, {});
dma_trait_impl!(crate::timer::Dma<Ch4>, TIM3, DMA2_CH6, 26u8, {});
dma_trait_impl!(crate::timer::Dma<Ch4>, TIM3, DMA2_CH7, 26u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM3, DMA1_CH0, 27u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM3, DMA1_CH1, 27u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM3, DMA1_CH2, 27u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM3, DMA1_CH3, 27u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM3, DMA1_CH4, 27u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM3, DMA1_CH5, 27u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM3, DMA1_CH6, 27u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM3, DMA1_CH7, 27u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM3, DMA2_CH0, 27u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM3, DMA2_CH1, 27u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM3, DMA2_CH2, 27u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM3, DMA2_CH3, 27u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM3, DMA2_CH4, 27u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM3, DMA2_CH5, 27u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM3, DMA2_CH6, 27u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM3, DMA2_CH7, 27u8, {});
dma_trait_impl!(crate::timer::Dma<Ch1>, TIM4, DMA1_CH0, 29u8, {});
dma_trait_impl!(crate::timer::Dma<Ch1>, TIM4, DMA1_CH1, 29u8, {});
dma_trait_impl!(crate::timer::Dma<Ch1>, TIM4, DMA1_CH2, 29u8, {});
dma_trait_impl!(crate::timer::Dma<Ch1>, TIM4, DMA1_CH3, 29u8, {});
dma_trait_impl!(crate::timer::Dma<Ch1>, TIM4, DMA1_CH4, 29u8, {});
dma_trait_impl!(crate::timer::Dma<Ch1>, TIM4, DMA1_CH5, 29u8, {});
dma_trait_impl!(crate::timer::Dma<Ch1>, TIM4, DMA1_CH6, 29u8, {});
dma_trait_impl!(crate::timer::Dma<Ch1>, TIM4, DMA1_CH7, 29u8, {});
dma_trait_impl!(crate::timer::Dma<Ch1>, TIM4, DMA2_CH0, 29u8, {});
dma_trait_impl!(crate::timer::Dma<Ch1>, TIM4, DMA2_CH1, 29u8, {});
dma_trait_impl!(crate::timer::Dma<Ch1>, TIM4, DMA2_CH2, 29u8, {});
dma_trait_impl!(crate::timer::Dma<Ch1>, TIM4, DMA2_CH3, 29u8, {});
dma_trait_impl!(crate::timer::Dma<Ch1>, TIM4, DMA2_CH4, 29u8, {});
dma_trait_impl!(crate::timer::Dma<Ch1>, TIM4, DMA2_CH5, 29u8, {});
dma_trait_impl!(crate::timer::Dma<Ch1>, TIM4, DMA2_CH6, 29u8, {});
dma_trait_impl!(crate::timer::Dma<Ch1>, TIM4, DMA2_CH7, 29u8, {});
dma_trait_impl!(crate::timer::Dma<Ch2>, TIM4, DMA1_CH0, 30u8, {});
dma_trait_impl!(crate::timer::Dma<Ch2>, TIM4, DMA1_CH1, 30u8, {});
dma_trait_impl!(crate::timer::Dma<Ch2>, TIM4, DMA1_CH2, 30u8, {});
dma_trait_impl!(crate::timer::Dma<Ch2>, TIM4, DMA1_CH3, 30u8, {});
dma_trait_impl!(crate::timer::Dma<Ch2>, TIM4, DMA1_CH4, 30u8, {});
dma_trait_impl!(crate::timer::Dma<Ch2>, TIM4, DMA1_CH5, 30u8, {});
dma_trait_impl!(crate::timer::Dma<Ch2>, TIM4, DMA1_CH6, 30u8, {});
dma_trait_impl!(crate::timer::Dma<Ch2>, TIM4, DMA1_CH7, 30u8, {});
dma_trait_impl!(crate::timer::Dma<Ch2>, TIM4, DMA2_CH0, 30u8, {});
dma_trait_impl!(crate::timer::Dma<Ch2>, TIM4, DMA2_CH1, 30u8, {});
dma_trait_impl!(crate::timer::Dma<Ch2>, TIM4, DMA2_CH2, 30u8, {});
dma_trait_impl!(crate::timer::Dma<Ch2>, TIM4, DMA2_CH3, 30u8, {});
dma_trait_impl!(crate::timer::Dma<Ch2>, TIM4, DMA2_CH4, 30u8, {});
dma_trait_impl!(crate::timer::Dma<Ch2>, TIM4, DMA2_CH5, 30u8, {});
dma_trait_impl!(crate::timer::Dma<Ch2>, TIM4, DMA2_CH6, 30u8, {});
dma_trait_impl!(crate::timer::Dma<Ch2>, TIM4, DMA2_CH7, 30u8, {});
dma_trait_impl!(crate::timer::Dma<Ch3>, TIM4, DMA1_CH0, 31u8, {});
dma_trait_impl!(crate::timer::Dma<Ch3>, TIM4, DMA1_CH1, 31u8, {});
dma_trait_impl!(crate::timer::Dma<Ch3>, TIM4, DMA1_CH2, 31u8, {});
dma_trait_impl!(crate::timer::Dma<Ch3>, TIM4, DMA1_CH3, 31u8, {});
dma_trait_impl!(crate::timer::Dma<Ch3>, TIM4, DMA1_CH4, 31u8, {});
dma_trait_impl!(crate::timer::Dma<Ch3>, TIM4, DMA1_CH5, 31u8, {});
dma_trait_impl!(crate::timer::Dma<Ch3>, TIM4, DMA1_CH6, 31u8, {});
dma_trait_impl!(crate::timer::Dma<Ch3>, TIM4, DMA1_CH7, 31u8, {});
dma_trait_impl!(crate::timer::Dma<Ch3>, TIM4, DMA2_CH0, 31u8, {});
dma_trait_impl!(crate::timer::Dma<Ch3>, TIM4, DMA2_CH1, 31u8, {});
dma_trait_impl!(crate::timer::Dma<Ch3>, TIM4, DMA2_CH2, 31u8, {});
dma_trait_impl!(crate::timer::Dma<Ch3>, TIM4, DMA2_CH3, 31u8, {});
dma_trait_impl!(crate::timer::Dma<Ch3>, TIM4, DMA2_CH4, 31u8, {});
dma_trait_impl!(crate::timer::Dma<Ch3>, TIM4, DMA2_CH5, 31u8, {});
dma_trait_impl!(crate::timer::Dma<Ch3>, TIM4, DMA2_CH6, 31u8, {});
dma_trait_impl!(crate::timer::Dma<Ch3>, TIM4, DMA2_CH7, 31u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM4, DMA1_CH0, 32u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM4, DMA1_CH1, 32u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM4, DMA1_CH2, 32u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM4, DMA1_CH3, 32u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM4, DMA1_CH4, 32u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM4, DMA1_CH5, 32u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM4, DMA1_CH6, 32u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM4, DMA1_CH7, 32u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM4, DMA2_CH0, 32u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM4, DMA2_CH1, 32u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM4, DMA2_CH2, 32u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM4, DMA2_CH3, 32u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM4, DMA2_CH4, 32u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM4, DMA2_CH5, 32u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM4, DMA2_CH6, 32u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM4, DMA2_CH7, 32u8, {});
dma_trait_impl!(crate::timer::Dma<Ch1>, TIM5, DMA1_CH0, 55u8, {});
dma_trait_impl!(crate::timer::Dma<Ch1>, TIM5, DMA1_CH1, 55u8, {});
dma_trait_impl!(crate::timer::Dma<Ch1>, TIM5, DMA1_CH2, 55u8, {});
dma_trait_impl!(crate::timer::Dma<Ch1>, TIM5, DMA1_CH3, 55u8, {});
dma_trait_impl!(crate::timer::Dma<Ch1>, TIM5, DMA1_CH4, 55u8, {});
dma_trait_impl!(crate::timer::Dma<Ch1>, TIM5, DMA1_CH5, 55u8, {});
dma_trait_impl!(crate::timer::Dma<Ch1>, TIM5, DMA1_CH6, 55u8, {});
dma_trait_impl!(crate::timer::Dma<Ch1>, TIM5, DMA1_CH7, 55u8, {});
dma_trait_impl!(crate::timer::Dma<Ch1>, TIM5, DMA2_CH0, 55u8, {});
dma_trait_impl!(crate::timer::Dma<Ch1>, TIM5, DMA2_CH1, 55u8, {});
dma_trait_impl!(crate::timer::Dma<Ch1>, TIM5, DMA2_CH2, 55u8, {});
dma_trait_impl!(crate::timer::Dma<Ch1>, TIM5, DMA2_CH3, 55u8, {});
dma_trait_impl!(crate::timer::Dma<Ch1>, TIM5, DMA2_CH4, 55u8, {});
dma_trait_impl!(crate::timer::Dma<Ch1>, TIM5, DMA2_CH5, 55u8, {});
dma_trait_impl!(crate::timer::Dma<Ch1>, TIM5, DMA2_CH6, 55u8, {});
dma_trait_impl!(crate::timer::Dma<Ch1>, TIM5, DMA2_CH7, 55u8, {});
dma_trait_impl!(crate::timer::Dma<Ch2>, TIM5, DMA1_CH0, 56u8, {});
dma_trait_impl!(crate::timer::Dma<Ch2>, TIM5, DMA1_CH1, 56u8, {});
dma_trait_impl!(crate::timer::Dma<Ch2>, TIM5, DMA1_CH2, 56u8, {});
dma_trait_impl!(crate::timer::Dma<Ch2>, TIM5, DMA1_CH3, 56u8, {});
dma_trait_impl!(crate::timer::Dma<Ch2>, TIM5, DMA1_CH4, 56u8, {});
dma_trait_impl!(crate::timer::Dma<Ch2>, TIM5, DMA1_CH5, 56u8, {});
dma_trait_impl!(crate::timer::Dma<Ch2>, TIM5, DMA1_CH6, 56u8, {});
dma_trait_impl!(crate::timer::Dma<Ch2>, TIM5, DMA1_CH7, 56u8, {});
dma_trait_impl!(crate::timer::Dma<Ch2>, TIM5, DMA2_CH0, 56u8, {});
dma_trait_impl!(crate::timer::Dma<Ch2>, TIM5, DMA2_CH1, 56u8, {});
dma_trait_impl!(crate::timer::Dma<Ch2>, TIM5, DMA2_CH2, 56u8, {});
dma_trait_impl!(crate::timer::Dma<Ch2>, TIM5, DMA2_CH3, 56u8, {});
dma_trait_impl!(crate::timer::Dma<Ch2>, TIM5, DMA2_CH4, 56u8, {});
dma_trait_impl!(crate::timer::Dma<Ch2>, TIM5, DMA2_CH5, 56u8, {});
dma_trait_impl!(crate::timer::Dma<Ch2>, TIM5, DMA2_CH6, 56u8, {});
dma_trait_impl!(crate::timer::Dma<Ch2>, TIM5, DMA2_CH7, 56u8, {});
dma_trait_impl!(crate::timer::Dma<Ch3>, TIM5, DMA1_CH0, 57u8, {});
dma_trait_impl!(crate::timer::Dma<Ch3>, TIM5, DMA1_CH1, 57u8, {});
dma_trait_impl!(crate::timer::Dma<Ch3>, TIM5, DMA1_CH2, 57u8, {});
dma_trait_impl!(crate::timer::Dma<Ch3>, TIM5, DMA1_CH3, 57u8, {});
dma_trait_impl!(crate::timer::Dma<Ch3>, TIM5, DMA1_CH4, 57u8, {});
dma_trait_impl!(crate::timer::Dma<Ch3>, TIM5, DMA1_CH5, 57u8, {});
dma_trait_impl!(crate::timer::Dma<Ch3>, TIM5, DMA1_CH6, 57u8, {});
dma_trait_impl!(crate::timer::Dma<Ch3>, TIM5, DMA1_CH7, 57u8, {});
dma_trait_impl!(crate::timer::Dma<Ch3>, TIM5, DMA2_CH0, 57u8, {});
dma_trait_impl!(crate::timer::Dma<Ch3>, TIM5, DMA2_CH1, 57u8, {});
dma_trait_impl!(crate::timer::Dma<Ch3>, TIM5, DMA2_CH2, 57u8, {});
dma_trait_impl!(crate::timer::Dma<Ch3>, TIM5, DMA2_CH3, 57u8, {});
dma_trait_impl!(crate::timer::Dma<Ch3>, TIM5, DMA2_CH4, 57u8, {});
dma_trait_impl!(crate::timer::Dma<Ch3>, TIM5, DMA2_CH5, 57u8, {});
dma_trait_impl!(crate::timer::Dma<Ch3>, TIM5, DMA2_CH6, 57u8, {});
dma_trait_impl!(crate::timer::Dma<Ch3>, TIM5, DMA2_CH7, 57u8, {});
dma_trait_impl!(crate::timer::Dma<Ch4>, TIM5, DMA1_CH0, 58u8, {});
dma_trait_impl!(crate::timer::Dma<Ch4>, TIM5, DMA1_CH1, 58u8, {});
dma_trait_impl!(crate::timer::Dma<Ch4>, TIM5, DMA1_CH2, 58u8, {});
dma_trait_impl!(crate::timer::Dma<Ch4>, TIM5, DMA1_CH3, 58u8, {});
dma_trait_impl!(crate::timer::Dma<Ch4>, TIM5, DMA1_CH4, 58u8, {});
dma_trait_impl!(crate::timer::Dma<Ch4>, TIM5, DMA1_CH5, 58u8, {});
dma_trait_impl!(crate::timer::Dma<Ch4>, TIM5, DMA1_CH6, 58u8, {});
dma_trait_impl!(crate::timer::Dma<Ch4>, TIM5, DMA1_CH7, 58u8, {});
dma_trait_impl!(crate::timer::Dma<Ch4>, TIM5, DMA2_CH0, 58u8, {});
dma_trait_impl!(crate::timer::Dma<Ch4>, TIM5, DMA2_CH1, 58u8, {});
dma_trait_impl!(crate::timer::Dma<Ch4>, TIM5, DMA2_CH2, 58u8, {});
dma_trait_impl!(crate::timer::Dma<Ch4>, TIM5, DMA2_CH3, 58u8, {});
dma_trait_impl!(crate::timer::Dma<Ch4>, TIM5, DMA2_CH4, 58u8, {});
dma_trait_impl!(crate::timer::Dma<Ch4>, TIM5, DMA2_CH5, 58u8, {});
dma_trait_impl!(crate::timer::Dma<Ch4>, TIM5, DMA2_CH6, 58u8, {});
dma_trait_impl!(crate::timer::Dma<Ch4>, TIM5, DMA2_CH7, 58u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM5, DMA1_CH0, 59u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM5, DMA1_CH1, 59u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM5, DMA1_CH2, 59u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM5, DMA1_CH3, 59u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM5, DMA1_CH4, 59u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM5, DMA1_CH5, 59u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM5, DMA1_CH6, 59u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM5, DMA1_CH7, 59u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM5, DMA2_CH0, 59u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM5, DMA2_CH1, 59u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM5, DMA2_CH2, 59u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM5, DMA2_CH3, 59u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM5, DMA2_CH4, 59u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM5, DMA2_CH5, 59u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM5, DMA2_CH6, 59u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM5, DMA2_CH7, 59u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM6, DMA1_CH0, 69u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM6, DMA1_CH1, 69u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM6, DMA1_CH2, 69u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM6, DMA1_CH3, 69u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM6, DMA1_CH4, 69u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM6, DMA1_CH5, 69u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM6, DMA1_CH6, 69u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM6, DMA1_CH7, 69u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM6, DMA2_CH0, 69u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM6, DMA2_CH1, 69u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM6, DMA2_CH2, 69u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM6, DMA2_CH3, 69u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM6, DMA2_CH4, 69u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM6, DMA2_CH5, 69u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM6, DMA2_CH6, 69u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM6, DMA2_CH7, 69u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM7, DMA1_CH0, 70u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM7, DMA1_CH1, 70u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM7, DMA1_CH2, 70u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM7, DMA1_CH3, 70u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM7, DMA1_CH4, 70u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM7, DMA1_CH5, 70u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM7, DMA1_CH6, 70u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM7, DMA1_CH7, 70u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM7, DMA2_CH0, 70u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM7, DMA2_CH1, 70u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM7, DMA2_CH2, 70u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM7, DMA2_CH3, 70u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM7, DMA2_CH4, 70u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM7, DMA2_CH5, 70u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM7, DMA2_CH6, 70u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM7, DMA2_CH7, 70u8, {});
dma_trait_impl!(crate::timer::Dma<Ch1>, TIM8, DMA1_CH0, 47u8, {});
dma_trait_impl!(crate::timer::Dma<Ch1>, TIM8, DMA1_CH1, 47u8, {});
dma_trait_impl!(crate::timer::Dma<Ch1>, TIM8, DMA1_CH2, 47u8, {});
dma_trait_impl!(crate::timer::Dma<Ch1>, TIM8, DMA1_CH3, 47u8, {});
dma_trait_impl!(crate::timer::Dma<Ch1>, TIM8, DMA1_CH4, 47u8, {});
dma_trait_impl!(crate::timer::Dma<Ch1>, TIM8, DMA1_CH5, 47u8, {});
dma_trait_impl!(crate::timer::Dma<Ch1>, TIM8, DMA1_CH6, 47u8, {});
dma_trait_impl!(crate::timer::Dma<Ch1>, TIM8, DMA1_CH7, 47u8, {});
dma_trait_impl!(crate::timer::Dma<Ch1>, TIM8, DMA2_CH0, 47u8, {});
dma_trait_impl!(crate::timer::Dma<Ch1>, TIM8, DMA2_CH1, 47u8, {});
dma_trait_impl!(crate::timer::Dma<Ch1>, TIM8, DMA2_CH2, 47u8, {});
dma_trait_impl!(crate::timer::Dma<Ch1>, TIM8, DMA2_CH3, 47u8, {});
dma_trait_impl!(crate::timer::Dma<Ch1>, TIM8, DMA2_CH4, 47u8, {});
dma_trait_impl!(crate::timer::Dma<Ch1>, TIM8, DMA2_CH5, 47u8, {});
dma_trait_impl!(crate::timer::Dma<Ch1>, TIM8, DMA2_CH6, 47u8, {});
dma_trait_impl!(crate::timer::Dma<Ch1>, TIM8, DMA2_CH7, 47u8, {});
dma_trait_impl!(crate::timer::Dma<Ch2>, TIM8, DMA1_CH0, 48u8, {});
dma_trait_impl!(crate::timer::Dma<Ch2>, TIM8, DMA1_CH1, 48u8, {});
dma_trait_impl!(crate::timer::Dma<Ch2>, TIM8, DMA1_CH2, 48u8, {});
dma_trait_impl!(crate::timer::Dma<Ch2>, TIM8, DMA1_CH3, 48u8, {});
dma_trait_impl!(crate::timer::Dma<Ch2>, TIM8, DMA1_CH4, 48u8, {});
dma_trait_impl!(crate::timer::Dma<Ch2>, TIM8, DMA1_CH5, 48u8, {});
dma_trait_impl!(crate::timer::Dma<Ch2>, TIM8, DMA1_CH6, 48u8, {});
dma_trait_impl!(crate::timer::Dma<Ch2>, TIM8, DMA1_CH7, 48u8, {});
dma_trait_impl!(crate::timer::Dma<Ch2>, TIM8, DMA2_CH0, 48u8, {});
dma_trait_impl!(crate::timer::Dma<Ch2>, TIM8, DMA2_CH1, 48u8, {});
dma_trait_impl!(crate::timer::Dma<Ch2>, TIM8, DMA2_CH2, 48u8, {});
dma_trait_impl!(crate::timer::Dma<Ch2>, TIM8, DMA2_CH3, 48u8, {});
dma_trait_impl!(crate::timer::Dma<Ch2>, TIM8, DMA2_CH4, 48u8, {});
dma_trait_impl!(crate::timer::Dma<Ch2>, TIM8, DMA2_CH5, 48u8, {});
dma_trait_impl!(crate::timer::Dma<Ch2>, TIM8, DMA2_CH6, 48u8, {});
dma_trait_impl!(crate::timer::Dma<Ch2>, TIM8, DMA2_CH7, 48u8, {});
dma_trait_impl!(crate::timer::Dma<Ch3>, TIM8, DMA1_CH0, 49u8, {});
dma_trait_impl!(crate::timer::Dma<Ch3>, TIM8, DMA1_CH1, 49u8, {});
dma_trait_impl!(crate::timer::Dma<Ch3>, TIM8, DMA1_CH2, 49u8, {});
dma_trait_impl!(crate::timer::Dma<Ch3>, TIM8, DMA1_CH3, 49u8, {});
dma_trait_impl!(crate::timer::Dma<Ch3>, TIM8, DMA1_CH4, 49u8, {});
dma_trait_impl!(crate::timer::Dma<Ch3>, TIM8, DMA1_CH5, 49u8, {});
dma_trait_impl!(crate::timer::Dma<Ch3>, TIM8, DMA1_CH6, 49u8, {});
dma_trait_impl!(crate::timer::Dma<Ch3>, TIM8, DMA1_CH7, 49u8, {});
dma_trait_impl!(crate::timer::Dma<Ch3>, TIM8, DMA2_CH0, 49u8, {});
dma_trait_impl!(crate::timer::Dma<Ch3>, TIM8, DMA2_CH1, 49u8, {});
dma_trait_impl!(crate::timer::Dma<Ch3>, TIM8, DMA2_CH2, 49u8, {});
dma_trait_impl!(crate::timer::Dma<Ch3>, TIM8, DMA2_CH3, 49u8, {});
dma_trait_impl!(crate::timer::Dma<Ch3>, TIM8, DMA2_CH4, 49u8, {});
dma_trait_impl!(crate::timer::Dma<Ch3>, TIM8, DMA2_CH5, 49u8, {});
dma_trait_impl!(crate::timer::Dma<Ch3>, TIM8, DMA2_CH6, 49u8, {});
dma_trait_impl!(crate::timer::Dma<Ch3>, TIM8, DMA2_CH7, 49u8, {});
dma_trait_impl!(crate::timer::Dma<Ch4>, TIM8, DMA1_CH0, 50u8, {});
dma_trait_impl!(crate::timer::Dma<Ch4>, TIM8, DMA1_CH1, 50u8, {});
dma_trait_impl!(crate::timer::Dma<Ch4>, TIM8, DMA1_CH2, 50u8, {});
dma_trait_impl!(crate::timer::Dma<Ch4>, TIM8, DMA1_CH3, 50u8, {});
dma_trait_impl!(crate::timer::Dma<Ch4>, TIM8, DMA1_CH4, 50u8, {});
dma_trait_impl!(crate::timer::Dma<Ch4>, TIM8, DMA1_CH5, 50u8, {});
dma_trait_impl!(crate::timer::Dma<Ch4>, TIM8, DMA1_CH6, 50u8, {});
dma_trait_impl!(crate::timer::Dma<Ch4>, TIM8, DMA1_CH7, 50u8, {});
dma_trait_impl!(crate::timer::Dma<Ch4>, TIM8, DMA2_CH0, 50u8, {});
dma_trait_impl!(crate::timer::Dma<Ch4>, TIM8, DMA2_CH1, 50u8, {});
dma_trait_impl!(crate::timer::Dma<Ch4>, TIM8, DMA2_CH2, 50u8, {});
dma_trait_impl!(crate::timer::Dma<Ch4>, TIM8, DMA2_CH3, 50u8, {});
dma_trait_impl!(crate::timer::Dma<Ch4>, TIM8, DMA2_CH4, 50u8, {});
dma_trait_impl!(crate::timer::Dma<Ch4>, TIM8, DMA2_CH5, 50u8, {});
dma_trait_impl!(crate::timer::Dma<Ch4>, TIM8, DMA2_CH6, 50u8, {});
dma_trait_impl!(crate::timer::Dma<Ch4>, TIM8, DMA2_CH7, 50u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM8, DMA1_CH0, 51u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM8, DMA1_CH1, 51u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM8, DMA1_CH2, 51u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM8, DMA1_CH3, 51u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM8, DMA1_CH4, 51u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM8, DMA1_CH5, 51u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM8, DMA1_CH6, 51u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM8, DMA1_CH7, 51u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM8, DMA2_CH0, 51u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM8, DMA2_CH1, 51u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM8, DMA2_CH2, 51u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM8, DMA2_CH3, 51u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM8, DMA2_CH4, 51u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM8, DMA2_CH5, 51u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM8, DMA2_CH6, 51u8, {});
dma_trait_impl!(crate::timer::UpDma, TIM8, DMA2_CH7, 51u8, {});
dma_trait_impl!(crate::usart::RxDma, UART4, DMA1_CH0, 63u8, {});
dma_trait_impl!(crate::usart::RxDma, UART4, DMA1_CH1, 63u8, {});
dma_trait_impl!(crate::usart::RxDma, UART4, DMA1_CH2, 63u8, {});
dma_trait_impl!(crate::usart::RxDma, UART4, DMA1_CH3, 63u8, {});
dma_trait_impl!(crate::usart::RxDma, UART4, DMA1_CH4, 63u8, {});
dma_trait_impl!(crate::usart::RxDma, UART4, DMA1_CH5, 63u8, {});
dma_trait_impl!(crate::usart::RxDma, UART4, DMA1_CH6, 63u8, {});
dma_trait_impl!(crate::usart::RxDma, UART4, DMA1_CH7, 63u8, {});
dma_trait_impl!(crate::usart::RxDma, UART4, DMA2_CH0, 63u8, {});
dma_trait_impl!(crate::usart::RxDma, UART4, DMA2_CH1, 63u8, {});
dma_trait_impl!(crate::usart::RxDma, UART4, DMA2_CH2, 63u8, {});
dma_trait_impl!(crate::usart::RxDma, UART4, DMA2_CH3, 63u8, {});
dma_trait_impl!(crate::usart::RxDma, UART4, DMA2_CH4, 63u8, {});
dma_trait_impl!(crate::usart::RxDma, UART4, DMA2_CH5, 63u8, {});
dma_trait_impl!(crate::usart::RxDma, UART4, DMA2_CH6, 63u8, {});
dma_trait_impl!(crate::usart::RxDma, UART4, DMA2_CH7, 63u8, {});
dma_trait_impl!(crate::usart::TxDma, UART4, DMA1_CH0, 64u8, {});
dma_trait_impl!(crate::usart::TxDma, UART4, DMA1_CH1, 64u8, {});
dma_trait_impl!(crate::usart::TxDma, UART4, DMA1_CH2, 64u8, {});
dma_trait_impl!(crate::usart::TxDma, UART4, DMA1_CH3, 64u8, {});
dma_trait_impl!(crate::usart::TxDma, UART4, DMA1_CH4, 64u8, {});
dma_trait_impl!(crate::usart::TxDma, UART4, DMA1_CH5, 64u8, {});
dma_trait_impl!(crate::usart::TxDma, UART4, DMA1_CH6, 64u8, {});
dma_trait_impl!(crate::usart::TxDma, UART4, DMA1_CH7, 64u8, {});
dma_trait_impl!(crate::usart::TxDma, UART4, DMA2_CH0, 64u8, {});
dma_trait_impl!(crate::usart::TxDma, UART4, DMA2_CH1, 64u8, {});
dma_trait_impl!(crate::usart::TxDma, UART4, DMA2_CH2, 64u8, {});
dma_trait_impl!(crate::usart::TxDma, UART4, DMA2_CH3, 64u8, {});
dma_trait_impl!(crate::usart::TxDma, UART4, DMA2_CH4, 64u8, {});
dma_trait_impl!(crate::usart::TxDma, UART4, DMA2_CH5, 64u8, {});
dma_trait_impl!(crate::usart::TxDma, UART4, DMA2_CH6, 64u8, {});
dma_trait_impl!(crate::usart::TxDma, UART4, DMA2_CH7, 64u8, {});
dma_trait_impl!(crate::usart::RxDma, UART5, DMA1_CH0, 65u8, {});
dma_trait_impl!(crate::usart::RxDma, UART5, DMA1_CH1, 65u8, {});
dma_trait_impl!(crate::usart::RxDma, UART5, DMA1_CH2, 65u8, {});
dma_trait_impl!(crate::usart::RxDma, UART5, DMA1_CH3, 65u8, {});
dma_trait_impl!(crate::usart::RxDma, UART5, DMA1_CH4, 65u8, {});
dma_trait_impl!(crate::usart::RxDma, UART5, DMA1_CH5, 65u8, {});
dma_trait_impl!(crate::usart::RxDma, UART5, DMA1_CH6, 65u8, {});
dma_trait_impl!(crate::usart::RxDma, UART5, DMA1_CH7, 65u8, {});
dma_trait_impl!(crate::usart::RxDma, UART5, DMA2_CH0, 65u8, {});
dma_trait_impl!(crate::usart::RxDma, UART5, DMA2_CH1, 65u8, {});
dma_trait_impl!(crate::usart::RxDma, UART5, DMA2_CH2, 65u8, {});
dma_trait_impl!(crate::usart::RxDma, UART5, DMA2_CH3, 65u8, {});
dma_trait_impl!(crate::usart::RxDma, UART5, DMA2_CH4, 65u8, {});
dma_trait_impl!(crate::usart::RxDma, UART5, DMA2_CH5, 65u8, {});
dma_trait_impl!(crate::usart::RxDma, UART5, DMA2_CH6, 65u8, {});
dma_trait_impl!(crate::usart::RxDma, UART5, DMA2_CH7, 65u8, {});
dma_trait_impl!(crate::usart::TxDma, UART5, DMA1_CH0, 66u8, {});
dma_trait_impl!(crate::usart::TxDma, UART5, DMA1_CH1, 66u8, {});
dma_trait_impl!(crate::usart::TxDma, UART5, DMA1_CH2, 66u8, {});
dma_trait_impl!(crate::usart::TxDma, UART5, DMA1_CH3, 66u8, {});
dma_trait_impl!(crate::usart::TxDma, UART5, DMA1_CH4, 66u8, {});
dma_trait_impl!(crate::usart::TxDma, UART5, DMA1_CH5, 66u8, {});
dma_trait_impl!(crate::usart::TxDma, UART5, DMA1_CH6, 66u8, {});
dma_trait_impl!(crate::usart::TxDma, UART5, DMA1_CH7, 66u8, {});
dma_trait_impl!(crate::usart::TxDma, UART5, DMA2_CH0, 66u8, {});
dma_trait_impl!(crate::usart::TxDma, UART5, DMA2_CH1, 66u8, {});
dma_trait_impl!(crate::usart::TxDma, UART5, DMA2_CH2, 66u8, {});
dma_trait_impl!(crate::usart::TxDma, UART5, DMA2_CH3, 66u8, {});
dma_trait_impl!(crate::usart::TxDma, UART5, DMA2_CH4, 66u8, {});
dma_trait_impl!(crate::usart::TxDma, UART5, DMA2_CH5, 66u8, {});
dma_trait_impl!(crate::usart::TxDma, UART5, DMA2_CH6, 66u8, {});
dma_trait_impl!(crate::usart::TxDma, UART5, DMA2_CH7, 66u8, {});
dma_trait_impl!(crate::usart::RxDma, UART7, DMA1_CH0, 79u8, {});
dma_trait_impl!(crate::usart::RxDma, UART7, DMA1_CH1, 79u8, {});
dma_trait_impl!(crate::usart::RxDma, UART7, DMA1_CH2, 79u8, {});
dma_trait_impl!(crate::usart::RxDma, UART7, DMA1_CH3, 79u8, {});
dma_trait_impl!(crate::usart::RxDma, UART7, DMA1_CH4, 79u8, {});
dma_trait_impl!(crate::usart::RxDma, UART7, DMA1_CH5, 79u8, {});
dma_trait_impl!(crate::usart::RxDma, UART7, DMA1_CH6, 79u8, {});
dma_trait_impl!(crate::usart::RxDma, UART7, DMA1_CH7, 79u8, {});
dma_trait_impl!(crate::usart::RxDma, UART7, DMA2_CH0, 79u8, {});
dma_trait_impl!(crate::usart::RxDma, UART7, DMA2_CH1, 79u8, {});
dma_trait_impl!(crate::usart::RxDma, UART7, DMA2_CH2, 79u8, {});
dma_trait_impl!(crate::usart::RxDma, UART7, DMA2_CH3, 79u8, {});
dma_trait_impl!(crate::usart::RxDma, UART7, DMA2_CH4, 79u8, {});
dma_trait_impl!(crate::usart::RxDma, UART7, DMA2_CH5, 79u8, {});
dma_trait_impl!(crate::usart::RxDma, UART7, DMA2_CH6, 79u8, {});
dma_trait_impl!(crate::usart::RxDma, UART7, DMA2_CH7, 79u8, {});
dma_trait_impl!(crate::usart::TxDma, UART7, DMA1_CH0, 80u8, {});
dma_trait_impl!(crate::usart::TxDma, UART7, DMA1_CH1, 80u8, {});
dma_trait_impl!(crate::usart::TxDma, UART7, DMA1_CH2, 80u8, {});
dma_trait_impl!(crate::usart::TxDma, UART7, DMA1_CH3, 80u8, {});
dma_trait_impl!(crate::usart::TxDma, UART7, DMA1_CH4, 80u8, {});
dma_trait_impl!(crate::usart::TxDma, UART7, DMA1_CH5, 80u8, {});
dma_trait_impl!(crate::usart::TxDma, UART7, DMA1_CH6, 80u8, {});
dma_trait_impl!(crate::usart::TxDma, UART7, DMA1_CH7, 80u8, {});
dma_trait_impl!(crate::usart::TxDma, UART7, DMA2_CH0, 80u8, {});
dma_trait_impl!(crate::usart::TxDma, UART7, DMA2_CH1, 80u8, {});
dma_trait_impl!(crate::usart::TxDma, UART7, DMA2_CH2, 80u8, {});
dma_trait_impl!(crate::usart::TxDma, UART7, DMA2_CH3, 80u8, {});
dma_trait_impl!(crate::usart::TxDma, UART7, DMA2_CH4, 80u8, {});
dma_trait_impl!(crate::usart::TxDma, UART7, DMA2_CH5, 80u8, {});
dma_trait_impl!(crate::usart::TxDma, UART7, DMA2_CH6, 80u8, {});
dma_trait_impl!(crate::usart::TxDma, UART7, DMA2_CH7, 80u8, {});
dma_trait_impl!(crate::usart::RxDma, UART8, DMA1_CH0, 81u8, {});
dma_trait_impl!(crate::usart::RxDma, UART8, DMA1_CH1, 81u8, {});
dma_trait_impl!(crate::usart::RxDma, UART8, DMA1_CH2, 81u8, {});
dma_trait_impl!(crate::usart::RxDma, UART8, DMA1_CH3, 81u8, {});
dma_trait_impl!(crate::usart::RxDma, UART8, DMA1_CH4, 81u8, {});
dma_trait_impl!(crate::usart::RxDma, UART8, DMA1_CH5, 81u8, {});
dma_trait_impl!(crate::usart::RxDma, UART8, DMA1_CH6, 81u8, {});
dma_trait_impl!(crate::usart::RxDma, UART8, DMA1_CH7, 81u8, {});
dma_trait_impl!(crate::usart::RxDma, UART8, DMA2_CH0, 81u8, {});
dma_trait_impl!(crate::usart::RxDma, UART8, DMA2_CH1, 81u8, {});
dma_trait_impl!(crate::usart::RxDma, UART8, DMA2_CH2, 81u8, {});
dma_trait_impl!(crate::usart::RxDma, UART8, DMA2_CH3, 81u8, {});
dma_trait_impl!(crate::usart::RxDma, UART8, DMA2_CH4, 81u8, {});
dma_trait_impl!(crate::usart::RxDma, UART8, DMA2_CH5, 81u8, {});
dma_trait_impl!(crate::usart::RxDma, UART8, DMA2_CH6, 81u8, {});
dma_trait_impl!(crate::usart::RxDma, UART8, DMA2_CH7, 81u8, {});
dma_trait_impl!(crate::usart::TxDma, UART8, DMA1_CH0, 82u8, {});
dma_trait_impl!(crate::usart::TxDma, UART8, DMA1_CH1, 82u8, {});
dma_trait_impl!(crate::usart::TxDma, UART8, DMA1_CH2, 82u8, {});
dma_trait_impl!(crate::usart::TxDma, UART8, DMA1_CH3, 82u8, {});
dma_trait_impl!(crate::usart::TxDma, UART8, DMA1_CH4, 82u8, {});
dma_trait_impl!(crate::usart::TxDma, UART8, DMA1_CH5, 82u8, {});
dma_trait_impl!(crate::usart::TxDma, UART8, DMA1_CH6, 82u8, {});
dma_trait_impl!(crate::usart::TxDma, UART8, DMA1_CH7, 82u8, {});
dma_trait_impl!(crate::usart::TxDma, UART8, DMA2_CH0, 82u8, {});
dma_trait_impl!(crate::usart::TxDma, UART8, DMA2_CH1, 82u8, {});
dma_trait_impl!(crate::usart::TxDma, UART8, DMA2_CH2, 82u8, {});
dma_trait_impl!(crate::usart::TxDma, UART8, DMA2_CH3, 82u8, {});
dma_trait_impl!(crate::usart::TxDma, UART8, DMA2_CH4, 82u8, {});
dma_trait_impl!(crate::usart::TxDma, UART8, DMA2_CH5, 82u8, {});
dma_trait_impl!(crate::usart::TxDma, UART8, DMA2_CH6, 82u8, {});
dma_trait_impl!(crate::usart::TxDma, UART8, DMA2_CH7, 82u8, {});
dma_trait_impl!(crate::usart::RxDma, UART9, DMA1_CH0, 116u8, {});
dma_trait_impl!(crate::usart::RxDma, UART9, DMA1_CH1, 116u8, {});
dma_trait_impl!(crate::usart::RxDma, UART9, DMA1_CH2, 116u8, {});
dma_trait_impl!(crate::usart::RxDma, UART9, DMA1_CH3, 116u8, {});
dma_trait_impl!(crate::usart::RxDma, UART9, DMA1_CH4, 116u8, {});
dma_trait_impl!(crate::usart::RxDma, UART9, DMA1_CH5, 116u8, {});
dma_trait_impl!(crate::usart::RxDma, UART9, DMA1_CH6, 116u8, {});
dma_trait_impl!(crate::usart::RxDma, UART9, DMA1_CH7, 116u8, {});
dma_trait_impl!(crate::usart::RxDma, UART9, DMA2_CH0, 116u8, {});
dma_trait_impl!(crate::usart::RxDma, UART9, DMA2_CH1, 116u8, {});
dma_trait_impl!(crate::usart::RxDma, UART9, DMA2_CH2, 116u8, {});
dma_trait_impl!(crate::usart::RxDma, UART9, DMA2_CH3, 116u8, {});
dma_trait_impl!(crate::usart::RxDma, UART9, DMA2_CH4, 116u8, {});
dma_trait_impl!(crate::usart::RxDma, UART9, DMA2_CH5, 116u8, {});
dma_trait_impl!(crate::usart::RxDma, UART9, DMA2_CH6, 116u8, {});
dma_trait_impl!(crate::usart::RxDma, UART9, DMA2_CH7, 116u8, {});
dma_trait_impl!(crate::usart::TxDma, UART9, DMA1_CH0, 117u8, {});
dma_trait_impl!(crate::usart::TxDma, UART9, DMA1_CH1, 117u8, {});
dma_trait_impl!(crate::usart::TxDma, UART9, DMA1_CH2, 117u8, {});
dma_trait_impl!(crate::usart::TxDma, UART9, DMA1_CH3, 117u8, {});
dma_trait_impl!(crate::usart::TxDma, UART9, DMA1_CH4, 117u8, {});
dma_trait_impl!(crate::usart::TxDma, UART9, DMA1_CH5, 117u8, {});
dma_trait_impl!(crate::usart::TxDma, UART9, DMA1_CH6, 117u8, {});
dma_trait_impl!(crate::usart::TxDma, UART9, DMA1_CH7, 117u8, {});
dma_trait_impl!(crate::usart::TxDma, UART9, DMA2_CH0, 117u8, {});
dma_trait_impl!(crate::usart::TxDma, UART9, DMA2_CH1, 117u8, {});
dma_trait_impl!(crate::usart::TxDma, UART9, DMA2_CH2, 117u8, {});
dma_trait_impl!(crate::usart::TxDma, UART9, DMA2_CH3, 117u8, {});
dma_trait_impl!(crate::usart::TxDma, UART9, DMA2_CH4, 117u8, {});
dma_trait_impl!(crate::usart::TxDma, UART9, DMA2_CH5, 117u8, {});
dma_trait_impl!(crate::usart::TxDma, UART9, DMA2_CH6, 117u8, {});
dma_trait_impl!(crate::usart::TxDma, UART9, DMA2_CH7, 117u8, {});
dma_trait_impl!(crate::usart::RxDma, USART1, DMA1_CH0, 41u8, {});
dma_trait_impl!(crate::usart::RxDma, USART1, DMA1_CH1, 41u8, {});
dma_trait_impl!(crate::usart::RxDma, USART1, DMA1_CH2, 41u8, {});
dma_trait_impl!(crate::usart::RxDma, USART1, DMA1_CH3, 41u8, {});
dma_trait_impl!(crate::usart::RxDma, USART1, DMA1_CH4, 41u8, {});
dma_trait_impl!(crate::usart::RxDma, USART1, DMA1_CH5, 41u8, {});
dma_trait_impl!(crate::usart::RxDma, USART1, DMA1_CH6, 41u8, {});
dma_trait_impl!(crate::usart::RxDma, USART1, DMA1_CH7, 41u8, {});
dma_trait_impl!(crate::usart::RxDma, USART1, DMA2_CH0, 41u8, {});
dma_trait_impl!(crate::usart::RxDma, USART1, DMA2_CH1, 41u8, {});
dma_trait_impl!(crate::usart::RxDma, USART1, DMA2_CH2, 41u8, {});
dma_trait_impl!(crate::usart::RxDma, USART1, DMA2_CH3, 41u8, {});
dma_trait_impl!(crate::usart::RxDma, USART1, DMA2_CH4, 41u8, {});
dma_trait_impl!(crate::usart::RxDma, USART1, DMA2_CH5, 41u8, {});
dma_trait_impl!(crate::usart::RxDma, USART1, DMA2_CH6, 41u8, {});
dma_trait_impl!(crate::usart::RxDma, USART1, DMA2_CH7, 41u8, {});
dma_trait_impl!(crate::usart::TxDma, USART1, DMA1_CH0, 42u8, {});
dma_trait_impl!(crate::usart::TxDma, USART1, DMA1_CH1, 42u8, {});
dma_trait_impl!(crate::usart::TxDma, USART1, DMA1_CH2, 42u8, {});
dma_trait_impl!(crate::usart::TxDma, USART1, DMA1_CH3, 42u8, {});
dma_trait_impl!(crate::usart::TxDma, USART1, DMA1_CH4, 42u8, {});
dma_trait_impl!(crate::usart::TxDma, USART1, DMA1_CH5, 42u8, {});
dma_trait_impl!(crate::usart::TxDma, USART1, DMA1_CH6, 42u8, {});
dma_trait_impl!(crate::usart::TxDma, USART1, DMA1_CH7, 42u8, {});
dma_trait_impl!(crate::usart::TxDma, USART1, DMA2_CH0, 42u8, {});
dma_trait_impl!(crate::usart::TxDma, USART1, DMA2_CH1, 42u8, {});
dma_trait_impl!(crate::usart::TxDma, USART1, DMA2_CH2, 42u8, {});
dma_trait_impl!(crate::usart::TxDma, USART1, DMA2_CH3, 42u8, {});
dma_trait_impl!(crate::usart::TxDma, USART1, DMA2_CH4, 42u8, {});
dma_trait_impl!(crate::usart::TxDma, USART1, DMA2_CH5, 42u8, {});
dma_trait_impl!(crate::usart::TxDma, USART1, DMA2_CH6, 42u8, {});
dma_trait_impl!(crate::usart::TxDma, USART1, DMA2_CH7, 42u8, {});
dma_trait_impl!(crate::usart::RxDma, USART10, DMA1_CH0, 118u8, {});
dma_trait_impl!(crate::usart::RxDma, USART10, DMA1_CH1, 118u8, {});
dma_trait_impl!(crate::usart::RxDma, USART10, DMA1_CH2, 118u8, {});
dma_trait_impl!(crate::usart::RxDma, USART10, DMA1_CH3, 118u8, {});
dma_trait_impl!(crate::usart::RxDma, USART10, DMA1_CH4, 118u8, {});
dma_trait_impl!(crate::usart::RxDma, USART10, DMA1_CH5, 118u8, {});
dma_trait_impl!(crate::usart::RxDma, USART10, DMA1_CH6, 118u8, {});
dma_trait_impl!(crate::usart::RxDma, USART10, DMA1_CH7, 118u8, {});
dma_trait_impl!(crate::usart::RxDma, USART10, DMA2_CH0, 118u8, {});
dma_trait_impl!(crate::usart::RxDma, USART10, DMA2_CH1, 118u8, {});
dma_trait_impl!(crate::usart::RxDma, USART10, DMA2_CH2, 118u8, {});
dma_trait_impl!(crate::usart::RxDma, USART10, DMA2_CH3, 118u8, {});
dma_trait_impl!(crate::usart::RxDma, USART10, DMA2_CH4, 118u8, {});
dma_trait_impl!(crate::usart::RxDma, USART10, DMA2_CH5, 118u8, {});
dma_trait_impl!(crate::usart::RxDma, USART10, DMA2_CH6, 118u8, {});
dma_trait_impl!(crate::usart::RxDma, USART10, DMA2_CH7, 118u8, {});
dma_trait_impl!(crate::usart::TxDma, USART10, DMA1_CH0, 119u8, {});
dma_trait_impl!(crate::usart::TxDma, USART10, DMA1_CH1, 119u8, {});
dma_trait_impl!(crate::usart::TxDma, USART10, DMA1_CH2, 119u8, {});
dma_trait_impl!(crate::usart::TxDma, USART10, DMA1_CH3, 119u8, {});
dma_trait_impl!(crate::usart::TxDma, USART10, DMA1_CH4, 119u8, {});
dma_trait_impl!(crate::usart::TxDma, USART10, DMA1_CH5, 119u8, {});
dma_trait_impl!(crate::usart::TxDma, USART10, DMA1_CH6, 119u8, {});
dma_trait_impl!(crate::usart::TxDma, USART10, DMA1_CH7, 119u8, {});
dma_trait_impl!(crate::usart::TxDma, USART10, DMA2_CH0, 119u8, {});
dma_trait_impl!(crate::usart::TxDma, USART10, DMA2_CH1, 119u8, {});
dma_trait_impl!(crate::usart::TxDma, USART10, DMA2_CH2, 119u8, {});
dma_trait_impl!(crate::usart::TxDma, USART10, DMA2_CH3, 119u8, {});
dma_trait_impl!(crate::usart::TxDma, USART10, DMA2_CH4, 119u8, {});
dma_trait_impl!(crate::usart::TxDma, USART10, DMA2_CH5, 119u8, {});
dma_trait_impl!(crate::usart::TxDma, USART10, DMA2_CH6, 119u8, {});
dma_trait_impl!(crate::usart::TxDma, USART10, DMA2_CH7, 119u8, {});
dma_trait_impl!(crate::usart::RxDma, USART2, DMA1_CH0, 43u8, {});
dma_trait_impl!(crate::usart::RxDma, USART2, DMA1_CH1, 43u8, {});
dma_trait_impl!(crate::usart::RxDma, USART2, DMA1_CH2, 43u8, {});
dma_trait_impl!(crate::usart::RxDma, USART2, DMA1_CH3, 43u8, {});
dma_trait_impl!(crate::usart::RxDma, USART2, DMA1_CH4, 43u8, {});
dma_trait_impl!(crate::usart::RxDma, USART2, DMA1_CH5, 43u8, {});
dma_trait_impl!(crate::usart::RxDma, USART2, DMA1_CH6, 43u8, {});
dma_trait_impl!(crate::usart::RxDma, USART2, DMA1_CH7, 43u8, {});
dma_trait_impl!(crate::usart::RxDma, USART2, DMA2_CH0, 43u8, {});
dma_trait_impl!(crate::usart::RxDma, USART2, DMA2_CH1, 43u8, {});
dma_trait_impl!(crate::usart::RxDma, USART2, DMA2_CH2, 43u8, {});
dma_trait_impl!(crate::usart::RxDma, USART2, DMA2_CH3, 43u8, {});
dma_trait_impl!(crate::usart::RxDma, USART2, DMA2_CH4, 43u8, {});
dma_trait_impl!(crate::usart::RxDma, USART2, DMA2_CH5, 43u8, {});
dma_trait_impl!(crate::usart::RxDma, USART2, DMA2_CH6, 43u8, {});
dma_trait_impl!(crate::usart::RxDma, USART2, DMA2_CH7, 43u8, {});
dma_trait_impl!(crate::usart::TxDma, USART2, DMA1_CH0, 44u8, {});
dma_trait_impl!(crate::usart::TxDma, USART2, DMA1_CH1, 44u8, {});
dma_trait_impl!(crate::usart::TxDma, USART2, DMA1_CH2, 44u8, {});
dma_trait_impl!(crate::usart::TxDma, USART2, DMA1_CH3, 44u8, {});
dma_trait_impl!(crate::usart::TxDma, USART2, DMA1_CH4, 44u8, {});
dma_trait_impl!(crate::usart::TxDma, USART2, DMA1_CH5, 44u8, {});
dma_trait_impl!(crate::usart::TxDma, USART2, DMA1_CH6, 44u8, {});
dma_trait_impl!(crate::usart::TxDma, USART2, DMA1_CH7, 44u8, {});
dma_trait_impl!(crate::usart::TxDma, USART2, DMA2_CH0, 44u8, {});
dma_trait_impl!(crate::usart::TxDma, USART2, DMA2_CH1, 44u8, {});
dma_trait_impl!(crate::usart::TxDma, USART2, DMA2_CH2, 44u8, {});
dma_trait_impl!(crate::usart::TxDma, USART2, DMA2_CH3, 44u8, {});
dma_trait_impl!(crate::usart::TxDma, USART2, DMA2_CH4, 44u8, {});
dma_trait_impl!(crate::usart::TxDma, USART2, DMA2_CH5, 44u8, {});
dma_trait_impl!(crate::usart::TxDma, USART2, DMA2_CH6, 44u8, {});
dma_trait_impl!(crate::usart::TxDma, USART2, DMA2_CH7, 44u8, {});
dma_trait_impl!(crate::usart::RxDma, USART3, DMA1_CH0, 45u8, {});
dma_trait_impl!(crate::usart::RxDma, USART3, DMA1_CH1, 45u8, {});
dma_trait_impl!(crate::usart::RxDma, USART3, DMA1_CH2, 45u8, {});
dma_trait_impl!(crate::usart::RxDma, USART3, DMA1_CH3, 45u8, {});
dma_trait_impl!(crate::usart::RxDma, USART3, DMA1_CH4, 45u8, {});
dma_trait_impl!(crate::usart::RxDma, USART3, DMA1_CH5, 45u8, {});
dma_trait_impl!(crate::usart::RxDma, USART3, DMA1_CH6, 45u8, {});
dma_trait_impl!(crate::usart::RxDma, USART3, DMA1_CH7, 45u8, {});
dma_trait_impl!(crate::usart::RxDma, USART3, DMA2_CH0, 45u8, {});
dma_trait_impl!(crate::usart::RxDma, USART3, DMA2_CH1, 45u8, {});
dma_trait_impl!(crate::usart::RxDma, USART3, DMA2_CH2, 45u8, {});
dma_trait_impl!(crate::usart::RxDma, USART3, DMA2_CH3, 45u8, {});
dma_trait_impl!(crate::usart::RxDma, USART3, DMA2_CH4, 45u8, {});
dma_trait_impl!(crate::usart::RxDma, USART3, DMA2_CH5, 45u8, {});
dma_trait_impl!(crate::usart::RxDma, USART3, DMA2_CH6, 45u8, {});
dma_trait_impl!(crate::usart::RxDma, USART3, DMA2_CH7, 45u8, {});
dma_trait_impl!(crate::usart::TxDma, USART3, DMA1_CH0, 46u8, {});
dma_trait_impl!(crate::usart::TxDma, USART3, DMA1_CH1, 46u8, {});
dma_trait_impl!(crate::usart::TxDma, USART3, DMA1_CH2, 46u8, {});
dma_trait_impl!(crate::usart::TxDma, USART3, DMA1_CH3, 46u8, {});
dma_trait_impl!(crate::usart::TxDma, USART3, DMA1_CH4, 46u8, {});
dma_trait_impl!(crate::usart::TxDma, USART3, DMA1_CH5, 46u8, {});
dma_trait_impl!(crate::usart::TxDma, USART3, DMA1_CH6, 46u8, {});
dma_trait_impl!(crate::usart::TxDma, USART3, DMA1_CH7, 46u8, {});
dma_trait_impl!(crate::usart::TxDma, USART3, DMA2_CH0, 46u8, {});
dma_trait_impl!(crate::usart::TxDma, USART3, DMA2_CH1, 46u8, {});
dma_trait_impl!(crate::usart::TxDma, USART3, DMA2_CH2, 46u8, {});
dma_trait_impl!(crate::usart::TxDma, USART3, DMA2_CH3, 46u8, {});
dma_trait_impl!(crate::usart::TxDma, USART3, DMA2_CH4, 46u8, {});
dma_trait_impl!(crate::usart::TxDma, USART3, DMA2_CH5, 46u8, {});
dma_trait_impl!(crate::usart::TxDma, USART3, DMA2_CH6, 46u8, {});
dma_trait_impl!(crate::usart::TxDma, USART3, DMA2_CH7, 46u8, {});
dma_trait_impl!(crate::usart::RxDma, USART6, DMA1_CH0, 71u8, {});
dma_trait_impl!(crate::usart::RxDma, USART6, DMA1_CH1, 71u8, {});
dma_trait_impl!(crate::usart::RxDma, USART6, DMA1_CH2, 71u8, {});
dma_trait_impl!(crate::usart::RxDma, USART6, DMA1_CH3, 71u8, {});
dma_trait_impl!(crate::usart::RxDma, USART6, DMA1_CH4, 71u8, {});
dma_trait_impl!(crate::usart::RxDma, USART6, DMA1_CH5, 71u8, {});
dma_trait_impl!(crate::usart::RxDma, USART6, DMA1_CH6, 71u8, {});
dma_trait_impl!(crate::usart::RxDma, USART6, DMA1_CH7, 71u8, {});
dma_trait_impl!(crate::usart::RxDma, USART6, DMA2_CH0, 71u8, {});
dma_trait_impl!(crate::usart::RxDma, USART6, DMA2_CH1, 71u8, {});
dma_trait_impl!(crate::usart::RxDma, USART6, DMA2_CH2, 71u8, {});
dma_trait_impl!(crate::usart::RxDma, USART6, DMA2_CH3, 71u8, {});
dma_trait_impl!(crate::usart::RxDma, USART6, DMA2_CH4, 71u8, {});
dma_trait_impl!(crate::usart::RxDma, USART6, DMA2_CH5, 71u8, {});
dma_trait_impl!(crate::usart::RxDma, USART6, DMA2_CH6, 71u8, {});
dma_trait_impl!(crate::usart::RxDma, USART6, DMA2_CH7, 71u8, {});
dma_trait_impl!(crate::usart::TxDma, USART6, DMA1_CH0, 72u8, {});
dma_trait_impl!(crate::usart::TxDma, USART6, DMA1_CH1, 72u8, {});
dma_trait_impl!(crate::usart::TxDma, USART6, DMA1_CH2, 72u8, {});
dma_trait_impl!(crate::usart::TxDma, USART6, DMA1_CH3, 72u8, {});
dma_trait_impl!(crate::usart::TxDma, USART6, DMA1_CH4, 72u8, {});
dma_trait_impl!(crate::usart::TxDma, USART6, DMA1_CH5, 72u8, {});
dma_trait_impl!(crate::usart::TxDma, USART6, DMA1_CH6, 72u8, {});
dma_trait_impl!(crate::usart::TxDma, USART6, DMA1_CH7, 72u8, {});
dma_trait_impl!(crate::usart::TxDma, USART6, DMA2_CH0, 72u8, {});
dma_trait_impl!(crate::usart::TxDma, USART6, DMA2_CH1, 72u8, {});
dma_trait_impl!(crate::usart::TxDma, USART6, DMA2_CH2, 72u8, {});
dma_trait_impl!(crate::usart::TxDma, USART6, DMA2_CH3, 72u8, {});
dma_trait_impl!(crate::usart::TxDma, USART6, DMA2_CH4, 72u8, {});
dma_trait_impl!(crate::usart::TxDma, USART6, DMA2_CH5, 72u8, {});
dma_trait_impl!(crate::usart::TxDma, USART6, DMA2_CH6, 72u8, {});
dma_trait_impl!(crate::usart::TxDma, USART6, DMA2_CH7, 72u8, {});
impl core::ops::Div<crate::pac::rcc::vals::Hpre> for crate::time::Hertz {
    type Output = crate::time::Hertz;
    fn div(self, rhs: crate::pac::rcc::vals::Hpre) -> Self::Output {
        match rhs {
            crate::pac::rcc::vals::Hpre::DIV1 => self * 1u32 / 1u32,
            crate::pac::rcc::vals::Hpre::DIV2 => self * 1u32 / 2u32,
            crate::pac::rcc::vals::Hpre::DIV4 => self * 1u32 / 4u32,
            crate::pac::rcc::vals::Hpre::DIV8 => self * 1u32 / 8u32,
            crate::pac::rcc::vals::Hpre::DIV16 => self * 1u32 / 16u32,
            crate::pac::rcc::vals::Hpre::DIV64 => self * 1u32 / 64u32,
            crate::pac::rcc::vals::Hpre::DIV128 => self * 1u32 / 128u32,
            crate::pac::rcc::vals::Hpre::DIV256 => self * 1u32 / 256u32,
            crate::pac::rcc::vals::Hpre::DIV512 => self * 1u32 / 512u32,
            #[allow(unreachable_patterns)]
            _ => unreachable!(),
        }
    }
}
impl core::ops::Mul<crate::pac::rcc::vals::Hpre> for crate::time::Hertz {
    type Output = crate::time::Hertz;
    fn mul(self, rhs: crate::pac::rcc::vals::Hpre) -> Self::Output {
        match rhs {
            crate::pac::rcc::vals::Hpre::DIV1 => self * 1u32 / 1u32,
            crate::pac::rcc::vals::Hpre::DIV2 => self * 2u32 / 1u32,
            crate::pac::rcc::vals::Hpre::DIV4 => self * 4u32 / 1u32,
            crate::pac::rcc::vals::Hpre::DIV8 => self * 8u32 / 1u32,
            crate::pac::rcc::vals::Hpre::DIV16 => self * 16u32 / 1u32,
            crate::pac::rcc::vals::Hpre::DIV64 => self * 64u32 / 1u32,
            crate::pac::rcc::vals::Hpre::DIV128 => self * 128u32 / 1u32,
            crate::pac::rcc::vals::Hpre::DIV256 => self * 256u32 / 1u32,
            crate::pac::rcc::vals::Hpre::DIV512 => self * 512u32 / 1u32,
            #[allow(unreachable_patterns)]
            _ => unreachable!(),
        }
    }
}
impl core::ops::Div<crate::pac::rcc::vals::Hsidiv> for crate::time::Hertz {
    type Output = crate::time::Hertz;
    fn div(self, rhs: crate::pac::rcc::vals::Hsidiv) -> Self::Output {
        match rhs {
            crate::pac::rcc::vals::Hsidiv::DIV1 => self * 1u32 / 1u32,
            crate::pac::rcc::vals::Hsidiv::DIV2 => self * 1u32 / 2u32,
            crate::pac::rcc::vals::Hsidiv::DIV4 => self * 1u32 / 4u32,
            crate::pac::rcc::vals::Hsidiv::DIV8 => self * 1u32 / 8u32,
            #[allow(unreachable_patterns)]
            _ => unreachable!(),
        }
    }
}
impl core::ops::Mul<crate::pac::rcc::vals::Hsidiv> for crate::time::Hertz {
    type Output = crate::time::Hertz;
    fn mul(self, rhs: crate::pac::rcc::vals::Hsidiv) -> Self::Output {
        match rhs {
            crate::pac::rcc::vals::Hsidiv::DIV1 => self * 1u32 / 1u32,
            crate::pac::rcc::vals::Hsidiv::DIV2 => self * 2u32 / 1u32,
            crate::pac::rcc::vals::Hsidiv::DIV4 => self * 4u32 / 1u32,
            crate::pac::rcc::vals::Hsidiv::DIV8 => self * 8u32 / 1u32,
            #[allow(unreachable_patterns)]
            _ => unreachable!(),
        }
    }
}
impl core::ops::Div<crate::pac::rcc::vals::Mcopre> for crate::time::Hertz {
    type Output = crate::time::Hertz;
    fn div(self, rhs: crate::pac::rcc::vals::Mcopre) -> Self::Output {
        match rhs {
            crate::pac::rcc::vals::Mcopre::DIV1 => self * 1u32 / 1u32,
            crate::pac::rcc::vals::Mcopre::DIV2 => self * 1u32 / 2u32,
            crate::pac::rcc::vals::Mcopre::DIV3 => self * 1u32 / 3u32,
            crate::pac::rcc::vals::Mcopre::DIV4 => self * 1u32 / 4u32,
            crate::pac::rcc::vals::Mcopre::DIV5 => self * 1u32 / 5u32,
            crate::pac::rcc::vals::Mcopre::DIV6 => self * 1u32 / 6u32,
            crate::pac::rcc::vals::Mcopre::DIV7 => self * 1u32 / 7u32,
            crate::pac::rcc::vals::Mcopre::DIV8 => self * 1u32 / 8u32,
            crate::pac::rcc::vals::Mcopre::DIV9 => self * 1u32 / 9u32,
            crate::pac::rcc::vals::Mcopre::DIV10 => self * 1u32 / 10u32,
            crate::pac::rcc::vals::Mcopre::DIV11 => self * 1u32 / 11u32,
            crate::pac::rcc::vals::Mcopre::DIV12 => self * 1u32 / 12u32,
            crate::pac::rcc::vals::Mcopre::DIV13 => self * 1u32 / 13u32,
            crate::pac::rcc::vals::Mcopre::DIV14 => self * 1u32 / 14u32,
            crate::pac::rcc::vals::Mcopre::DIV15 => self * 1u32 / 15u32,
            #[allow(unreachable_patterns)]
            _ => unreachable!(),
        }
    }
}
impl core::ops::Mul<crate::pac::rcc::vals::Mcopre> for crate::time::Hertz {
    type Output = crate::time::Hertz;
    fn mul(self, rhs: crate::pac::rcc::vals::Mcopre) -> Self::Output {
        match rhs {
            crate::pac::rcc::vals::Mcopre::DIV1 => self * 1u32 / 1u32,
            crate::pac::rcc::vals::Mcopre::DIV2 => self * 2u32 / 1u32,
            crate::pac::rcc::vals::Mcopre::DIV3 => self * 3u32 / 1u32,
            crate::pac::rcc::vals::Mcopre::DIV4 => self * 4u32 / 1u32,
            crate::pac::rcc::vals::Mcopre::DIV5 => self * 5u32 / 1u32,
            crate::pac::rcc::vals::Mcopre::DIV6 => self * 6u32 / 1u32,
            crate::pac::rcc::vals::Mcopre::DIV7 => self * 7u32 / 1u32,
            crate::pac::rcc::vals::Mcopre::DIV8 => self * 8u32 / 1u32,
            crate::pac::rcc::vals::Mcopre::DIV9 => self * 9u32 / 1u32,
            crate::pac::rcc::vals::Mcopre::DIV10 => self * 10u32 / 1u32,
            crate::pac::rcc::vals::Mcopre::DIV11 => self * 11u32 / 1u32,
            crate::pac::rcc::vals::Mcopre::DIV12 => self * 12u32 / 1u32,
            crate::pac::rcc::vals::Mcopre::DIV13 => self * 13u32 / 1u32,
            crate::pac::rcc::vals::Mcopre::DIV14 => self * 14u32 / 1u32,
            crate::pac::rcc::vals::Mcopre::DIV15 => self * 15u32 / 1u32,
            #[allow(unreachable_patterns)]
            _ => unreachable!(),
        }
    }
}
impl core::ops::Div<crate::pac::rcc::vals::Plldiv> for crate::time::Hertz {
    type Output = crate::time::Hertz;
    fn div(self, rhs: crate::pac::rcc::vals::Plldiv) -> Self::Output {
        match rhs {
            crate::pac::rcc::vals::Plldiv::DIV1 => self * 1u32 / 1u32,
            crate::pac::rcc::vals::Plldiv::DIV2 => self * 1u32 / 2u32,
            crate::pac::rcc::vals::Plldiv::DIV3 => self * 1u32 / 3u32,
            crate::pac::rcc::vals::Plldiv::DIV4 => self * 1u32 / 4u32,
            crate::pac::rcc::vals::Plldiv::DIV5 => self * 1u32 / 5u32,
            crate::pac::rcc::vals::Plldiv::DIV6 => self * 1u32 / 6u32,
            crate::pac::rcc::vals::Plldiv::DIV7 => self * 1u32 / 7u32,
            crate::pac::rcc::vals::Plldiv::DIV8 => self * 1u32 / 8u32,
            crate::pac::rcc::vals::Plldiv::DIV9 => self * 1u32 / 9u32,
            crate::pac::rcc::vals::Plldiv::DIV10 => self * 1u32 / 10u32,
            crate::pac::rcc::vals::Plldiv::DIV11 => self * 1u32 / 11u32,
            crate::pac::rcc::vals::Plldiv::DIV12 => self * 1u32 / 12u32,
            crate::pac::rcc::vals::Plldiv::DIV13 => self * 1u32 / 13u32,
            crate::pac::rcc::vals::Plldiv::DIV14 => self * 1u32 / 14u32,
            crate::pac::rcc::vals::Plldiv::DIV15 => self * 1u32 / 15u32,
            crate::pac::rcc::vals::Plldiv::DIV16 => self * 1u32 / 16u32,
            crate::pac::rcc::vals::Plldiv::DIV17 => self * 1u32 / 17u32,
            crate::pac::rcc::vals::Plldiv::DIV18 => self * 1u32 / 18u32,
            crate::pac::rcc::vals::Plldiv::DIV19 => self * 1u32 / 19u32,
            crate::pac::rcc::vals::Plldiv::DIV20 => self * 1u32 / 20u32,
            crate::pac::rcc::vals::Plldiv::DIV21 => self * 1u32 / 21u32,
            crate::pac::rcc::vals::Plldiv::DIV22 => self * 1u32 / 22u32,
            crate::pac::rcc::vals::Plldiv::DIV23 => self * 1u32 / 23u32,
            crate::pac::rcc::vals::Plldiv::DIV24 => self * 1u32 / 24u32,
            crate::pac::rcc::vals::Plldiv::DIV25 => self * 1u32 / 25u32,
            crate::pac::rcc::vals::Plldiv::DIV26 => self * 1u32 / 26u32,
            crate::pac::rcc::vals::Plldiv::DIV27 => self * 1u32 / 27u32,
            crate::pac::rcc::vals::Plldiv::DIV28 => self * 1u32 / 28u32,
            crate::pac::rcc::vals::Plldiv::DIV29 => self * 1u32 / 29u32,
            crate::pac::rcc::vals::Plldiv::DIV30 => self * 1u32 / 30u32,
            crate::pac::rcc::vals::Plldiv::DIV31 => self * 1u32 / 31u32,
            crate::pac::rcc::vals::Plldiv::DIV32 => self * 1u32 / 32u32,
            crate::pac::rcc::vals::Plldiv::DIV33 => self * 1u32 / 33u32,
            crate::pac::rcc::vals::Plldiv::DIV34 => self * 1u32 / 34u32,
            crate::pac::rcc::vals::Plldiv::DIV35 => self * 1u32 / 35u32,
            crate::pac::rcc::vals::Plldiv::DIV36 => self * 1u32 / 36u32,
            crate::pac::rcc::vals::Plldiv::DIV37 => self * 1u32 / 37u32,
            crate::pac::rcc::vals::Plldiv::DIV38 => self * 1u32 / 38u32,
            crate::pac::rcc::vals::Plldiv::DIV39 => self * 1u32 / 39u32,
            crate::pac::rcc::vals::Plldiv::DIV40 => self * 1u32 / 40u32,
            crate::pac::rcc::vals::Plldiv::DIV41 => self * 1u32 / 41u32,
            crate::pac::rcc::vals::Plldiv::DIV42 => self * 1u32 / 42u32,
            crate::pac::rcc::vals::Plldiv::DIV43 => self * 1u32 / 43u32,
            crate::pac::rcc::vals::Plldiv::DIV44 => self * 1u32 / 44u32,
            crate::pac::rcc::vals::Plldiv::DIV45 => self * 1u32 / 45u32,
            crate::pac::rcc::vals::Plldiv::DIV46 => self * 1u32 / 46u32,
            crate::pac::rcc::vals::Plldiv::DIV47 => self * 1u32 / 47u32,
            crate::pac::rcc::vals::Plldiv::DIV48 => self * 1u32 / 48u32,
            crate::pac::rcc::vals::Plldiv::DIV49 => self * 1u32 / 49u32,
            crate::pac::rcc::vals::Plldiv::DIV50 => self * 1u32 / 50u32,
            crate::pac::rcc::vals::Plldiv::DIV51 => self * 1u32 / 51u32,
            crate::pac::rcc::vals::Plldiv::DIV52 => self * 1u32 / 52u32,
            crate::pac::rcc::vals::Plldiv::DIV53 => self * 1u32 / 53u32,
            crate::pac::rcc::vals::Plldiv::DIV54 => self * 1u32 / 54u32,
            crate::pac::rcc::vals::Plldiv::DIV55 => self * 1u32 / 55u32,
            crate::pac::rcc::vals::Plldiv::DIV56 => self * 1u32 / 56u32,
            crate::pac::rcc::vals::Plldiv::DIV57 => self * 1u32 / 57u32,
            crate::pac::rcc::vals::Plldiv::DIV58 => self * 1u32 / 58u32,
            crate::pac::rcc::vals::Plldiv::DIV59 => self * 1u32 / 59u32,
            crate::pac::rcc::vals::Plldiv::DIV60 => self * 1u32 / 60u32,
            crate::pac::rcc::vals::Plldiv::DIV61 => self * 1u32 / 61u32,
            crate::pac::rcc::vals::Plldiv::DIV62 => self * 1u32 / 62u32,
            crate::pac::rcc::vals::Plldiv::DIV63 => self * 1u32 / 63u32,
            crate::pac::rcc::vals::Plldiv::DIV64 => self * 1u32 / 64u32,
            crate::pac::rcc::vals::Plldiv::DIV65 => self * 1u32 / 65u32,
            crate::pac::rcc::vals::Plldiv::DIV66 => self * 1u32 / 66u32,
            crate::pac::rcc::vals::Plldiv::DIV67 => self * 1u32 / 67u32,
            crate::pac::rcc::vals::Plldiv::DIV68 => self * 1u32 / 68u32,
            crate::pac::rcc::vals::Plldiv::DIV69 => self * 1u32 / 69u32,
            crate::pac::rcc::vals::Plldiv::DIV70 => self * 1u32 / 70u32,
            crate::pac::rcc::vals::Plldiv::DIV71 => self * 1u32 / 71u32,
            crate::pac::rcc::vals::Plldiv::DIV72 => self * 1u32 / 72u32,
            crate::pac::rcc::vals::Plldiv::DIV73 => self * 1u32 / 73u32,
            crate::pac::rcc::vals::Plldiv::DIV74 => self * 1u32 / 74u32,
            crate::pac::rcc::vals::Plldiv::DIV75 => self * 1u32 / 75u32,
            crate::pac::rcc::vals::Plldiv::DIV76 => self * 1u32 / 76u32,
            crate::pac::rcc::vals::Plldiv::DIV77 => self * 1u32 / 77u32,
            crate::pac::rcc::vals::Plldiv::DIV78 => self * 1u32 / 78u32,
            crate::pac::rcc::vals::Plldiv::DIV79 => self * 1u32 / 79u32,
            crate::pac::rcc::vals::Plldiv::DIV80 => self * 1u32 / 80u32,
            crate::pac::rcc::vals::Plldiv::DIV81 => self * 1u32 / 81u32,
            crate::pac::rcc::vals::Plldiv::DIV82 => self * 1u32 / 82u32,
            crate::pac::rcc::vals::Plldiv::DIV83 => self * 1u32 / 83u32,
            crate::pac::rcc::vals::Plldiv::DIV84 => self * 1u32 / 84u32,
            crate::pac::rcc::vals::Plldiv::DIV85 => self * 1u32 / 85u32,
            crate::pac::rcc::vals::Plldiv::DIV86 => self * 1u32 / 86u32,
            crate::pac::rcc::vals::Plldiv::DIV87 => self * 1u32 / 87u32,
            crate::pac::rcc::vals::Plldiv::DIV88 => self * 1u32 / 88u32,
            crate::pac::rcc::vals::Plldiv::DIV89 => self * 1u32 / 89u32,
            crate::pac::rcc::vals::Plldiv::DIV90 => self * 1u32 / 90u32,
            crate::pac::rcc::vals::Plldiv::DIV91 => self * 1u32 / 91u32,
            crate::pac::rcc::vals::Plldiv::DIV92 => self * 1u32 / 92u32,
            crate::pac::rcc::vals::Plldiv::DIV93 => self * 1u32 / 93u32,
            crate::pac::rcc::vals::Plldiv::DIV94 => self * 1u32 / 94u32,
            crate::pac::rcc::vals::Plldiv::DIV95 => self * 1u32 / 95u32,
            crate::pac::rcc::vals::Plldiv::DIV96 => self * 1u32 / 96u32,
            crate::pac::rcc::vals::Plldiv::DIV97 => self * 1u32 / 97u32,
            crate::pac::rcc::vals::Plldiv::DIV98 => self * 1u32 / 98u32,
            crate::pac::rcc::vals::Plldiv::DIV99 => self * 1u32 / 99u32,
            crate::pac::rcc::vals::Plldiv::DIV100 => self * 1u32 / 100u32,
            crate::pac::rcc::vals::Plldiv::DIV101 => self * 1u32 / 101u32,
            crate::pac::rcc::vals::Plldiv::DIV102 => self * 1u32 / 102u32,
            crate::pac::rcc::vals::Plldiv::DIV103 => self * 1u32 / 103u32,
            crate::pac::rcc::vals::Plldiv::DIV104 => self * 1u32 / 104u32,
            crate::pac::rcc::vals::Plldiv::DIV105 => self * 1u32 / 105u32,
            crate::pac::rcc::vals::Plldiv::DIV106 => self * 1u32 / 106u32,
            crate::pac::rcc::vals::Plldiv::DIV107 => self * 1u32 / 107u32,
            crate::pac::rcc::vals::Plldiv::DIV108 => self * 1u32 / 108u32,
            crate::pac::rcc::vals::Plldiv::DIV109 => self * 1u32 / 109u32,
            crate::pac::rcc::vals::Plldiv::DIV110 => self * 1u32 / 110u32,
            crate::pac::rcc::vals::Plldiv::DIV111 => self * 1u32 / 111u32,
            crate::pac::rcc::vals::Plldiv::DIV112 => self * 1u32 / 112u32,
            crate::pac::rcc::vals::Plldiv::DIV113 => self * 1u32 / 113u32,
            crate::pac::rcc::vals::Plldiv::DIV114 => self * 1u32 / 114u32,
            crate::pac::rcc::vals::Plldiv::DIV115 => self * 1u32 / 115u32,
            crate::pac::rcc::vals::Plldiv::DIV116 => self * 1u32 / 116u32,
            crate::pac::rcc::vals::Plldiv::DIV117 => self * 1u32 / 117u32,
            crate::pac::rcc::vals::Plldiv::DIV118 => self * 1u32 / 118u32,
            crate::pac::rcc::vals::Plldiv::DIV119 => self * 1u32 / 119u32,
            crate::pac::rcc::vals::Plldiv::DIV120 => self * 1u32 / 120u32,
            crate::pac::rcc::vals::Plldiv::DIV121 => self * 1u32 / 121u32,
            crate::pac::rcc::vals::Plldiv::DIV122 => self * 1u32 / 122u32,
            crate::pac::rcc::vals::Plldiv::DIV123 => self * 1u32 / 123u32,
            crate::pac::rcc::vals::Plldiv::DIV124 => self * 1u32 / 124u32,
            crate::pac::rcc::vals::Plldiv::DIV125 => self * 1u32 / 125u32,
            crate::pac::rcc::vals::Plldiv::DIV126 => self * 1u32 / 126u32,
            crate::pac::rcc::vals::Plldiv::DIV127 => self * 1u32 / 127u32,
            crate::pac::rcc::vals::Plldiv::DIV128 => self * 1u32 / 128u32,
            #[allow(unreachable_patterns)]
            _ => unreachable!(),
        }
    }
}
impl core::ops::Mul<crate::pac::rcc::vals::Plldiv> for crate::time::Hertz {
    type Output = crate::time::Hertz;
    fn mul(self, rhs: crate::pac::rcc::vals::Plldiv) -> Self::Output {
        match rhs {
            crate::pac::rcc::vals::Plldiv::DIV1 => self * 1u32 / 1u32,
            crate::pac::rcc::vals::Plldiv::DIV2 => self * 2u32 / 1u32,
            crate::pac::rcc::vals::Plldiv::DIV3 => self * 3u32 / 1u32,
            crate::pac::rcc::vals::Plldiv::DIV4 => self * 4u32 / 1u32,
            crate::pac::rcc::vals::Plldiv::DIV5 => self * 5u32 / 1u32,
            crate::pac::rcc::vals::Plldiv::DIV6 => self * 6u32 / 1u32,
            crate::pac::rcc::vals::Plldiv::DIV7 => self * 7u32 / 1u32,
            crate::pac::rcc::vals::Plldiv::DIV8 => self * 8u32 / 1u32,
            crate::pac::rcc::vals::Plldiv::DIV9 => self * 9u32 / 1u32,
            crate::pac::rcc::vals::Plldiv::DIV10 => self * 10u32 / 1u32,
            crate::pac::rcc::vals::Plldiv::DIV11 => self * 11u32 / 1u32,
            crate::pac::rcc::vals::Plldiv::DIV12 => self * 12u32 / 1u32,
            crate::pac::rcc::vals::Plldiv::DIV13 => self * 13u32 / 1u32,
            crate::pac::rcc::vals::Plldiv::DIV14 => self * 14u32 / 1u32,
            crate::pac::rcc::vals::Plldiv::DIV15 => self * 15u32 / 1u32,
            crate::pac::rcc::vals::Plldiv::DIV16 => self * 16u32 / 1u32,
            crate::pac::rcc::vals::Plldiv::DIV17 => self * 17u32 / 1u32,
            crate::pac::rcc::vals::Plldiv::DIV18 => self * 18u32 / 1u32,
            crate::pac::rcc::vals::Plldiv::DIV19 => self * 19u32 / 1u32,
            crate::pac::rcc::vals::Plldiv::DIV20 => self * 20u32 / 1u32,
            crate::pac::rcc::vals::Plldiv::DIV21 => self * 21u32 / 1u32,
            crate::pac::rcc::vals::Plldiv::DIV22 => self * 22u32 / 1u32,
            crate::pac::rcc::vals::Plldiv::DIV23 => self * 23u32 / 1u32,
            crate::pac::rcc::vals::Plldiv::DIV24 => self * 24u32 / 1u32,
            crate::pac::rcc::vals::Plldiv::DIV25 => self * 25u32 / 1u32,
            crate::pac::rcc::vals::Plldiv::DIV26 => self * 26u32 / 1u32,
            crate::pac::rcc::vals::Plldiv::DIV27 => self * 27u32 / 1u32,
            crate::pac::rcc::vals::Plldiv::DIV28 => self * 28u32 / 1u32,
            crate::pac::rcc::vals::Plldiv::DIV29 => self * 29u32 / 1u32,
            crate::pac::rcc::vals::Plldiv::DIV30 => self * 30u32 / 1u32,
            crate::pac::rcc::vals::Plldiv::DIV31 => self * 31u32 / 1u32,
            crate::pac::rcc::vals::Plldiv::DIV32 => self * 32u32 / 1u32,
            crate::pac::rcc::vals::Plldiv::DIV33 => self * 33u32 / 1u32,
            crate::pac::rcc::vals::Plldiv::DIV34 => self * 34u32 / 1u32,
            crate::pac::rcc::vals::Plldiv::DIV35 => self * 35u32 / 1u32,
            crate::pac::rcc::vals::Plldiv::DIV36 => self * 36u32 / 1u32,
            crate::pac::rcc::vals::Plldiv::DIV37 => self * 37u32 / 1u32,
            crate::pac::rcc::vals::Plldiv::DIV38 => self * 38u32 / 1u32,
            crate::pac::rcc::vals::Plldiv::DIV39 => self * 39u32 / 1u32,
            crate::pac::rcc::vals::Plldiv::DIV40 => self * 40u32 / 1u32,
            crate::pac::rcc::vals::Plldiv::DIV41 => self * 41u32 / 1u32,
            crate::pac::rcc::vals::Plldiv::DIV42 => self * 42u32 / 1u32,
            crate::pac::rcc::vals::Plldiv::DIV43 => self * 43u32 / 1u32,
            crate::pac::rcc::vals::Plldiv::DIV44 => self * 44u32 / 1u32,
            crate::pac::rcc::vals::Plldiv::DIV45 => self * 45u32 / 1u32,
            crate::pac::rcc::vals::Plldiv::DIV46 => self * 46u32 / 1u32,
            crate::pac::rcc::vals::Plldiv::DIV47 => self * 47u32 / 1u32,
            crate::pac::rcc::vals::Plldiv::DIV48 => self * 48u32 / 1u32,
            crate::pac::rcc::vals::Plldiv::DIV49 => self * 49u32 / 1u32,
            crate::pac::rcc::vals::Plldiv::DIV50 => self * 50u32 / 1u32,
            crate::pac::rcc::vals::Plldiv::DIV51 => self * 51u32 / 1u32,
            crate::pac::rcc::vals::Plldiv::DIV52 => self * 52u32 / 1u32,
            crate::pac::rcc::vals::Plldiv::DIV53 => self * 53u32 / 1u32,
            crate::pac::rcc::vals::Plldiv::DIV54 => self * 54u32 / 1u32,
            crate::pac::rcc::vals::Plldiv::DIV55 => self * 55u32 / 1u32,
            crate::pac::rcc::vals::Plldiv::DIV56 => self * 56u32 / 1u32,
            crate::pac::rcc::vals::Plldiv::DIV57 => self * 57u32 / 1u32,
            crate::pac::rcc::vals::Plldiv::DIV58 => self * 58u32 / 1u32,
            crate::pac::rcc::vals::Plldiv::DIV59 => self * 59u32 / 1u32,
            crate::pac::rcc::vals::Plldiv::DIV60 => self * 60u32 / 1u32,
            crate::pac::rcc::vals::Plldiv::DIV61 => self * 61u32 / 1u32,
            crate::pac::rcc::vals::Plldiv::DIV62 => self * 62u32 / 1u32,
            crate::pac::rcc::vals::Plldiv::DIV63 => self * 63u32 / 1u32,
            crate::pac::rcc::vals::Plldiv::DIV64 => self * 64u32 / 1u32,
            crate::pac::rcc::vals::Plldiv::DIV65 => self * 65u32 / 1u32,
            crate::pac::rcc::vals::Plldiv::DIV66 => self * 66u32 / 1u32,
            crate::pac::rcc::vals::Plldiv::DIV67 => self * 67u32 / 1u32,
            crate::pac::rcc::vals::Plldiv::DIV68 => self * 68u32 / 1u32,
            crate::pac::rcc::vals::Plldiv::DIV69 => self * 69u32 / 1u32,
            crate::pac::rcc::vals::Plldiv::DIV70 => self * 70u32 / 1u32,
            crate::pac::rcc::vals::Plldiv::DIV71 => self * 71u32 / 1u32,
            crate::pac::rcc::vals::Plldiv::DIV72 => self * 72u32 / 1u32,
            crate::pac::rcc::vals::Plldiv::DIV73 => self * 73u32 / 1u32,
            crate::pac::rcc::vals::Plldiv::DIV74 => self * 74u32 / 1u32,
            crate::pac::rcc::vals::Plldiv::DIV75 => self * 75u32 / 1u32,
            crate::pac::rcc::vals::Plldiv::DIV76 => self * 76u32 / 1u32,
            crate::pac::rcc::vals::Plldiv::DIV77 => self * 77u32 / 1u32,
            crate::pac::rcc::vals::Plldiv::DIV78 => self * 78u32 / 1u32,
            crate::pac::rcc::vals::Plldiv::DIV79 => self * 79u32 / 1u32,
            crate::pac::rcc::vals::Plldiv::DIV80 => self * 80u32 / 1u32,
            crate::pac::rcc::vals::Plldiv::DIV81 => self * 81u32 / 1u32,
            crate::pac::rcc::vals::Plldiv::DIV82 => self * 82u32 / 1u32,
            crate::pac::rcc::vals::Plldiv::DIV83 => self * 83u32 / 1u32,
            crate::pac::rcc::vals::Plldiv::DIV84 => self * 84u32 / 1u32,
            crate::pac::rcc::vals::Plldiv::DIV85 => self * 85u32 / 1u32,
            crate::pac::rcc::vals::Plldiv::DIV86 => self * 86u32 / 1u32,
            crate::pac::rcc::vals::Plldiv::DIV87 => self * 87u32 / 1u32,
            crate::pac::rcc::vals::Plldiv::DIV88 => self * 88u32 / 1u32,
            crate::pac::rcc::vals::Plldiv::DIV89 => self * 89u32 / 1u32,
            crate::pac::rcc::vals::Plldiv::DIV90 => self * 90u32 / 1u32,
            crate::pac::rcc::vals::Plldiv::DIV91 => self * 91u32 / 1u32,
            crate::pac::rcc::vals::Plldiv::DIV92 => self * 92u32 / 1u32,
            crate::pac::rcc::vals::Plldiv::DIV93 => self * 93u32 / 1u32,
            crate::pac::rcc::vals::Plldiv::DIV94 => self * 94u32 / 1u32,
            crate::pac::rcc::vals::Plldiv::DIV95 => self * 95u32 / 1u32,
            crate::pac::rcc::vals::Plldiv::DIV96 => self * 96u32 / 1u32,
            crate::pac::rcc::vals::Plldiv::DIV97 => self * 97u32 / 1u32,
            crate::pac::rcc::vals::Plldiv::DIV98 => self * 98u32 / 1u32,
            crate::pac::rcc::vals::Plldiv::DIV99 => self * 99u32 / 1u32,
            crate::pac::rcc::vals::Plldiv::DIV100 => self * 100u32 / 1u32,
            crate::pac::rcc::vals::Plldiv::DIV101 => self * 101u32 / 1u32,
            crate::pac::rcc::vals::Plldiv::DIV102 => self * 102u32 / 1u32,
            crate::pac::rcc::vals::Plldiv::DIV103 => self * 103u32 / 1u32,
            crate::pac::rcc::vals::Plldiv::DIV104 => self * 104u32 / 1u32,
            crate::pac::rcc::vals::Plldiv::DIV105 => self * 105u32 / 1u32,
            crate::pac::rcc::vals::Plldiv::DIV106 => self * 106u32 / 1u32,
            crate::pac::rcc::vals::Plldiv::DIV107 => self * 107u32 / 1u32,
            crate::pac::rcc::vals::Plldiv::DIV108 => self * 108u32 / 1u32,
            crate::pac::rcc::vals::Plldiv::DIV109 => self * 109u32 / 1u32,
            crate::pac::rcc::vals::Plldiv::DIV110 => self * 110u32 / 1u32,
            crate::pac::rcc::vals::Plldiv::DIV111 => self * 111u32 / 1u32,
            crate::pac::rcc::vals::Plldiv::DIV112 => self * 112u32 / 1u32,
            crate::pac::rcc::vals::Plldiv::DIV113 => self * 113u32 / 1u32,
            crate::pac::rcc::vals::Plldiv::DIV114 => self * 114u32 / 1u32,
            crate::pac::rcc::vals::Plldiv::DIV115 => self * 115u32 / 1u32,
            crate::pac::rcc::vals::Plldiv::DIV116 => self * 116u32 / 1u32,
            crate::pac::rcc::vals::Plldiv::DIV117 => self * 117u32 / 1u32,
            crate::pac::rcc::vals::Plldiv::DIV118 => self * 118u32 / 1u32,
            crate::pac::rcc::vals::Plldiv::DIV119 => self * 119u32 / 1u32,
            crate::pac::rcc::vals::Plldiv::DIV120 => self * 120u32 / 1u32,
            crate::pac::rcc::vals::Plldiv::DIV121 => self * 121u32 / 1u32,
            crate::pac::rcc::vals::Plldiv::DIV122 => self * 122u32 / 1u32,
            crate::pac::rcc::vals::Plldiv::DIV123 => self * 123u32 / 1u32,
            crate::pac::rcc::vals::Plldiv::DIV124 => self * 124u32 / 1u32,
            crate::pac::rcc::vals::Plldiv::DIV125 => self * 125u32 / 1u32,
            crate::pac::rcc::vals::Plldiv::DIV126 => self * 126u32 / 1u32,
            crate::pac::rcc::vals::Plldiv::DIV127 => self * 127u32 / 1u32,
            crate::pac::rcc::vals::Plldiv::DIV128 => self * 128u32 / 1u32,
            #[allow(unreachable_patterns)]
            _ => unreachable!(),
        }
    }
}
impl core::ops::Div<crate::pac::rcc::vals::Pllm> for crate::time::Hertz {
    type Output = crate::time::Hertz;
    fn div(self, rhs: crate::pac::rcc::vals::Pllm) -> Self::Output {
        match rhs {
            crate::pac::rcc::vals::Pllm::DIV1 => self * 1u32 / 1u32,
            crate::pac::rcc::vals::Pllm::DIV2 => self * 1u32 / 2u32,
            crate::pac::rcc::vals::Pllm::DIV3 => self * 1u32 / 3u32,
            crate::pac::rcc::vals::Pllm::DIV4 => self * 1u32 / 4u32,
            crate::pac::rcc::vals::Pllm::DIV5 => self * 1u32 / 5u32,
            crate::pac::rcc::vals::Pllm::DIV6 => self * 1u32 / 6u32,
            crate::pac::rcc::vals::Pllm::DIV7 => self * 1u32 / 7u32,
            crate::pac::rcc::vals::Pllm::DIV8 => self * 1u32 / 8u32,
            crate::pac::rcc::vals::Pllm::DIV9 => self * 1u32 / 9u32,
            crate::pac::rcc::vals::Pllm::DIV10 => self * 1u32 / 10u32,
            crate::pac::rcc::vals::Pllm::DIV11 => self * 1u32 / 11u32,
            crate::pac::rcc::vals::Pllm::DIV12 => self * 1u32 / 12u32,
            crate::pac::rcc::vals::Pllm::DIV13 => self * 1u32 / 13u32,
            crate::pac::rcc::vals::Pllm::DIV14 => self * 1u32 / 14u32,
            crate::pac::rcc::vals::Pllm::DIV15 => self * 1u32 / 15u32,
            crate::pac::rcc::vals::Pllm::DIV16 => self * 1u32 / 16u32,
            crate::pac::rcc::vals::Pllm::DIV17 => self * 1u32 / 17u32,
            crate::pac::rcc::vals::Pllm::DIV18 => self * 1u32 / 18u32,
            crate::pac::rcc::vals::Pllm::DIV19 => self * 1u32 / 19u32,
            crate::pac::rcc::vals::Pllm::DIV20 => self * 1u32 / 20u32,
            crate::pac::rcc::vals::Pllm::DIV21 => self * 1u32 / 21u32,
            crate::pac::rcc::vals::Pllm::DIV22 => self * 1u32 / 22u32,
            crate::pac::rcc::vals::Pllm::DIV23 => self * 1u32 / 23u32,
            crate::pac::rcc::vals::Pllm::DIV24 => self * 1u32 / 24u32,
            crate::pac::rcc::vals::Pllm::DIV25 => self * 1u32 / 25u32,
            crate::pac::rcc::vals::Pllm::DIV26 => self * 1u32 / 26u32,
            crate::pac::rcc::vals::Pllm::DIV27 => self * 1u32 / 27u32,
            crate::pac::rcc::vals::Pllm::DIV28 => self * 1u32 / 28u32,
            crate::pac::rcc::vals::Pllm::DIV29 => self * 1u32 / 29u32,
            crate::pac::rcc::vals::Pllm::DIV30 => self * 1u32 / 30u32,
            crate::pac::rcc::vals::Pllm::DIV31 => self * 1u32 / 31u32,
            crate::pac::rcc::vals::Pllm::DIV32 => self * 1u32 / 32u32,
            crate::pac::rcc::vals::Pllm::DIV33 => self * 1u32 / 33u32,
            crate::pac::rcc::vals::Pllm::DIV34 => self * 1u32 / 34u32,
            crate::pac::rcc::vals::Pllm::DIV35 => self * 1u32 / 35u32,
            crate::pac::rcc::vals::Pllm::DIV36 => self * 1u32 / 36u32,
            crate::pac::rcc::vals::Pllm::DIV37 => self * 1u32 / 37u32,
            crate::pac::rcc::vals::Pllm::DIV38 => self * 1u32 / 38u32,
            crate::pac::rcc::vals::Pllm::DIV39 => self * 1u32 / 39u32,
            crate::pac::rcc::vals::Pllm::DIV40 => self * 1u32 / 40u32,
            crate::pac::rcc::vals::Pllm::DIV41 => self * 1u32 / 41u32,
            crate::pac::rcc::vals::Pllm::DIV42 => self * 1u32 / 42u32,
            crate::pac::rcc::vals::Pllm::DIV43 => self * 1u32 / 43u32,
            crate::pac::rcc::vals::Pllm::DIV44 => self * 1u32 / 44u32,
            crate::pac::rcc::vals::Pllm::DIV45 => self * 1u32 / 45u32,
            crate::pac::rcc::vals::Pllm::DIV46 => self * 1u32 / 46u32,
            crate::pac::rcc::vals::Pllm::DIV47 => self * 1u32 / 47u32,
            crate::pac::rcc::vals::Pllm::DIV48 => self * 1u32 / 48u32,
            crate::pac::rcc::vals::Pllm::DIV49 => self * 1u32 / 49u32,
            crate::pac::rcc::vals::Pllm::DIV50 => self * 1u32 / 50u32,
            crate::pac::rcc::vals::Pllm::DIV51 => self * 1u32 / 51u32,
            crate::pac::rcc::vals::Pllm::DIV52 => self * 1u32 / 52u32,
            crate::pac::rcc::vals::Pllm::DIV53 => self * 1u32 / 53u32,
            crate::pac::rcc::vals::Pllm::DIV54 => self * 1u32 / 54u32,
            crate::pac::rcc::vals::Pllm::DIV55 => self * 1u32 / 55u32,
            crate::pac::rcc::vals::Pllm::DIV56 => self * 1u32 / 56u32,
            crate::pac::rcc::vals::Pllm::DIV57 => self * 1u32 / 57u32,
            crate::pac::rcc::vals::Pllm::DIV58 => self * 1u32 / 58u32,
            crate::pac::rcc::vals::Pllm::DIV59 => self * 1u32 / 59u32,
            crate::pac::rcc::vals::Pllm::DIV60 => self * 1u32 / 60u32,
            crate::pac::rcc::vals::Pllm::DIV61 => self * 1u32 / 61u32,
            crate::pac::rcc::vals::Pllm::DIV62 => self * 1u32 / 62u32,
            #[allow(unreachable_patterns)]
            _ => unreachable!(),
        }
    }
}
impl core::ops::Mul<crate::pac::rcc::vals::Pllm> for crate::time::Hertz {
    type Output = crate::time::Hertz;
    fn mul(self, rhs: crate::pac::rcc::vals::Pllm) -> Self::Output {
        match rhs {
            crate::pac::rcc::vals::Pllm::DIV1 => self * 1u32 / 1u32,
            crate::pac::rcc::vals::Pllm::DIV2 => self * 2u32 / 1u32,
            crate::pac::rcc::vals::Pllm::DIV3 => self * 3u32 / 1u32,
            crate::pac::rcc::vals::Pllm::DIV4 => self * 4u32 / 1u32,
            crate::pac::rcc::vals::Pllm::DIV5 => self * 5u32 / 1u32,
            crate::pac::rcc::vals::Pllm::DIV6 => self * 6u32 / 1u32,
            crate::pac::rcc::vals::Pllm::DIV7 => self * 7u32 / 1u32,
            crate::pac::rcc::vals::Pllm::DIV8 => self * 8u32 / 1u32,
            crate::pac::rcc::vals::Pllm::DIV9 => self * 9u32 / 1u32,
            crate::pac::rcc::vals::Pllm::DIV10 => self * 10u32 / 1u32,
            crate::pac::rcc::vals::Pllm::DIV11 => self * 11u32 / 1u32,
            crate::pac::rcc::vals::Pllm::DIV12 => self * 12u32 / 1u32,
            crate::pac::rcc::vals::Pllm::DIV13 => self * 13u32 / 1u32,
            crate::pac::rcc::vals::Pllm::DIV14 => self * 14u32 / 1u32,
            crate::pac::rcc::vals::Pllm::DIV15 => self * 15u32 / 1u32,
            crate::pac::rcc::vals::Pllm::DIV16 => self * 16u32 / 1u32,
            crate::pac::rcc::vals::Pllm::DIV17 => self * 17u32 / 1u32,
            crate::pac::rcc::vals::Pllm::DIV18 => self * 18u32 / 1u32,
            crate::pac::rcc::vals::Pllm::DIV19 => self * 19u32 / 1u32,
            crate::pac::rcc::vals::Pllm::DIV20 => self * 20u32 / 1u32,
            crate::pac::rcc::vals::Pllm::DIV21 => self * 21u32 / 1u32,
            crate::pac::rcc::vals::Pllm::DIV22 => self * 22u32 / 1u32,
            crate::pac::rcc::vals::Pllm::DIV23 => self * 23u32 / 1u32,
            crate::pac::rcc::vals::Pllm::DIV24 => self * 24u32 / 1u32,
            crate::pac::rcc::vals::Pllm::DIV25 => self * 25u32 / 1u32,
            crate::pac::rcc::vals::Pllm::DIV26 => self * 26u32 / 1u32,
            crate::pac::rcc::vals::Pllm::DIV27 => self * 27u32 / 1u32,
            crate::pac::rcc::vals::Pllm::DIV28 => self * 28u32 / 1u32,
            crate::pac::rcc::vals::Pllm::DIV29 => self * 29u32 / 1u32,
            crate::pac::rcc::vals::Pllm::DIV30 => self * 30u32 / 1u32,
            crate::pac::rcc::vals::Pllm::DIV31 => self * 31u32 / 1u32,
            crate::pac::rcc::vals::Pllm::DIV32 => self * 32u32 / 1u32,
            crate::pac::rcc::vals::Pllm::DIV33 => self * 33u32 / 1u32,
            crate::pac::rcc::vals::Pllm::DIV34 => self * 34u32 / 1u32,
            crate::pac::rcc::vals::Pllm::DIV35 => self * 35u32 / 1u32,
            crate::pac::rcc::vals::Pllm::DIV36 => self * 36u32 / 1u32,
            crate::pac::rcc::vals::Pllm::DIV37 => self * 37u32 / 1u32,
            crate::pac::rcc::vals::Pllm::DIV38 => self * 38u32 / 1u32,
            crate::pac::rcc::vals::Pllm::DIV39 => self * 39u32 / 1u32,
            crate::pac::rcc::vals::Pllm::DIV40 => self * 40u32 / 1u32,
            crate::pac::rcc::vals::Pllm::DIV41 => self * 41u32 / 1u32,
            crate::pac::rcc::vals::Pllm::DIV42 => self * 42u32 / 1u32,
            crate::pac::rcc::vals::Pllm::DIV43 => self * 43u32 / 1u32,
            crate::pac::rcc::vals::Pllm::DIV44 => self * 44u32 / 1u32,
            crate::pac::rcc::vals::Pllm::DIV45 => self * 45u32 / 1u32,
            crate::pac::rcc::vals::Pllm::DIV46 => self * 46u32 / 1u32,
            crate::pac::rcc::vals::Pllm::DIV47 => self * 47u32 / 1u32,
            crate::pac::rcc::vals::Pllm::DIV48 => self * 48u32 / 1u32,
            crate::pac::rcc::vals::Pllm::DIV49 => self * 49u32 / 1u32,
            crate::pac::rcc::vals::Pllm::DIV50 => self * 50u32 / 1u32,
            crate::pac::rcc::vals::Pllm::DIV51 => self * 51u32 / 1u32,
            crate::pac::rcc::vals::Pllm::DIV52 => self * 52u32 / 1u32,
            crate::pac::rcc::vals::Pllm::DIV53 => self * 53u32 / 1u32,
            crate::pac::rcc::vals::Pllm::DIV54 => self * 54u32 / 1u32,
            crate::pac::rcc::vals::Pllm::DIV55 => self * 55u32 / 1u32,
            crate::pac::rcc::vals::Pllm::DIV56 => self * 56u32 / 1u32,
            crate::pac::rcc::vals::Pllm::DIV57 => self * 57u32 / 1u32,
            crate::pac::rcc::vals::Pllm::DIV58 => self * 58u32 / 1u32,
            crate::pac::rcc::vals::Pllm::DIV59 => self * 59u32 / 1u32,
            crate::pac::rcc::vals::Pllm::DIV60 => self * 60u32 / 1u32,
            crate::pac::rcc::vals::Pllm::DIV61 => self * 61u32 / 1u32,
            crate::pac::rcc::vals::Pllm::DIV62 => self * 62u32 / 1u32,
            #[allow(unreachable_patterns)]
            _ => unreachable!(),
        }
    }
}
impl core::ops::Div<crate::pac::rcc::vals::Plln> for crate::time::Hertz {
    type Output = crate::time::Hertz;
    fn div(self, rhs: crate::pac::rcc::vals::Plln) -> Self::Output {
        match rhs {
            crate::pac::rcc::vals::Plln::MUL4 => self * 1u32 / 4u32,
            crate::pac::rcc::vals::Plln::MUL5 => self * 1u32 / 5u32,
            crate::pac::rcc::vals::Plln::MUL6 => self * 1u32 / 6u32,
            crate::pac::rcc::vals::Plln::MUL7 => self * 1u32 / 7u32,
            crate::pac::rcc::vals::Plln::MUL8 => self * 1u32 / 8u32,
            crate::pac::rcc::vals::Plln::MUL9 => self * 1u32 / 9u32,
            crate::pac::rcc::vals::Plln::MUL10 => self * 1u32 / 10u32,
            crate::pac::rcc::vals::Plln::MUL11 => self * 1u32 / 11u32,
            crate::pac::rcc::vals::Plln::MUL12 => self * 1u32 / 12u32,
            crate::pac::rcc::vals::Plln::MUL13 => self * 1u32 / 13u32,
            crate::pac::rcc::vals::Plln::MUL14 => self * 1u32 / 14u32,
            crate::pac::rcc::vals::Plln::MUL15 => self * 1u32 / 15u32,
            crate::pac::rcc::vals::Plln::MUL16 => self * 1u32 / 16u32,
            crate::pac::rcc::vals::Plln::MUL17 => self * 1u32 / 17u32,
            crate::pac::rcc::vals::Plln::MUL18 => self * 1u32 / 18u32,
            crate::pac::rcc::vals::Plln::MUL19 => self * 1u32 / 19u32,
            crate::pac::rcc::vals::Plln::MUL20 => self * 1u32 / 20u32,
            crate::pac::rcc::vals::Plln::MUL21 => self * 1u32 / 21u32,
            crate::pac::rcc::vals::Plln::MUL22 => self * 1u32 / 22u32,
            crate::pac::rcc::vals::Plln::MUL23 => self * 1u32 / 23u32,
            crate::pac::rcc::vals::Plln::MUL24 => self * 1u32 / 24u32,
            crate::pac::rcc::vals::Plln::MUL25 => self * 1u32 / 25u32,
            crate::pac::rcc::vals::Plln::MUL26 => self * 1u32 / 26u32,
            crate::pac::rcc::vals::Plln::MUL27 => self * 1u32 / 27u32,
            crate::pac::rcc::vals::Plln::MUL28 => self * 1u32 / 28u32,
            crate::pac::rcc::vals::Plln::MUL29 => self * 1u32 / 29u32,
            crate::pac::rcc::vals::Plln::MUL30 => self * 1u32 / 30u32,
            crate::pac::rcc::vals::Plln::MUL31 => self * 1u32 / 31u32,
            crate::pac::rcc::vals::Plln::MUL32 => self * 1u32 / 32u32,
            crate::pac::rcc::vals::Plln::MUL33 => self * 1u32 / 33u32,
            crate::pac::rcc::vals::Plln::MUL34 => self * 1u32 / 34u32,
            crate::pac::rcc::vals::Plln::MUL35 => self * 1u32 / 35u32,
            crate::pac::rcc::vals::Plln::MUL36 => self * 1u32 / 36u32,
            crate::pac::rcc::vals::Plln::MUL37 => self * 1u32 / 37u32,
            crate::pac::rcc::vals::Plln::MUL38 => self * 1u32 / 38u32,
            crate::pac::rcc::vals::Plln::MUL39 => self * 1u32 / 39u32,
            crate::pac::rcc::vals::Plln::MUL40 => self * 1u32 / 40u32,
            crate::pac::rcc::vals::Plln::MUL41 => self * 1u32 / 41u32,
            crate::pac::rcc::vals::Plln::MUL42 => self * 1u32 / 42u32,
            crate::pac::rcc::vals::Plln::MUL43 => self * 1u32 / 43u32,
            crate::pac::rcc::vals::Plln::MUL44 => self * 1u32 / 44u32,
            crate::pac::rcc::vals::Plln::MUL45 => self * 1u32 / 45u32,
            crate::pac::rcc::vals::Plln::MUL46 => self * 1u32 / 46u32,
            crate::pac::rcc::vals::Plln::MUL47 => self * 1u32 / 47u32,
            crate::pac::rcc::vals::Plln::MUL48 => self * 1u32 / 48u32,
            crate::pac::rcc::vals::Plln::MUL49 => self * 1u32 / 49u32,
            crate::pac::rcc::vals::Plln::MUL50 => self * 1u32 / 50u32,
            crate::pac::rcc::vals::Plln::MUL51 => self * 1u32 / 51u32,
            crate::pac::rcc::vals::Plln::MUL52 => self * 1u32 / 52u32,
            crate::pac::rcc::vals::Plln::MUL53 => self * 1u32 / 53u32,
            crate::pac::rcc::vals::Plln::MUL54 => self * 1u32 / 54u32,
            crate::pac::rcc::vals::Plln::MUL55 => self * 1u32 / 55u32,
            crate::pac::rcc::vals::Plln::MUL56 => self * 1u32 / 56u32,
            crate::pac::rcc::vals::Plln::MUL57 => self * 1u32 / 57u32,
            crate::pac::rcc::vals::Plln::MUL58 => self * 1u32 / 58u32,
            crate::pac::rcc::vals::Plln::MUL59 => self * 1u32 / 59u32,
            crate::pac::rcc::vals::Plln::MUL60 => self * 1u32 / 60u32,
            crate::pac::rcc::vals::Plln::MUL61 => self * 1u32 / 61u32,
            crate::pac::rcc::vals::Plln::MUL62 => self * 1u32 / 62u32,
            crate::pac::rcc::vals::Plln::MUL63 => self * 1u32 / 63u32,
            crate::pac::rcc::vals::Plln::MUL64 => self * 1u32 / 64u32,
            crate::pac::rcc::vals::Plln::MUL65 => self * 1u32 / 65u32,
            crate::pac::rcc::vals::Plln::MUL66 => self * 1u32 / 66u32,
            crate::pac::rcc::vals::Plln::MUL67 => self * 1u32 / 67u32,
            crate::pac::rcc::vals::Plln::MUL68 => self * 1u32 / 68u32,
            crate::pac::rcc::vals::Plln::MUL69 => self * 1u32 / 69u32,
            crate::pac::rcc::vals::Plln::MUL70 => self * 1u32 / 70u32,
            crate::pac::rcc::vals::Plln::MUL71 => self * 1u32 / 71u32,
            crate::pac::rcc::vals::Plln::MUL72 => self * 1u32 / 72u32,
            crate::pac::rcc::vals::Plln::MUL73 => self * 1u32 / 73u32,
            crate::pac::rcc::vals::Plln::MUL74 => self * 1u32 / 74u32,
            crate::pac::rcc::vals::Plln::MUL75 => self * 1u32 / 75u32,
            crate::pac::rcc::vals::Plln::MUL76 => self * 1u32 / 76u32,
            crate::pac::rcc::vals::Plln::MUL77 => self * 1u32 / 77u32,
            crate::pac::rcc::vals::Plln::MUL78 => self * 1u32 / 78u32,
            crate::pac::rcc::vals::Plln::MUL79 => self * 1u32 / 79u32,
            crate::pac::rcc::vals::Plln::MUL80 => self * 1u32 / 80u32,
            crate::pac::rcc::vals::Plln::MUL81 => self * 1u32 / 81u32,
            crate::pac::rcc::vals::Plln::MUL82 => self * 1u32 / 82u32,
            crate::pac::rcc::vals::Plln::MUL83 => self * 1u32 / 83u32,
            crate::pac::rcc::vals::Plln::MUL84 => self * 1u32 / 84u32,
            crate::pac::rcc::vals::Plln::MUL85 => self * 1u32 / 85u32,
            crate::pac::rcc::vals::Plln::MUL86 => self * 1u32 / 86u32,
            crate::pac::rcc::vals::Plln::MUL87 => self * 1u32 / 87u32,
            crate::pac::rcc::vals::Plln::MUL88 => self * 1u32 / 88u32,
            crate::pac::rcc::vals::Plln::MUL89 => self * 1u32 / 89u32,
            crate::pac::rcc::vals::Plln::MUL90 => self * 1u32 / 90u32,
            crate::pac::rcc::vals::Plln::MUL91 => self * 1u32 / 91u32,
            crate::pac::rcc::vals::Plln::MUL92 => self * 1u32 / 92u32,
            crate::pac::rcc::vals::Plln::MUL93 => self * 1u32 / 93u32,
            crate::pac::rcc::vals::Plln::MUL94 => self * 1u32 / 94u32,
            crate::pac::rcc::vals::Plln::MUL95 => self * 1u32 / 95u32,
            crate::pac::rcc::vals::Plln::MUL96 => self * 1u32 / 96u32,
            crate::pac::rcc::vals::Plln::MUL97 => self * 1u32 / 97u32,
            crate::pac::rcc::vals::Plln::MUL98 => self * 1u32 / 98u32,
            crate::pac::rcc::vals::Plln::MUL99 => self * 1u32 / 99u32,
            crate::pac::rcc::vals::Plln::MUL100 => self * 1u32 / 100u32,
            crate::pac::rcc::vals::Plln::MUL101 => self * 1u32 / 101u32,
            crate::pac::rcc::vals::Plln::MUL102 => self * 1u32 / 102u32,
            crate::pac::rcc::vals::Plln::MUL103 => self * 1u32 / 103u32,
            crate::pac::rcc::vals::Plln::MUL104 => self * 1u32 / 104u32,
            crate::pac::rcc::vals::Plln::MUL105 => self * 1u32 / 105u32,
            crate::pac::rcc::vals::Plln::MUL106 => self * 1u32 / 106u32,
            crate::pac::rcc::vals::Plln::MUL107 => self * 1u32 / 107u32,
            crate::pac::rcc::vals::Plln::MUL108 => self * 1u32 / 108u32,
            crate::pac::rcc::vals::Plln::MUL109 => self * 1u32 / 109u32,
            crate::pac::rcc::vals::Plln::MUL110 => self * 1u32 / 110u32,
            crate::pac::rcc::vals::Plln::MUL111 => self * 1u32 / 111u32,
            crate::pac::rcc::vals::Plln::MUL112 => self * 1u32 / 112u32,
            crate::pac::rcc::vals::Plln::MUL113 => self * 1u32 / 113u32,
            crate::pac::rcc::vals::Plln::MUL114 => self * 1u32 / 114u32,
            crate::pac::rcc::vals::Plln::MUL115 => self * 1u32 / 115u32,
            crate::pac::rcc::vals::Plln::MUL116 => self * 1u32 / 116u32,
            crate::pac::rcc::vals::Plln::MUL117 => self * 1u32 / 117u32,
            crate::pac::rcc::vals::Plln::MUL118 => self * 1u32 / 118u32,
            crate::pac::rcc::vals::Plln::MUL119 => self * 1u32 / 119u32,
            crate::pac::rcc::vals::Plln::MUL120 => self * 1u32 / 120u32,
            crate::pac::rcc::vals::Plln::MUL121 => self * 1u32 / 121u32,
            crate::pac::rcc::vals::Plln::MUL122 => self * 1u32 / 122u32,
            crate::pac::rcc::vals::Plln::MUL123 => self * 1u32 / 123u32,
            crate::pac::rcc::vals::Plln::MUL124 => self * 1u32 / 124u32,
            crate::pac::rcc::vals::Plln::MUL125 => self * 1u32 / 125u32,
            crate::pac::rcc::vals::Plln::MUL126 => self * 1u32 / 126u32,
            crate::pac::rcc::vals::Plln::MUL127 => self * 1u32 / 127u32,
            crate::pac::rcc::vals::Plln::MUL128 => self * 1u32 / 128u32,
            crate::pac::rcc::vals::Plln::MUL129 => self * 1u32 / 129u32,
            crate::pac::rcc::vals::Plln::MUL130 => self * 1u32 / 130u32,
            crate::pac::rcc::vals::Plln::MUL131 => self * 1u32 / 131u32,
            crate::pac::rcc::vals::Plln::MUL132 => self * 1u32 / 132u32,
            crate::pac::rcc::vals::Plln::MUL133 => self * 1u32 / 133u32,
            crate::pac::rcc::vals::Plln::MUL134 => self * 1u32 / 134u32,
            crate::pac::rcc::vals::Plln::MUL135 => self * 1u32 / 135u32,
            crate::pac::rcc::vals::Plln::MUL136 => self * 1u32 / 136u32,
            crate::pac::rcc::vals::Plln::MUL137 => self * 1u32 / 137u32,
            crate::pac::rcc::vals::Plln::MUL138 => self * 1u32 / 138u32,
            crate::pac::rcc::vals::Plln::MUL139 => self * 1u32 / 139u32,
            crate::pac::rcc::vals::Plln::MUL140 => self * 1u32 / 140u32,
            crate::pac::rcc::vals::Plln::MUL141 => self * 1u32 / 141u32,
            crate::pac::rcc::vals::Plln::MUL142 => self * 1u32 / 142u32,
            crate::pac::rcc::vals::Plln::MUL143 => self * 1u32 / 143u32,
            crate::pac::rcc::vals::Plln::MUL144 => self * 1u32 / 144u32,
            crate::pac::rcc::vals::Plln::MUL145 => self * 1u32 / 145u32,
            crate::pac::rcc::vals::Plln::MUL146 => self * 1u32 / 146u32,
            crate::pac::rcc::vals::Plln::MUL147 => self * 1u32 / 147u32,
            crate::pac::rcc::vals::Plln::MUL148 => self * 1u32 / 148u32,
            crate::pac::rcc::vals::Plln::MUL149 => self * 1u32 / 149u32,
            crate::pac::rcc::vals::Plln::MUL150 => self * 1u32 / 150u32,
            crate::pac::rcc::vals::Plln::MUL151 => self * 1u32 / 151u32,
            crate::pac::rcc::vals::Plln::MUL152 => self * 1u32 / 152u32,
            crate::pac::rcc::vals::Plln::MUL153 => self * 1u32 / 153u32,
            crate::pac::rcc::vals::Plln::MUL154 => self * 1u32 / 154u32,
            crate::pac::rcc::vals::Plln::MUL155 => self * 1u32 / 155u32,
            crate::pac::rcc::vals::Plln::MUL156 => self * 1u32 / 156u32,
            crate::pac::rcc::vals::Plln::MUL157 => self * 1u32 / 157u32,
            crate::pac::rcc::vals::Plln::MUL158 => self * 1u32 / 158u32,
            crate::pac::rcc::vals::Plln::MUL159 => self * 1u32 / 159u32,
            crate::pac::rcc::vals::Plln::MUL160 => self * 1u32 / 160u32,
            crate::pac::rcc::vals::Plln::MUL161 => self * 1u32 / 161u32,
            crate::pac::rcc::vals::Plln::MUL162 => self * 1u32 / 162u32,
            crate::pac::rcc::vals::Plln::MUL163 => self * 1u32 / 163u32,
            crate::pac::rcc::vals::Plln::MUL164 => self * 1u32 / 164u32,
            crate::pac::rcc::vals::Plln::MUL165 => self * 1u32 / 165u32,
            crate::pac::rcc::vals::Plln::MUL166 => self * 1u32 / 166u32,
            crate::pac::rcc::vals::Plln::MUL167 => self * 1u32 / 167u32,
            crate::pac::rcc::vals::Plln::MUL168 => self * 1u32 / 168u32,
            crate::pac::rcc::vals::Plln::MUL169 => self * 1u32 / 169u32,
            crate::pac::rcc::vals::Plln::MUL170 => self * 1u32 / 170u32,
            crate::pac::rcc::vals::Plln::MUL171 => self * 1u32 / 171u32,
            crate::pac::rcc::vals::Plln::MUL172 => self * 1u32 / 172u32,
            crate::pac::rcc::vals::Plln::MUL173 => self * 1u32 / 173u32,
            crate::pac::rcc::vals::Plln::MUL174 => self * 1u32 / 174u32,
            crate::pac::rcc::vals::Plln::MUL175 => self * 1u32 / 175u32,
            crate::pac::rcc::vals::Plln::MUL176 => self * 1u32 / 176u32,
            crate::pac::rcc::vals::Plln::MUL177 => self * 1u32 / 177u32,
            crate::pac::rcc::vals::Plln::MUL178 => self * 1u32 / 178u32,
            crate::pac::rcc::vals::Plln::MUL179 => self * 1u32 / 179u32,
            crate::pac::rcc::vals::Plln::MUL180 => self * 1u32 / 180u32,
            crate::pac::rcc::vals::Plln::MUL181 => self * 1u32 / 181u32,
            crate::pac::rcc::vals::Plln::MUL182 => self * 1u32 / 182u32,
            crate::pac::rcc::vals::Plln::MUL183 => self * 1u32 / 183u32,
            crate::pac::rcc::vals::Plln::MUL184 => self * 1u32 / 184u32,
            crate::pac::rcc::vals::Plln::MUL185 => self * 1u32 / 185u32,
            crate::pac::rcc::vals::Plln::MUL186 => self * 1u32 / 186u32,
            crate::pac::rcc::vals::Plln::MUL187 => self * 1u32 / 187u32,
            crate::pac::rcc::vals::Plln::MUL188 => self * 1u32 / 188u32,
            crate::pac::rcc::vals::Plln::MUL189 => self * 1u32 / 189u32,
            crate::pac::rcc::vals::Plln::MUL190 => self * 1u32 / 190u32,
            crate::pac::rcc::vals::Plln::MUL191 => self * 1u32 / 191u32,
            crate::pac::rcc::vals::Plln::MUL192 => self * 1u32 / 192u32,
            crate::pac::rcc::vals::Plln::MUL193 => self * 1u32 / 193u32,
            crate::pac::rcc::vals::Plln::MUL194 => self * 1u32 / 194u32,
            crate::pac::rcc::vals::Plln::MUL195 => self * 1u32 / 195u32,
            crate::pac::rcc::vals::Plln::MUL196 => self * 1u32 / 196u32,
            crate::pac::rcc::vals::Plln::MUL197 => self * 1u32 / 197u32,
            crate::pac::rcc::vals::Plln::MUL198 => self * 1u32 / 198u32,
            crate::pac::rcc::vals::Plln::MUL199 => self * 1u32 / 199u32,
            crate::pac::rcc::vals::Plln::MUL200 => self * 1u32 / 200u32,
            crate::pac::rcc::vals::Plln::MUL201 => self * 1u32 / 201u32,
            crate::pac::rcc::vals::Plln::MUL202 => self * 1u32 / 202u32,
            crate::pac::rcc::vals::Plln::MUL203 => self * 1u32 / 203u32,
            crate::pac::rcc::vals::Plln::MUL204 => self * 1u32 / 204u32,
            crate::pac::rcc::vals::Plln::MUL205 => self * 1u32 / 205u32,
            crate::pac::rcc::vals::Plln::MUL206 => self * 1u32 / 206u32,
            crate::pac::rcc::vals::Plln::MUL207 => self * 1u32 / 207u32,
            crate::pac::rcc::vals::Plln::MUL208 => self * 1u32 / 208u32,
            crate::pac::rcc::vals::Plln::MUL209 => self * 1u32 / 209u32,
            crate::pac::rcc::vals::Plln::MUL210 => self * 1u32 / 210u32,
            crate::pac::rcc::vals::Plln::MUL211 => self * 1u32 / 211u32,
            crate::pac::rcc::vals::Plln::MUL212 => self * 1u32 / 212u32,
            crate::pac::rcc::vals::Plln::MUL213 => self * 1u32 / 213u32,
            crate::pac::rcc::vals::Plln::MUL214 => self * 1u32 / 214u32,
            crate::pac::rcc::vals::Plln::MUL215 => self * 1u32 / 215u32,
            crate::pac::rcc::vals::Plln::MUL216 => self * 1u32 / 216u32,
            crate::pac::rcc::vals::Plln::MUL217 => self * 1u32 / 217u32,
            crate::pac::rcc::vals::Plln::MUL218 => self * 1u32 / 218u32,
            crate::pac::rcc::vals::Plln::MUL219 => self * 1u32 / 219u32,
            crate::pac::rcc::vals::Plln::MUL220 => self * 1u32 / 220u32,
            crate::pac::rcc::vals::Plln::MUL221 => self * 1u32 / 221u32,
            crate::pac::rcc::vals::Plln::MUL222 => self * 1u32 / 222u32,
            crate::pac::rcc::vals::Plln::MUL223 => self * 1u32 / 223u32,
            crate::pac::rcc::vals::Plln::MUL224 => self * 1u32 / 224u32,
            crate::pac::rcc::vals::Plln::MUL225 => self * 1u32 / 225u32,
            crate::pac::rcc::vals::Plln::MUL226 => self * 1u32 / 226u32,
            crate::pac::rcc::vals::Plln::MUL227 => self * 1u32 / 227u32,
            crate::pac::rcc::vals::Plln::MUL228 => self * 1u32 / 228u32,
            crate::pac::rcc::vals::Plln::MUL229 => self * 1u32 / 229u32,
            crate::pac::rcc::vals::Plln::MUL230 => self * 1u32 / 230u32,
            crate::pac::rcc::vals::Plln::MUL231 => self * 1u32 / 231u32,
            crate::pac::rcc::vals::Plln::MUL232 => self * 1u32 / 232u32,
            crate::pac::rcc::vals::Plln::MUL233 => self * 1u32 / 233u32,
            crate::pac::rcc::vals::Plln::MUL234 => self * 1u32 / 234u32,
            crate::pac::rcc::vals::Plln::MUL235 => self * 1u32 / 235u32,
            crate::pac::rcc::vals::Plln::MUL236 => self * 1u32 / 236u32,
            crate::pac::rcc::vals::Plln::MUL237 => self * 1u32 / 237u32,
            crate::pac::rcc::vals::Plln::MUL238 => self * 1u32 / 238u32,
            crate::pac::rcc::vals::Plln::MUL239 => self * 1u32 / 239u32,
            crate::pac::rcc::vals::Plln::MUL240 => self * 1u32 / 240u32,
            crate::pac::rcc::vals::Plln::MUL241 => self * 1u32 / 241u32,
            crate::pac::rcc::vals::Plln::MUL242 => self * 1u32 / 242u32,
            crate::pac::rcc::vals::Plln::MUL243 => self * 1u32 / 243u32,
            crate::pac::rcc::vals::Plln::MUL244 => self * 1u32 / 244u32,
            crate::pac::rcc::vals::Plln::MUL245 => self * 1u32 / 245u32,
            crate::pac::rcc::vals::Plln::MUL246 => self * 1u32 / 246u32,
            crate::pac::rcc::vals::Plln::MUL247 => self * 1u32 / 247u32,
            crate::pac::rcc::vals::Plln::MUL248 => self * 1u32 / 248u32,
            crate::pac::rcc::vals::Plln::MUL249 => self * 1u32 / 249u32,
            crate::pac::rcc::vals::Plln::MUL250 => self * 1u32 / 250u32,
            crate::pac::rcc::vals::Plln::MUL251 => self * 1u32 / 251u32,
            crate::pac::rcc::vals::Plln::MUL252 => self * 1u32 / 252u32,
            crate::pac::rcc::vals::Plln::MUL253 => self * 1u32 / 253u32,
            crate::pac::rcc::vals::Plln::MUL254 => self * 1u32 / 254u32,
            crate::pac::rcc::vals::Plln::MUL255 => self * 1u32 / 255u32,
            crate::pac::rcc::vals::Plln::MUL256 => self * 1u32 / 256u32,
            crate::pac::rcc::vals::Plln::MUL257 => self * 1u32 / 257u32,
            crate::pac::rcc::vals::Plln::MUL258 => self * 1u32 / 258u32,
            crate::pac::rcc::vals::Plln::MUL259 => self * 1u32 / 259u32,
            crate::pac::rcc::vals::Plln::MUL260 => self * 1u32 / 260u32,
            crate::pac::rcc::vals::Plln::MUL261 => self * 1u32 / 261u32,
            crate::pac::rcc::vals::Plln::MUL262 => self * 1u32 / 262u32,
            crate::pac::rcc::vals::Plln::MUL263 => self * 1u32 / 263u32,
            crate::pac::rcc::vals::Plln::MUL264 => self * 1u32 / 264u32,
            crate::pac::rcc::vals::Plln::MUL265 => self * 1u32 / 265u32,
            crate::pac::rcc::vals::Plln::MUL266 => self * 1u32 / 266u32,
            crate::pac::rcc::vals::Plln::MUL267 => self * 1u32 / 267u32,
            crate::pac::rcc::vals::Plln::MUL268 => self * 1u32 / 268u32,
            crate::pac::rcc::vals::Plln::MUL269 => self * 1u32 / 269u32,
            crate::pac::rcc::vals::Plln::MUL270 => self * 1u32 / 270u32,
            crate::pac::rcc::vals::Plln::MUL271 => self * 1u32 / 271u32,
            crate::pac::rcc::vals::Plln::MUL272 => self * 1u32 / 272u32,
            crate::pac::rcc::vals::Plln::MUL273 => self * 1u32 / 273u32,
            crate::pac::rcc::vals::Plln::MUL274 => self * 1u32 / 274u32,
            crate::pac::rcc::vals::Plln::MUL275 => self * 1u32 / 275u32,
            crate::pac::rcc::vals::Plln::MUL276 => self * 1u32 / 276u32,
            crate::pac::rcc::vals::Plln::MUL277 => self * 1u32 / 277u32,
            crate::pac::rcc::vals::Plln::MUL278 => self * 1u32 / 278u32,
            crate::pac::rcc::vals::Plln::MUL279 => self * 1u32 / 279u32,
            crate::pac::rcc::vals::Plln::MUL280 => self * 1u32 / 280u32,
            crate::pac::rcc::vals::Plln::MUL281 => self * 1u32 / 281u32,
            crate::pac::rcc::vals::Plln::MUL282 => self * 1u32 / 282u32,
            crate::pac::rcc::vals::Plln::MUL283 => self * 1u32 / 283u32,
            crate::pac::rcc::vals::Plln::MUL284 => self * 1u32 / 284u32,
            crate::pac::rcc::vals::Plln::MUL285 => self * 1u32 / 285u32,
            crate::pac::rcc::vals::Plln::MUL286 => self * 1u32 / 286u32,
            crate::pac::rcc::vals::Plln::MUL287 => self * 1u32 / 287u32,
            crate::pac::rcc::vals::Plln::MUL288 => self * 1u32 / 288u32,
            crate::pac::rcc::vals::Plln::MUL289 => self * 1u32 / 289u32,
            crate::pac::rcc::vals::Plln::MUL290 => self * 1u32 / 290u32,
            crate::pac::rcc::vals::Plln::MUL291 => self * 1u32 / 291u32,
            crate::pac::rcc::vals::Plln::MUL292 => self * 1u32 / 292u32,
            crate::pac::rcc::vals::Plln::MUL293 => self * 1u32 / 293u32,
            crate::pac::rcc::vals::Plln::MUL294 => self * 1u32 / 294u32,
            crate::pac::rcc::vals::Plln::MUL295 => self * 1u32 / 295u32,
            crate::pac::rcc::vals::Plln::MUL296 => self * 1u32 / 296u32,
            crate::pac::rcc::vals::Plln::MUL297 => self * 1u32 / 297u32,
            crate::pac::rcc::vals::Plln::MUL298 => self * 1u32 / 298u32,
            crate::pac::rcc::vals::Plln::MUL299 => self * 1u32 / 299u32,
            crate::pac::rcc::vals::Plln::MUL300 => self * 1u32 / 300u32,
            crate::pac::rcc::vals::Plln::MUL301 => self * 1u32 / 301u32,
            crate::pac::rcc::vals::Plln::MUL302 => self * 1u32 / 302u32,
            crate::pac::rcc::vals::Plln::MUL303 => self * 1u32 / 303u32,
            crate::pac::rcc::vals::Plln::MUL304 => self * 1u32 / 304u32,
            crate::pac::rcc::vals::Plln::MUL305 => self * 1u32 / 305u32,
            crate::pac::rcc::vals::Plln::MUL306 => self * 1u32 / 306u32,
            crate::pac::rcc::vals::Plln::MUL307 => self * 1u32 / 307u32,
            crate::pac::rcc::vals::Plln::MUL308 => self * 1u32 / 308u32,
            crate::pac::rcc::vals::Plln::MUL309 => self * 1u32 / 309u32,
            crate::pac::rcc::vals::Plln::MUL310 => self * 1u32 / 310u32,
            crate::pac::rcc::vals::Plln::MUL311 => self * 1u32 / 311u32,
            crate::pac::rcc::vals::Plln::MUL312 => self * 1u32 / 312u32,
            crate::pac::rcc::vals::Plln::MUL313 => self * 1u32 / 313u32,
            crate::pac::rcc::vals::Plln::MUL314 => self * 1u32 / 314u32,
            crate::pac::rcc::vals::Plln::MUL315 => self * 1u32 / 315u32,
            crate::pac::rcc::vals::Plln::MUL316 => self * 1u32 / 316u32,
            crate::pac::rcc::vals::Plln::MUL317 => self * 1u32 / 317u32,
            crate::pac::rcc::vals::Plln::MUL318 => self * 1u32 / 318u32,
            crate::pac::rcc::vals::Plln::MUL319 => self * 1u32 / 319u32,
            crate::pac::rcc::vals::Plln::MUL320 => self * 1u32 / 320u32,
            crate::pac::rcc::vals::Plln::MUL321 => self * 1u32 / 321u32,
            crate::pac::rcc::vals::Plln::MUL322 => self * 1u32 / 322u32,
            crate::pac::rcc::vals::Plln::MUL323 => self * 1u32 / 323u32,
            crate::pac::rcc::vals::Plln::MUL324 => self * 1u32 / 324u32,
            crate::pac::rcc::vals::Plln::MUL325 => self * 1u32 / 325u32,
            crate::pac::rcc::vals::Plln::MUL326 => self * 1u32 / 326u32,
            crate::pac::rcc::vals::Plln::MUL327 => self * 1u32 / 327u32,
            crate::pac::rcc::vals::Plln::MUL328 => self * 1u32 / 328u32,
            crate::pac::rcc::vals::Plln::MUL329 => self * 1u32 / 329u32,
            crate::pac::rcc::vals::Plln::MUL330 => self * 1u32 / 330u32,
            crate::pac::rcc::vals::Plln::MUL331 => self * 1u32 / 331u32,
            crate::pac::rcc::vals::Plln::MUL332 => self * 1u32 / 332u32,
            crate::pac::rcc::vals::Plln::MUL333 => self * 1u32 / 333u32,
            crate::pac::rcc::vals::Plln::MUL334 => self * 1u32 / 334u32,
            crate::pac::rcc::vals::Plln::MUL335 => self * 1u32 / 335u32,
            crate::pac::rcc::vals::Plln::MUL336 => self * 1u32 / 336u32,
            crate::pac::rcc::vals::Plln::MUL337 => self * 1u32 / 337u32,
            crate::pac::rcc::vals::Plln::MUL338 => self * 1u32 / 338u32,
            crate::pac::rcc::vals::Plln::MUL339 => self * 1u32 / 339u32,
            crate::pac::rcc::vals::Plln::MUL340 => self * 1u32 / 340u32,
            crate::pac::rcc::vals::Plln::MUL341 => self * 1u32 / 341u32,
            crate::pac::rcc::vals::Plln::MUL342 => self * 1u32 / 342u32,
            crate::pac::rcc::vals::Plln::MUL343 => self * 1u32 / 343u32,
            crate::pac::rcc::vals::Plln::MUL344 => self * 1u32 / 344u32,
            crate::pac::rcc::vals::Plln::MUL345 => self * 1u32 / 345u32,
            crate::pac::rcc::vals::Plln::MUL346 => self * 1u32 / 346u32,
            crate::pac::rcc::vals::Plln::MUL347 => self * 1u32 / 347u32,
            crate::pac::rcc::vals::Plln::MUL348 => self * 1u32 / 348u32,
            crate::pac::rcc::vals::Plln::MUL349 => self * 1u32 / 349u32,
            crate::pac::rcc::vals::Plln::MUL350 => self * 1u32 / 350u32,
            crate::pac::rcc::vals::Plln::MUL351 => self * 1u32 / 351u32,
            crate::pac::rcc::vals::Plln::MUL352 => self * 1u32 / 352u32,
            crate::pac::rcc::vals::Plln::MUL353 => self * 1u32 / 353u32,
            crate::pac::rcc::vals::Plln::MUL354 => self * 1u32 / 354u32,
            crate::pac::rcc::vals::Plln::MUL355 => self * 1u32 / 355u32,
            crate::pac::rcc::vals::Plln::MUL356 => self * 1u32 / 356u32,
            crate::pac::rcc::vals::Plln::MUL357 => self * 1u32 / 357u32,
            crate::pac::rcc::vals::Plln::MUL358 => self * 1u32 / 358u32,
            crate::pac::rcc::vals::Plln::MUL359 => self * 1u32 / 359u32,
            crate::pac::rcc::vals::Plln::MUL360 => self * 1u32 / 360u32,
            crate::pac::rcc::vals::Plln::MUL361 => self * 1u32 / 361u32,
            crate::pac::rcc::vals::Plln::MUL362 => self * 1u32 / 362u32,
            crate::pac::rcc::vals::Plln::MUL363 => self * 1u32 / 363u32,
            crate::pac::rcc::vals::Plln::MUL364 => self * 1u32 / 364u32,
            crate::pac::rcc::vals::Plln::MUL365 => self * 1u32 / 365u32,
            crate::pac::rcc::vals::Plln::MUL366 => self * 1u32 / 366u32,
            crate::pac::rcc::vals::Plln::MUL367 => self * 1u32 / 367u32,
            crate::pac::rcc::vals::Plln::MUL368 => self * 1u32 / 368u32,
            crate::pac::rcc::vals::Plln::MUL369 => self * 1u32 / 369u32,
            crate::pac::rcc::vals::Plln::MUL370 => self * 1u32 / 370u32,
            crate::pac::rcc::vals::Plln::MUL371 => self * 1u32 / 371u32,
            crate::pac::rcc::vals::Plln::MUL372 => self * 1u32 / 372u32,
            crate::pac::rcc::vals::Plln::MUL373 => self * 1u32 / 373u32,
            crate::pac::rcc::vals::Plln::MUL374 => self * 1u32 / 374u32,
            crate::pac::rcc::vals::Plln::MUL375 => self * 1u32 / 375u32,
            crate::pac::rcc::vals::Plln::MUL376 => self * 1u32 / 376u32,
            crate::pac::rcc::vals::Plln::MUL377 => self * 1u32 / 377u32,
            crate::pac::rcc::vals::Plln::MUL378 => self * 1u32 / 378u32,
            crate::pac::rcc::vals::Plln::MUL379 => self * 1u32 / 379u32,
            crate::pac::rcc::vals::Plln::MUL380 => self * 1u32 / 380u32,
            crate::pac::rcc::vals::Plln::MUL381 => self * 1u32 / 381u32,
            crate::pac::rcc::vals::Plln::MUL382 => self * 1u32 / 382u32,
            crate::pac::rcc::vals::Plln::MUL383 => self * 1u32 / 383u32,
            crate::pac::rcc::vals::Plln::MUL384 => self * 1u32 / 384u32,
            crate::pac::rcc::vals::Plln::MUL385 => self * 1u32 / 385u32,
            crate::pac::rcc::vals::Plln::MUL386 => self * 1u32 / 386u32,
            crate::pac::rcc::vals::Plln::MUL387 => self * 1u32 / 387u32,
            crate::pac::rcc::vals::Plln::MUL388 => self * 1u32 / 388u32,
            crate::pac::rcc::vals::Plln::MUL389 => self * 1u32 / 389u32,
            crate::pac::rcc::vals::Plln::MUL390 => self * 1u32 / 390u32,
            crate::pac::rcc::vals::Plln::MUL391 => self * 1u32 / 391u32,
            crate::pac::rcc::vals::Plln::MUL392 => self * 1u32 / 392u32,
            crate::pac::rcc::vals::Plln::MUL393 => self * 1u32 / 393u32,
            crate::pac::rcc::vals::Plln::MUL394 => self * 1u32 / 394u32,
            crate::pac::rcc::vals::Plln::MUL395 => self * 1u32 / 395u32,
            crate::pac::rcc::vals::Plln::MUL396 => self * 1u32 / 396u32,
            crate::pac::rcc::vals::Plln::MUL397 => self * 1u32 / 397u32,
            crate::pac::rcc::vals::Plln::MUL398 => self * 1u32 / 398u32,
            crate::pac::rcc::vals::Plln::MUL399 => self * 1u32 / 399u32,
            crate::pac::rcc::vals::Plln::MUL400 => self * 1u32 / 400u32,
            crate::pac::rcc::vals::Plln::MUL401 => self * 1u32 / 401u32,
            crate::pac::rcc::vals::Plln::MUL402 => self * 1u32 / 402u32,
            crate::pac::rcc::vals::Plln::MUL403 => self * 1u32 / 403u32,
            crate::pac::rcc::vals::Plln::MUL404 => self * 1u32 / 404u32,
            crate::pac::rcc::vals::Plln::MUL405 => self * 1u32 / 405u32,
            crate::pac::rcc::vals::Plln::MUL406 => self * 1u32 / 406u32,
            crate::pac::rcc::vals::Plln::MUL407 => self * 1u32 / 407u32,
            crate::pac::rcc::vals::Plln::MUL408 => self * 1u32 / 408u32,
            crate::pac::rcc::vals::Plln::MUL409 => self * 1u32 / 409u32,
            crate::pac::rcc::vals::Plln::MUL410 => self * 1u32 / 410u32,
            crate::pac::rcc::vals::Plln::MUL411 => self * 1u32 / 411u32,
            crate::pac::rcc::vals::Plln::MUL412 => self * 1u32 / 412u32,
            crate::pac::rcc::vals::Plln::MUL413 => self * 1u32 / 413u32,
            crate::pac::rcc::vals::Plln::MUL414 => self * 1u32 / 414u32,
            crate::pac::rcc::vals::Plln::MUL415 => self * 1u32 / 415u32,
            crate::pac::rcc::vals::Plln::MUL416 => self * 1u32 / 416u32,
            crate::pac::rcc::vals::Plln::MUL417 => self * 1u32 / 417u32,
            crate::pac::rcc::vals::Plln::MUL418 => self * 1u32 / 418u32,
            crate::pac::rcc::vals::Plln::MUL419 => self * 1u32 / 419u32,
            crate::pac::rcc::vals::Plln::MUL420 => self * 1u32 / 420u32,
            crate::pac::rcc::vals::Plln::MUL421 => self * 1u32 / 421u32,
            crate::pac::rcc::vals::Plln::MUL422 => self * 1u32 / 422u32,
            crate::pac::rcc::vals::Plln::MUL423 => self * 1u32 / 423u32,
            crate::pac::rcc::vals::Plln::MUL424 => self * 1u32 / 424u32,
            crate::pac::rcc::vals::Plln::MUL425 => self * 1u32 / 425u32,
            crate::pac::rcc::vals::Plln::MUL426 => self * 1u32 / 426u32,
            crate::pac::rcc::vals::Plln::MUL427 => self * 1u32 / 427u32,
            crate::pac::rcc::vals::Plln::MUL428 => self * 1u32 / 428u32,
            crate::pac::rcc::vals::Plln::MUL429 => self * 1u32 / 429u32,
            crate::pac::rcc::vals::Plln::MUL430 => self * 1u32 / 430u32,
            crate::pac::rcc::vals::Plln::MUL431 => self * 1u32 / 431u32,
            crate::pac::rcc::vals::Plln::MUL432 => self * 1u32 / 432u32,
            crate::pac::rcc::vals::Plln::MUL433 => self * 1u32 / 433u32,
            crate::pac::rcc::vals::Plln::MUL434 => self * 1u32 / 434u32,
            crate::pac::rcc::vals::Plln::MUL435 => self * 1u32 / 435u32,
            crate::pac::rcc::vals::Plln::MUL436 => self * 1u32 / 436u32,
            crate::pac::rcc::vals::Plln::MUL437 => self * 1u32 / 437u32,
            crate::pac::rcc::vals::Plln::MUL438 => self * 1u32 / 438u32,
            crate::pac::rcc::vals::Plln::MUL439 => self * 1u32 / 439u32,
            crate::pac::rcc::vals::Plln::MUL440 => self * 1u32 / 440u32,
            crate::pac::rcc::vals::Plln::MUL441 => self * 1u32 / 441u32,
            crate::pac::rcc::vals::Plln::MUL442 => self * 1u32 / 442u32,
            crate::pac::rcc::vals::Plln::MUL443 => self * 1u32 / 443u32,
            crate::pac::rcc::vals::Plln::MUL444 => self * 1u32 / 444u32,
            crate::pac::rcc::vals::Plln::MUL445 => self * 1u32 / 445u32,
            crate::pac::rcc::vals::Plln::MUL446 => self * 1u32 / 446u32,
            crate::pac::rcc::vals::Plln::MUL447 => self * 1u32 / 447u32,
            crate::pac::rcc::vals::Plln::MUL448 => self * 1u32 / 448u32,
            crate::pac::rcc::vals::Plln::MUL449 => self * 1u32 / 449u32,
            crate::pac::rcc::vals::Plln::MUL450 => self * 1u32 / 450u32,
            crate::pac::rcc::vals::Plln::MUL451 => self * 1u32 / 451u32,
            crate::pac::rcc::vals::Plln::MUL452 => self * 1u32 / 452u32,
            crate::pac::rcc::vals::Plln::MUL453 => self * 1u32 / 453u32,
            crate::pac::rcc::vals::Plln::MUL454 => self * 1u32 / 454u32,
            crate::pac::rcc::vals::Plln::MUL455 => self * 1u32 / 455u32,
            crate::pac::rcc::vals::Plln::MUL456 => self * 1u32 / 456u32,
            crate::pac::rcc::vals::Plln::MUL457 => self * 1u32 / 457u32,
            crate::pac::rcc::vals::Plln::MUL458 => self * 1u32 / 458u32,
            crate::pac::rcc::vals::Plln::MUL459 => self * 1u32 / 459u32,
            crate::pac::rcc::vals::Plln::MUL460 => self * 1u32 / 460u32,
            crate::pac::rcc::vals::Plln::MUL461 => self * 1u32 / 461u32,
            crate::pac::rcc::vals::Plln::MUL462 => self * 1u32 / 462u32,
            crate::pac::rcc::vals::Plln::MUL463 => self * 1u32 / 463u32,
            crate::pac::rcc::vals::Plln::MUL464 => self * 1u32 / 464u32,
            crate::pac::rcc::vals::Plln::MUL465 => self * 1u32 / 465u32,
            crate::pac::rcc::vals::Plln::MUL466 => self * 1u32 / 466u32,
            crate::pac::rcc::vals::Plln::MUL467 => self * 1u32 / 467u32,
            crate::pac::rcc::vals::Plln::MUL468 => self * 1u32 / 468u32,
            crate::pac::rcc::vals::Plln::MUL469 => self * 1u32 / 469u32,
            crate::pac::rcc::vals::Plln::MUL470 => self * 1u32 / 470u32,
            crate::pac::rcc::vals::Plln::MUL471 => self * 1u32 / 471u32,
            crate::pac::rcc::vals::Plln::MUL472 => self * 1u32 / 472u32,
            crate::pac::rcc::vals::Plln::MUL473 => self * 1u32 / 473u32,
            crate::pac::rcc::vals::Plln::MUL474 => self * 1u32 / 474u32,
            crate::pac::rcc::vals::Plln::MUL475 => self * 1u32 / 475u32,
            crate::pac::rcc::vals::Plln::MUL476 => self * 1u32 / 476u32,
            crate::pac::rcc::vals::Plln::MUL477 => self * 1u32 / 477u32,
            crate::pac::rcc::vals::Plln::MUL478 => self * 1u32 / 478u32,
            crate::pac::rcc::vals::Plln::MUL479 => self * 1u32 / 479u32,
            crate::pac::rcc::vals::Plln::MUL480 => self * 1u32 / 480u32,
            crate::pac::rcc::vals::Plln::MUL481 => self * 1u32 / 481u32,
            crate::pac::rcc::vals::Plln::MUL482 => self * 1u32 / 482u32,
            crate::pac::rcc::vals::Plln::MUL483 => self * 1u32 / 483u32,
            crate::pac::rcc::vals::Plln::MUL484 => self * 1u32 / 484u32,
            crate::pac::rcc::vals::Plln::MUL485 => self * 1u32 / 485u32,
            crate::pac::rcc::vals::Plln::MUL486 => self * 1u32 / 486u32,
            crate::pac::rcc::vals::Plln::MUL487 => self * 1u32 / 487u32,
            crate::pac::rcc::vals::Plln::MUL488 => self * 1u32 / 488u32,
            crate::pac::rcc::vals::Plln::MUL489 => self * 1u32 / 489u32,
            crate::pac::rcc::vals::Plln::MUL490 => self * 1u32 / 490u32,
            crate::pac::rcc::vals::Plln::MUL491 => self * 1u32 / 491u32,
            crate::pac::rcc::vals::Plln::MUL492 => self * 1u32 / 492u32,
            crate::pac::rcc::vals::Plln::MUL493 => self * 1u32 / 493u32,
            crate::pac::rcc::vals::Plln::MUL494 => self * 1u32 / 494u32,
            crate::pac::rcc::vals::Plln::MUL495 => self * 1u32 / 495u32,
            crate::pac::rcc::vals::Plln::MUL496 => self * 1u32 / 496u32,
            crate::pac::rcc::vals::Plln::MUL497 => self * 1u32 / 497u32,
            crate::pac::rcc::vals::Plln::MUL498 => self * 1u32 / 498u32,
            crate::pac::rcc::vals::Plln::MUL499 => self * 1u32 / 499u32,
            crate::pac::rcc::vals::Plln::MUL500 => self * 1u32 / 500u32,
            crate::pac::rcc::vals::Plln::MUL501 => self * 1u32 / 501u32,
            crate::pac::rcc::vals::Plln::MUL502 => self * 1u32 / 502u32,
            crate::pac::rcc::vals::Plln::MUL503 => self * 1u32 / 503u32,
            crate::pac::rcc::vals::Plln::MUL504 => self * 1u32 / 504u32,
            crate::pac::rcc::vals::Plln::MUL505 => self * 1u32 / 505u32,
            crate::pac::rcc::vals::Plln::MUL506 => self * 1u32 / 506u32,
            crate::pac::rcc::vals::Plln::MUL507 => self * 1u32 / 507u32,
            crate::pac::rcc::vals::Plln::MUL508 => self * 1u32 / 508u32,
            crate::pac::rcc::vals::Plln::MUL509 => self * 1u32 / 509u32,
            crate::pac::rcc::vals::Plln::MUL510 => self * 1u32 / 510u32,
            crate::pac::rcc::vals::Plln::MUL511 => self * 1u32 / 511u32,
            crate::pac::rcc::vals::Plln::MUL512 => self * 1u32 / 512u32,
            #[allow(unreachable_patterns)]
            _ => unreachable!(),
        }
    }
}
impl core::ops::Mul<crate::pac::rcc::vals::Plln> for crate::time::Hertz {
    type Output = crate::time::Hertz;
    fn mul(self, rhs: crate::pac::rcc::vals::Plln) -> Self::Output {
        match rhs {
            crate::pac::rcc::vals::Plln::MUL4 => self * 4u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL5 => self * 5u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL6 => self * 6u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL7 => self * 7u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL8 => self * 8u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL9 => self * 9u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL10 => self * 10u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL11 => self * 11u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL12 => self * 12u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL13 => self * 13u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL14 => self * 14u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL15 => self * 15u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL16 => self * 16u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL17 => self * 17u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL18 => self * 18u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL19 => self * 19u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL20 => self * 20u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL21 => self * 21u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL22 => self * 22u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL23 => self * 23u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL24 => self * 24u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL25 => self * 25u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL26 => self * 26u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL27 => self * 27u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL28 => self * 28u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL29 => self * 29u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL30 => self * 30u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL31 => self * 31u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL32 => self * 32u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL33 => self * 33u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL34 => self * 34u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL35 => self * 35u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL36 => self * 36u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL37 => self * 37u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL38 => self * 38u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL39 => self * 39u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL40 => self * 40u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL41 => self * 41u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL42 => self * 42u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL43 => self * 43u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL44 => self * 44u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL45 => self * 45u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL46 => self * 46u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL47 => self * 47u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL48 => self * 48u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL49 => self * 49u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL50 => self * 50u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL51 => self * 51u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL52 => self * 52u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL53 => self * 53u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL54 => self * 54u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL55 => self * 55u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL56 => self * 56u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL57 => self * 57u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL58 => self * 58u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL59 => self * 59u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL60 => self * 60u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL61 => self * 61u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL62 => self * 62u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL63 => self * 63u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL64 => self * 64u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL65 => self * 65u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL66 => self * 66u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL67 => self * 67u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL68 => self * 68u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL69 => self * 69u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL70 => self * 70u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL71 => self * 71u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL72 => self * 72u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL73 => self * 73u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL74 => self * 74u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL75 => self * 75u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL76 => self * 76u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL77 => self * 77u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL78 => self * 78u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL79 => self * 79u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL80 => self * 80u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL81 => self * 81u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL82 => self * 82u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL83 => self * 83u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL84 => self * 84u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL85 => self * 85u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL86 => self * 86u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL87 => self * 87u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL88 => self * 88u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL89 => self * 89u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL90 => self * 90u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL91 => self * 91u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL92 => self * 92u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL93 => self * 93u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL94 => self * 94u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL95 => self * 95u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL96 => self * 96u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL97 => self * 97u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL98 => self * 98u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL99 => self * 99u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL100 => self * 100u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL101 => self * 101u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL102 => self * 102u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL103 => self * 103u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL104 => self * 104u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL105 => self * 105u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL106 => self * 106u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL107 => self * 107u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL108 => self * 108u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL109 => self * 109u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL110 => self * 110u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL111 => self * 111u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL112 => self * 112u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL113 => self * 113u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL114 => self * 114u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL115 => self * 115u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL116 => self * 116u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL117 => self * 117u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL118 => self * 118u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL119 => self * 119u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL120 => self * 120u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL121 => self * 121u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL122 => self * 122u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL123 => self * 123u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL124 => self * 124u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL125 => self * 125u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL126 => self * 126u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL127 => self * 127u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL128 => self * 128u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL129 => self * 129u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL130 => self * 130u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL131 => self * 131u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL132 => self * 132u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL133 => self * 133u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL134 => self * 134u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL135 => self * 135u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL136 => self * 136u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL137 => self * 137u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL138 => self * 138u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL139 => self * 139u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL140 => self * 140u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL141 => self * 141u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL142 => self * 142u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL143 => self * 143u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL144 => self * 144u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL145 => self * 145u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL146 => self * 146u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL147 => self * 147u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL148 => self * 148u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL149 => self * 149u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL150 => self * 150u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL151 => self * 151u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL152 => self * 152u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL153 => self * 153u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL154 => self * 154u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL155 => self * 155u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL156 => self * 156u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL157 => self * 157u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL158 => self * 158u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL159 => self * 159u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL160 => self * 160u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL161 => self * 161u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL162 => self * 162u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL163 => self * 163u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL164 => self * 164u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL165 => self * 165u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL166 => self * 166u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL167 => self * 167u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL168 => self * 168u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL169 => self * 169u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL170 => self * 170u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL171 => self * 171u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL172 => self * 172u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL173 => self * 173u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL174 => self * 174u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL175 => self * 175u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL176 => self * 176u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL177 => self * 177u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL178 => self * 178u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL179 => self * 179u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL180 => self * 180u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL181 => self * 181u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL182 => self * 182u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL183 => self * 183u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL184 => self * 184u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL185 => self * 185u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL186 => self * 186u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL187 => self * 187u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL188 => self * 188u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL189 => self * 189u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL190 => self * 190u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL191 => self * 191u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL192 => self * 192u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL193 => self * 193u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL194 => self * 194u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL195 => self * 195u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL196 => self * 196u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL197 => self * 197u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL198 => self * 198u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL199 => self * 199u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL200 => self * 200u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL201 => self * 201u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL202 => self * 202u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL203 => self * 203u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL204 => self * 204u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL205 => self * 205u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL206 => self * 206u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL207 => self * 207u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL208 => self * 208u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL209 => self * 209u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL210 => self * 210u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL211 => self * 211u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL212 => self * 212u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL213 => self * 213u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL214 => self * 214u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL215 => self * 215u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL216 => self * 216u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL217 => self * 217u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL218 => self * 218u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL219 => self * 219u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL220 => self * 220u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL221 => self * 221u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL222 => self * 222u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL223 => self * 223u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL224 => self * 224u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL225 => self * 225u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL226 => self * 226u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL227 => self * 227u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL228 => self * 228u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL229 => self * 229u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL230 => self * 230u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL231 => self * 231u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL232 => self * 232u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL233 => self * 233u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL234 => self * 234u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL235 => self * 235u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL236 => self * 236u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL237 => self * 237u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL238 => self * 238u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL239 => self * 239u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL240 => self * 240u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL241 => self * 241u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL242 => self * 242u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL243 => self * 243u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL244 => self * 244u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL245 => self * 245u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL246 => self * 246u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL247 => self * 247u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL248 => self * 248u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL249 => self * 249u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL250 => self * 250u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL251 => self * 251u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL252 => self * 252u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL253 => self * 253u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL254 => self * 254u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL255 => self * 255u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL256 => self * 256u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL257 => self * 257u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL258 => self * 258u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL259 => self * 259u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL260 => self * 260u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL261 => self * 261u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL262 => self * 262u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL263 => self * 263u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL264 => self * 264u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL265 => self * 265u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL266 => self * 266u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL267 => self * 267u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL268 => self * 268u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL269 => self * 269u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL270 => self * 270u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL271 => self * 271u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL272 => self * 272u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL273 => self * 273u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL274 => self * 274u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL275 => self * 275u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL276 => self * 276u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL277 => self * 277u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL278 => self * 278u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL279 => self * 279u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL280 => self * 280u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL281 => self * 281u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL282 => self * 282u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL283 => self * 283u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL284 => self * 284u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL285 => self * 285u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL286 => self * 286u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL287 => self * 287u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL288 => self * 288u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL289 => self * 289u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL290 => self * 290u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL291 => self * 291u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL292 => self * 292u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL293 => self * 293u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL294 => self * 294u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL295 => self * 295u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL296 => self * 296u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL297 => self * 297u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL298 => self * 298u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL299 => self * 299u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL300 => self * 300u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL301 => self * 301u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL302 => self * 302u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL303 => self * 303u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL304 => self * 304u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL305 => self * 305u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL306 => self * 306u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL307 => self * 307u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL308 => self * 308u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL309 => self * 309u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL310 => self * 310u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL311 => self * 311u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL312 => self * 312u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL313 => self * 313u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL314 => self * 314u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL315 => self * 315u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL316 => self * 316u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL317 => self * 317u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL318 => self * 318u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL319 => self * 319u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL320 => self * 320u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL321 => self * 321u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL322 => self * 322u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL323 => self * 323u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL324 => self * 324u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL325 => self * 325u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL326 => self * 326u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL327 => self * 327u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL328 => self * 328u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL329 => self * 329u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL330 => self * 330u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL331 => self * 331u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL332 => self * 332u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL333 => self * 333u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL334 => self * 334u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL335 => self * 335u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL336 => self * 336u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL337 => self * 337u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL338 => self * 338u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL339 => self * 339u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL340 => self * 340u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL341 => self * 341u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL342 => self * 342u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL343 => self * 343u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL344 => self * 344u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL345 => self * 345u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL346 => self * 346u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL347 => self * 347u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL348 => self * 348u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL349 => self * 349u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL350 => self * 350u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL351 => self * 351u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL352 => self * 352u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL353 => self * 353u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL354 => self * 354u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL355 => self * 355u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL356 => self * 356u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL357 => self * 357u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL358 => self * 358u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL359 => self * 359u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL360 => self * 360u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL361 => self * 361u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL362 => self * 362u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL363 => self * 363u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL364 => self * 364u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL365 => self * 365u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL366 => self * 366u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL367 => self * 367u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL368 => self * 368u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL369 => self * 369u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL370 => self * 370u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL371 => self * 371u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL372 => self * 372u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL373 => self * 373u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL374 => self * 374u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL375 => self * 375u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL376 => self * 376u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL377 => self * 377u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL378 => self * 378u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL379 => self * 379u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL380 => self * 380u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL381 => self * 381u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL382 => self * 382u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL383 => self * 383u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL384 => self * 384u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL385 => self * 385u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL386 => self * 386u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL387 => self * 387u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL388 => self * 388u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL389 => self * 389u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL390 => self * 390u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL391 => self * 391u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL392 => self * 392u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL393 => self * 393u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL394 => self * 394u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL395 => self * 395u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL396 => self * 396u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL397 => self * 397u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL398 => self * 398u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL399 => self * 399u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL400 => self * 400u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL401 => self * 401u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL402 => self * 402u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL403 => self * 403u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL404 => self * 404u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL405 => self * 405u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL406 => self * 406u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL407 => self * 407u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL408 => self * 408u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL409 => self * 409u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL410 => self * 410u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL411 => self * 411u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL412 => self * 412u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL413 => self * 413u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL414 => self * 414u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL415 => self * 415u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL416 => self * 416u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL417 => self * 417u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL418 => self * 418u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL419 => self * 419u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL420 => self * 420u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL421 => self * 421u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL422 => self * 422u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL423 => self * 423u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL424 => self * 424u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL425 => self * 425u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL426 => self * 426u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL427 => self * 427u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL428 => self * 428u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL429 => self * 429u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL430 => self * 430u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL431 => self * 431u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL432 => self * 432u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL433 => self * 433u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL434 => self * 434u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL435 => self * 435u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL436 => self * 436u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL437 => self * 437u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL438 => self * 438u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL439 => self * 439u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL440 => self * 440u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL441 => self * 441u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL442 => self * 442u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL443 => self * 443u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL444 => self * 444u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL445 => self * 445u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL446 => self * 446u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL447 => self * 447u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL448 => self * 448u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL449 => self * 449u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL450 => self * 450u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL451 => self * 451u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL452 => self * 452u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL453 => self * 453u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL454 => self * 454u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL455 => self * 455u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL456 => self * 456u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL457 => self * 457u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL458 => self * 458u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL459 => self * 459u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL460 => self * 460u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL461 => self * 461u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL462 => self * 462u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL463 => self * 463u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL464 => self * 464u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL465 => self * 465u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL466 => self * 466u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL467 => self * 467u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL468 => self * 468u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL469 => self * 469u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL470 => self * 470u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL471 => self * 471u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL472 => self * 472u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL473 => self * 473u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL474 => self * 474u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL475 => self * 475u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL476 => self * 476u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL477 => self * 477u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL478 => self * 478u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL479 => self * 479u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL480 => self * 480u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL481 => self * 481u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL482 => self * 482u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL483 => self * 483u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL484 => self * 484u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL485 => self * 485u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL486 => self * 486u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL487 => self * 487u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL488 => self * 488u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL489 => self * 489u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL490 => self * 490u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL491 => self * 491u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL492 => self * 492u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL493 => self * 493u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL494 => self * 494u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL495 => self * 495u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL496 => self * 496u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL497 => self * 497u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL498 => self * 498u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL499 => self * 499u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL500 => self * 500u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL501 => self * 501u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL502 => self * 502u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL503 => self * 503u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL504 => self * 504u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL505 => self * 505u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL506 => self * 506u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL507 => self * 507u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL508 => self * 508u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL509 => self * 509u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL510 => self * 510u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL511 => self * 511u32 / 1u32,
            crate::pac::rcc::vals::Plln::MUL512 => self * 512u32 / 1u32,
            #[allow(unreachable_patterns)]
            _ => unreachable!(),
        }
    }
}
impl core::ops::Div<crate::pac::rcc::vals::Ppre> for crate::time::Hertz {
    type Output = crate::time::Hertz;
    fn div(self, rhs: crate::pac::rcc::vals::Ppre) -> Self::Output {
        match rhs {
            crate::pac::rcc::vals::Ppre::DIV1 => self * 1u32 / 1u32,
            crate::pac::rcc::vals::Ppre::DIV2 => self * 1u32 / 2u32,
            crate::pac::rcc::vals::Ppre::DIV4 => self * 1u32 / 4u32,
            crate::pac::rcc::vals::Ppre::DIV8 => self * 1u32 / 8u32,
            crate::pac::rcc::vals::Ppre::DIV16 => self * 1u32 / 16u32,
            #[allow(unreachable_patterns)]
            _ => unreachable!(),
        }
    }
}
impl core::ops::Mul<crate::pac::rcc::vals::Ppre> for crate::time::Hertz {
    type Output = crate::time::Hertz;
    fn mul(self, rhs: crate::pac::rcc::vals::Ppre) -> Self::Output {
        match rhs {
            crate::pac::rcc::vals::Ppre::DIV1 => self * 1u32 / 1u32,
            crate::pac::rcc::vals::Ppre::DIV2 => self * 2u32 / 1u32,
            crate::pac::rcc::vals::Ppre::DIV4 => self * 4u32 / 1u32,
            crate::pac::rcc::vals::Ppre::DIV8 => self * 8u32 / 1u32,
            crate::pac::rcc::vals::Ppre::DIV16 => self * 16u32 / 1u32,
            #[allow(unreachable_patterns)]
            _ => unreachable!(),
        }
    }
}
#[allow(non_camel_case_types)]
pub mod peripheral_interrupts {
    pub mod ADC1 {
        pub type GLOBAL = crate::interrupt::typelevel::ADC;
    }
    pub mod ADC12_COMMON {}
    pub mod ADC2 {
        pub type GLOBAL = crate::interrupt::typelevel::ADC;
    }
    pub mod ADC3 {
        pub type GLOBAL = crate::interrupt::typelevel::ADC3;
    }
    pub mod ADC3_COMMON {}
    pub mod BDMA {
        pub type CH0 = crate::interrupt::typelevel::BDMA_CHANNEL0;
        pub type CH1 = crate::interrupt::typelevel::BDMA_CHANNEL1;
        pub type CH2 = crate::interrupt::typelevel::BDMA_CHANNEL2;
        pub type CH3 = crate::interrupt::typelevel::BDMA_CHANNEL3;
        pub type CH4 = crate::interrupt::typelevel::BDMA_CHANNEL4;
        pub type CH5 = crate::interrupt::typelevel::BDMA_CHANNEL5;
        pub type CH6 = crate::interrupt::typelevel::BDMA_CHANNEL6;
        pub type CH7 = crate::interrupt::typelevel::BDMA_CHANNEL7;
    }
    pub mod CEC {
        pub type GLOBAL = crate::interrupt::typelevel::CEC;
    }
    pub mod COMP1 {
        pub type WKUP = crate::interrupt::typelevel::COMP;
    }
    pub mod COMP2 {
        pub type WKUP = crate::interrupt::typelevel::COMP;
    }
    pub mod CORDIC {
        pub type GLOBAL = crate::interrupt::typelevel::CORDIC;
    }
    pub mod CRC {}
    pub mod CRS {}
    pub mod DAC1 {
        pub type GLOBAL = crate::interrupt::typelevel::TIM6_DAC;
    }
    pub mod DBGMCU {}
    pub mod DCMI {
        pub type GLOBAL = crate::interrupt::typelevel::DCMI_PSSI;
    }
    pub mod DFSDM1 {
        pub type FLT0 = crate::interrupt::typelevel::DFSDM1_FLT0;
        pub type FLT1 = crate::interrupt::typelevel::DFSDM1_FLT1;
        pub type FLT2 = crate::interrupt::typelevel::DFSDM1_FLT2;
        pub type FLT3 = crate::interrupt::typelevel::DFSDM1_FLT3;
    }
    pub mod DMA1 {
        pub type CH0 = crate::interrupt::typelevel::DMA1_STREAM0;
        pub type CH1 = crate::interrupt::typelevel::DMA1_STREAM1;
        pub type CH2 = crate::interrupt::typelevel::DMA1_STREAM2;
        pub type CH3 = crate::interrupt::typelevel::DMA1_STREAM3;
        pub type CH4 = crate::interrupt::typelevel::DMA1_STREAM4;
        pub type CH5 = crate::interrupt::typelevel::DMA1_STREAM5;
        pub type CH6 = crate::interrupt::typelevel::DMA1_STREAM6;
        pub type CH7 = crate::interrupt::typelevel::DMA1_STREAM7;
    }
    pub mod DMA2 {
        pub type CH0 = crate::interrupt::typelevel::DMA2_STREAM0;
        pub type CH1 = crate::interrupt::typelevel::DMA2_STREAM1;
        pub type CH2 = crate::interrupt::typelevel::DMA2_STREAM2;
        pub type CH3 = crate::interrupt::typelevel::DMA2_STREAM3;
        pub type CH4 = crate::interrupt::typelevel::DMA2_STREAM4;
        pub type CH5 = crate::interrupt::typelevel::DMA2_STREAM5;
        pub type CH6 = crate::interrupt::typelevel::DMA2_STREAM6;
        pub type CH7 = crate::interrupt::typelevel::DMA2_STREAM7;
    }
    pub mod DMA2D {
        pub type GLOBAL = crate::interrupt::typelevel::DMA2D;
    }
    pub mod DMAMUX1 {
        pub type OVR = crate::interrupt::typelevel::DMAMUX1_OVR;
    }
    pub mod DMAMUX2 {
        pub type OVR = crate::interrupt::typelevel::DMAMUX2_OVR;
    }
    pub mod DTS {
        pub type GLOBAL = crate::interrupt::typelevel::DTS;
    }
    pub mod ETH {
        pub type GLOBAL = crate::interrupt::typelevel::ETH;
        pub type WKUP = crate::interrupt::typelevel::ETH_WKUP;
    }
    pub mod EXTI {
        pub type EXTI0 = crate::interrupt::typelevel::EXTI0;
        pub type EXTI1 = crate::interrupt::typelevel::EXTI1;
        pub type EXTI10 = crate::interrupt::typelevel::EXTI15_10;
        pub type EXTI11 = crate::interrupt::typelevel::EXTI15_10;
        pub type EXTI12 = crate::interrupt::typelevel::EXTI15_10;
        pub type EXTI13 = crate::interrupt::typelevel::EXTI15_10;
        pub type EXTI14 = crate::interrupt::typelevel::EXTI15_10;
        pub type EXTI15 = crate::interrupt::typelevel::EXTI15_10;
        pub type EXTI2 = crate::interrupt::typelevel::EXTI2;
        pub type EXTI3 = crate::interrupt::typelevel::EXTI3;
        pub type EXTI4 = crate::interrupt::typelevel::EXTI4;
        pub type EXTI5 = crate::interrupt::typelevel::EXTI9_5;
        pub type EXTI6 = crate::interrupt::typelevel::EXTI9_5;
        pub type EXTI7 = crate::interrupt::typelevel::EXTI9_5;
        pub type EXTI8 = crate::interrupt::typelevel::EXTI9_5;
        pub type EXTI9 = crate::interrupt::typelevel::EXTI9_5;
    }
    pub mod FDCAN1 {
        pub type CAL = crate::interrupt::typelevel::FDCAN_CAL;
        pub type IT0 = crate::interrupt::typelevel::FDCAN1_IT0;
        pub type IT1 = crate::interrupt::typelevel::FDCAN1_IT1;
    }
    pub mod FDCAN2 {
        pub type CAL = crate::interrupt::typelevel::FDCAN_CAL;
        pub type IT0 = crate::interrupt::typelevel::FDCAN2_IT0;
        pub type IT1 = crate::interrupt::typelevel::FDCAN2_IT1;
    }
    pub mod FDCAN3 {
        pub type IT0 = crate::interrupt::typelevel::FDCAN3_IT0;
        pub type IT1 = crate::interrupt::typelevel::FDCAN3_IT1;
    }
    pub mod FDCANRAM {}
    pub mod FLASH {
        pub type GLOBAL = crate::interrupt::typelevel::FLASH;
    }
    pub mod FMAC {
        pub type GLOBAL = crate::interrupt::typelevel::FMAC;
    }
    pub mod FMC {
        pub type GLOBAL = crate::interrupt::typelevel::FMC;
    }
    pub mod GPIOA {}
    pub mod GPIOB {}
    pub mod GPIOC {}
    pub mod GPIOD {}
    pub mod GPIOE {}
    pub mod GPIOF {}
    pub mod GPIOG {}
    pub mod GPIOH {}
    pub mod GPIOJ {}
    pub mod GPIOK {}
    pub mod HSEM {}
    pub mod I2C1 {
        pub type ER = crate::interrupt::typelevel::I2C1_ER;
        pub type EV = crate::interrupt::typelevel::I2C1_EV;
    }
    pub mod I2C2 {
        pub type ER = crate::interrupt::typelevel::I2C2_ER;
        pub type EV = crate::interrupt::typelevel::I2C2_EV;
    }
    pub mod I2C3 {
        pub type ER = crate::interrupt::typelevel::I2C3_ER;
        pub type EV = crate::interrupt::typelevel::I2C3_EV;
    }
    pub mod I2C4 {
        pub type ER = crate::interrupt::typelevel::I2C4_ER;
        pub type EV = crate::interrupt::typelevel::I2C4_EV;
    }
    pub mod I2C5 {
        pub type ER = crate::interrupt::typelevel::I2C5_ER;
        pub type EV = crate::interrupt::typelevel::I2C5_EV;
    }
    pub mod IWDG1 {}
    pub mod LPTIM1 {
        pub type GLOBAL = crate::interrupt::typelevel::LPTIM1;
    }
    pub mod LPTIM2 {
        pub type GLOBAL = crate::interrupt::typelevel::LPTIM2;
    }
    pub mod LPTIM3 {
        pub type GLOBAL = crate::interrupt::typelevel::LPTIM3;
    }
    pub mod LPTIM4 {
        pub type GLOBAL = crate::interrupt::typelevel::LPTIM4;
    }
    pub mod LPTIM5 {
        pub type GLOBAL = crate::interrupt::typelevel::LPTIM5;
    }
    pub mod LPUART1 {
        pub type GLOBAL = crate::interrupt::typelevel::LPUART1;
    }
    pub mod LTDC {
        pub type ER = crate::interrupt::typelevel::LTDC_ER;
        pub type GLOBAL = crate::interrupt::typelevel::LTDC;
    }
    pub mod MDIOS {
        pub type GLOBAL = crate::interrupt::typelevel::MDIOS;
        pub type WKUP = crate::interrupt::typelevel::MDIOS_WKUP;
    }
    pub mod MDMA {
        pub type GLOBAL = crate::interrupt::typelevel::MDMA;
    }
    pub mod OCTOSPI1 {
        pub type GLOBAL = crate::interrupt::typelevel::OCTOSPI1;
    }
    pub mod OCTOSPI2 {
        pub type GLOBAL = crate::interrupt::typelevel::OCTOSPI2;
    }
    pub mod OCTOSPIM {}
    pub mod OPAMP1 {}
    pub mod OPAMP2 {}
    pub mod PSSI {
        pub type GLOBAL = crate::interrupt::typelevel::DCMI_PSSI;
    }
    pub mod PWR {}
    pub mod RCC {
        pub type CRS = crate::interrupt::typelevel::CRS;
        pub type GLOBAL = crate::interrupt::typelevel::RCC;
    }
    pub mod RNG {
        pub type GLOBAL = crate::interrupt::typelevel::RNG;
    }
    pub mod RTC {
        pub type ALARM = crate::interrupt::typelevel::RTC_ALARM;
        pub type STAMP = crate::interrupt::typelevel::TAMP_STAMP;
        pub type TAMP = crate::interrupt::typelevel::TAMP_STAMP;
        pub type WKUP = crate::interrupt::typelevel::RTC_WKUP;
    }
    pub mod SAI1 {
        pub type A = crate::interrupt::typelevel::SAI1;
        pub type B = crate::interrupt::typelevel::SAI1;
    }
    pub mod SAI4 {
        pub type A = crate::interrupt::typelevel::SAI4;
        pub type B = crate::interrupt::typelevel::SAI4;
    }
    pub mod SDMMC1 {
        pub type GLOBAL = crate::interrupt::typelevel::SDMMC1;
    }
    pub mod SDMMC2 {
        pub type GLOBAL = crate::interrupt::typelevel::SDMMC2;
    }
    pub mod SPDIFRX1 {
        pub type GLOBAL = crate::interrupt::typelevel::SPDIF_RX;
    }
    pub mod SPI1 {
        pub type GLOBAL = crate::interrupt::typelevel::SPI1;
    }
    pub mod SPI2 {
        pub type GLOBAL = crate::interrupt::typelevel::SPI2;
    }
    pub mod SPI3 {
        pub type GLOBAL = crate::interrupt::typelevel::SPI3;
    }
    pub mod SPI4 {
        pub type GLOBAL = crate::interrupt::typelevel::SPI4;
    }
    pub mod SPI6 {
        pub type GLOBAL = crate::interrupt::typelevel::SPI6;
    }
    pub mod SWPMI1 {
        pub type GLOBAL = crate::interrupt::typelevel::SWPMI1;
    }
    pub mod SYSCFG {}
    pub mod TIM1 {
        pub type BRK = crate::interrupt::typelevel::TIM1_BRK;
        pub type CC = crate::interrupt::typelevel::TIM1_CC;
        pub type COM = crate::interrupt::typelevel::TIM1_TRG_COM;
        pub type TRG = crate::interrupt::typelevel::TIM1_TRG_COM;
        pub type UP = crate::interrupt::typelevel::TIM1_UP;
    }
    pub mod TIM12 {
        pub type BRK = crate::interrupt::typelevel::TIM8_BRK_TIM12;
        pub type CC = crate::interrupt::typelevel::TIM8_BRK_TIM12;
        pub type COM = crate::interrupt::typelevel::TIM8_BRK_TIM12;
        pub type TRG = crate::interrupt::typelevel::TIM8_BRK_TIM12;
        pub type UP = crate::interrupt::typelevel::TIM8_BRK_TIM12;
    }
    pub mod TIM13 {
        pub type BRK = crate::interrupt::typelevel::TIM8_UP_TIM13;
        pub type CC = crate::interrupt::typelevel::TIM8_UP_TIM13;
        pub type COM = crate::interrupt::typelevel::TIM8_UP_TIM13;
        pub type TRG = crate::interrupt::typelevel::TIM8_UP_TIM13;
        pub type UP = crate::interrupt::typelevel::TIM8_UP_TIM13;
    }
    pub mod TIM14 {
        pub type BRK = crate::interrupt::typelevel::TIM8_TRG_COM_TIM14;
        pub type CC = crate::interrupt::typelevel::TIM8_TRG_COM_TIM14;
        pub type COM = crate::interrupt::typelevel::TIM8_TRG_COM_TIM14;
        pub type TRG = crate::interrupt::typelevel::TIM8_TRG_COM_TIM14;
        pub type UP = crate::interrupt::typelevel::TIM8_TRG_COM_TIM14;
    }
    pub mod TIM15 {
        pub type BRK = crate::interrupt::typelevel::TIM15;
        pub type CC = crate::interrupt::typelevel::TIM15;
        pub type COM = crate::interrupt::typelevel::TIM15;
        pub type TRG = crate::interrupt::typelevel::TIM15;
        pub type UP = crate::interrupt::typelevel::TIM15;
    }
    pub mod TIM16 {
        pub type BRK = crate::interrupt::typelevel::TIM16;
        pub type CC = crate::interrupt::typelevel::TIM16;
        pub type COM = crate::interrupt::typelevel::TIM16;
        pub type TRG = crate::interrupt::typelevel::TIM16;
        pub type UP = crate::interrupt::typelevel::TIM16;
    }
    pub mod TIM17 {
        pub type BRK = crate::interrupt::typelevel::TIM17;
        pub type CC = crate::interrupt::typelevel::TIM17;
        pub type COM = crate::interrupt::typelevel::TIM17;
        pub type TRG = crate::interrupt::typelevel::TIM17;
        pub type UP = crate::interrupt::typelevel::TIM17;
    }
    pub mod TIM2 {
        pub type BRK = crate::interrupt::typelevel::TIM2;
        pub type CC = crate::interrupt::typelevel::TIM2;
        pub type COM = crate::interrupt::typelevel::TIM2;
        pub type TRG = crate::interrupt::typelevel::TIM2;
        pub type UP = crate::interrupt::typelevel::TIM2;
    }
    pub mod TIM23 {
        pub type BRK = crate::interrupt::typelevel::TIM23;
        pub type CC = crate::interrupt::typelevel::TIM23;
        pub type COM = crate::interrupt::typelevel::TIM23;
        pub type TRG = crate::interrupt::typelevel::TIM23;
        pub type UP = crate::interrupt::typelevel::TIM23;
    }
    pub mod TIM24 {
        pub type BRK = crate::interrupt::typelevel::TIM24;
        pub type CC = crate::interrupt::typelevel::TIM24;
        pub type COM = crate::interrupt::typelevel::TIM24;
        pub type TRG = crate::interrupt::typelevel::TIM24;
        pub type UP = crate::interrupt::typelevel::TIM24;
    }
    pub mod TIM3 {
        pub type BRK = crate::interrupt::typelevel::TIM3;
        pub type CC = crate::interrupt::typelevel::TIM3;
        pub type COM = crate::interrupt::typelevel::TIM3;
        pub type TRG = crate::interrupt::typelevel::TIM3;
        pub type UP = crate::interrupt::typelevel::TIM3;
    }
    pub mod TIM4 {
        pub type BRK = crate::interrupt::typelevel::TIM4;
        pub type CC = crate::interrupt::typelevel::TIM4;
        pub type COM = crate::interrupt::typelevel::TIM4;
        pub type TRG = crate::interrupt::typelevel::TIM4;
        pub type UP = crate::interrupt::typelevel::TIM4;
    }
    pub mod TIM5 {
        pub type BRK = crate::interrupt::typelevel::TIM5;
        pub type CC = crate::interrupt::typelevel::TIM5;
        pub type COM = crate::interrupt::typelevel::TIM5;
        pub type TRG = crate::interrupt::typelevel::TIM5;
        pub type UP = crate::interrupt::typelevel::TIM5;
    }
    pub mod TIM6 {
        pub type BRK = crate::interrupt::typelevel::TIM6_DAC;
        pub type CC = crate::interrupt::typelevel::TIM6_DAC;
        pub type COM = crate::interrupt::typelevel::TIM6_DAC;
        pub type TRG = crate::interrupt::typelevel::TIM6_DAC;
        pub type UP = crate::interrupt::typelevel::TIM6_DAC;
    }
    pub mod TIM7 {
        pub type BRK = crate::interrupt::typelevel::TIM7;
        pub type CC = crate::interrupt::typelevel::TIM7;
        pub type COM = crate::interrupt::typelevel::TIM7;
        pub type TRG = crate::interrupt::typelevel::TIM7;
        pub type UP = crate::interrupt::typelevel::TIM7;
    }
    pub mod TIM8 {
        pub type BRK = crate::interrupt::typelevel::TIM8_BRK_TIM12;
        pub type CC = crate::interrupt::typelevel::TIM8_CC;
        pub type COM = crate::interrupt::typelevel::TIM8_TRG_COM_TIM14;
        pub type TRG = crate::interrupt::typelevel::TIM8_TRG_COM_TIM14;
        pub type UP = crate::interrupt::typelevel::TIM8_UP_TIM13;
    }
    pub mod UART4 {
        pub type GLOBAL = crate::interrupt::typelevel::UART4;
    }
    pub mod UART5 {
        pub type GLOBAL = crate::interrupt::typelevel::UART5;
    }
    pub mod UART7 {
        pub type GLOBAL = crate::interrupt::typelevel::UART7;
    }
    pub mod UART8 {
        pub type GLOBAL = crate::interrupt::typelevel::UART8;
    }
    pub mod UART9 {
        pub type GLOBAL = crate::interrupt::typelevel::UART9;
    }
    pub mod UID {}
    pub mod USART1 {
        pub type GLOBAL = crate::interrupt::typelevel::USART1;
    }
    pub mod USART10 {
        pub type GLOBAL = crate::interrupt::typelevel::USART10;
    }
    pub mod USART2 {
        pub type GLOBAL = crate::interrupt::typelevel::USART2;
    }
    pub mod USART3 {
        pub type GLOBAL = crate::interrupt::typelevel::USART3;
    }
    pub mod USART6 {
        pub type GLOBAL = crate::interrupt::typelevel::USART6;
    }
    pub mod USB_OTG_HS {
        pub type EP1_IN = crate::interrupt::typelevel::OTG_HS_EP1_IN;
        pub type EP1_OUT = crate::interrupt::typelevel::OTG_HS_EP1_OUT;
        pub type GLOBAL = crate::interrupt::typelevel::OTG_HS;
        pub type WKUP = crate::interrupt::typelevel::OTG_HS_WKUP;
    }
    pub mod VREFBUF {}
    pub mod WWDG1 {
        pub type GLOBAL = crate::interrupt::typelevel::WWDG;
        pub type RST = crate::interrupt::typelevel::WWDG;
    }
}
dma_channel_impl!(DMA1_CH0, 0u8);
dma_channel_impl!(DMA1_CH1, 1u8);
dma_channel_impl!(DMA1_CH2, 2u8);
dma_channel_impl!(DMA1_CH3, 3u8);
dma_channel_impl!(DMA1_CH4, 4u8);
dma_channel_impl!(DMA1_CH5, 5u8);
dma_channel_impl!(DMA1_CH6, 6u8);
dma_channel_impl!(DMA1_CH7, 7u8);
dma_channel_impl!(DMA2_CH0, 8u8);
dma_channel_impl!(DMA2_CH1, 9u8);
dma_channel_impl!(DMA2_CH2, 10u8);
dma_channel_impl!(DMA2_CH3, 11u8);
dma_channel_impl!(DMA2_CH4, 12u8);
dma_channel_impl!(DMA2_CH5, 13u8);
dma_channel_impl!(DMA2_CH6, 14u8);
dma_channel_impl!(DMA2_CH7, 15u8);
dma_channel_impl!(BDMA_CH0, 16u8);
dma_channel_impl!(BDMA_CH1, 17u8);
dma_channel_impl!(BDMA_CH2, 18u8);
dma_channel_impl!(BDMA_CH3, 19u8);
dma_channel_impl!(BDMA_CH4, 20u8);
dma_channel_impl!(BDMA_CH5, 21u8);
dma_channel_impl!(BDMA_CH6, 22u8);
dma_channel_impl!(BDMA_CH7, 23u8);
#[cfg(feature = "rt")]
#[crate::interrupt]
unsafe fn BDMA_CHANNEL0() {
    <crate::peripherals::BDMA_CH0 as crate::dma::ChannelInterrupt>::on_irq();
}
#[cfg(feature = "rt")]
#[crate::interrupt]
unsafe fn BDMA_CHANNEL1() {
    <crate::peripherals::BDMA_CH1 as crate::dma::ChannelInterrupt>::on_irq();
}
#[cfg(feature = "rt")]
#[crate::interrupt]
unsafe fn BDMA_CHANNEL2() {
    <crate::peripherals::BDMA_CH2 as crate::dma::ChannelInterrupt>::on_irq();
}
#[cfg(feature = "rt")]
#[crate::interrupt]
unsafe fn BDMA_CHANNEL3() {
    <crate::peripherals::BDMA_CH3 as crate::dma::ChannelInterrupt>::on_irq();
}
#[cfg(feature = "rt")]
#[crate::interrupt]
unsafe fn BDMA_CHANNEL4() {
    <crate::peripherals::BDMA_CH4 as crate::dma::ChannelInterrupt>::on_irq();
}
#[cfg(feature = "rt")]
#[crate::interrupt]
unsafe fn BDMA_CHANNEL5() {
    <crate::peripherals::BDMA_CH5 as crate::dma::ChannelInterrupt>::on_irq();
}
#[cfg(feature = "rt")]
#[crate::interrupt]
unsafe fn BDMA_CHANNEL6() {
    <crate::peripherals::BDMA_CH6 as crate::dma::ChannelInterrupt>::on_irq();
}
#[cfg(feature = "rt")]
#[crate::interrupt]
unsafe fn BDMA_CHANNEL7() {
    <crate::peripherals::BDMA_CH7 as crate::dma::ChannelInterrupt>::on_irq();
}
#[cfg(feature = "rt")]
#[crate::interrupt]
unsafe fn DMA1_STREAM0() {
    <crate::peripherals::DMA1_CH0 as crate::dma::ChannelInterrupt>::on_irq();
}
#[cfg(feature = "rt")]
#[crate::interrupt]
unsafe fn DMA1_STREAM1() {
    <crate::peripherals::DMA1_CH1 as crate::dma::ChannelInterrupt>::on_irq();
}
#[cfg(feature = "rt")]
#[crate::interrupt]
unsafe fn DMA1_STREAM2() {
    <crate::peripherals::DMA1_CH2 as crate::dma::ChannelInterrupt>::on_irq();
}
#[cfg(feature = "rt")]
#[crate::interrupt]
unsafe fn DMA1_STREAM3() {
    <crate::peripherals::DMA1_CH3 as crate::dma::ChannelInterrupt>::on_irq();
}
#[cfg(feature = "rt")]
#[crate::interrupt]
unsafe fn DMA1_STREAM4() {
    <crate::peripherals::DMA1_CH4 as crate::dma::ChannelInterrupt>::on_irq();
}
#[cfg(feature = "rt")]
#[crate::interrupt]
unsafe fn DMA1_STREAM5() {
    <crate::peripherals::DMA1_CH5 as crate::dma::ChannelInterrupt>::on_irq();
}
#[cfg(feature = "rt")]
#[crate::interrupt]
unsafe fn DMA1_STREAM6() {
    <crate::peripherals::DMA1_CH6 as crate::dma::ChannelInterrupt>::on_irq();
}
#[cfg(feature = "rt")]
#[crate::interrupt]
unsafe fn DMA1_STREAM7() {
    <crate::peripherals::DMA1_CH7 as crate::dma::ChannelInterrupt>::on_irq();
}
#[cfg(feature = "rt")]
#[crate::interrupt]
unsafe fn DMA2_STREAM0() {
    <crate::peripherals::DMA2_CH0 as crate::dma::ChannelInterrupt>::on_irq();
}
#[cfg(feature = "rt")]
#[crate::interrupt]
unsafe fn DMA2_STREAM1() {
    <crate::peripherals::DMA2_CH1 as crate::dma::ChannelInterrupt>::on_irq();
}
#[cfg(feature = "rt")]
#[crate::interrupt]
unsafe fn DMA2_STREAM2() {
    <crate::peripherals::DMA2_CH2 as crate::dma::ChannelInterrupt>::on_irq();
}
#[cfg(feature = "rt")]
#[crate::interrupt]
unsafe fn DMA2_STREAM3() {
    <crate::peripherals::DMA2_CH3 as crate::dma::ChannelInterrupt>::on_irq();
}
#[cfg(feature = "rt")]
#[crate::interrupt]
unsafe fn DMA2_STREAM4() {
    <crate::peripherals::DMA2_CH4 as crate::dma::ChannelInterrupt>::on_irq();
}
#[cfg(feature = "rt")]
#[crate::interrupt]
unsafe fn DMA2_STREAM5() {
    <crate::peripherals::DMA2_CH5 as crate::dma::ChannelInterrupt>::on_irq();
}
#[cfg(feature = "rt")]
#[crate::interrupt]
unsafe fn DMA2_STREAM6() {
    <crate::peripherals::DMA2_CH6 as crate::dma::ChannelInterrupt>::on_irq();
}
#[cfg(feature = "rt")]
#[crate::interrupt]
unsafe fn DMA2_STREAM7() {
    <crate::peripherals::DMA2_CH7 as crate::dma::ChannelInterrupt>::on_irq();
}
pub(crate) const DMA_CHANNELS: &[crate::dma::ChannelInfo] = &[
    crate::dma::ChannelInfo {
        dma: crate::dma::DmaInfo::Dma(crate::pac::DMA1),
        num: 0usize,
        dmamux: crate::dma::DmamuxInfo {
            mux: crate::pac::DMAMUX1,
            num: 0usize,
        },
    },
    crate::dma::ChannelInfo {
        dma: crate::dma::DmaInfo::Dma(crate::pac::DMA1),
        num: 1usize,
        dmamux: crate::dma::DmamuxInfo {
            mux: crate::pac::DMAMUX1,
            num: 1usize,
        },
    },
    crate::dma::ChannelInfo {
        dma: crate::dma::DmaInfo::Dma(crate::pac::DMA1),
        num: 2usize,
        dmamux: crate::dma::DmamuxInfo {
            mux: crate::pac::DMAMUX1,
            num: 2usize,
        },
    },
    crate::dma::ChannelInfo {
        dma: crate::dma::DmaInfo::Dma(crate::pac::DMA1),
        num: 3usize,
        dmamux: crate::dma::DmamuxInfo {
            mux: crate::pac::DMAMUX1,
            num: 3usize,
        },
    },
    crate::dma::ChannelInfo {
        dma: crate::dma::DmaInfo::Dma(crate::pac::DMA1),
        num: 4usize,
        dmamux: crate::dma::DmamuxInfo {
            mux: crate::pac::DMAMUX1,
            num: 4usize,
        },
    },
    crate::dma::ChannelInfo {
        dma: crate::dma::DmaInfo::Dma(crate::pac::DMA1),
        num: 5usize,
        dmamux: crate::dma::DmamuxInfo {
            mux: crate::pac::DMAMUX1,
            num: 5usize,
        },
    },
    crate::dma::ChannelInfo {
        dma: crate::dma::DmaInfo::Dma(crate::pac::DMA1),
        num: 6usize,
        dmamux: crate::dma::DmamuxInfo {
            mux: crate::pac::DMAMUX1,
            num: 6usize,
        },
    },
    crate::dma::ChannelInfo {
        dma: crate::dma::DmaInfo::Dma(crate::pac::DMA1),
        num: 7usize,
        dmamux: crate::dma::DmamuxInfo {
            mux: crate::pac::DMAMUX1,
            num: 7usize,
        },
    },
    crate::dma::ChannelInfo {
        dma: crate::dma::DmaInfo::Dma(crate::pac::DMA2),
        num: 0usize,
        dmamux: crate::dma::DmamuxInfo {
            mux: crate::pac::DMAMUX1,
            num: 8usize,
        },
    },
    crate::dma::ChannelInfo {
        dma: crate::dma::DmaInfo::Dma(crate::pac::DMA2),
        num: 1usize,
        dmamux: crate::dma::DmamuxInfo {
            mux: crate::pac::DMAMUX1,
            num: 9usize,
        },
    },
    crate::dma::ChannelInfo {
        dma: crate::dma::DmaInfo::Dma(crate::pac::DMA2),
        num: 2usize,
        dmamux: crate::dma::DmamuxInfo {
            mux: crate::pac::DMAMUX1,
            num: 10usize,
        },
    },
    crate::dma::ChannelInfo {
        dma: crate::dma::DmaInfo::Dma(crate::pac::DMA2),
        num: 3usize,
        dmamux: crate::dma::DmamuxInfo {
            mux: crate::pac::DMAMUX1,
            num: 11usize,
        },
    },
    crate::dma::ChannelInfo {
        dma: crate::dma::DmaInfo::Dma(crate::pac::DMA2),
        num: 4usize,
        dmamux: crate::dma::DmamuxInfo {
            mux: crate::pac::DMAMUX1,
            num: 12usize,
        },
    },
    crate::dma::ChannelInfo {
        dma: crate::dma::DmaInfo::Dma(crate::pac::DMA2),
        num: 5usize,
        dmamux: crate::dma::DmamuxInfo {
            mux: crate::pac::DMAMUX1,
            num: 13usize,
        },
    },
    crate::dma::ChannelInfo {
        dma: crate::dma::DmaInfo::Dma(crate::pac::DMA2),
        num: 6usize,
        dmamux: crate::dma::DmamuxInfo {
            mux: crate::pac::DMAMUX1,
            num: 14usize,
        },
    },
    crate::dma::ChannelInfo {
        dma: crate::dma::DmaInfo::Dma(crate::pac::DMA2),
        num: 7usize,
        dmamux: crate::dma::DmamuxInfo {
            mux: crate::pac::DMAMUX1,
            num: 15usize,
        },
    },
    crate::dma::ChannelInfo {
        dma: crate::dma::DmaInfo::Bdma(crate::pac::BDMA),
        num: 0usize,
        dmamux: crate::dma::DmamuxInfo {
            mux: crate::pac::DMAMUX2,
            num: 0usize,
        },
    },
    crate::dma::ChannelInfo {
        dma: crate::dma::DmaInfo::Bdma(crate::pac::BDMA),
        num: 1usize,
        dmamux: crate::dma::DmamuxInfo {
            mux: crate::pac::DMAMUX2,
            num: 1usize,
        },
    },
    crate::dma::ChannelInfo {
        dma: crate::dma::DmaInfo::Bdma(crate::pac::BDMA),
        num: 2usize,
        dmamux: crate::dma::DmamuxInfo {
            mux: crate::pac::DMAMUX2,
            num: 2usize,
        },
    },
    crate::dma::ChannelInfo {
        dma: crate::dma::DmaInfo::Bdma(crate::pac::BDMA),
        num: 3usize,
        dmamux: crate::dma::DmamuxInfo {
            mux: crate::pac::DMAMUX2,
            num: 3usize,
        },
    },
    crate::dma::ChannelInfo {
        dma: crate::dma::DmaInfo::Bdma(crate::pac::BDMA),
        num: 4usize,
        dmamux: crate::dma::DmamuxInfo {
            mux: crate::pac::DMAMUX2,
            num: 4usize,
        },
    },
    crate::dma::ChannelInfo {
        dma: crate::dma::DmaInfo::Bdma(crate::pac::BDMA),
        num: 5usize,
        dmamux: crate::dma::DmamuxInfo {
            mux: crate::pac::DMAMUX2,
            num: 5usize,
        },
    },
    crate::dma::ChannelInfo {
        dma: crate::dma::DmaInfo::Bdma(crate::pac::BDMA),
        num: 6usize,
        dmamux: crate::dma::DmamuxInfo {
            mux: crate::pac::DMAMUX2,
            num: 6usize,
        },
    },
    crate::dma::ChannelInfo {
        dma: crate::dma::DmaInfo::Bdma(crate::pac::BDMA),
        num: 7usize,
        dmamux: crate::dma::DmamuxInfo {
            mux: crate::pac::DMAMUX2,
            num: 7usize,
        },
    },
];
pub fn gpio_block(n: usize) -> crate::pac::gpio::Gpio {
    unsafe { crate::pac::gpio::Gpio::from_ptr((1476526080usize + 1024usize * n) as _) }
}
pub const FLASH_BASE: usize = 134217728usize;
pub const FLASH_SIZE: usize = 1048576usize;
pub const WRITE_SIZE: usize = 32usize;
